...
| statusList | 输出 | 通道状态数组，用于返回每个通道的当前状态，与channelList一一对应。<br>该参数不能为空指针。<br>调用者分配的数组，至少包含listNum个元素的空间。<br>该数组元素类型为int32_t，元素取值为HcommChannelStatus枚举值，其定义及用法请参见[HcommChannelStatus](../../datatype_definition/HcommChannelStatus.md)。 |
...
std::vector<int32_t> statuses(channelNum, HCOMM_CHANNEL_STATUS_READY);
