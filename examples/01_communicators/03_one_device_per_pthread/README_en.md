...
This sample demonstrates how to use the `HcclCommInitRootInfoConfig()` API to initialize a communicator in a single process, with each thread managing one NPU device. It covers the following features:

- Device detection: Query the number of available devices using the `aclrtGetDeviceCount()` API.
...
- Initialize the communicator in each thread based on the rootinfo identification information using the `HcclCommInitRootInfoConfig()` API.
...
└── one_device_per_pthread    # Compiled executable
```
