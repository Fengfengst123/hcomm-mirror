...
| 2 | The address planning is the same as layer 1. The address type is IP address. RoCE IP source: prefer parsing the topology XML (/var/run/ascend-topologyd/virtualTopology.xml) to extract the NPU×NIC affinity groups; when the XML is unavailable (file missing, parsing failure, or no NIC definitions), fall back to the driver topo interface — enumerate the system NIC names, then query the topo type of each NPU and NIC pair one by one via the driver interface dcmiv2_get_topo_info_by_device_id_and_nic_name; an NPU×NIC pair whose topo type is DCMI_TOPO_TYPE_UB is treated as affined and used to build the affinity matrix; finally distribute the RoCE IP to each NPU round-robin based on the affinity. |
