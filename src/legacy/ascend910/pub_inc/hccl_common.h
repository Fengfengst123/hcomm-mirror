...
constexpr u32 AICPU_ZERO_COPY_MAX_DEVICE_NUM_A3
    = 32; // 零拷贝相关在A3场景上的最大设备数，影响外部组件使用，不能随意修改
...
constexpr uint32_t HCCL_EXCHANGE_INFO_LEN = 2048; // HCCL一致性校验交换信息最大长度 socket底层约束2k
                                                  /* error message相关 */
/* EI0004 */
...
using RemoteRankInfo = struct TagRemoteRankInfo {
    ...
    s32 remoteSdid;
    TagRemoteRankInfo()
        : remoteDeviceId(INVALID_INT),
          remoteRank(INVALID_UINT),
          remotePid(INVALID_INT),
          remoteSdid(INVALID_INT)
    {}
