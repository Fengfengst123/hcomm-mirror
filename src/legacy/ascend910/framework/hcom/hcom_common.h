...
using HcomInfo = struct HcomInfoTag {
    ...
    std::unordered_map<std::string, HcclGroupParams>
        hcomGroupMap; // 每个group的信息(kname为服务器的server_id,按照服务器区分)
    ...
    HcomInfoTag() : pComm(nullptr), devId(-1), cloudFlag(false), isHcomInit(false) {}
