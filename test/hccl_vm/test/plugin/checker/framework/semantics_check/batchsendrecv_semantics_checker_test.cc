        ...
        bool splitOutput = false)
    {
        for (RankId rankId = 0; rankId < rankSize; ++rankId) {
            ...
            if (dataSize != 0) {
                ...
                rankMemSemantics[BufferType::OUTPUT].emplace(first.startAddr, first);
                if (firstSize != dataSize) {
                    ...
                    rankMemSemantics[BufferType::OUTPUT].emplace(second.startAddr, second);
                }
            }
            allRankMemSemantics[rankId] = rankMemSemantics;
        }
...
TEST_F(BatchSendRecvSemanticsCheckerTest, ValidSemantics_TwoRanks)
{
    ...
    for (DeviceId i = 0; i < rankSize; i++)
        rankToDevice[i] = i;
    HcclResult result = TaskCheckBatchSendRecvSemantics(allRankMemSemantics, rankSize, dataSize, rankToDevice);
...
TEST_F(BatchSendRecvSemanticsCheckerTest, Abnormal_ExpectedRankSetIsNotInferredFromActualMap)
{
    ...
    rankMemSemantics[BufferType::OUTPUT].emplace(output.startAddr, output);
    allRankMemSemantics[0] = rankMemSemantics;

    std::vector<DeviceId> rankToDevice(2);
    for (DeviceId i = 0; i < 2; i++)
        rankToDevice[i] = i;
    EXPECT_EQ(TaskCheckBatchSendRecvSemantics(allRankMemSemantics, 2, dataSize, rankToDevice), HcclResult::HCCL_E_PARA);
}
...
TEST_F(BatchSendRecvSemanticsCheckerTest, RingValidSemantics_FourRanksWithSplitOutput)
{
    ...
    for (DeviceId i = 0; i < 4; i++)
        rankToDevice[i] = i;
    EXPECT_EQ(
        TaskCheckBatchSendRecvRingSemantics(allRankMemSemantics, 4, 1024, rankToDevice), HcclResult::HCCL_SUCCESS);
}
...
TEST_F(BatchSendRecvSemanticsCheckerTest, RingValidSemantics_ZeroDataSize)
{
    ...
    for (DeviceId i = 0; i < 2; i++)
        rankToDevice[i] = i;
    EXPECT_EQ(TaskCheckBatchSendRecvRingSemantics(allRankMemSemantics, 2, 0, rankToDevice), HcclResult::HCCL_SUCCESS);
}
...
TEST_F(BatchSendRecvSemanticsCheckerTest, RingAbnormal_SingleRank)
{
    ...
    for (DeviceId i = 0; i < 1; i++)
        rankToDevice[i] = i;
    EXPECT_EQ(TaskCheckBatchSendRecvRingSemantics(allRankMemSemantics, 1, 1024, rankToDevice), HcclResult::HCCL_E_PARA);
}
...
TEST_F(BatchSendRecvSemanticsCheckerTest, RingAbnormal_MissingRank)
{
    ...
    for (DeviceId i = 0; i < 4; i++)
        rankToDevice[i] = i;
    EXPECT_EQ(TaskCheckBatchSendRecvRingSemantics(allRankMemSemantics, 4, 1024, rankToDevice), HcclResult::HCCL_E_PARA);
}
...
TEST_F(BatchSendRecvSemanticsCheckerTest, RingAbnormal_WrongSourceRank)
{
    ...
    output.srcBufs.insert(SrcBufDes(0, BufferType::INPUT, 0));
    allRankMemSemantics[2][BufferType::OUTPUT].emplace(output.startAddr, output);
...
TEST_F(BatchSendRecvSemanticsCheckerTest, RingAbnormal_WrongSourceOffset)
{
    ...
    allRankMemSemantics[0][BufferType::OUTPUT].emplace(output.startAddr, output);

    std::vector<DeviceId> rankToDevice(2);
    for (DeviceId i = 0; i < 2; i++)
        rankToDevice[i] = i;
    EXPECT_EQ(TaskCheckBatchSendRecvRingSemantics(allRankMemSemantics, 2, 1024, rankToDevice), HcclResult::HCCL_E_PARA);
}
