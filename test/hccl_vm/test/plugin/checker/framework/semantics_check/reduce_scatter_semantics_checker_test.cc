...
TEST_F(ReduceScatterSemanticsCheckerTest, ValidSemantics_TwoRanks)
{
    ...
    for (DeviceId i = 0; i < rankSize; i++)
        rankToDevice[i] = i;
    HcclResult result
        = TaskCheckReduceScatterSemantics(allRankMemSemantics, dataSize, HcclReduceOp::HCCL_REDUCE_SUM, rankToDevice);
...
TEST_F(ReduceScatterSemanticsCheckerTest, ValidSemantics_ReduceProd)
{
    ...
    for (DeviceId i = 0; i < rankSize; i++)
        rankToDevice[i] = i;
    HcclResult result
        = TaskCheckReduceScatterSemantics(allRankMemSemantics, dataSize, HcclReduceOp::HCCL_REDUCE_PROD, rankToDevice);
    EXPECT_EQ(result, HcclResult::HCCL_SUCCESS);
}
