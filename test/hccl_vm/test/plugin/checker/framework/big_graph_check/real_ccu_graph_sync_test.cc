...
class RealCcuGraphSyncTest : public testing::Test {
  protected:
    ...
    static CcuNotify MakeNotify(DeviceId recordDevice, DeviceId waitDevice,
                                uint32_t dieId, uint16_t ckeId,
                                uint16_t ckeMask) {
        CcuNotify notify;
    ...
    static CcuTraceInfo MakeTrace(const TaskPosition &position,
                                  const char *role, uint32_t instrId) {
        CcuTraceInfo trace;
        trace.valid = true;
        trace.opName = "ccu";
        trace.nodeRole = role;
    ...
    TaskRecordCCU *AddRecord(const CcuNotify &notify,
                             const TaskPosition &position, uint32_t instrId) {
        TaskRecordCCU *node =
            AddNode(std::make_unique<TaskRecordCCU>(notify, ProtocolType::CCU),
                    position);
        node->SetCcuTrace(MakeTrace(position, "post", instrId));
        return node;
    }

    TaskWaitCCU *AddWait(const CcuNotify &notify, const TaskPosition &position,
                         uint32_t instrId) {
        TaskWaitCCU *node = AddNode(
            std::make_unique<TaskWaitCCU>(notify, ProtocolType::CCU), position);
    ...
    TaskStart *AddLoopStart(const TaskPosition &position, uint32_t instrId) {
        TaskStart *node =
            AddNode(std::make_unique<TaskStart>(BoundaryType::LOOP), position);
    ...
    TaskEnd *AddLoopEnd(const TaskPosition &position, uint32_t instrId) {
        TaskEnd *node =
            AddNode(std::make_unique<TaskEnd>(BoundaryType::LOOP), position);
    ...
    Graph BuildRealGraph() {
        ...
        graph.rank0InitialPost =
            AddRecord(MakeNotify(0, 0, 1, 192, 0xffff), rank0Operator0, 25);
        graph.rank0PostBits01 =
            AddRecord(MakeNotify(0, 1, 1, 256, 0x3), rank0Operator0, 27);
        graph.rank0WaitBits01 = AddWait(
            MakeNotify(INVALID_DEVICE_ID, 0, 1, 256, 0x3), rank0Operator1, 29);
        auto rank0Write =
            AddNode(std::make_unique<TaskTransMem>(
                        Slice(0, MemType::INPUT, 0x800),
                        Slice(1, MemType::OUTPUT, 0), ProtocolType::CCU),
                    rank0Operator1);
        graph.rank0PostBit1Local =
            AddRecord(MakeNotify(0, 0, 1, 224, 0x2), rank0Operator1, 63);
        TaskStart *loopStart = AddLoopStart(rank0Operator1, 128);
        TaskBatchTransMem *loopCopyIn =
            AddBatchCopy(rank0Operator1, Slice(0, MemType::INPUT, 0),
                         Slice(0, MemType::MS_CCU, 0x8000000), 128);
        TaskRecordCCU *loopPost1 =
            AddRecord(MakeNotify(0, 0, 1, 0, 0x1), rank0Operator1, 128);
        TaskWaitCCU *loopWait1 =
            AddWait(MakeNotify(0, 0, 1, 0, 0x1), rank0Operator1, 128);
        TaskBatchTransMem *loopCopyOut =
            AddBatchCopy(rank0Operator1, Slice(0, MemType::MS_CCU, 0x8000000),
                         Slice(0, MemType::OUTPUT, 0), 128);
        TaskRecordCCU *loopPost2 =
            AddRecord(MakeNotify(0, 0, 1, 0, 0x1), rank0Operator1, 128);
        TaskWaitCCU *loopWait2 =
            AddWait(MakeNotify(0, 0, 1, 0, 0x1), rank0Operator1, 128);
        TaskEnd *loopEnd = AddLoopEnd(rank0Operator1, 128);
        TaskWaitCCU *rank0WaitBit1Local =
            AddWait(MakeNotify(0, 0, 1, 224, 0x2), rank0Operator1, 136);
        graph.rank0WaitBit1Local = rank0WaitBit1Local;
        graph.rank0PostBit3 =
            AddRecord(MakeNotify(0, 1, 1, 256, 0x8), rank0Operator2, 142);
        graph.rank0WaitBit3 = AddWait(
            MakeNotify(INVALID_DEVICE_ID, 0, 1, 256, 0x8), rank0Operator2, 143);
        ...
        TaskRecordCCU *rank1InitialPost =
            AddRecord(MakeNotify(1, 1, 1, 192, 0xffff), rank1Operator0, 17);
        graph.rank1PostBits01 =
            AddRecord(MakeNotify(1, 0, 1, 256, 0x3), rank1Operator0, 19);
        graph.rank1WaitBits01 = AddWait(
            MakeNotify(INVALID_DEVICE_ID, 1, 1, 256, 0x3), rank1Operator1, 21);
        graph.rank1PostBit3 =
            AddRecord(MakeNotify(1, 0, 1, 256, 0x8), rank1Operator2, 33);
        graph.rank1WaitBit3 = AddWait(
            MakeNotify(INVALID_DEVICE_ID, 1, 1, 256, 0x8), rank1Operator2, 34);
...
TEST_F(RealCcuGraphSyncTest, AddingWaitForExistingCcuMaskBitIsRejected) {
    ...
    TaskWaitCCU *extraWait = AddWait(
        MakeNotify(INVALID_DEVICE_ID, 1, 1, 256, 0x1), Position(3, 1), 35);
...
TEST_F(RealCcuGraphSyncTest, AddingRecordForExistingCcuMaskBitIsRejected) {
    Graph graph = BuildRealGraph();
    TaskRecordCCU *extraPost =
        AddRecord(MakeNotify(3, 1, 1, 256, 0x1), Position(3, 3), 36);
