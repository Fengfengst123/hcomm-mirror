...
class RealAivGraphSyncTest : public testing::Test {
  protected:
    ...
    static TaskPosition Position(OperatorId operatorId, RankId rankId,
                                 StreamId streamId, QueueId queueId,
                                 uint32_t blockId = 0, uint32_t taskId = 0) {
    ...
    static AivPipeEvent MakeEvent(RankId rankId, uint32_t blockId,
    ...
    static AivFlagSync MakeFlag(RankId /*currentRank*/, DeviceId ownerDevice,
    ...
    TaskAivPipeBarrier *AddBarrier(const TaskPosition &position,
                                   uint32_t taskId) {
        AivBarrierInfo info;
        info.pipeType = 3;
        info.merged = true;
        info.memberTaskIds = {taskId, taskId + 1, taskId + 2};
        return AddNode(std::make_unique<TaskAivPipeBarrier>(std::move(info)),
                       position);
    }

    TaskAivSyncAll *AddSyncAll(const TaskPosition &position,
                               uint32_t syncRound) {
        AivSyncAllInfo info;
        info.syncRound = syncRound;
        info.merged = true;
        info.memberTaskIds = {18, 19, 20, 267, 268, 269};
        return AddNode(std::make_unique<TaskAivSyncAll>(std::move(info)),
                       position);
    }

    TaskBatchTransMem *AddBatchCopy(const TaskPosition &position, MemSlice src,
                                    MemSlice dst) {
    ...
    TaskEnd *AddSubGraphEnd(const TaskPosition &position) {
        return AddNode(std::make_unique<TaskEnd>(BoundaryType::AIV_SUB_GRAPH),
                       position);
    }

    TaskAivPipeBarrier *BuildEventPipeline(TaskStart *subStart, RankId rankId,
                                           OperatorId operatorId,
                                           uint32_t blockId, QueueId queueId,
                                           TaskAivSetFlag **firstSet,
                                           TaskAivWaitFlag **firstWait) {
        const TaskPosition pipePosition = Position(
            operatorId, rankId, INVALID_STREAM_ID, queueId, blockId, 0);
        TaskAivPipeBarrier *initialBarrier =
            AddBarrier(pipePosition, blockId * 249);
        TaskAivSetFlag *set0 =
            AddNode(std::make_unique<TaskAivSetFlag>(
                        MakeEvent(rankId, blockId, 1, 2, 0)),
                    Position(operatorId, rankId, INVALID_STREAM_ID, queueId,
                             blockId, 1));
        TaskAivWaitFlag *wait0 =
            AddNode(std::make_unique<TaskAivWaitFlag>(
                        MakeEvent(rankId, blockId, 1, 2, 0)),
                    Position(operatorId + 1, rankId, INVALID_STREAM_ID, queueId,
                             blockId, 2));
        TaskBatchTransMem *copy = AddBatchCopy(
            Position(operatorId + 1, rankId, INVALID_STREAM_ID, queueId + 1,
                     blockId, 3),
            Slice(rankId, MemType::AIV_COMM, 0xa00000 + blockId * 0x2aaaa),
            Slice(rankId, MemType::AIV_UB, 0));
        TaskAivSetFlag *set1 =
            AddNode(std::make_unique<TaskAivSetFlag>(
                        MakeEvent(rankId, blockId, 2, 1, 1)),
                    Position(operatorId + 1, rankId, INVALID_STREAM_ID, queueId,
                             blockId, 4));
        TaskAivWaitFlag *wait1 =
            AddNode(std::make_unique<TaskAivWaitFlag>(
                        MakeEvent(rankId, blockId, 2, 1, 1)),
                    Position(operatorId + 2, rankId, INVALID_STREAM_ID,
                             queueId + 1, blockId, 5));
        TaskBatchTransMem *copyOut =
            AddBatchCopy(Position(operatorId + 2, rankId, INVALID_STREAM_ID,
                                  queueId + 1, blockId, 6),
                         Slice(rankId, MemType::AIV_UB, 0),
                         Slice(rankId, MemType::AIV_COMM, 0x100000));
        TaskAivPipeBarrier *finalBarrier =
            AddBarrier(Position(operatorId + 2, rankId, INVALID_STREAM_ID,
                                queueId, blockId, 7),
                       blockId * 247 + 7);
    ...
    Graph BuildRealGraph() {
        ...
        TaskStart *rank0Start =
            AddNode(std::make_unique<TaskStart>(BoundaryType::AIV_SUB_GRAPH),
                    SubGraphPosition(0, 0));
        TaskStart *rank1Start =
            AddNode(std::make_unique<TaskStart>(BoundaryType::AIV_SUB_GRAPH),
        ...
        for (uint32_t blockId = 0; blockId < 3; ++blockId) {
            ...
            rank1Pipelines.push_back(
                BuildEventPipeline(rank1Start, 1, 0, blockId,
                                   3072 + blockId * 3, nullptr, nullptr));
        }
        ...
        TaskAivRecvFlag *flagRecv0Local = AddNode(
            std::make_unique<TaskAivRecvFlag>(MakeFlag(0, 0, 0x900000, 2)),
            Position(1, 0, INVALID_STREAM_ID, 3072, 0, 11));
        graph.flagRecv0Remote = AddNode(
            std::make_unique<TaskAivRecvFlag>(MakeFlag(1, 0, 0x900000, 2)),
            Position(1, 1, INVALID_STREAM_ID, 3072, 0, 12));
        TaskAivSendFlag *flagSend1 = AddNode(
            std::make_unique<TaskAivSendFlag>(MakeFlag(1, 1, 0x900000, 2)),
            Position(0, 1, INVALID_STREAM_ID, 3072, 0, 13));
        TaskAivRecvFlag *flagRecv1Local = AddNode(
            std::make_unique<TaskAivRecvFlag>(MakeFlag(1, 1, 0x900000, 2)),
            Position(1, 1, INVALID_STREAM_ID, 3072, 0, 14));
        TaskAivRecvFlag *flagRecv1Remote = AddNode(
            std::make_unique<TaskAivRecvFlag>(MakeFlag(0, 1, 0x900000, 2)),
            Position(1, 0, INVALID_STREAM_ID, 3072, 0, 15));
...
TEST_F(RealAivGraphSyncTest, AddingEventWaitAcrossOperatorIsRejected) {
    Graph graph = BuildRealGraph();
    TaskAivWaitFlag *extraWait =
        AddNode(std::make_unique<TaskAivWaitFlag>(MakeEvent(0, 0, 1, 2, 0)),
                Position(3, 0, INVALID_STREAM_ID, 3081, 0, 20));
...
TEST_F(RealAivGraphSyncTest, AddingEventSetAcrossOperatorIsRejected) {
    Graph graph = BuildRealGraph();
    TaskAivSetFlag *extraSet =
        AddNode(std::make_unique<TaskAivSetFlag>(MakeEvent(0, 0, 1, 2, 0)),
                Position(3, 0, INVALID_STREAM_ID, 3081, 0, 21));
...
TEST_F(RealAivGraphSyncTest, AddingSecondSendForFlagFanoutIsRejected) {
    Graph graph = BuildRealGraph();
    TaskAivSendFlag *extraSend =
        AddNode(std::make_unique<TaskAivSendFlag>(MakeFlag(0, 0, 0x900000, 2)),
                Position(3, 0, INVALID_STREAM_ID, 3081, 0, 22));
