...
class BigGraphCheckerV3SyncTest : public testing::Test {
  protected:
    ...
    static TaskPosition MakePosition(OperatorId operatorId, RankId rankId,
    ...
    TaskRecordAICPU *AddAicpuRecord(uint32_t notifyId,
    ...
    TaskWaitAICPU *AddAicpuWait(uint32_t notifyId,
                                const TaskPosition &position) {
        AicpuNotify notify;
        notify.recordDeviceId = 0;
        notify.waitDeviceId = 2;
    ...
    TaskAivSetFlag *AddAivEventSet(int32_t eventId,
                                   const TaskPosition &position) {
        return AddNode(std::make_unique<TaskAivSetFlag>(MakeEvent(eventId)),
                       position);
    }

    TaskAivWaitFlag *AddAivEventWait(int32_t eventId,
                                     const TaskPosition &position) {
        return AddNode(std::make_unique<TaskAivWaitFlag>(MakeEvent(eventId)),
                       position);
    }

    TaskAivSendFlag *AddAivFlagSend(uint64_t commInfoOffset,
                                    const TaskPosition &position) {
        return AddNode(
            std::make_unique<TaskAivSendFlag>(MakeFlag(commInfoOffset)),
            position);
    }

    TaskAivRecvFlag *AddAivFlagRecv(uint64_t commInfoOffset,
                                    const TaskPosition &position) {
        return AddNode(
            std::make_unique<TaskAivRecvFlag>(MakeFlag(commInfoOffset)),
            position);
    }

    void ExpectConflict(TaskStart *start, size_t minimumBucketCount,
                        size_t minimumPairCount) {
    ...
    void ExpectSuccess(TaskStart *start, size_t minimumBucketCount,
                       size_t minimumPairCount) {
        SyncConflictCheckStats stats;
...
TEST_F(BigGraphCheckerV3SyncTest,
       CrossOperatorAicpuOnePostToManyWaitsIsRejected) {
    ...
    TaskEnd *barrier =
        AddNode(std::make_unique<TaskEnd>(BoundaryType::MAIN_GRAPH), op2Rank2);
...
TEST_F(BigGraphCheckerV3SyncTest,
       CrossOperatorAicpuManyPostsToOneWaitIsRejected) {
    TaskStart *start = AddMainStart();
    const TaskPosition op0Rank0 = MakePosition(0, 0, 0, 0);
    const TaskPosition op1Rank1 = MakePosition(1, 1, 1, 1);
    const TaskPosition op2Rank2 = MakePosition(2, 2, 2, 2);
...
TEST_F(BigGraphCheckerV3SyncTest,
       CrossOperatorAivEventOnePostToManyWaitsIsRejected) {
    TaskStart *start = AddMainStart();
    const TaskPosition op0Rank0 = MakePosition(0, 0, 0, 0);
    const TaskPosition op1Rank1 = MakePosition(1, 1, 1, 1);
    const TaskPosition op2Rank2 = MakePosition(2, 2, 2, 2);
...
TEST_F(BigGraphCheckerV3SyncTest,
       CrossOperatorAivEventManyPostsToOneWaitIsRejected) {
    TaskStart *start = AddMainStart();
    const TaskPosition op0Rank0 = MakePosition(0, 0, 0, 0);
    const TaskPosition op1Rank1 = MakePosition(1, 1, 1, 1);
    const TaskPosition op2Rank2 = MakePosition(2, 2, 2, 2);
...
TEST_F(BigGraphCheckerV3SyncTest,
       CrossOperatorAivFlagOnePostToManyWaitsIsAllowed) {
    TaskStart *start = AddMainStart();
    const TaskPosition op0Rank0 = MakePosition(0, 0, 0, 0);
    const TaskPosition op1Rank1 = MakePosition(1, 1, 1, 1);
    const TaskPosition op2Rank2 = MakePosition(2, 2, 2, 2);
...
TEST_F(BigGraphCheckerV3SyncTest,
       CrossOperatorAivFlagManyPostsToOneWaitIsRejected) {
    TaskStart *start = AddMainStart();
    const TaskPosition op0Rank0 = MakePosition(0, 0, 0, 0);
    const TaskPosition op1Rank1 = MakePosition(1, 1, 1, 1);
    const TaskPosition op2Rank2 = MakePosition(2, 2, 2, 2);
...
TEST_F(BigGraphCheckerV3SyncTest,
       CrossOperatorCcuOnePostToManyWaitsIsRejected) {
    TaskStart *start = AddMainStart();
    const TaskPosition op0Rank0 = MakePosition(0, 0, 0, 0);
    const TaskPosition op1Rank1 = MakePosition(1, 1, 1, 1);
    const TaskPosition op2Rank2 = MakePosition(2, 2, 2, 2);
...
TEST_F(BigGraphCheckerV3SyncTest,
       CrossOperatorCcuManyPostsToOneWaitIsRejected) {
    TaskStart *start = AddMainStart();
    const TaskPosition op0Rank0 = MakePosition(0, 0, 0, 0);
    const TaskPosition op1Rank1 = MakePosition(1, 1, 1, 1);
    const TaskPosition op2Rank2 = MakePosition(2, 2, 2, 2);
