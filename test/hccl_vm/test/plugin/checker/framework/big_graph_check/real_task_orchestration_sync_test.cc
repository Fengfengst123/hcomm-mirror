...
class RealTaskOrchestrationSyncTest : public testing::Test {
  protected:
    ...
    TaskRecordAICPU *AddRecord(DeviceId recordDevice, DeviceId waitDevice,
                               uint32_t notifyId,
    ...
    TaskWaitAICPU *AddWait(DeviceId recordDevice, DeviceId waitDevice,
                           uint32_t notifyId, const TaskPosition &position) {
        AicpuNotify notify;
        notify.recordDeviceId = recordDevice;
        notify.waitDeviceId = waitDevice;
        notify.notifyId = notifyId;
    ...
    Graph BuildRealOrchestration(bool includeRecord24 = true,
                                 uint32_t record24NotifyId = 24) {
        Graph graph;
        ...
        auto rank0TransMem =
            AddNode(std::make_unique<TaskTransMem>(
                        Slice(0, MemType::INPUT, 0),
                        Slice(0, MemType::CCL, 0x1000), ProtocolType::SDMA),
                    rank0Stream11);
        graph.rank0Wait11 = AddWait(0, 0, 11, rank0Stream11);
        auto rank0Reduce =
            AddNode(std::make_unique<TaskReduce>(Slice(0, MemType::CCL, 0x1000),
                                                 Slice(0, MemType::OUTPUT, 0),
                                                 0, 0, ProtocolType::SDMA),
                    rank0Stream11);
        ...
        if (includeRecord24) {
            graph.rank0Record24 =
                AddRecord(0, 1, record24NotifyId, rank0Stream12);
        }
        ...
        auto rank1TransMem =
            AddNode(std::make_unique<TaskTransMem>(
                        Slice(1, MemType::INPUT, 0),
                        Slice(1, MemType::CCL, 0x2000), ProtocolType::SDMA),
                    rank1Stream14);
...
TEST_F(RealTaskOrchestrationSyncTest,
       AddingWaitTaskForExistingNotifyIsRejected) {
    Graph graph = BuildRealOrchestration();
    const TaskPosition rank0Stream12 = Position(2, 0, 12);
...
TEST_F(RealTaskOrchestrationSyncTest,
       AddingRecordTaskForExistingNotifyIsRejected) {
    Graph graph = BuildRealOrchestration();
    const TaskPosition rank0Stream11 = Position(1, 0, 11);
...
TEST_F(RealTaskOrchestrationSyncTest,
       DeletingRecordTaskLeavesConsumerWithoutProducer) {
    Graph graph = BuildRealOrchestration(false);
    ExpectConflict(graph.start);
}

TEST_F(RealTaskOrchestrationSyncTest,
       ModifyingRecordTaskNotifyIdBreaksExistingPair) {
