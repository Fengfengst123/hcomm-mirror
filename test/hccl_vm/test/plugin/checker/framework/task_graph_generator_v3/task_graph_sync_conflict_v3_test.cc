...
class SyncConflictTest : public testing::Test {
  protected:
    ...
    static TaskPosition MakePosition(RankId rankId, StreamId streamId,
                                     QueueId queueId) {
    ...
    static TaskPosition MakeLane(size_t lane) {
        return MakePosition(static_cast<RankId>(lane / 4U),
                            static_cast<StreamId>(lane % 4U),
                            static_cast<QueueId>(lane));
    }
    ...
    TaskWaitAICPU *AddWait(uint32_t notifyId) {
        ...
        notify.notifyId = notifyId;
        return AddNode(
            std::make_unique<TaskWaitAICPU>(notify, ProtocolType::SDMA));
    }

    TaskRecordAICPU *AddRecord(uint32_t notifyId,
                               const TaskPosition &position) {
        return AddNode(std::make_unique<TaskRecordAICPU>(
                           MakeAicpuNotify(notifyId), ProtocolType::SDMA),
                       position);
    }

    TaskWaitAICPU *AddWait(uint32_t notifyId, const TaskPosition &position) {
        return AddNode(std::make_unique<TaskWaitAICPU>(
                           MakeAicpuNotify(notifyId), ProtocolType::SDMA),
                       position);
    }
    ...
    TaskRecordCCU *AddCcuRecord(uint16_t mask) {
        ...
        notify.ckeMask = mask;
        return AddNode(
            std::make_unique<TaskRecordCCU>(notify, ProtocolType::CCU));
    }
    ...
    TaskWaitCCU *AddCcuWait(uint16_t mask) {
        ...
        notify.ckeMask = mask;
        return AddNode(
            std::make_unique<TaskWaitCCU>(notify, ProtocolType::CCU));
    }
    ...
    static AivPipeEvent MakeEvent(int32_t eventId, RankId /*rankId*/ = 0,
                                  uint64_t /*launchIdx*/ = 1,
                                  uint32_t /*blockId*/ = 2,
    ...
    static AivFlagSync MakeFlag(uint64_t commInfoOffset,
                                DeviceId flagOwnerDevice = 0,
                                uint64_t /*launchIdx*/ = 1, int32_t value = 0) {
        AivFlagSync flag;
...
TEST_F(SyncConflictTest, NullNotifyResourceIsIgnored) {
    TaskStart *start = AddMainStart();
    TaskRecordAICPU *post = AddNode(std::make_unique<TaskRecordAICPU>(
        MakeAicpuNotify(INVALID_NOTIFY_ID), ProtocolType::SDMA));
    TaskWaitAICPU *wait = AddNode(std::make_unique<TaskWaitAICPU>(
        MakeAicpuNotify(INVALID_NOTIFY_ID), ProtocolType::SDMA));
    AddEdge(start, post);
    AddEdge(post, wait);
...
TEST_F(SyncConflictTest, OrderedReuseReachesNextPostAndStartWait) {
    ...
    TaskNode *order2 =
        AddNode(std::make_unique<TaskEnd>(BoundaryType::MAIN_GRAPH));
    TaskWaitAICPU *wait2 = AddWait(1);
    AddEdge(start, post1);
    AddEdge(post1, wait1);
    AddEdge(wait1, post2);
...
TEST_F(SyncConflictTest, StartWaitLaneMatchRequiresRankStreamAndQueue) {
    ...
    for (const TaskPosition &syncLane : mismatchedLanes) {
        ...
        TaskNode *unrelatedOrder = AddNode(
            std::make_unique<TaskEnd>(BoundaryType::MAIN_GRAPH), waitLane);
...
TEST_F(SyncConflictTest, CcuOrderedReuseAcrossMultiplePairsIsAllowed) {
    ...
    TaskRecordCCU *post1 = AddNode(
        std::make_unique<TaskRecordCCU>(
            CcuNotify{INVALID_CHANNEL_ID, 0, 0, 0, 1, 1}, ProtocolType::CCU),
        lane);
    TaskWaitCCU *wait1 = AddNode(
        std::make_unique<TaskWaitCCU>(
            CcuNotify{INVALID_CHANNEL_ID, 0, 0, 0, 1, 1}, ProtocolType::CCU),
        lane);
...
TEST_F(SyncConflictTest, CcuResourceFieldsCreateIndependentBuckets) {
    auto makeNotify = [](DeviceId waitDeviceId, uint32_t dieId,
                         uint16_t ckeId) {
        CcuNotify notify;
    ...
    for (const CcuNotify &notify : resources) {
        TaskRecordCCU *post = AddNode(
            std::make_unique<TaskRecordCCU>(notify, ProtocolType::CCU), lane);
        TaskWaitCCU *wait = AddNode(
            std::make_unique<TaskWaitCCU>(notify, ProtocolType::CCU), lane);
        AddEdge(start, post);
        AddEdge(post, wait);
    }
...
TEST_F(SyncConflictTest, AivEventAndFlagResourcesAreDistinct) {
    ...
    TaskAivWaitFlag *waitFlag =
        AddNode(std::make_unique<TaskAivWaitFlag>(event));

    AivFlagSync flag;
    flag.flagOwnerDevice = 0;
    flag.commInfoOffset = 0x100;
    TaskAivSendFlag *sendFlag =
        AddNode(std::make_unique<TaskAivSendFlag>(flag));
    TaskAivRecvFlag *recvFlag =
        AddNode(std::make_unique<TaskAivRecvFlag>(flag));
...
TEST_F(SyncConflictTest, AivEventOrderedReuseAcrossMultiplePairsIsAllowed) {
    ...
    TaskAivSetFlag *set1 =
        AddNode(std::make_unique<TaskAivSetFlag>(event), lane);
    TaskAivWaitFlag *wait1 =
        AddNode(std::make_unique<TaskAivWaitFlag>(event), lane);
    TaskAivSetFlag *set2 =
        AddNode(std::make_unique<TaskAivSetFlag>(event), lane);
    TaskAivWaitFlag *wait2 =
        AddNode(std::make_unique<TaskAivWaitFlag>(event), lane);
    AddEdge(start, set1);
    AddEdge(set1, wait1);
    AddEdge(wait1, set2);
...
TEST_F(SyncConflictTest, AivEventWaitWithoutMatchingPostIsRejected) {
    TaskStart *start = AddMainStart();
    TaskAivSetFlag *set =
        AddNode(std::make_unique<TaskAivSetFlag>(MakeEvent(1)));
    TaskAivWaitFlag *wait =
        AddNode(std::make_unique<TaskAivWaitFlag>(MakeEvent(2)));
    AddEdge(start, set);
    AddEdge(set, wait);
...
TEST_F(SyncConflictTest, AivEventFieldsCreateIndependentBuckets) {
    const std::vector<AivPipeEvent> resources{
        MakeEvent(1, 0, 1, 2, 3, 4), MakeEvent(1, 1, 1, 2, 3, 4),
        MakeEvent(1, 0, 2, 2, 3, 4), MakeEvent(1, 0, 1, 3, 3, 4),
        MakeEvent(1, 0, 1, 2, 4, 4), MakeEvent(1, 0, 1, 2, 3, 5),
        MakeEvent(2, 0, 1, 2, 3, 4),
    };

    TaskStart *start = AddMainStart();
    for (const AivPipeEvent &event : resources) {
        TaskAivSetFlag *setFlag =
            AddNode(std::make_unique<TaskAivSetFlag>(event));
        TaskAivWaitFlag *waitFlag =
            AddNode(std::make_unique<TaskAivWaitFlag>(event));
        AddEdge(start, setFlag);
        AddEdge(setFlag, waitFlag);
...
TEST_F(SyncConflictTest, AivFlagOrderedReuseAcrossMultiplePairsIsAllowed) {
    ...
    TaskAivSendFlag *send1 =
        AddNode(std::make_unique<TaskAivSendFlag>(flag), lane);
    TaskAivRecvFlag *recv1 =
        AddNode(std::make_unique<TaskAivRecvFlag>(flag), lane);
    TaskAivSendFlag *send2 =
        AddNode(std::make_unique<TaskAivSendFlag>(flag), lane);
    TaskAivRecvFlag *recv2 =
        AddNode(std::make_unique<TaskAivRecvFlag>(flag), lane);
    AddEdge(start, send1);
    AddEdge(send1, recv1);
    AddEdge(recv1, send2);
...
TEST_F(SyncConflictTest, AivFlagValuesCreateIndependentBuckets) {
    ...
    TaskAivSendFlag *send1 =
        AddNode(std::make_unique<TaskAivSendFlag>(flagValue1));
    TaskAivRecvFlag *recv1 =
        AddNode(std::make_unique<TaskAivRecvFlag>(flagValue1));
    TaskAivSendFlag *send2 =
        AddNode(std::make_unique<TaskAivSendFlag>(flagValue2));
    TaskAivRecvFlag *recv2 =
        AddNode(std::make_unique<TaskAivRecvFlag>(flagValue2));
    AddEdge(start, send1);
    AddEdge(send1, recv1);
    AddEdge(start, send2);
    AddEdge(recv1, send2);
...
TEST_F(SyncConflictTest, AivFlagDifferentValuesAllowConcurrentReceives) {
    ...
    TaskAivSendFlag *send1 =
        AddNode(std::make_unique<TaskAivSendFlag>(firstFlag), producerLane);
    TaskAivRecvFlag *recv1 =
        AddNode(std::make_unique<TaskAivRecvFlag>(firstFlag), firstReceiveLane);
    TaskAivSendFlag *send2 =
        AddNode(std::make_unique<TaskAivSendFlag>(secondFlag), producerLane);
    TaskAivRecvFlag *recv2 = AddNode(
        std::make_unique<TaskAivRecvFlag>(secondFlag), secondReceiveLane);
    AddEdge(start, send1);
    AddEdge(send1, recv1);
    AddEdge(recv1, send2);
    AddEdge(send2, recv2);
...
TEST_F(SyncConflictTest, AivFlagDifferentValuesStillRequireProducerOrdering) {
    ...
    TaskAivSendFlag *send1 =
        AddNode(std::make_unique<TaskAivSendFlag>(firstFlag), firstLane);
    TaskAivRecvFlag *recv1 =
        AddNode(std::make_unique<TaskAivRecvFlag>(firstFlag), firstLane);
    TaskAivSendFlag *send2 =
        AddNode(std::make_unique<TaskAivSendFlag>(secondFlag), firstLane);
    TaskAivRecvFlag *recv2 = AddNode(
        std::make_unique<TaskAivRecvFlag>(secondFlag), secondReceiveLane);
    AddEdge(start, send1);
    AddEdge(send1, recv1);
...
TEST_F(SyncConflictTest,
       AivFlagFanOutGroupsRequireAllPreviousReceivesToReachNextGroup) {
    TaskStart *start = AddMainStart();
    ...
    TaskAivSendFlag *send2 =
        AddNode(std::make_unique<TaskAivSendFlag>(flag), lane);
    TaskAivRecvFlag *recv3 =
        AddNode(std::make_unique<TaskAivRecvFlag>(flag), lane);
    TaskAivRecvFlag *recv4 =
        AddNode(std::make_unique<TaskAivRecvFlag>(flag), lane);
    AddEdge(start, send1);
    AddEdge(send1, recv1);
    AddEdge(send1, recv2);
...
TEST_F(SyncConflictTest,
       AivFlagFanOutGroupsRejectMissingPreviousReceiveOrdering) {
    TaskStart *start = AddMainStart();
    const AivFlagSync flag = MakeFlag(0xa000, 0, 1, 7);
    const TaskPosition lane = MakeLane(0);
    TaskAivSendFlag *send1 =
...
TEST_F(SyncConflictTest, UnmatchedAivPostAfterWaitIsAllowed) {
    ...
    TaskAivSetFlag *unmatchedSet =
        AddNode(std::make_unique<TaskAivSetFlag>(event));
    TaskAivSetFlag *secondUnmatchedSet =
        AddNode(std::make_unique<TaskAivSetFlag>(event));
    AddEdge(start, firstSet);
    AddEdge(firstSet, wait);
    ...
    for (size_t round = 0; round < roundCount; ++round) {
        ...
        for (size_t lane = 0; lane < laneCount; ++lane) {
            ...
            TaskRecordAICPU *post =
                AddRecord(static_cast<uint32_t>(lane + 1U), position);
            TaskWaitAICPU *wait =
                AddWait(static_cast<uint32_t>(lane + 1U), position);
