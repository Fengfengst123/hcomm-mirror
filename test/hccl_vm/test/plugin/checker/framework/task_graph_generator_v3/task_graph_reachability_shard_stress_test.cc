...
TEST_F(ShardReachabilityStressTest, RandomDagShardMatchesDense) {
    for (uint32_t round = 0; round < 200; ++round) {
        ...
        ASSERT_EQ(builder.BuildClosureForNodes(allIds, shardedClosure),
                  HCCL_SUCCESS)
            << "round=" << round;
        ...
        for (const NodeId fromId : allIds) {
            for (const NodeId toId : allIds) {
                ...
                ASSERT_EQ(
                    IsReachable(denseClosure, fromId, toId, denseReachable),
                    HCCL_SUCCESS);
                ASSERT_EQ(
                    IsReachable(shardedClosure, fromId, toId, shardedReachable),
                    HCCL_SUCCESS);
                ASSERT_EQ(shardedReachable, denseReachable)
                    << "round=" << round << ", from=" << fromId
                    << ", to=" << toId;
            }
        }
        ...
        for (uint32_t shardRound = 0; shardRound < 5; ++shardRound) {
            ...
            ASSERT_EQ(builder.BuildClosureForNodes(subsetIds, subsetClosure),
                      HCCL_SUCCESS)
...
TEST_F(ShardReachabilityStressTest, OrphanEdgeIntoReachableSet) {
    ...
    const std::vector<NodeId> closureIds = {taskA->GetNodeId(),
                                            taskB->GetNodeId()};
...
TEST_F(ShardReachabilityStressTest, ChildOfStartAndRegularNode) {
    ...
    EXPECT_EQ(builder.Init(start_), HCCL_SUCCESS);
    ReachabilityClosure closure;
    EXPECT_EQ(builder.BuildClosureForNodes(
                  {taskA->GetNodeId(), taskB->GetNodeId(), taskC->GetNodeId()},
                  closure),
              HCCL_SUCCESS);
    bool reachable = false;
    EXPECT_EQ(
        IsReachable(closure, taskA->GetNodeId(), taskC->GetNodeId(), reachable),
        HCCL_SUCCESS);
...
TEST_F(ShardReachabilityStressTest, DeepChainHighIndegreeWideFanout) {
    ...
    for (size_t i = 0; i < chainLen; ++i) {
        chain.push_back(
            AddTransMem(0U, static_cast<uint64_t>(i) * 0x100U, 0x100U));
    }
