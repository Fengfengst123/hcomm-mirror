...
extern std::map<RankId, std::map<u32, HcclSim::ChannelsPerDie>>
...
constexpr uint64_t AIV_COMM_INFO_SYNC_CELL_BYTES =
    AivCommInfoLayout::SYNC_CELL_BYTES;

uint64_t AivCommInfoFlagOffset(uint64_t cellIndex) {
    return AIV_COMM_INFO_FLAG1_OFFSET +
           cellIndex * AIV_COMM_INFO_SYNC_CELL_BYTES;
}
...
class TempAivInstallDir {
  public:
    ...
    std::string TaskJsonPath(uint32_t rank, uint64_t launch) const {
        return dataDir_ + "/hcclvm_aiv_device" + std::to_string(rank) +
               "_launch" + std::to_string(launch) + "_task.json";
    }
...
Json Slice(uint32_t bufferType, uint32_t deviceId, uint64_t offset,
           uint64_t size) {
    return Json{{"bufferType", bufferType},
                {"deviceId", deviceId},
                {"offset", offset},
...
Json RuntimeTask(uint32_t taskType, const std::string &name, uint32_t taskId,
                 uint32_t rank, uint32_t block, uint32_t pipe, Json payload) {
...
Json MemCopy(uint32_t taskId, uint32_t rank, uint32_t block, uint32_t pipe,
             uint32_t srcRank, uint32_t dstRank, uint32_t srcType,
             uint64_t srcOffset, uint32_t dstType, uint64_t dstOffset,
             uint64_t size = 64) {
    return RuntimeTask(TASK_MEM_COPY, "MemCopy", taskId, rank, block, pipe,
                       Json{{"src", Slice(srcType, srcRank, srcOffset, size)},
                            {"dst", Slice(dstType, dstRank, dstOffset, size)}});
}

Json Reduce(uint32_t taskId, uint32_t rank, uint32_t block, uint32_t pipe,
            uint32_t srcRank, uint32_t dstRank, uint32_t srcType,
            uint64_t srcOffset, uint32_t dstType, uint64_t dstOffset,
            uint64_t size = 64, uint32_t dataType = 0, uint32_t reduceOp = 0) {
...
Json SetFlag(uint32_t taskId, uint32_t rank, uint32_t block, uint32_t pipe,
             uint32_t srcPipe, uint32_t dstPipe, int32_t eventId) {
    return RuntimeTask(
        TASK_SET_FLAG, "SetFlag", taskId, rank, block, pipe,
        Json{{"srcPipe", srcPipe}, {"dstPipe", dstPipe}, {"eventId", eventId}});
}

Json WaitFlag(uint32_t taskId, uint32_t rank, uint32_t block, uint32_t pipe,
              uint32_t srcPipe, uint32_t dstPipe, int32_t eventId) {
    return RuntimeTask(
        TASK_WAIT_FLAG, "WaitFlag", taskId, rank, block, pipe,
        Json{{"srcPipe", srcPipe}, {"dstPipe", dstPipe}, {"eventId", eventId}});
}
...
Json NormalTask(uint32_t taskId, uint32_t rank, uint32_t block, uint32_t pipe) {
    const uint64_t base = 0x20000ULL + static_cast<uint64_t>(rank) * 0x4000ULL +
                          static_cast<uint64_t>(block) * 0x400ULL +
                          static_cast<uint64_t>(pipe) * 0x80ULL;
    return MemCopy(taskId, rank, block, pipe, rank, rank, BUFFER_UB,
                   base + 0x2000ULL, BUFFER_UB, base, 32);
}

Json PipeBarrier(uint32_t taskId, uint32_t rank, uint32_t block, uint32_t pipe,
                 const std::vector<uint32_t> &members) {
    return RuntimeTask(
        TASK_PIPE_BARRIER, "PipeBarrier", taskId, rank, block, pipe,
        Json{{"pipeType", PIPE_ALL}, {"barrierGroupTaskIds", members}});
}

Json SyncAll(uint32_t taskId, uint32_t rank, uint32_t block, uint32_t pipe,
             uint32_t round) {
    return RuntimeTask(TASK_SYNC_ALL, "SyncAll", taskId, rank, block, pipe,
                       Json{{"syncRound", round}});
}

Json SendFlag(uint32_t taskId, uint32_t rank, uint32_t block, uint32_t pipe,
              uint32_t ownerRank, uint64_t offset, int32_t value) {
    return RuntimeTask(
        TASK_SEND_FLAG, "SendFlag", taskId, rank, block, pipe,
        Json{{"targetRank", ownerRank},
             {"flagBuffer",
              Slice(BUFFER_AIV_COMM_INFO, ownerRank, offset, sizeof(int32_t))},
             {"flagValue", value}});
}

Json RecvFlag(uint32_t taskId, uint32_t rank, uint32_t block, uint32_t pipe,
              uint32_t ownerRank, uint64_t offset, int32_t value) {
    return RuntimeTask(
...
Json Block(uint32_t blockIdx, std::vector<Json> pipe0, std::vector<Json> pipe1,
...
Json Snapshot(uint32_t rank, uint32_t rankSize, uint64_t launch,
...
Json NormalSnapshot(uint32_t rank, uint32_t rankSize, uint64_t launch,
                    uint32_t blockCount = 1) {
    std::vector<Json> blocks;
    for (uint32_t block = 0; block < blockCount; ++block) {
        const uint32_t base = 1000U + block * 10U;
        blocks.push_back(Block(block, {NormalTask(base + 0U, rank, block, 0)},
                               {NormalTask(base + 1U, rank, block, 1)},
                               {NormalTask(base + 2U, rank, block, 2)}));
    }
    return Snapshot(rank, rankSize, launch, std::move(blocks));
}

Json PipeBarrierSnapshot(uint32_t rank, uint32_t rankSize, uint64_t launch,
                         uint32_t rounds = 1) {
    ...
    for (uint32_t round = 0; round < rounds; ++round) {
        ...
        pipe0.push_back(
            PipeBarrier(base + 0U, rank, 0, 0, {base + 1U, base + 2U}));
        pipe1.push_back(
            PipeBarrier(base + 1U, rank, 0, 1, {base + 0U, base + 2U}));
        pipe2.push_back(
            PipeBarrier(base + 2U, rank, 0, 2, {base + 0U, base + 1U}));
    }
    return Snapshot(
        rank, rankSize, launch,
        {Block(0, std::move(pipe0), std::move(pipe1), std::move(pipe2))});
}

Json PipeBarrierMissingMemberSnapshot(uint32_t rank, uint32_t rankSize,
                                      uint64_t launch) {
...
Json SyncAllSnapshot(uint32_t rank, uint32_t rankSize, uint64_t launch,
                     uint32_t rounds = 1) {
    std::vector<Json> blocks;
    for (uint32_t block = 0; block < 2; ++block) {
        ...
        blocks.push_back(
            Block(block, std::move(pipe0), std::move(pipe1), std::move(pipe2)));
    }
    return Snapshot(rank, rankSize, launch, std::move(blocks));
}

Json SyncAllMissingMemberSnapshot(uint32_t rank, uint32_t rankSize,
                                  uint64_t launch) {
    std::vector<Json> blocks;
    blocks.push_back(Block(0, {SyncAll(200, rank, 0, 0, 0)},
                           {SyncAll(201, rank, 0, 1, 0)},
                           {NormalTask(202, rank, 0, 2)}));
    blocks.push_back(Block(1, {SyncAll(210, rank, 1, 0, 0)},
                           {SyncAll(211, rank, 1, 1, 0)},
...
Json CombinationSnapshot(uint32_t rank, uint32_t rankSize, uint64_t launch) {
    ...
    blocks.push_back(
        Block(0,
              {PipeBarrier(100, rank, 0, 0, {101, 102}),
               SyncAll(200, rank, 0, 0, 0), NormalTask(300, rank, 0, 0)},
              {PipeBarrier(101, rank, 0, 1, {100, 102}),
               SyncAll(201, rank, 0, 1, 0), NormalTask(301, rank, 0, 1)},
              {PipeBarrier(102, rank, 0, 2, {100, 101}),
               SyncAll(202, rank, 0, 2, 0), NormalTask(302, rank, 0, 2)}));
    blocks.push_back(
        Block(1, {SyncAll(210, rank, 1, 0, 0), NormalTask(310, rank, 1, 0)},
...
Json UbSnapshot(uint32_t rank, uint32_t rankSize, uint64_t launch,
                UbMode mode) {
    if (mode == UbMode::PARALLEL_CONFLICT) {
        return Snapshot(rank, rankSize, launch,
                        {Block(0, {NormalTask(10, rank, 0, 0)},
                               {MemCopy(20, rank, 0, 1, rank, rank, BUFFER_UB,
                                        0x4000, BUFFER_UB, 0x1000)},
                               {MemCopy(21, rank, 0, 2, rank, rank, BUFFER_UB,
                                        0x5000, BUFFER_UB, 0x1000)})});
    }
    ...
    if (mode == UbMode::ORDERED_OVERLAP) {
        return Snapshot(rank, rankSize, launch,
                        {Block(0, {NormalTask(10, rank, 0, 0)},
                               {MemCopy(20, rank, 0, 1, rank, rank, BUFFER_UB,
                                        0x4000, BUFFER_UB, 0x1000),
                                MemCopy(21, rank, 0, 1, rank, rank, BUFFER_UB,
                                        0x5000, BUFFER_UB, 0x1000)},
...
Json CclSnapshot(uint32_t rank, uint32_t rankSize, uint64_t launch,
                 CclMode mode) {
    if (mode == CclMode::WRITER_LOW) {
        return Snapshot(rank, rankSize, launch,
                        {Block(0, {NormalTask(10, rank, 0, 0)},
                               {MemCopy(20, rank, 0, 1, rank, 0, BUFFER_UB,
                                        0x4000, BUFFER_CCL, 0x1000)},
                               {NormalTask(12, rank, 0, 2)})});
    }
...
Json SendRecvSnapshot(uint32_t rank, uint32_t rankSize, uint64_t launch) {
    if (rank == 0) {
        return Snapshot(
            rank, rankSize, launch,
            {Block(0,
                   {SendFlag(50, rank, 0, 0, 1, AivCommInfoFlagOffset(1), 1),
    ...
    return Snapshot(
        rank, rankSize, launch,
        {Block(0,
               {RecvFlag(50 + rank, rank, 0, 0, rank,
                         AivCommInfoFlagOffset(rank), 1)},
               {NormalTask(11, rank, 0, 1)}, {NormalTask(12, rank, 0, 2)})});
}

Json SendRecvSnapshotWithUnalignedFlagVirtualAddr(uint32_t rank,
                                                  uint32_t rankSize,
                                                  uint64_t launch) {
        ...
        for (const char *pipeName : {"scalarTasks", "mte2Tasks", "mte3Tasks"}) {
            for (Json &task : block[pipeName]) {
                ...
                if (payload.contains("flagBuffer")) {
                    ...
                    flagBuffer["virtualAddr"] =
                        0x1000001ULL + flagBuffer["offset"].get<uint64_t>();
                }
            }
        }
    }
...
Json CpGm2GMSnapshot(uint32_t rank, uint32_t rankSize, uint64_t launch,
                     uint32_t iterationCount, bool reduceOut,
    ...
    const uint64_t externalOffsetBase =
        flagMem ? AIV_COMM_INFO_FLAG1_OFFSET : 0;
    for (uint32_t index = 0; index < iterationCount; ++index) {
        ...
        const uint64_t offset = externalOffsetBase +
                                static_cast<uint64_t>(index) *
                                    (sliceSize + (externalGap ? sliceSize : 0));
        mte2.push_back(MemCopy(base + 0, rank, 0, PIPE_MTE2, rank, rank,
                               extSrcType, offset, BUFFER_UB, 0, sliceSize));
        mte2.push_back(
            SetFlag(base + 1, rank, 0, PIPE_MTE2, PIPE_MTE2, PIPE_MTE3, 0));
        mte2.push_back(
            WaitFlag(base + 5, rank, 0, PIPE_MTE2, PIPE_MTE3, PIPE_MTE2, 1));

        mte3.push_back(
            WaitFlag(base + 2, rank, 0, PIPE_MTE3, PIPE_MTE2, PIPE_MTE3, 0));
        if (reduceOut) {
            mte3.push_back(Reduce(base + 3, rank, 0, PIPE_MTE3, rank, rank,
                                  BUFFER_UB, 0, extDstType, offset, sliceSize));
        } else {
            mte3.push_back(MemCopy(base + 3, rank, 0, PIPE_MTE3, rank, rank,
                                   BUFFER_UB, 0, extDstType, offset,
                                   sliceSize));
        }
        mte3.push_back(
            SetFlag(base + 4, rank, 0, PIPE_MTE3, PIPE_MTE3, PIPE_MTE2, 1));
    }
    return Snapshot(rank, rankSize, launch,
                    {Block(0, {}, std::move(mte2), std::move(mte3))});
}

Json CpGm2GMCrossDeviceSnapshot(uint32_t rank, uint32_t rankSize,
                                uint64_t launch) {
...
Json CpGm2GMVirtualAddrGapSnapshot(uint32_t rank, uint32_t rankSize,
                                   uint64_t launch) {
    Json snapshot = CpGm2GMSnapshot(rank, rankSize, launch, 2, false);
    Json &mte2Copy = snapshot["aivCores"][0]["mte2Tasks"][3]["payload"]["src"];
    Json &mte3Copy = snapshot["aivCores"][0]["mte3Tasks"][4]["payload"]["dst"];
    ...
    mte2Copy["virtualAddr"] =
        mte2Copy["virtualAddr"].get<uint64_t>() + 0x1000ULL;
    mte3Copy["virtualAddr"] =
        mte3Copy["virtualAddr"].get<uint64_t>() + 0x1000ULL;
    return snapshot;
}

Json CpGm2GMPipeBoundarySnapshot(uint32_t rank, uint32_t rankSize,
                                 uint64_t launch) {
    ...
    for (uint32_t index = 0; index < 2; ++index) {
        ...
        mte2.push_back(MemCopy(base + 0, rank, 0, PIPE_MTE2, rank, rank,
                               BUFFER_INPUT, offset, BUFFER_UB, 0, sliceSize));
        mte2.push_back(
            SetFlag(base + 1, rank, 0, PIPE_MTE2, PIPE_MTE2, PIPE_MTE3, 0));
        mte2.push_back(
            WaitFlag(base + 5, rank, 0, PIPE_MTE2, PIPE_MTE3, PIPE_MTE2, 1));
        ...
        mte3.push_back(
            WaitFlag(base + 2, rank, 0, PIPE_MTE3, PIPE_MTE2, PIPE_MTE3, 0));
        mte3.push_back(MemCopy(base + 3, rank, 0, PIPE_MTE3, rank, rank,
                               BUFFER_UB, 0, BUFFER_OUTPUT, offset, sliceSize));
        mte3.push_back(
            SetFlag(base + 4, rank, 0, PIPE_MTE3, PIPE_MTE3, PIPE_MTE2, 1));
    }
...
RunResult RunAivCase(const CaseConfig &config, bool runMemConflict = false) {
    ...
    for (uint32_t rank = 0; rank < config.rankSize; ++rank) {
        for (uint64_t launch : config.launches) {
            ...
            WriteJson(install->TaskJsonPath(rank, launch),
                      config.snapshotFactory(rank, launch));
        }
    }
    ...
    if (result.genRet == HCCL_SUCCESS && runMemConflict) {
        result.memRet = V3::CheckDataMoveTaskMemConflict(
            result.graph->GetMainStartNode(), &result.memStats);
    }
    HcclSim::StorageManager::GetInstance().Reset();
    return result;
}
...
size_t CountMergedPipeBarrier(const V3::TaskGraphGeneratorV3 &graph) {
    ...
    for (const auto &node : graph.GetNodes()) {
        if (node == nullptr ||
            node->GetType() != V3::TaskType::AIV_PIPE_BARRIER) {
            continue;
        }
        const auto *barrier =
            dynamic_cast<const V3::TaskAivPipeBarrier *>(node.get());
...
size_t CountMergedSyncAll(const V3::TaskGraphGeneratorV3 &graph,
                          size_t expectedMembers) {
    size_t count = 0;
    for (const auto &node : graph.GetNodes()) {
        ...
        const auto *syncAll =
            dynamic_cast<const V3::TaskAivSyncAll *>(node.get());
...
size_t CountDirectEdges(const V3::TaskGraphGeneratorV3 &graph,
                        V3::TaskType parentType, V3::TaskType childType) {
    size_t count = 0;
    for (const auto &node : graph.GetNodes()) {
...
std::vector<const V3::TaskNode *>
CollectNodesByType(const V3::TaskGraphGeneratorV3 &graph, V3::TaskType type) {
...
const V3::TaskNode *FindNodeByTaskId(const V3::TaskGraphGeneratorV3 &graph,
                                     uint32_t rank, uint32_t pipe,
                                     uint32_t taskId) {
    for (const auto &node : graph.GetNodes()) {
        if (node == nullptr) {
            continue;
        ...
        if (position.rankId == rank && position.launchIdx == 0 &&
            position.blockId == 0 && position.pipe == pipe &&
...
size_t CountLinkedNodesByTypeAndPipe(const std::vector<V3::TaskNode *> &nodes,
                                     V3::TaskType type, uint32_t pipe,
                                     uint32_t minTaskId) {
    size_t count = 0;
    for (const V3::TaskNode *node : nodes) {
        ...
        const V3::TaskPosition &position = node->GetPosition();
        if (node->GetType() == type && position.pipe == pipe &&
            position.taskId > minTaskId) {
            ++count;
        }
    }
    return count;
...
TEST_F(AivStTest, AIV_ST_2P_001_PipeBarrierMergePositive) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_001", 2, {0}, [](uint32_t rank, uint64_t launch) {
             return PipeBarrierSnapshot(rank, 2, launch);
         }});
...
TEST_F(AivStTest, AIV_ST_2P_002_PipeBarrierMissingMemberNegative) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_002", 2, {0}, [](uint32_t rank, uint64_t launch) {
             return rank == 0
                        ? PipeBarrierMissingMemberSnapshot(rank, 2, launch)
...
TEST_F(AivStTest, AIV_ST_2P_003_SyncAllMergePositive) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_003", 2, {0}, [](uint32_t rank, uint64_t launch) {
             return SyncAllSnapshot(rank, 2, launch);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_2P_004_SyncAllMissingMemberNegative) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_004", 2, {0}, [](uint32_t rank, uint64_t launch) {
             return rank == 0 ? SyncAllMissingMemberSnapshot(rank, 2, launch)
                              : SyncAllSnapshot(rank, 2, launch);
         }});
    EXPECT_NE(result.genRet, HCCL_SUCCESS);
}
...
TEST_F(AivStTest, AIV_ST_2P_005_PipeBarrierTwoRoundMergePositive) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_005", 2, {0}, [](uint32_t rank, uint64_t launch) {
             return PipeBarrierSnapshot(rank, 2, launch, 2);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_2P_006_SyncAllTwoRoundMergePositive) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_006", 2, {0}, [](uint32_t rank, uint64_t launch) {
             return SyncAllSnapshot(rank, 2, launch, 2);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_2P_007_MultiLaunchPositive) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_007", 2, {0, 1}, [](uint32_t rank, uint64_t launch) {
             return NormalSnapshot(rank, 2, launch);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_2P_008_MultiLaunchMissingFileNegative) {
    CaseConfig config{"AIV_ST_2P_008",
                      2,
                      {0, 1},
...
TEST_F(AivStTest, AIV_ST_2P_009_UbParallelWriteConflict) {
    RunResult result =
        RunAivCase({"AIV_ST_2P_009",
                    2,
                    {0},
                    [](uint32_t rank, uint64_t launch) {
                        return UbSnapshot(rank, 2, launch,
                                          rank == 0 ? UbMode::PARALLEL_CONFLICT
                                                    : UbMode::NORMAL);
                    }},
                   true);
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_2P_010_UbParallelWriteNoConflict) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_010",
         ...
         [](uint32_t rank, uint64_t launch) {
             return UbSnapshot(rank, 2, launch,
                               rank == 0 ? UbMode::PARALLEL_NO_CONFLICT
                                         : UbMode::NORMAL);
         }},
        true);
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_2P_011_UbOrderedOverlapNoConflict) {
    RunResult result =
        RunAivCase({"AIV_ST_2P_011",
                    2,
                    {0},
                    [](uint32_t rank, uint64_t launch) {
                        return UbSnapshot(rank, 2, launch,
                                          rank == 0 ? UbMode::ORDERED_OVERLAP
                                                    : UbMode::NORMAL);
                    }},
                   true);
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_4P_001_PipeBarrierMergePositive) {
    RunResult result = RunAivCase(
        {"AIV_ST_4P_001", 4, {0}, [](uint32_t rank, uint64_t launch) {
             return PipeBarrierSnapshot(rank, 4, launch);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
    EXPECT_EQ(result.graph->GetAivExpandStats().pipeBarrierMergeCount, 4U);
    EXPECT_EQ(CountMergedPipeBarrier(*result.graph), 4U);
...
TEST_F(AivStTest, AIV_ST_4P_002_MultiLaunchPositive) {
    RunResult result = RunAivCase(
        {"AIV_ST_4P_002", 4, {0, 1}, [](uint32_t rank, uint64_t launch) {
             return NormalSnapshot(rank, 4, launch);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_4P_003_SendRecvFlagCrossRankPositive) {
    RunResult result = RunAivCase(
        {"AIV_ST_4P_003", 4, {0}, [](uint32_t rank, uint64_t launch) {
             return SendRecvSnapshot(rank, 4, launch);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
    EXPECT_EQ(result.graph->GetAivExpandStats().sendRecvEdgeCount, 3U);
    EXPECT_EQ(CountDirectEdges(*result.graph, V3::TaskType::AIV_SEND_FLAG,
                               V3::TaskType::AIV_RECV_FLAG),
              3U);
}

TEST_F(AivStTest,
       AIV_ST_4P_003A_SendRecvFlagUsesBufferOffsetNotVirtualAddress) {
    RunResult result = RunAivCase(
        {"AIV_ST_4P_003A", 4, {0}, [](uint32_t rank, uint64_t launch) {
             return SendRecvSnapshotWithUnalignedFlagVirtualAddr(rank, 4,
                                                                 launch);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);

    V3::SingleTaskCheckStats stats;
    EXPECT_EQ(V3::CheckTaskMem(result.graph->GetMainStartNode(), &stats),
              HCCL_SUCCESS);
    EXPECT_EQ(result.graph->GetAivExpandStats().sendRecvEdgeCount, 3U);

    const auto sendFlags =
        CollectNodesByType(*result.graph, V3::TaskType::AIV_SEND_FLAG);
    ...
    for (const V3::TaskNode *node : sendFlags) {
        ...
        EXPECT_EQ(sendFlag->GetFlag().commInfoOffset %
                      AIV_COMM_INFO_SYNC_CELL_BYTES,
                  0U);
...
TEST_F(AivStTest, AIV_ST_4P_004_UbParallelWriteConflict) {
    RunResult result =
        RunAivCase({"AIV_ST_4P_004",
                    4,
                    {0},
                    [](uint32_t rank, uint64_t launch) {
                        return UbSnapshot(rank, 4, launch,
                                          rank == 0 ? UbMode::PARALLEL_CONFLICT
                                                    : UbMode::NORMAL);
                    }},
                   true);
...
TEST_F(AivStTest, AIV_ST_4P_007_CombinationPositive) {
    RunResult result =
        RunAivCase({"AIV_ST_4P_007",
                    4,
                    {0, 1},
                    [](uint32_t rank, uint64_t launch) {
                        return CombinationSnapshot(rank, 4, launch);
                    }},
                   true);
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
    EXPECT_EQ(result.memRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_2P_012_CclCrossRankParallelWriteConflict) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_012",
         ...
         {0},
         [](uint32_t rank, uint64_t launch) {
             return CclSnapshot(rank, 2, launch,
                                rank == 0 ? CclMode::WRITER_LOW
                                          : CclMode::WRITER_SAME_RANGE);
         }},
        true);
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
    EXPECT_EQ(result.memRet, HCCL_E_MEMORY);
    ...
    RunResult result = RunAivCase(
        {"AIV_ST_4P_008",
         ...
         [](uint32_t rank, uint64_t launch) {
             ...
             if (rank == 1) {
                 return CclSnapshot(rank, 4, launch,
                                    CclMode::WRITER_SAME_RANGE);
             }
...
TEST_F(AivStTest, AIV_ST_2P_014_CpGm2GMMemCopyLoopMergePositive) {
    RunResult result =
        RunAivCase({"AIV_ST_2P_014",
                    2,
                    {0},
                    [](uint32_t rank, uint64_t launch) {
                        return CpGm2GMSnapshot(rank, 2, launch, 3, false);
                    }},
                   true);
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
    EXPECT_EQ(result.memRet, HCCL_SUCCESS);
    ...
    EXPECT_EQ(result.graph->GetAivExpandStats().cpGmMergedOriginalNodeCount,
              36U);
    ...
    EXPECT_EQ(
        CollectNodesByType(*result.graph, V3::TaskType::BATCH_TRANS_MEM).size(),
        4U);
}

TEST_F(AivStTest, AIV_ST_2P_015_CpGm2GMReduceLoopMergePositive) {
    RunResult result =
        RunAivCase({"AIV_ST_2P_015",
                    2,
                    {0},
                    [](uint32_t rank, uint64_t launch) {
                        return CpGm2GMSnapshot(rank, 2, launch, 2, true);
                    }},
                   true);
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
    EXPECT_EQ(result.memRet, HCCL_SUCCESS);
    ...
    EXPECT_EQ(
        ...
        2U);
    EXPECT_EQ(
        CollectNodesByType(*result.graph, V3::TaskType::BATCH_REDUCE).size(),
        2U);
}

TEST_F(AivStTest, AIV_ST_2P_016_CpGm2GMSingleIterationNoMerge) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_016", 2, {0}, [](uint32_t rank, uint64_t launch) {
             return CpGm2GMSnapshot(rank, 2, launch, 1, false);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
...
TEST_F(AivStTest, AIV_ST_2P_017_CpGm2GMFlagMemLoopMergePositive) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_017", 2, {0}, [](uint32_t rank, uint64_t launch) {
             return CpGm2GMSnapshot(rank, 2, launch, 3, false, true);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
    EXPECT_EQ(result.graph->GetAivExpandStats().cpGmLoopMergeCount, 2U);
    EXPECT_EQ(result.graph->GetAivExpandStats().cpGmMergedIterationCount, 6U);
...
TEST_F(AivStTest, AIV_ST_2P_018_CpGm2GMExternalGapNoMerge) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_018", 2, {0}, [](uint32_t rank, uint64_t launch) {
...
TEST_F(AivStTest, AIV_ST_2P_020_CpGm2GMCrossDeviceNoMerge) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_020", 2, {0}, [](uint32_t rank, uint64_t launch) {
...
TEST_F(AivStTest, AIV_ST_2P_021_CpGm2GMVirtualAddrGapNoMerge) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_021", 2, {0}, [](uint32_t rank, uint64_t launch) {
...
TEST_F(AivStTest, AIV_ST_2P_019_CpGm2GMMte3BoundaryKeepPipe) {
    RunResult result = RunAivCase(
        {"AIV_ST_2P_019", 2, {0}, [](uint32_t rank, uint64_t launch) {
             return CpGm2GMPipeBoundarySnapshot(rank, 2, launch);
         }});
    ASSERT_EQ(result.genRet, HCCL_SUCCESS);
    EXPECT_EQ(result.graph->GetAivExpandStats().cpGmLoopMergeCount, 2U);
    ...
    for (uint32_t rank = 0; rank < 2; ++rank) {
        const V3::TaskNode *mte3Before =
            FindNodeByTaskId(*result.graph, rank, PIPE_MTE3, 91);
        const V3::TaskNode *mte3After =
            FindNodeByTaskId(*result.graph, rank, PIPE_MTE3, 1001);
        ASSERT_NE(mte3Before, nullptr);
        ASSERT_NE(mte3After, nullptr);
        EXPECT_EQ(CountLinkedNodesByTypeAndPipe(mte3Before->GetChildren(),
                                                V3::TaskType::AIV_WAIT_FLAG,
                                                PIPE_MTE3, minSyntheticTaskId),
                  1U);
        EXPECT_EQ(CountLinkedNodesByTypeAndPipe(mte3Before->GetChildren(),
                                                V3::TaskType::BATCH_TRANS_MEM,
                                                PIPE_MTE2, minSyntheticTaskId),
                  0U);
        EXPECT_EQ(CountLinkedNodesByTypeAndPipe(mte3After->GetParents(),
                                                V3::TaskType::AIV_SET_FLAG,
                                                PIPE_MTE3, minSyntheticTaskId),
                  1U);
        EXPECT_EQ(CountLinkedNodesByTypeAndPipe(mte3After->GetParents(),
                                                V3::TaskType::AIV_WAIT_FLAG,
