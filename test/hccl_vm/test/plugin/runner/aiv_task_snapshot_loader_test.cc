  ...
  protected:
    void SetUp() override {
        testDir_ = fs::temp_directory_path() /
                   ("aiv_test_" + std::to_string(::getpid()));
    ...
    std::string WriteTaskFile(uint32_t rankId, uint32_t launchIndex,
                              const json &content) {
        std::string fileName = "hcclvm_aiv_device" + std::to_string(rankId) +
                               "_launch" + std::to_string(launchIndex) +
                               "_task.json";
    ...
    static json MakeDataSlice(uint32_t bufferType, uint64_t offset,
                              uint64_t size) {
    ...
    static json MakeSetFlagPayload(uint32_t srcPipe, uint32_t dstPipe,
                                   int32_t eventId) {
        return {
            {"srcPipe", srcPipe}, {"dstPipe", dstPipe}, {"eventId", eventId}};
    }

    static json MakeWaitFlagPayload(uint32_t srcPipe, uint32_t dstPipe,
                                    int32_t eventId) {
        return {
            {"srcPipe", srcPipe}, {"dstPipe", dstPipe}, {"eventId", eventId}};
    }
    ...
    static json
    MakePipeBarrierPayload(uint32_t pipeType,
                           const std::vector<uint32_t> &barrierGroupTaskIds) {
        return {{"pipeType", pipeType},
                {"barrierGroupTaskIds", barrierGroupTaskIds}};
    }

    static json MakeSendFlagPayload(uint32_t rank, uint64_t commInfoOffset,
                                    int32_t flagValue) {
        return {
            {"targetRank", rank},
    ...
    static json MakeRecvFlagPayload(uint32_t rank, uint64_t commInfoOffset,
                                    int32_t targetValue) {
        return {
            {"targetRank", rank},
            {"flagBuffer", MakeDataSlice(4, commInfoOffset, sizeof(int32_t))},
    ...
    static json MakeTaskJson(uint32_t taskType, uint32_t taskId,
                             uint32_t rankId, uint32_t blockId,
                             uint32_t curPipe, const json &payload) {
        return {{"taskType", taskType}, {"taskId", taskId},
                {"rankId", rankId},     {"blockId", blockId},
                {"curPipe", curPipe},   {"payload", payload}};
    }
    ...
    static json MakeBlockJson(uint32_t blockIdx,
        ...
        return {{"blockIdx", blockIdx},
                {"scalarTasks", scalarTasks},
                {"mte2Tasks", mte2Tasks},
                {"mte3Tasks", mte3Tasks}};
    }
...
TEST_F(AivTaskSnapshotLoaderTest,
       IsAivExpansionModeEnabled_EnvNotSet_ReturnsFalse) {
    unsetenv("HCCL_OP_EXPANSION_MODE");
    EXPECT_FALSE(IsAivExpansionModeEnabled());
}

TEST_F(AivTaskSnapshotLoaderTest,
       IsAivExpansionModeEnabled_EnvAiv_ReturnsTrue) {
...
TEST_F(AivTaskSnapshotLoaderTest,
       IsAivExpansionModeEnabled_EnvOther_ReturnsFalse) {
...
TEST_F(AivTaskSnapshotLoaderTest,
       IsAivExpansionModeEnabled_EnvEmpty_ReturnsFalse) {
...
TEST_F(AivTaskSnapshotLoaderTest, LoadByLaunchDirect_MemCopy_Success) {
    ...
    auto block =
        MakeBlockJson(0, json::array({task}), json::array(), json::array());
    json content = {{"rank", 0},
                    {"rankSize", 4},
                    {"launchIndex", 0},
    ...
    std::string errorMsg;
    EXPECT_TRUE(
        AivTaskSnapshotLoader::LoadRuntimeTasks(0, 0, snapshot, &errorMsg));
    ASSERT_EQ(snapshot.blocks.size(), 1u);
    ASSERT_EQ(snapshot.blocks[0].scalarTasks.size(), 1u);
    EXPECT_EQ(snapshot.blocks[0].scalarTasks[0]->GetTaskType(),
              AivSim::AivTaskType::MEM_COPY);
}
...
TEST_F(AivTaskSnapshotLoaderTest, LoadByLaunchDirect_Reduce_Success) {
    ...
    auto block =
        MakeBlockJson(0, json::array(), json::array({task}), json::array());
    json content = {{"rank", 1},
                    {"rankSize", 2},
                    {"launchIndex", 1},
                    {"aivCores", json::array({block})}};
    ...
    std::string errorMsg;
    EXPECT_TRUE(
        AivTaskSnapshotLoader::LoadRuntimeTasks(1, 1, snapshot, &errorMsg));
    ASSERT_EQ(snapshot.blocks.size(), 1u);
    ASSERT_EQ(snapshot.blocks[0].mte2Tasks.size(), 1u);
    EXPECT_EQ(snapshot.blocks[0].mte2Tasks[0]->GetTaskType(),
              AivSim::AivTaskType::REDUCE);
}
...
TEST_F(AivTaskSnapshotLoaderTest, LoadByLaunchDirect_PipeBarrier_Success) {
    ...
    ASSERT_EQ(snapshot.blocks[0].scalarTasks.size(), 1u);
    EXPECT_EQ(snapshot.blocks[0].scalarTasks[0]->GetTaskType(),
              AivSim::AivTaskType::PIPE_BARRIER);
}
...
TEST_F(AivTaskSnapshotLoaderTest, LoadByLaunchDirect_RecvFlag_Success) {
    ...
    auto block =
        MakeBlockJson(1, json::array(), json::array(), json::array({task}));
    json content = {{"rank", 0},
                    {"rankSize", 3},
                    {"launchIndex", 2},
                    {"aivCores", json::array({block})}};
    ...
    EXPECT_EQ(snapshot.blocks[0].mte3Tasks[0]->GetTaskType(),
              AivSim::AivTaskType::RECV_FLAG);
}

TEST_F(AivTaskSnapshotLoaderTest, LoadByLaunchDirect_AllTaskTypes_Success) {
    auto ds = MakeDataSlice(0, 0, 64);
    auto memCopy =
        MakeTaskJson(0, 1, 0, 0, 0, MakeMemCopyPayload(0, 1, ds, ds));
    ...
    auto block = MakeBlockJson(
        0,
        json::array({memCopy, setFlag, waitFlag, barrier, sendFlag, recvFlag}),
...
TEST_F(AivTaskSnapshotLoaderTest, LoadByLaunchDirect_MultiBlock_Success) {
    auto task = MakeTaskJson(0, 1, 0, 0, 0,
                             MakeMemCopyPayload(0, 1, MakeDataSlice(0, 0, 8),
                                                MakeDataSlice(1, 0, 8)));
    auto block0 =
        MakeBlockJson(0, json::array({task}), json::array(), json::array());
    auto block1 =
        MakeBlockJson(1, json::array(), json::array({task}), json::array());
    auto block2 =
        MakeBlockJson(2, json::array(), json::array(), json::array({task}));
    json content = {{"rank", 0},
                    {"rankSize", 1},
                    {"launchIndex", 0},
...
TEST_F(AivTaskSnapshotLoaderTest, LoadByLaunchDirect_BarrierGroup_Success) {
    auto barrier1 =
        MakeTaskJson(4, 10, 0, 0, 0, MakePipeBarrierPayload(0, {11}));
    auto barrier2 =
        MakeTaskJson(4, 11, 0, 0, 0, MakePipeBarrierPayload(0, {10}));
...
TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_ErrorNullPointer_Succeeds) {
    auto task = MakeTaskJson(0, 1, 0, 0, 0,
                             MakeMemCopyPayload(0, 1, MakeDataSlice(0, 0, 8),
                                                MakeDataSlice(1, 0, 8)));
    json content = {
    ...
    AivRuntimeTaskSnapshot snapshot;
    EXPECT_TRUE(
        AivTaskSnapshotLoader::LoadRuntimeTasks(0, 0, snapshot, nullptr));
}
...
TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_FileNotExist_ReturnsFalse) {
    AivRuntimeTaskSnapshot snapshot;
    std::string errorMsg;
    EXPECT_FALSE(
        AivTaskSnapshotLoader::LoadRuntimeTasks(99, 0, snapshot, &errorMsg));
    EXPECT_FALSE(errorMsg.empty());
}

TEST_F(AivTaskSnapshotLoaderTest, LoadByLaunchDirect_InvalidJson_ReturnsFalse) {
    std::ofstream ofs(testDir_ / "data" /
                      "hcclvm_aiv_device0_launch0_task.json");
    ofs << "not valid json";
    ofs.close();
...
TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_MissingAivCores_ReturnsFalse) {
...
TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_AivCoresNotArray_ReturnsFalse) {
    json content = {{"rank", 0},
                    {"rankSize", 1},
                    {"launchIndex", 0},
                    {"aivCores", "not_array"}};
...
TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_TaskArrayNotArray_ReturnsFalse) {
...
TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_UnsupportedTaskType_ReturnsFalse) {
    ...
    EXPECT_FALSE(
        AivTaskSnapshotLoader::LoadRuntimeTasks(0, 0, snapshot, &errorMsg));
    EXPECT_TRUE(errorMsg.find("unsupported AIV runtime task type") !=
                std::string::npos);
}

TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_MemCopyMissingSrcSlice_ReturnsFalse) {
...
TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_ReduceMissingDstSlice_ReturnsFalse) {
...
TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_BarrierGroupUnresolvable_ReturnsFalse) {
    // barrier1 references task 999 which doesn't exist
    auto barrier1 =
        MakeTaskJson(4, 10, 0, 0, 0, MakePipeBarrierPayload(0, {999}));
    json content = {
        {"rank", 0},
        {"rankSize", 1},
        {"launchIndex", 0},
    ...
    EXPECT_FALSE(
        AivTaskSnapshotLoader::LoadRuntimeTasks(0, 0, snapshot, &errorMsg));
    EXPECT_TRUE(errorMsg.find("failed to resolve barrier group taskId") !=
                std::string::npos);
}
...
TEST_F(AivTaskSnapshotLoaderTest,
       LoadByLaunchDirect_TaskDefaultValues_Success) {
