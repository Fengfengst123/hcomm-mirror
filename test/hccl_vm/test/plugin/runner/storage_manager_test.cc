...
class StorageManagerTest : public testing::Test {
  protected:
    ...
    std::string CreateTempDir() {
        auto dir =
            std::filesystem::temp_directory_path() / "storage_mgr_test_XXXXXX";
    ...
    void SetupSynDataForTrans() {
        ...
        mgr.m_synData.model_info.comm.chip_type =
            static_cast<uint16_t>(DevType::DEV_TYPE_950);
    ...
    bool WriteSynDataFile(const std::string &dir, const std::string &dataId) {
        char fileName[256];
        snprintf(fileName, sizeof(fileName), "/%s_hcclvm_syn_data.bin",
                 dataId.c_str());
        ...
        StorageManager::GetInstance().m_synData.header.magic =
            HCCLVM_SYN_FILE_MAGIC;
        StorageManager::GetInstance().m_synData.header.version = 1;
        StorageManager::GetInstance().m_synData.header.count = 0;
        auto ret =
            HcclVmSynDataWrite(fp, StorageManager::GetInstance().m_synData);
        fclose(fp);
    ...
    bool WriteInstrDataFile(const std::string &dir, const std::string &dataId) {
        char fileName[256];
        snprintf(fileName, sizeof(fileName), "/%s_hcclvm_instr_data.bin",
                 dataId.c_str());
        std::string dataDir = dir + "/data";
        std::filesystem::create_directories(dataDir);
        std::string fullPath = dataDir + fileName;
    ...
    bool WriteTaskDataFile(const std::string &dir, const std::string &dataId) {
        char fileName[256];
        snprintf(fileName, sizeof(fileName), "/%s_hcclvm_task_data.bin",
                 dataId.c_str());
        std::string dataDir = dir + "/data";
        std::filesystem::create_directories(dataDir);
        std::string fullPath = dataDir + fileName;
...
TEST_F(StorageManagerTest, GetAllRankTaskQueues_Default_ReturnsEmpty) {
    AllRankTaskQueues &queues =
        StorageManager::GetInstance().GetAllRankTaskQueues();
...
TEST_F(StorageManagerTest, LoadHcclVmSynthesisData_EmptyDataId_ReturnsError) {
    ...
    auto ret =
        StorageManager::GetInstance().LoadHcclVmSynthesisData(detail, channels);
...
TEST_F(StorageManagerTest, DumpAllRankInputOutput_Disabled_Safe) {
    ...
    EXPECT_NO_THROW(
        StorageManager::GetInstance().DumpAllRankInputOutput(emptyData));
}
...
TEST_F(StorageManagerTest,
       ConvertTaskQueue_ValidTaskMeta_RankSizeZero_ReturnsError) {
    HcclVmTaskMetaData taskMetaData;
