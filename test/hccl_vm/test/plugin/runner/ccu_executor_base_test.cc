...
class MockCcuExecutor : public CcuExecutorBase {
  public:
    MockCcuExecutor(int streamId, int rankId, int dieId, const CcuInstr &instr,
...
class CcuExecutorBaseTest : public testing::Test {
  protected:
    void SetUp() override {
        ...
        simulator_ = std::make_unique<CcuSimulator>(0, 0, 0, 10, 10,
                                                    RunnerCcuVersion::CCU_V1);
        executor_ =
            std::make_unique<MockCcuExecutor>(0, 0, 0, instr, simulator_.get());
    }
...
TEST_F(CcuExecutorBaseTest, ParseMSList) {
    ...
    auto sim = std::make_unique<CcuSimulator>(0, 0, 0, 10, 10,
                                              RunnerCcuVersion::CCU_V1);
...
TEST_F(CcuExecutorBaseTest, WaitCkeProcess_MaskNotMatched_DebugLog) {
    ...
    CcuInstr instr;
    memset(&instr, 0, sizeof(CcuInstr));
    auto sim2 = std::make_unique<CcuSimulator>(0, 0, 0, 10, 10,
                                               RunnerCcuVersion::CCU_V1);
