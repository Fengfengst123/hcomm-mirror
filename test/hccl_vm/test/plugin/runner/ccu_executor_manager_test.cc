...
class TestExecutor : public CcuExecutorBase {
  public:
    TestExecutor(int streamId, int rankId, int dieId, const CcuInstr &instr,
...
TEST_F(CcuExecutorManagerTest, RegFuncAndGetFunc) {
    ...
    auto createFunc = [](int streamId, int rankId, int dieId,
                         const CcuInstr &instr, CcuSimulator *ccuSimulator) {
        return std::make_unique<TestExecutor>(streamId, rankId, dieId, instr,
                                              ccuSimulator);
    };

    CcuExecutorCreateFuncMgr::Instance().RegFunc(RunnerCcuVersion::CCU_V1,
                                                 testType, createFunc);

    auto retrievedFunc = CcuExecutorCreateFuncMgr::Instance().GetFunc(
        RunnerCcuVersion::CCU_V1, testType);
...
TEST_F(CcuExecutorManagerTest, GetFuncNonExistent) {
    ...
    auto retrievedFunc = CcuExecutorCreateFuncMgr::Instance().GetFunc(
        RunnerCcuVersion::CCU_V1, nonExistentType);
...
TEST_F(CcuExecutorManagerTest, FactoryMakeInstanceRegistered) {
    ...
    auto executor = CcuExecutorFactory::MakeCcuExecutorInstance(
        RunnerCcuVersion::CCU_V1, testType, 0, 0, 0, instr_, nullptr);
...
TEST_F(CcuExecutorManagerTest, CreateFuncRegister) {
    ...
    CcuExecutorCreateFuncRegister reg(
        ...
        [](int streamId, int rankId, int dieId, const CcuInstr &instr,
           CcuSimulator *ccuSimulator) {
    ...
    auto retrievedFunc = CcuExecutorCreateFuncMgr::Instance().GetFunc(
        RunnerCcuVersion::CCU_V1, header.header);
    EXPECT_NE(retrievedFunc, nullptr);
}

// Test: Register multiple functions
TEST_F(CcuExecutorManagerTest, RegisterMultipleFunctions) {
    auto createFunc1 = [](int streamId, int rankId, int dieId,
                          const CcuInstr &instr, CcuSimulator *ccuSimulator) {
        return std::make_unique<TestExecutor>(streamId, rankId, dieId, instr,
                                              ccuSimulator);
    };
    ...
    EXPECT_NE(CcuExecutorCreateFuncMgr::Instance().GetFunc(
                  RunnerCcuVersion::CCU_V1, 0x1000),
              nullptr);
    EXPECT_NE(CcuExecutorCreateFuncMgr::Instance().GetFunc(
                  RunnerCcuVersion::CCU_V1, 0x2000),
              nullptr);
    EXPECT_NE(CcuExecutorCreateFuncMgr::Instance().GetFunc(
                  RunnerCcuVersion::CCU_V1, 0x3000),
              nullptr);
}
...
TEST_F(CcuExecutorManagerTest, FactoryCorrectParameters) {
    ...
    auto createFunc = [](int streamId, int rankId, int dieId,
                         const CcuInstr &instr, CcuSimulator *ccuSimulator) {
        auto executor = std::make_unique<TestExecutor>(streamId, rankId, dieId,
                                                       instr, ccuSimulator);
        return executor;
    };
    ...
    auto executor = CcuExecutorFactory::MakeCcuExecutorInstance(
        RunnerCcuVersion::CCU_V1, testType, testStreamId, testRankId, testDieId,
        instr_, nullptr);
    EXPECT_NE(executor, nullptr);
...
TEST_F(CcuExecutorManagerTest, CreateFuncWithSimulator) {
    ...
    auto simulator = std::make_unique<CcuSimulator>(0, 0, 0, 10, 10,
...
TEST_F(CcuExecutorManagerTest, MultipleFactoryCalls) {
    ...
    auto executor1 = CcuExecutorFactory::MakeCcuExecutorInstance(
        RunnerCcuVersion::CCU_V1, testType, 0, 0, 0, instr_, nullptr);
    auto executor2 = CcuExecutorFactory::MakeCcuExecutorInstance(
        RunnerCcuVersion::CCU_V1, testType, 1, 1, 1, instr_, nullptr);
    auto executor3 = CcuExecutorFactory::MakeCcuExecutorInstance(
        RunnerCcuVersion::CCU_V1, testType, 2, 2, 2, instr_, nullptr);
