  ...
  protected:
    void SetUp() override {
        testDir_ = fs::temp_directory_path() /
                   ("aiv_exec_test_" + std::to_string(::getpid()));
    ...
    std::string WriteTaskFile(uint32_t deviceId, uint32_t launchIndex,
                              const json &content) {
        std::string fileName = "hcclvm_aiv_device" + std::to_string(deviceId) +
    ...
    static json MakeMemCopyPayload(const json &src, const json &dst) {
        return {{"src", src}, {"dst", dst}};
    }

    static json MakeReducePayload(const json &src, const json &dst,
                                  uint32_t dataType = 0,
                                  uint32_t reduceOp = 0) {
        return {{"src", src},
                {"dst", dst},
...
TEST_F(AivGraphExecutorTest, Execute_PipeBarrierSingleTask_Success) {
    auto barrier =
        MakeTaskJson(4, 5, 0, 0, 0, MakePipeBarrierPayload(0, json::array()));
...
TEST_F(AivGraphExecutorTest, Execute_PipeBarrierGroup_Success) {
    ...
    auto block =
        MakeBlockJson(0, json::array({barrier1}), json::array({barrier2}));
    json content = {{"rank", 0},
                    {"rankSize", 1},
                    {"launchIndex", 0},
                    {"aivCores", json::array({block})}};
...
TEST_F(AivGraphExecutorTest, Execute_MemCopyLenMismatch_ReturnsError) {
    ...
    AivGraphExecutor executor(0, 0);
    ASSERT_TRUE(executor.Init());
    EXPECT_EQ(executor.Execute(),
              HcclSim::HcclVmResult::HCCL_SIM_VRT_ERROR_CMD);
}
...
TEST_F(AivGraphExecutorTest, Execute_ReduceLenMismatch_ReturnsError) {
    ...
    auto task =
        MakeTaskJson(1, 1, 0, 0, 1, MakeReducePayload(srcDs, dstDs, 4, 0));
...
TEST_F(AivGraphExecutorTest, Execute_MixedFlagAndBarrierTasks_Success) {
    ...
    auto barrier =
        MakeTaskJson(4, 3, 0, 0, 0, MakePipeBarrierPayload(0, json::array()));
