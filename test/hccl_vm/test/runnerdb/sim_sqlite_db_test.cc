...
TEST_F(SimSqliteDbTest, HcommResourceTables_PreserveEndpointAndMemoryRecords) {
    ...
    uint64_t endpointId =
        SimRunnerSqliteDB::Instance().Add<sim::HcommEndpoint>(endpoint);
    ASSERT_GT(endpointId, 0);

    auto foundEndpoint =
        SimRunnerSqliteDB::Instance().Find<sim::HcommEndpoint>(endpointId);
    ...
    uint64_t memoryId =
        SimRunnerSqliteDB::Instance().Add<sim::HcommMemReg>(memory);
    ASSERT_GT(memoryId, 0);

    auto foundMemory =
        SimRunnerSqliteDB::Instance().Find<sim::HcommMemReg>(memoryId);
...
TEST_F(SimSqliteDbTest, QueryServer_ReturnsMatchingRecords) {
    ...
    auto results = SimRunnerSqliteDB::Instance().QueryList<sim::Server>(
...
TEST_F(SimSqliteDbTest, QueryServer_WithPredicate_ReturnsFilteredRecords) {
    ...
    auto results = SimRunnerSqliteDB::Instance().QueryList<sim::Server>(
        [](const sim::Server &s) { return s.pod_id == 300; });
...
TEST_F(SimSqliteDbTest, QueryServer_ReturnsFirstMatch) {
    ...
    auto result = SimRunnerSqliteDB::Instance().Query<sim::Server>(
        [](const sim::Server &s) { return s.pod_id > 150; });
...
TEST_F(SimSqliteDbTest, QueryServer_NoMatch_ReturnsFalse) {
    auto result = SimRunnerSqliteDB::Instance().Query<sim::Server>(
        [](const sim::Server &s) { return s.pod_id > 999999; });
...
TEST_F(SimSqliteDbTest, UpdateServer_ModifiesRecord) {
    ...
    bool ret = SimRunnerSqliteDB::Instance().Update<sim::Server>(
        id, [](sim::Server &s) { s.pod_id = 999; });
...
TEST_F(SimSqliteDbTest, UpdateServer_InvalidId_ReturnsFalse) {
    bool ret = SimRunnerSqliteDB::Instance().Update<sim::Server>(
        99999, [](sim::Server &s) { s.pod_id = 999; });
...
TEST_F(SimSqliteDbTest, AddMultipleDevices_MaintainsDataIntegrity) {
    ...
    auto results = SimRunnerSqliteDB::Instance().QueryList<sim::Device>(
...
TEST_F(SimSqliteDbTest, ClearAll_ClearsAllTables) {
    ...
    auto afterAdd = SimRunnerSqliteDB::Instance().QueryList<sim::Server>(
        [](const sim::Server &s) { return s.pod_id == 9999; });
...
TEST_F(SimSqliteDbHostTest, QueryHostByIpAddr) {
    ...
    auto result =
        SimRunnerSqliteDB::Instance().Query<sim::Host>([](const sim::Host &h) {
...
TEST_F(SimSqliteDbContextTest, GetDefaultContext) {
    ...
    auto result = SimRunnerSqliteDB::Instance().Query<sim::Context>(
        [](const sim::Context &c) {
...
TEST_F(SimSqliteDbStreamTest, UpdateStream_ModifiesRecord) {
    ...
    bool ret = SimRunnerSqliteDB::Instance().Update<sim::Stream>(
        id, [](sim::Stream &s) {
            s.priority = 10;
...
TEST_F(SimSqliteDbStreamTest, GetStreamsByCtxId) {
    ...
    auto results = SimRunnerSqliteDB::Instance().QueryList<sim::Stream>(
