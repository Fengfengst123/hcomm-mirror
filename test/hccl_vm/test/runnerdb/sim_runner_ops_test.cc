...
TEST_F(SimRunnerOpsGetCurrDeviceIdTest,
       GetCurrDeviceId_InvalidCtx_ReturnsZero) {
    // 设置一个不存在的 context id
    sim::SetCurrCtxTls(99999);
    uint64_t deviceId = sim::GetCurrDeviceId();
    EXPECT_EQ(deviceId, 0);
...
TEST_F(SimRunnerOpsGetCurrDeviceIdTest,
       GetCurrDeviceId_MissingContextDevice_ReturnsZero) {
...
TEST_F(SimRunnerOpsGetDeviceIdByCtxIdTest,
       GetDeviceIdByCtxId_InvalidCtx_ReturnsZero) {
    uint64_t deviceId = sim::GetDeviceIdByCtxId(99999);
    EXPECT_EQ(deviceId, 0);
}

TEST_F(SimRunnerOpsGetDeviceIdByCtxIdTest,
       GetDeviceIdByCtxId_NoDevice_ReturnsZero) {
...
TEST_F(SimRunnerOpsGetCurrDeviceIdTest,
       GetCurrDeviceKey_InvalidCtx_ReturnsZero) {
    sim::SetCurrCtxTls(99999);
    uint64_t deviceKey = sim::GetCurrDeviceKey();
    EXPECT_EQ(deviceKey, 0);
}
