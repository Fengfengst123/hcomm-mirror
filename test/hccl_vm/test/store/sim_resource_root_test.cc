...
TEST_F(SimResourceRootTest, BuildName_PrependsPid) {
    ...
    // 返回完整文件路径
    EXPECT_EQ(sim::SimResourceRoot::BuildName("ra_sock_pool_0"),
              "/tmp/sim_resource_root_test_99999/ra_sock_pool_0");
    EXPECT_EQ(sim::SimResourceRoot::BuildName("/HcclCommPool"),
              "/tmp/sim_resource_root_test_99999/HcclCommPool");
    EXPECT_EQ(sim::SimResourceRoot::BuildName(""),
              "/tmp/sim_resource_root_test_99999/");
}
...
TEST_F(SimResourceRootTest, Init_CreatesRootAndSyncDirs) {
    ...
    EXPECT_EQ(sim::SimResourceRoot::GetInstance().GetSyncDir(),
              std::string(kTestRoot) + "/sync");
}
...
TEST_F(SimResourceRootTest, BuildName_StripsLeadingSlash) {
    EXPECT_EQ(sim::SimResourceRoot::BuildName("//ra_sock"),
              "/tmp/sim_resource_root_test_99999/ra_sock");
}
