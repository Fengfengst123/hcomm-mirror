...
TEST(HcclDataDumpTest, CreateMemoryInfo_AddsOnlyNonZeroBuffers) {
    ...
    EXPECT_EQ(CreateMemoryInfo(synData, memInfo, 3),
    ...
    EXPECT_EQ(synData.memory_info.data[0].buffer_type,
              static_cast<uint8_t>(BufferType::INPUT));
    EXPECT_EQ(synData.memory_info.data[0].start_addr, 0x1000U);
    EXPECT_EQ(synData.memory_info.data[0].size, 128U);
    EXPECT_EQ(synData.memory_info.data[1].buffer_type,
              static_cast<uint8_t>(BufferType::OUTPUT));
    EXPECT_EQ(synData.memory_info.data[1].start_addr, 0x2000U);
    EXPECT_EQ(synData.memory_info.data[1].size, 256U);
}

TEST(HcclDataDumpTest,
     CreateSimTaskMetaData_SkipsShortBlobAndDeserializesValidTasks) {
    ...
    EXPECT_EQ(CreateSimTaskMetaData(taskMeta, compositeDataMap, {}),
              HcclVmResult::HCCL_SIM_SUCCESS);
