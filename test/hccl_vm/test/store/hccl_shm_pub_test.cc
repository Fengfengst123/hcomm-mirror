...
TEST_F(HcclShmPubTest,
       GetAddrByOffset_ReturnsErrorWhenOutputAlreadyHoldsPointer) {
    VmUniquePtr addrPtr(reinterpret_cast<void *>(0x1),
                        VmPtrReleaser{sim::PhyMemBlock{}});
    EXPECT_EQ(GetAddrByOffset(TEST_DEV_BASE, addrPtr),
              HcclVmResult::HCCL_SIM_E_PARA);
}

TEST_F(HcclShmPubTest, GetAddrByOffset_ReturnsErrorWhenVirtualMemMissing) {
    VmUniquePtr addrPtr(nullptr);
    EXPECT_EQ(GetAddrByOffset(TEST_DEV_BASE, addrPtr),
              HcclVmResult::HCCL_SIM_E_PTR);
    EXPECT_EQ(addrPtr.get(), nullptr);
}

TEST_F(HcclShmPubTest,
       GetAddrByOffset_MapsRunnerDbVirtualAddressToSharedMemory) {
    void *shm = sim::MemoryManager::GetInstance().AllocMemByName(TEST_MEM_NAME,
                                                                 TEST_MEM_SIZE);
    ...
    VmUniquePtr addrPtr(nullptr);
    EXPECT_EQ(GetAddrByOffset(TEST_DEV_BASE + TEST_DEV_OFFSET, addrPtr),
...
TEST_F(HcclShmPubTest, InsertTaskToCollection_ReturnsErrorForNullTask) {
    uint32_t index = 0;
    EXPECT_EQ(InsertTaskToCollection(nullptr, &index),
              HcclVmResult::HCCL_SIM_E_PARA);
}

TEST_F(HcclShmPubTest, InsertTaskToCollection_ReturnsErrorForNullIndex) {
    HcclTaskMetaData task{};
    EXPECT_EQ(InsertTaskToCollection(&task, nullptr),
              HcclVmResult::HCCL_SIM_E_PARA);
}
...
TEST_F(HcclShmPubTest, InsertTaskToCollection_InsertsSerializedTaskIntoOpDb) {
    ...
    uint32_t index = 0;
    EXPECT_EQ(InsertTaskToCollection(&task, &index),
              HcclVmResult::HCCL_SIM_SUCCESS);
...
TEST_F(HcclShmPubTest,
       InsertTaskToCollection_RejectsNonSyncTaskWithoutCommunicator) {
    ASSERT_EQ(sim::InitOpDataDb(), 0);
