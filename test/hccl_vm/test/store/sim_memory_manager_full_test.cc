...
TEST_F(MemoryManagerTest, AllocMemByName_Normal_Success) {
    void *ptr =
        sim::MemoryManager::GetInstance().AllocMemByName("test_mem", 1024);
...
TEST_F(MemoryManagerTest, AllocMemByName_ZeroSize_Fail) {
    void *ptr =
        sim::MemoryManager::GetInstance().AllocMemByName("test_mem_zero", 0);
    EXPECT_EQ(ptr, nullptr);
}

TEST_F(MemoryManagerTest, AllocMemByName_DuplicateName_Fail) {
    void *ptr1 =
        sim::MemoryManager::GetInstance().AllocMemByName("test_mem_dup", 1024);
...
TEST_F(MemoryManagerTest, FreeMemByName_Normal_Success) {
    void *ptr =
        sim::MemoryManager::GetInstance().AllocMemByName("test_mem_free", 1024);
    EXPECT_NE(ptr, nullptr);
...
TEST_F(MemoryManagerTest, FreeMemByName_NotExist_DoNothing) {
    EXPECT_NO_THROW(
        sim::MemoryManager::GetInstance().FreeMemByName("not_exist_mem"));
}

TEST_F(MemoryManagerTest, LockUnlockMemByName_Normal_Success) {
    void *ptr =
        sim::MemoryManager::GetInstance().AllocMemByName("test_mem_lock", 1024);
    EXPECT_NE(ptr, nullptr);
    EXPECT_NO_THROW(
        sim::MemoryManager::GetInstance().LockMemByName("test_mem_lock"));
    EXPECT_NO_THROW(
        sim::MemoryManager::GetInstance().UnlockMemByName("test_mem_lock"));
...
TEST_F(MemoryManagerTest, LockUnlockMemByName_NotExist_DoNothing) {
    EXPECT_NO_THROW(
        sim::MemoryManager::GetInstance().LockMemByName("not_exist"));
    EXPECT_NO_THROW(
        sim::MemoryManager::GetInstance().UnlockMemByName("not_exist"));
}

TEST_F(MemoryManagerTest, AcquireMemByName_NotExist_Fail) {
    void *ptr =
        sim::MemoryManager::GetInstance().AcquireMemByName("not_exist_acquire");
    EXPECT_EQ(ptr, nullptr);
}
...
TEST_F(MemoryManagerTest, AcquireMemByName_AfterAlloc_Success) {
    void *ptr1 =
        sim::MemoryManager::GetInstance().AllocMemByName("test_acquire", 1024);
    EXPECT_NE(ptr1, nullptr);
    void *ptr2 =
        sim::MemoryManager::GetInstance().AcquireMemByName("test_acquire");
...
TEST_F(MemoryManagerTest, ReleaseMemByName_Normal_Success) {
    void *ptr =
        sim::MemoryManager::GetInstance().AllocMemByName("test_release", 1024);
    EXPECT_NE(ptr, nullptr);
    EXPECT_NO_THROW(
        sim::MemoryManager::GetInstance().ReleaseMemByName("test_release"));
}

TEST_F(MemoryManagerTest, ReleaseMemByName_NullName_DoNothing) {
    EXPECT_NO_THROW(
        sim::MemoryManager::GetInstance().ReleaseMemByName(nullptr));
}

TEST_F(MemoryManagerTest, ReleaseMemByName_NotExist_DoNothing) {
    EXPECT_NO_THROW(sim::MemoryManager::GetInstance().ReleaseMemByName(
        "not_exist_release"));
}
