...
TEST_F(HcclProxyCommonTest, GetDataTypeSize_Uint16) {
    uint32_t size = 0;
    int result =
        sim::GetDataTypeSize(HcclDataType::HCCL_DATA_TYPE_UINT16, size);
    EXPECT_EQ(result, 0);
    EXPECT_EQ(size, 2);
}
...
TEST_F(HcclProxyCommonTest, GetDataTypeSize_Uint32) {
    uint32_t size = 0;
    int result =
        sim::GetDataTypeSize(HcclDataType::HCCL_DATA_TYPE_UINT32, size);
    EXPECT_EQ(result, 0);
    EXPECT_EQ(size, 4);
}
...
TEST_F(HcclProxyCommonTest, GetDataTypeSize_Uint64) {
    uint32_t size = 0;
    int result =
        sim::GetDataTypeSize(HcclDataType::HCCL_DATA_TYPE_UINT64, size);
    EXPECT_EQ(result, 0);
    EXPECT_EQ(size, 8);
}
...
TEST_F(HcclProxyCommonTest, GetDataTypeSize_Int128) {
    uint32_t size = 0;
    int result =
        sim::GetDataTypeSize(HcclDataType::HCCL_DATA_TYPE_INT128, size);
    EXPECT_EQ(result, 0);
    EXPECT_EQ(size, 16);
}
...
TEST_F(HcclProxyCommonTest, GetDataTypeSize_Fp8E4M3) {
    uint32_t size = 0;
    int result =
        sim::GetDataTypeSize(HcclDataType::HCCL_DATA_TYPE_FP8E4M3, size);
    EXPECT_EQ(result, 0);
    EXPECT_EQ(size, 1);
}
...
TEST_F(HcclProxyCommonTest, GetDataTypeSize_Fp8E5M2) {
    uint32_t size = 0;
    int result =
        sim::GetDataTypeSize(HcclDataType::HCCL_DATA_TYPE_FP8E5M2, size);
    EXPECT_EQ(result, 0);
    EXPECT_EQ(size, 1);
}
...
TEST_F(HcclProxyCommonTest, GetDataTypeSize_Fp8E8M0) {
    uint32_t size = 0;
    int result =
        sim::GetDataTypeSize(HcclDataType::HCCL_DATA_TYPE_FP8E8M0, size);
    EXPECT_EQ(result, 0);
    EXPECT_EQ(size, 1);
}
...
TEST_F(HcclProxyCommonTest, GetDataTypeSize_InvalidType) {
    uint32_t size = 0;
    int result =
        sim::GetDataTypeSize(HcclDataType::HCCL_DATA_TYPE_RESERVED, size);
...
TEST_F(HcclProxyCommonTest, DataTypeSizeMap_ContainsExpectedEntries) {
    ...
    EXPECT_TRUE(
        sim::DATA_TYPE_SIZE_MAP.count(HcclDataType::HCCL_DATA_TYPE_INT8) > 0);
    EXPECT_TRUE(
        sim::DATA_TYPE_SIZE_MAP.count(HcclDataType::HCCL_DATA_TYPE_FP16) > 0);
    EXPECT_TRUE(
        sim::DATA_TYPE_SIZE_MAP.count(HcclDataType::HCCL_DATA_TYPE_INT128) > 0);
}
