...
void SetupTestData() {
    ...
    g_localDevId = RunnerDB::Add<sim::Device>(devLocal);

    sim::Device devRemote{};
    devRemote.server_id = 1;
    devRemote.logic_id = 2;
    devRemote.physical_id = 2;
    g_remoteDevId = RunnerDB::Add<sim::Device>(devRemote);

    SetCurDeviceKey(static_cast<uint32_t>(g_localDevId));

    sim::VirtualMemBlock localMem{};
    localMem.start_ptr = kLocalVirBase;
    localMem.dev_mapped_ptr = 0;
    localMem.size = kDevAddrRange;
    localMem.ctx_id = 1;
    localMem.device_id = g_localDevId;
    localMem.src_type = static_cast<uint8_t>(sim::VIR_MEM_TYPE_DEV);
    RunnerDB::Add<sim::VirtualMemBlock>(localMem);

    sim::VirtualMemBlock remoteMem{};
    ...
    remoteMem.src_type = static_cast<uint8_t>(sim::VIR_MEM_TYPE_DEV);
    RunnerDB::Add<sim::VirtualMemBlock>(remoteMem);

    sim::Communicator self{};
    std::strncpy(self.comm_id, kTestCommName, sizeof(self.comm_id) - 1);
    self.rank_size = 2;
    self.rank_id = 1;
    self.device_id = g_localDevId;
    g_commKey = RunnerDB::Add<sim::Communicator>(self);

    sim::Communicator peer{};
    std::strncpy(peer.comm_id, kTestCommName, sizeof(peer.comm_id) - 1);
    peer.rank_size = 2;
    peer.rank_id = 2;
    peer.device_id = g_remoteDevId;
    RunnerDB::Add<sim::Communicator>(peer);
}
...
std::tuple<sim::HcclThread, sim::HcclChannel, uint32_t, uint32_t>
MakeContext() {
    sim::HcclThread thread{};
    thread.commId = g_commKey;
    thread.streamId = 34;
    sim::HcclChannel channel{};
    channel.id = 56;
...
TEST_F(DataOpDeviceStubTest, CopyTaskMapsDirectionAndAddresses) {
    ...
    HcclTaskMetaData task =
        MakeCopyTask(thread, rankId, rankId, src, remoteRankId, dst, 128);
...
TEST_F(DataOpDeviceStubTest, ReduceTaskKeepsElementCount) {
    ...
    HcclTaskMetaData task = MakeReduceTask(
        thread, rankId, 2, reinterpret_cast<const void *>(0x3000ULL), 1,
        reinterpret_cast<const void *>(0x4000ULL), 17, 4, 1);
...
TEST_F(DataOpDeviceStubTest, A5ReduceValidationRejectsInvalidAndOverflow) {
    ...
    EXPECT_EQ(
        ValidateA5Reduce(12, 0, std::numeric_limits<uint64_t>::max(), typeSize),
        HCCL_E_PARA);
}

TEST_F(DataOpDeviceStubTest,
       TimeoutValidationTruncatesAndRejectsInvalidValues) {
    ...
    EXPECT_EQ(
        NormalizeTimeout(std::numeric_limits<float>::quiet_NaN(), normalized),
        HCCL_E_PARA);
...
TEST_F(DataOpDeviceStubTest, BatchWriteAndReadMapOppositeDirections) {
    ...
    EXPECT_EQ(ExpandBatchDescriptor(write, 0, thread, channel, rankId,
                                    remoteRankId, tasks),
              HCCL_SUCCESS);
    ...
    EXPECT_EQ(ExpandBatchDescriptor(read, 1, thread, channel, rankId,
                                    remoteRankId, tasks),
              HCCL_SUCCESS);
...
TEST_F(DataOpDeviceStubTest, UnsupportedAndInvalidBatchTypesDoNotAppendTasks) {
    ...
    EXPECT_EQ(ExpandBatchDescriptor(desc, 0, thread, channel, rankId,
                                    remoteRankId, tasks),
              HCCL_E_NOT_SUPPORT);
    desc.transType = TRANSFER_TYPE_NOTIFY_WAIT_DEFAULT;
    EXPECT_EQ(ExpandBatchDescriptor(desc, 1, thread, channel, rankId,
                                    remoteRankId, tasks),
              HCCL_E_NOT_SUPPORT);
    desc.transType = 99;
    EXPECT_EQ(ExpandBatchDescriptor(desc, 2, thread, channel, rankId,
                                    remoteRankId, tasks),
              HCCL_E_PARA);
...
TEST_F(DataOpDeviceStubTest,
       PublicParameterErrorsAreStableAcrossRepeatedCalls) {
    EXPECT_EQ(
        HcommLocalCopyOnThread(1, nullptr, reinterpret_cast<void *>(1), 0),
        HCCL_E_PARA);
    EXPECT_EQ(HcommLocalReduceOnThread(1, reinterpret_cast<void *>(1), nullptr,
                                       0, 4, 0),
              HCCL_E_PARA);
