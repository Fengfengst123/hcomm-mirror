...
TEST_F(AscendHalStubTest, halEschedQueryInfo_ReturnsSuccess) {
    ...
    drvError_t result = halEschedQueryInfo(
        0, ESCHED_QUERY_TYPE::QUERY_TYPE_LOCAL_GRP_ID, &inPut, &outPut);
    EXPECT_EQ(result, DRV_ERROR_NONE);
}
...
TEST_F(AscendHalStubTest, halEschedRegisterAckFunc_ReturnsSuccess) {
    drvError_t result =
        halEschedRegisterAckFunc(0, EVENT_ID::EVENT_RANDOM_KERNEL, nullptr);
    EXPECT_EQ(result, DRV_ERROR_NONE);
}
...
TEST_F(AscendHalStubTest, drvQueryProcessHostPid_ReturnsSuccess) {
    ...
    drvError_t result =
        drvQueryProcessHostPid(0, &chip_id, &vfid, &host_pid, &cp_type);
    EXPECT_EQ(result, DRV_ERROR_NONE);
}
...
TEST_F(AscendHalStubTest, halGetDeviceInfo_ModuleTypeSystem) {
    ...
    drvError_t result =
        halGetDeviceInfo(0, MODULE_TYPE_SYSTEM, INFO_TYPE_CORE_NUM, &value);
    EXPECT_EQ(result, DRV_ERROR_NONE);
    EXPECT_EQ(value, 1);
}
...
TEST_F(AscendHalStubTest, halSensorNodeUpdateState_ReturnsSuccess) {
    drvError_t result = halSensorNodeUpdateState(
        0, 0, 0, halGeneralEventType_t::GENERAL_EVENT_TYPE_OCCUR);
    EXPECT_EQ(result, DRV_ERROR_NONE);
}
...
TEST_F(AscendHalStubTest, halGrpQuery_ReturnsZero) {
    ...
    int result = halGrpQuery(GroupQueryCmdType::GRP_QUERY_GROUP, nullptr, 0,
                             nullptr, &outLen);
    EXPECT_EQ(result, 0);
}
