...
int HandlePipeCmdSetDevId(uint8_t *payload, uint32_t payloadLen) {
    ...
    HCCL_VM_INFO(
        "Process[{}] PIPE_RSP_SET_DEV_ID, set rankId = [{}], deviceKey = [{}]",
        getpid(), curRankId, curDeviceKey);
...
int HandlePipeCmdGetDevPtr(uint8_t *payload, uint32_t payloadLen) {
    ...
    void *shmptr =
        sim::MemoryManager::GetInstance().AcquireMemByName(req->memName);
    if (shmptr == nullptr) {
        donePayload.ptr = 0;
        HCCL_VM_ERROR("acquire {} shm failed.", req->memName);
    } else {
        donePayload.ptr =
            static_cast<uint64_t>(reinterpret_cast<uintptr_t>(shmptr));
    }
...
static void ExecuteAicpuKernel(uint32_t rankId, ExecKernelPayload *kernelReq,
    ...
    std::string libDir =
        InstallPath::ResolveToInstallRoot("lib/" + GetArchStr()) + "/" +
        kernelSo;
    KernelFn fn =
        sim::KernelLibManager::GetInstance().GetOrLoadFunc(libDir, kernelName);
    if (!fn) {
    ...
    if (argsPtr == nullptr) {
        HCCL_VM_ERROR("[device] rankId[{}] init func handle failed null ptr.",
                      rankId);
        return;
    }
...
int HandlePipeCmdExecKernel(uint8_t *payload, uint32_t payloadLen) {
    ...
    ExecKernelPayload *kernelReq =
        reinterpret_cast<ExecKernelPayload *>(payload);
    uint32_t rankId = 0;
    GetCurRankId(&rankId);
    std::string kernelName(kernelReq->kernelName);
    uint32_t argsLen = payloadLen - sizeof(ExecKernelPayload);
    void *argsPtr = payload + sizeof(ExecKernelPayload);
    HCCL_VM_INFO("[device] rankId[{}] executing kernel:{}, argsLen:{}", rankId,
                 kernelName, argsLen);
    ...
    if (kernelReq->opDetailId != 0) {
        ...
        if (sim::QueryOpDetailIdentity(kernelReq->opDetailId, commId, opRankId,
                                       opDeviceId) != 0 ||
            commId == 0) {
            HCCL_VM_ERROR(
                "[device] invalid task identity for opDetailId={}, commId={}",
                kernelReq->opDetailId, commId);
            RspExecKernelPayload donePayload{-1};
            DeviceSendMsg(PIPE_RSP_EXEC_KERNEL, &donePayload,
                          sizeof(donePayload));
            return -1;
        }
        // InsertOpTask 仍通过该值把 device 任务写入本次 opDetail 的任务表；
        // 它不参与任务 metadata 的身份生成。
        sim::g_currOpDetailId = kernelReq->opDetailId;
    } else {
        // 普通 kernel 不产生带通信域身份的
        // SQE，执行前必须清掉上一次算子的上下文。
        sim::g_currOpDetailId = 0;
    }
