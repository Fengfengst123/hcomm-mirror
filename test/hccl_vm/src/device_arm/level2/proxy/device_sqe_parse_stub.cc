...
bool ResolveCurrentTaskIdentity(TaskIdentity &identity) {
    if (sim::g_currOpDetailId == 0 ||
        sim::QueryOpDetailIdentity(sim::g_currOpDetailId, identity.commId,
                                   identity.rankId, identity.deviceId) != 0 ||
        identity.commId == 0) {
        HCCL_VM_ERROR("cannot resolve task identity for opDetailId={}",
                      sim::g_currOpDetailId);
        return false;
    }
    return true;
}
...
void SetRemoteNotifyEndpoints(HcclTaskMetaData &task, uint32_t remoteDeviceId) {
    ...
    HCCL_VM_INFO("record Notify endpoints, localDeviceId={}, "
...
void ParseDavidSDMASqeWithIdentity(uint32_t streamId, void *sqeBuf,
                                   const TaskIdentity &identity);
void ParseDavidNotifySqeWithIdentity(uint32_t streamId, void *sqeBuf,
                                     bool isPost, const TaskIdentity &identity);
void ParseDavidUDMASqeWithIdentity(uint32_t streamId, void *sqeBuf,
                                   const TaskIdentity &identity);
void ParseDavidUBReadWriteSqeWithIdentity(uint64_t wqeAddr, uint16_t streamId,
                                          uint32_t jettyId, bool isRead,
                                          const TaskIdentity &identity);
void ParseDavidUBWriteWithNotifySqeWithIdentity(uint64_t wqeAddr,
                                                uint16_t streamId,
                                                uint32_t jettyId,
                                                const TaskIdentity &identity);
...
void ParseA5SqeFromSqBuffer(uint32_t devId, struct halSqCqConfigInfo *info) {
    ...
    if (tail >= head) {
        // 数据未绕圈，直接拷贝
        memcpy(tempBuffer, sqBuffer + head * HCCL_SQE_SIZE,
               sqeCnt * HCCL_SQE_SIZE);
    } else {
        // 数据绕圈，分两次拷贝
        int firstPart =
            HCCL_SQE_MAX_CNT - head; // 从head到缓冲区  末尾的拷贝数量
        int secondPart = tail;       // 从缓冲区开头到tail的拷贝数量
        memcpy(tempBuffer, sqBuffer + head * HCCL_SQE_SIZE,
               firstPart * HCCL_SQE_SIZE);
        memcpy(tempBuffer + firstPart * HCCL_SQE_SIZE, sqBuffer,
               secondPart * HCCL_SQE_SIZE);
    }

    for (int i = 0; i < sqeCnt; i++) {
        int sqOffIndex = i;
        void *sqeBuf =
            static_cast<uint8_t *>(tempBuffer) + sqOffIndex * HCCL_SQE_SIZE;
        Rt91095StarsSqeHeader *header =
            reinterpret_cast<Rt91095StarsSqeHeader *>(sqeBuf);
        switch (header->type) {
        ...
        default: {
            HCCL_VM_ERROR("not support sqe type[{}].",
...
void ParseDavidSDMASqeWithIdentity(uint32_t streamId, void *sqeBuf,
    ...
    Rt91095StarsMemcpySqe *sqe =
        reinterpret_cast<Rt91095StarsMemcpySqe *>(sqeBuf);
    ...
    uint64_t srcOffset = TransLocalAddrToVirtual(GetFull64BitAddr(
        sqe->u.strideMode0.srcAddrLow, sqe->u.strideMode0.srcAddrHigh));
    uint64_t dstOffset = TransLocalAddrToVirtual(GetFull64BitAddr(
        sqe->u.strideMode0.dstAddrLow, sqe->u.strideMode0.dstAddrHigh));
    uint64_t srcDeviceId = GetDeviceIdByDevAddr(srcOffset);
    uint64_t dstDeviceId = GetDeviceIdByDevAddr(dstOffset);
...
void ParseDavidNotifySqeWithIdentity(uint32_t streamId, void *sqeBuf,
    ...
    Rt91095StarsNotifySqe *sqe =
        reinterpret_cast<Rt91095StarsNotifySqe *>(sqeBuf);
    HcclTaskMetaData taskMeta;
    SetTaskIdentity(taskMeta, identity);
    taskMeta.streamId = streamId;
    taskMeta.jettyId = UINT32_MAX;
    taskMeta.taskType = isPost ? HccLTaskMetaType::NOTIFY_RECORD
                               : HccLTaskMetaType::NOTIFY_WAIT;
    const uint64_t notifyId = sqe->notifyId;
...
void ParseDavidUDMASqeWithIdentity(uint32_t streamId, void *sqeBuf,
    ...
    if (ciValue >= piValue) {
        HCCL_VM_ERROR("jettyId[{}] ciVal[{}] piValue[{}] streamId[{}].",
                      jettyId, ciValue, piValue, streamId);
        return;
    }

    HCCL_VM_INFO("jettyId[{}] ciVal[{}] piValue[{}] streamId[{}].", jettyId,
                 ciValue, piValue, streamId);
    for (auto index = ciValue; index < piValue; index++) {
        ...
        case static_cast<int>(UdmaSqOpcode::UDMA_OPC_WRITE): {
            ParseDavidUBReadWriteSqeWithIdentity(wqeAddr, streamId, jettyId,
                                                 false, identity);
            break;
        }
        case static_cast<int>(UdmaSqOpcode::UDMA_OPC_WRITE_WITH_NOTIFY): {
            ParseDavidUBWriteWithNotifySqeWithIdentity(wqeAddr, streamId,
                                                       jettyId, identity);
            index++; // 占128字节两个WQE
            break;
        }
        ...
        default: {
            HCCL_VM_ERROR("not support opcode[{}].",
                          static_cast<uint32_t>(ubCommon->opcode));
            return;
        }
        }
    }
...
void ParseDavidUBReadWriteSqeWithIdentity(uint64_t wqeAddr, uint16_t streamId,
    ...
    if (!GetRmtDeviceIdByEid(
            reinterpret_cast<const uint8_t *>(ubWqe->comm.rmtEid),
            rmtDeviceId)) {
        HCCL_VM_ERROR("skip UDMA SQE because remote device resolution failed, "
                      "streamId={}, jettyId={}",
                      streamId, jettyId);
        return;
    }
    ...
    if (ubWqe->comm.inlineEn == 1) {
        uint64_t notifyAddr =
            GetFull64BitAddr(ubWqe->comm.rmtAddrLow, ubWqe->comm.rmtAddrHigh);
    ...
    uint64_t locAddr =
        GetFull64BitAddr(ubWqe->u.sge.dataAddrLow, ubWqe->u.sge.dataAddrHigh);
    uint64_t rmtAddr =
        GetFull64BitAddr(ubWqe->comm.rmtAddrLow, ubWqe->comm.rmtAddrHigh);
    locAddr = TransLocalAddrToVirtual(locAddr);
    rmtAddr = TransRemoteAddrToVirtualByDeviceId(rmtAddr, rmtDeviceId);
    uint64_t srcOffset = isRead ? rmtAddr : locAddr;
    uint64_t dstOffset = isRead ? locAddr : rmtAddr;
    uint32_t srcDeviceId = GetDeviceIdByDevAddr(srcOffset);
    uint32_t dstDeviceId = GetDeviceIdByDevAddr(dstOffset);
    taskMeta.taskType = HccLTaskMetaType::MEM_CPY;
    taskMeta.taskData.transMem.srcOffset = srcOffset;
    taskMeta.taskData.transMem.dstOffset = dstOffset;
    taskMeta.taskData.transMem.dstDeviceId = dstDeviceId;
    ...
    if (ubWqe->comm.udfFlag == 1) {
        taskMeta.taskType = HccLTaskMetaType::REDUCE;
        taskMeta.taskData.reduce.reduceOp =
            ParseUbReduceTypeDavid(ubWqe->comm.inlinedata.udfData.reduceOp);
        taskMeta.taskData.reduce.dataType =
            ParseUbDataTypeDavid(ubWqe->comm.inlinedata.udfData.reduceType);
        taskMeta.taskData.reduce.srcOffset = srcOffset;
        taskMeta.taskData.reduce.dstOffset = dstOffset;
        taskMeta.taskData.reduce.dstDeviceId = dstDeviceId;
        taskMeta.taskData.reduce.srcDeviceId = srcDeviceId;
...
void ParseDavidUBWriteWithNotifySqeWithIdentity(uint64_t wqeAddr,
    ...
    UdmaSqeWriteWithNotify *ubWqe =
        reinterpret_cast<UdmaSqeWriteWithNotify *>(wqeAddr);
    ...
    if (!GetRmtDeviceIdByEid(
            reinterpret_cast<const uint8_t *>(ubWqe->comm.rmtEid),
            rmtDeviceId)) {
        HCCL_VM_ERROR("skip UDMA write-with-notify SQE because remote device "
                      "resolution failed, "
                      "streamId={}, jettyId={}",
                      streamId, jettyId);
        return;
    ...
    uint64_t dstOffset = TransRemoteAddrToVirtualByDeviceId(
        GetFull64BitAddr(ubWqe->comm.rmtAddrLow, ubWqe->comm.rmtAddrHigh),
        rmtDeviceId);
    uint64_t srcOffset = TransLocalAddrToVirtual(GetFull64BitAddr(
        ubWqe->localU.sge.dataAddrLow, ubWqe->localU.sge.dataAddrHigh));
    ...
    if (ubWqe->comm.udfFlag == 1) { // Reduce操作标识
        ...
        taskMeta1.taskData.reduce.reduceOp =
            ParseUbReduceTypeDavid(ubWqe->comm.inlinedata.udfData.reduceOp);
        taskMeta1.taskData.reduce.dataType =
            ParseUbDataTypeDavid(ubWqe->comm.inlinedata.udfData.reduceType);
    ...
    uint64_t notifyAddr = GetFull64BitAddr(ubWqe->notify.notifyAddrLow,
                                           ubWqe->notify.notifyAddrHigh);
...
void ParseDavidUBReadWriteSqe(uint64_t wqeAddr, uint16_t streamId,
                              uint32_t jettyId, bool isRead) {
    TaskIdentity identity{};
    if (ResolveCurrentTaskIdentity(identity)) {
        ParseDavidUBReadWriteSqeWithIdentity(wqeAddr, streamId, jettyId, isRead,
                                             identity);
    }
}

void ParseDavidUBWriteWithNotifySqe(uint64_t wqeAddr, uint16_t streamId,
                                    uint32_t jettyId) {
    TaskIdentity identity{};
    if (ResolveCurrentTaskIdentity(identity)) {
        ParseDavidUBWriteWithNotifySqeWithIdentity(wqeAddr, streamId, jettyId,
                                                   identity);
    }
}

uint64_t GetFull64BitAddr(uint32_t lowAddr, uint32_t highAddr) {
    return (static_cast<uint64_t>(highAddr) << SHIFT_BIT32) |
           static_cast<uint64_t>(lowAddr);
}
...
HcclReduceOp ParseReduceTypeDavid(uint8_t result) {
    ...
    if (DavidReduceOpMap.find(static_cast<uint32_t>(reduceType)) !=
        DavidReduceOpMap.end()) {
...
HcclDataType ParseDataTypeDavid(uint8_t result) {
    ...
    if (DavidDataTypeMap.find(static_cast<uint32_t>(dataType)) !=
        DavidDataTypeMap.end()) {
...
void PrintTaskMetaData(const HcclTaskMetaData &taskMeta) {
    ...
    case HccLTaskMetaType::MEM_CPY:
        HCCL_VM_INFO(
            ...
            pid, taskMeta.rankId, taskMeta.streamId,
            taskMeta.taskData.transMem.srcOffset,
            taskMeta.taskData.transMem.dstOffset,
            taskMeta.taskData.transMem.len,
            taskMeta.taskData.transMem.srcDeviceId,
            taskMeta.taskData.transMem.dstDeviceId);
        break;
    case HccLTaskMetaType::REDUCE:
        HCCL_VM_INFO("pid{}: rankId[{}] streamId[{}] taskType[REDUCE], "
                     ...
                     taskMeta.taskData.reduce.srcDeviceId,
                     taskMeta.taskData.reduce.dstDeviceId,
                     taskMeta.taskData.reduce.reduceOp,
                     taskMeta.taskData.reduce.dataType);
        break;
    case HccLTaskMetaType::NOTIFY_WAIT:
        HCCL_VM_INFO("pid{}: rankId[{}] streamId[{}] taskType[NOTIFY_WAIT], "
                     "notifyId[{}], recordDeviceId[{}], waitDeviceId[{}]",
                     pid, taskMeta.rankId, taskMeta.streamId,
                     taskMeta.taskData.notify.notifyId,
                     taskMeta.taskData.notify.srcDeviceId,
                     taskMeta.taskData.notify.dstDeviceId);
        break;
    case HccLTaskMetaType::NOTIFY_RECORD:
        HCCL_VM_INFO("pid{}: rankId[{}] streamId[{}] taskType[NOTIFY_RECORD], "
                     "notifyId[{}], recordDeviceId[{}], waitDeviceId[{}]",
                     pid, taskMeta.rankId, taskMeta.streamId,
                     taskMeta.taskData.notify.notifyId,
                     taskMeta.taskData.notify.srcDeviceId,
...
bool GetRmtDeviceIdByEid(const uint8_t *eid, uint32_t &deviceId) {
    ...
    if (sim::GetEndPointByEid(address, endPoint) != 0) {
        HCCL_VM_ERROR("failed to resolve remote device by eid={}",
                      address.EidToHexString());
        return false;
    }
