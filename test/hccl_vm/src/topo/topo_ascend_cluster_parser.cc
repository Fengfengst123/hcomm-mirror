...
constexpr uint8_t EID_SUPERPOD_ID_HIGH_MASK =
    0x0F;                               // eid[4]低4位: super pod id[8:5]
...
void eid_hex_to_uint8(const char *eid_str, uint8_t *eid_out) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        uint8_t low =
            (lowIdx < dataLen) ? hex_char_to_val(hex_data[lowIdx]) : 0;
...
static uint8_t eid_hex_byte_at(const std::string &eid, size_t byteIdx) {
    ...
    long start =
        static_cast<long>(hexLen) - static_cast<long>((16 - byteIdx) * 2);
    if (start < 0) {
        return 0;
    }
    uint8_t hi = hex_char_to_val(eid[off + start]);
    uint8_t lo = (start + 1 < static_cast<long>(hexLen))
                     ? hex_char_to_val(eid[off + start + 1])
                     : 0;
...
static uint8_t eid_die_id(const std::string &eid) {
    return static_cast<uint8_t>((eid_hex_byte_at(eid, 5) >> EID_DIE_ID_SHIFT) &
                                EID_DIE_ID_MASK);
}

// 按EID地址分配规则解码85~80位: 端口号
// (普通物理端口0-62; PG端口固定0x3F)
static uint8_t eid_port_num(const std::string &eid) {
    return eid_hex_byte_at(eid, 5) & EID_PORT_NUM_MASK;
}
...
static bool eid_is_pg_port(const std::string &eid) {
    return (eid_port_num(eid) == EID_PORT_NUM_PG);
}
 ...
 */
HcclVmResult
AscendClusterTopoParser::InitClusterTopo(const std::string &clusterDir) {
    ...
    if (status != ParseStatus::OK) {
        HCCL_VM_ERROR("Failed to parse cluster directory (status={:d})",
    ...
    ClusterTopoDumper::DumpToFile(
        network_, InstallPath::ResolveToInstallRoot("data/" + outputFile_));
 ...
 * 刷新TopoMetaConfig表项(记录"ranktable")与进程通信域配置缓存
 */
HcclVmResult AscendClusterTopoParser::ParseRanktableAndInitCommDomain(
    const std::string &ranktablePath) {
    HCCL_VM_DEBUG("Enter ParseRanktableAndInitCommDomain, ranktablePath: {}",
                  ranktablePath);

    TopoMeta topoMeta;
    if (ParseRanktable(ranktablePath, topoMeta) !=
        HcclVmResult::HCCL_SIM_SUCCESS) {
        HCCL_VM_ERROR("parse rank table failed");
        return HcclVmResult::HCCL_SIM_E_INTERNAL;
    }
    // ranktable模式: 刷新TopoMetaConfig表项与进程通信域配置缓存
    if (!sim::RefreshTopoMetaConfig("ranktable", &topoMeta)) {
 ...
 */
HcclVmResult
AscendClusterTopoParser::InitCommunicationDomain(const TopoMeta &topoMeta,
                                                 bool withRanktable) {
    ...
    for (const auto &[superPodId, superPod] : topoMeta) {
        for (const auto &[serverId, server] : superPod) {
            auto serverKey = sim::GetServerKeyById(superPodId, serverId);
            for (uint32_t deviceIdx = 0; deviceIdx < server.size();
                 ++deviceIdx) {
                auto physicId = server[deviceIdx];
                // todo: 暂时不考虑容器场景，userId == logicDevId
                auto userId = deviceIdx;
                if (sim::UpdateDeviceLogicId(serverKey, physicId, deviceIdx,
                                             userId) != ACL_SUCCESS) {
                    HCCL_VM_ERROR(
...
HcclVmResult
AscendClusterTopoParser::InitDynamicModelData(const TopoMeta &topoMeta) {
    for (const auto &[superPodId, superPod] : topoMeta) {
        for (const auto &[serverId, server] : superPod) {
            auto serverKey = sim::GetServerKeyById(superPodId, serverId);
            uint32_t deviceCount = server.size();

            RunnerDB::Update<sim::Server>(serverKey,
                                          [deviceCount](sim::Server &dev) {
                                              dev.used_dev_num = deviceCount;
                                          });
            ...
            for (uint32_t deviceIdx = 0; deviceIdx < deviceCount; ++deviceIdx) {
                ...
                auto devRet = RunnerDB::GetOneByPred<sim::Device>(
                    [serverKey, phyId](const sim::Device &d) {
                        return d.server_id == serverKey &&
                               d.physical_id == phyId;
                    });
                if (!devRet.second) {
                    HCCL_VM_ERROR("device not found: serverKey={}, phyDevId={}",
                                  serverKey, phyId);
                    return HcclVmResult::HCCL_SIM_E_INTERNAL;
                }
                auto deviceKey = devRet.first.id;

                // 校验该 device 至少关联一个 CCU
                auto deviceAllCcu = RunnerDB::GetByPred<sim::Ccu>(
                    [deviceKey](const sim::Ccu &d) {
...
HcclVmResult
AscendClusterTopoParser::BuildLevelList(const Server &server, int srcDevPhyId,
                                        const std::set<int> &commDomainLocalIds,
                                        uint32_t spIdx, uint32_t srvIdx,
                                        json &levelList, bool skipLayerAbove2,
                                        const std::set<int> &allowedNetLayers) {
    const Device *device = server.GetDevice(srcDevPhyId);
    if (!device) {
        HCCL_VM_ERROR(
            "cannot find device by phyDevId {:d} in superpod{:d}/server{:d}",
    ...
    for (const auto &link : server.links) {
        if (link.sideA.localId == srcDevPhyId ||
            link.sideB.localId == srcDevPhyId) {
    ...
    auto buildNetInstanceId = [](uint32_t sp, uint32_t srv, int layer,
                                 uint32_t niid) -> std::string {
        if (layer >= 2) {
            return "cluster";
        }
        if (layer == 0) {
            return "sp_" + std::to_string(sp) + "_srv_" + std::to_string(srv) +
                   "_idx_" + std::to_string(niid);
        }
    ...
    auto parseNetInstanceIdx = [](const std::string &netInstanceId,
                                  uint32_t &idxK) -> bool {
        uint32_t spTmp = 0;
        uint32_t srvTmp = 0;
        uint32_t layTmp = 0;
        // 新格式 net_layer=0: sp_{sp}_srv_{srv}_idx_{k}
        if (sscanf(netInstanceId.c_str(), "sp_%u_srv_%u_idx_%u", &spTmp,
                   &srvTmp, &idxK) == 3) {
            return true;
        }
        ...
        if (sscanf(netInstanceId.c_str(), "sp_%u_srv_%u_lay_%u_idx_%u", &spTmp,
                   &srvTmp, &layTmp, &idxK) == 4) {
            return true;
        }
        return false;
    };
    ...
    for (const auto &[netLayer, links] : layerLinks) {
        ...
        if (!allowedNetLayers.empty() &&
            allowedNetLayers.find(netLayer) == allowedNetLayers.end()) {
            continue;
        }
        ...
        if (!p2pLinks.empty()) {
            ...
            for (const auto *link : p2pLinks) {
                ...
                for (const auto &port : device->ports) {
                    if (port.layer == netLayer &&
                        port.linkType == LinkType::PEER2PEER &&
                        !port.netInstanceId.empty() &&
                        std::find(srcPort->portIds.begin(),
                                  srcPort->portIds.end(),
                                  port.portId) != srcPort->portIds.end()) {
                        uint32_t parsedK = 0;
                ...
                if (niidToLevelEntry.find(idxK) == niidToLevelEntry.end()) {
                    ...
                    levelEntry["net_instance_id"] =
                        buildNetInstanceId(spIdx, srvIdx, netLayer, idxK);
                ...
                int peerId = (link->sideA.localId == srcDevPhyId)
                                 ? link->sideB.localId
                                 : link->sideA.localId;
                if (commDomainLocalIds.find(peerId) ==
                    commDomainLocalIds.end()) {
                    continue;
                }
        ...
        if (!p2nLinks.empty()) {
            ...
            for (const auto &port : device->ports) {
                if (port.layer == netLayer &&
                    port.linkType == LinkType::PEER2NET && !port.eid.empty()) {
                    niidToEidPorts[port.netInstanceId][port.eid].push_back(
                        port.portId);
            ...
            if (!niidToEidPorts.empty()) {
                ...
                for (const auto &[rootinfoNiid, eidToPorts] : niidToEidPorts) {
                    ...
                    for (const auto &port : device->ports) {
                        if (port.layer == netLayer &&
                            port.linkType == LinkType::PEER2NET &&
                            !port.eid.empty()) {
                    ...
                    for (const auto &[eid, ports] : eidToPorts) {
                        ...
                        if (cleanEid.size() > 2 && cleanEid[0] == '0' &&
                            (cleanEid[1] == 'x' || cleanEid[1] == 'X')) {
                        ...
                        addrEntry["plane_id"] = (planeIt != eidToPlaneId.end())
                                                    ? planeIt->second
                                                    : "plane_pg_0";
    ...
    for (const auto &port : device->ports) {
        if (port.linkType == LinkType::PEER2NET && !port.eid.empty() &&
            ...
            // net_layer总层数 > 3时, 超出部分(前3层之外)不输出
            if (!allowedNetLayers.empty() &&
                allowedNetLayers.find(port.layer) == allowedNetLayers.end()) {
                continue;
            }
            ...
            for (const auto &p : device->ports) {
                if (p.layer == netLayer && p.linkType == LinkType::PEER2NET &&
                    !p.eid.empty()) {
        ...
        for (const auto &entry : levelList) {
            ...
            if (it != levelIndex.end()) {
                for (const auto &addr : entry["rank_addr_list"]) {
                    mergedLevelList[it->second]["rank_addr_list"].push_back(
                        addr);
                }
            } else {
...
json AscendClusterTopoParser::BuildRankEntry(
    const Server &server, int srcDevPhyId,
    const std::set<int> &commDomainLocalIds, uint32_t spIdx, uint32_t srvIdx,
    uint32_t rankId, bool skipLayerAbove2,
    const std::set<int> &allowedNetLayers) {
    json rankEntry;
    ...
    BuildLevelList(server, srcDevPhyId, commDomainLocalIds, spIdx, srvIdx,
                   rankEntry["level_list"], skipLayerAbove2, allowedNetLayers);

    return rankEntry;
}

HcclVmResult
AscendClusterTopoParser::CreateRankTableFile(const TopoMeta &topoMeta) {
    json rankTable;
    ...
    // 跨超节点通信域(topoMeta包含不止一个超节点/pod)时, layer3不过滤:
    // 不执行"单server跳过net_layer>=3"与"net_layer总层数>3只保留前3层"两条裁剪规则
    bool crossSuperPod = (topoMeta.size() > 1);
    bool skipLayerAbove2 = (serverNum == 1) && !crossSuperPod;
    if (skipLayerAbove2) {
        HCCL_VM_DEBUG("single server communication domain (server num = {:d}), "
                      "net_layer >= 3 entries are skipped in ranktable.json",
                      serverNum);
    }
    if (crossSuperPod) {
        HCCL_VM_DEBUG(
            "cross super pod communication domain (pod num = {:d}), "
            "net_layer >= 3 entries are not filtered in ranktable.json",
            topoMeta.size());
    }
    ...
    for (const auto &[podId, superPodMeta] : topoMeta) {
        ...
        for (const auto &[serverId, serverMeta] : superPodMeta) {
            if (serverId >= superPod.servers.size()) {
                HCCL_VM_ERROR("server id {:d} out of range in superpod {:d}",
                              serverId, podId);
                return HCCL_SIM_E_NOT_FOUND;
            }
            ...
            // 0,1,2,3时, 只输出net_layer 0,1,2的内容); 跨超节点通信域不裁剪:
            // layer3正常输出
            std::set<int> allowedNetLayers =
                crossSuperPod ? std::set<int>{}
                              : CollectAllowedNetLayers(server);
            if (!allowedNetLayers.empty()) {
                HCCL_VM_DEBUG(
            ...
            std::set<int> commDomainLocalIds(serverMeta.begin(),
                                             serverMeta.end());

            for (uint32_t deviceIdx = 0; deviceIdx < serverMeta.size();
                 ++deviceIdx) {
                ...
                rankTable["rank_list"].push_back(BuildRankEntry(
                    server, phyDevId, commDomainLocalIds, podId, serverId,
                    rankId, skipLayerAbove2, allowedNetLayers));
                rankId++;
            }
        }
    }
...
HcclVmResult AscendClusterTopoParser::InitClusterStaticTopoData() {
    // 静态拓扑批量写入合并为单事务提交: Server/Device/Ccu/Port/EndPoint等
    // 十余万条记录逐条INSERT的独立事务提交开销是start阶段的主要耗时之一
    sim::SqliteWriteTransaction writeTxn;
...
uint64_t AscendClusterTopoParser::InitServer(uint32_t superPodId,
                                             const Server &server) {
    HCCL_VM_DEBUG("Enter InitServer: {}, eid= {}", server.serverId,
                  server.hostEid.c_str());
    sim::Server serverDb;
    serverDb.pod_id = superPodId;
    serverDb.server_id = server.serverId;
    strncpy(serverDb.hardware_type, server.hardwareType.c_str(),
            sizeof(serverDb.hardware_type) - 1);
    ...
    HCCL_VM_DEBUG("host eid decoded: ip={}, superpod_id[8:5]={:d}",
                  host.ip_addr, eid_superpod_id_high(server.hostEid));
...
uint64_t AscendClusterTopoParser::InitPort(uint64_t deviceKey,
                                           const Port &portIn) {
    sim::Port portDb{};
    ...
    endPoint.type = 0;
    // port/port_group配置protocols含UBOE时(如ascend950_server_topo_normal_uboe.yaml
    // 的port_group layer2 UBOE端口), 标记EndPoint为UBOE端口
    endPoint.is_uboe =
        std::find(portIn.protocols.begin(), portIn.protocols.end(),
                  Protocol::UBOE) != portIn.protocols.end();
    // 空EID端口(如rootinfo中被裁剪层的端口)不绑定EndPoint
    static const uint8_t zeroEid[16] = {0};
    if (!portIn.eid.empty()) {
        ...
        HCCL_VM_DEBUG(
            "port {} eid decoded: ip={}, die_id={:d}, port_num={:d}(PG={})",
            portIn.portId.c_str(), endPoint.ip_addr, eid_die_id(portIn.eid),
            eid_port_num(portIn.eid), eid_is_pg_port(portIn.eid));
        if (memcmp(endPoint.eid, zeroEid, sizeof(endPoint.eid)) != 0) {
            auto endPointKey = RunnerDB::Add<sim::EndPoint>(endPoint);
            ipToEndpointId_.emplace(std::string(endPoint.ip_addr), endPointKey);
            ...
            auto endPointPortMappingKey =
                RunnerDB::Add<sim::EndPointPortMapping>(endPointPortMapping);
        }
    }
    return portKey;
}

uint64_t AscendClusterTopoParser::InitDevice(uint64_t serverKey,
...
uint64_t AscendClusterTopoParser::FindEndPointIdByIp(const std::string &ip) {
    ...
    auto ret =
        RunnerDB::GetOneByPred<sim::EndPoint>([&ip](const sim::EndPoint &d) {
            return strcmp(d.ip_addr, ip.c_str()) == 0;
        });
    if (!ret.second) {
        return 0;
    }
    ipToEndpointId_.emplace(ip, ret.first.id);
    return ret.first.id;
}
...
HcclVmResult AscendClusterTopoParser::AddLinkInfo(
    ...
    if (srcPort != nullptr && srcPort->localId != -1 && !srcPort->eid.empty()) {
        ...
        // 更新Endpoint status
        RunnerDB::Update<sim::EndPoint>(
            srcEndPointId, [](sim::EndPoint &ep) { ep.status = 1; });
    }
    ...
    if (dstPort != nullptr && dstPort->localId != -1 && !dstPort->eid.empty()) {
        ...
        // 更新Endpoint status
        RunnerDB::Update<sim::EndPoint>(
            dstEndPointId, [](sim::EndPoint &ep) { ep.status = 1; });
    }
...
HcclVmResult
AscendClusterTopoParser::ParseRanktable(const std::string &ranktablePath,
                                        TopoMeta &topoMeta) {
    ...
    for (const auto &rank : rankTable["rank_list"]) {
        ...
        for (const auto &level : levelList) {
            if (level.value("net_layer", -1) == 0) {
                ...
                if (sscanf(netInstanceId.c_str(), "superPod%u_rack%u", &spIdx,
                           &srvIdx) == 2) {
                    found = true;
                    break;
                }
        ...
        if (!found) {
            for (const auto &level : levelList) {
                const auto &addrList =
                    level.value("rank_addr_list", json::array());
                if (!addrList.empty()) {
                    ...
                    if (it != network_.eidIndex.end() && !it->second.empty()) {
                        spIdx =
                            static_cast<uint32_t>(std::get<0>(it->second[0]));
                        srvIdx =
                            static_cast<uint32_t>(std::get<1>(it->second[0]));
                        found = true;
                        break;
                    }
                }
