...
bool IsCcuDagLayoutNode(const V3TaskNode *node) {
    ...
    return node->HasCcuTrace() ||
           IsV3Boundary(node, BoundaryType::CCU_SUB_GRAPH);
}
...
const char *TaskTypeKeyName(TaskType type) {
    ...
    case TaskType::MODEL_EXEC_START:
        return "MODEL_EXEC_START";
    case TaskType::MODEL_EXEC_START_SUB:
        return "MODEL_EXEC_START_SUB";
    case TaskType::MODEL_EXEC_END_SUB:
        return "MODEL_EXEC_END_SUB";
    case TaskType::MODEL_EXEC_END:
        return "MODEL_EXEC_END";
    default:
        return "INVALID";
    }
}
...
std::string MakeCompactMemSliceLabel(const MemSlice &slice) {
    ...
    os << "{device=" << FormatGraphvizIdValue(slice.deviceId, INVALID_DEVICE_ID)
       << ", type=" << DescribeMemType(slice.memType) << ",offset=0x"
       << std::hex << slice.offset << ",len=0x" << slice.len << std::dec << "}";
    return os.str();
}
...
std::string MakeAicpuNotifyLabel(const AicpuNotify &notify) {
    std::ostringstream os;
    os << "notify{srcDevice="
       << FormatGraphvizIdValue(notify.recordDeviceId, INVALID_DEVICE_ID)
       << ", dstDevice="
       << FormatGraphvizIdValue(notify.waitDeviceId, INVALID_DEVICE_ID)
       << ", notifyId="
       << FormatGraphvizIdValue(notify.notifyId, INVALID_NOTIFY_ID) << "}";
    return os.str();
}
...
std::string MakeCcuNotifyLabel(const CcuNotify &notify) {
    ...
    os << "notify{srcDevice="
       ...
       << ", mask="
       << FormatGraphvizHexValue(notify.ckeMask, INVALID_CCU_CKE_MASK) << "}";
    return os.str();
}
...
QueueId GetV3GraphvizQueueId(const V3TaskNode *node) {
    ...
    if (node->HasCcuTrace() &&
        node->GetCcuTrace().queueId != INVALID_QUEUE_ID) {
        return node->GetCcuTrace().queueId;
    }
    return node->GetPosition().queueId;
}

std::string GetV3GraphvizCommLabel(const TaskPosition &position,
                                   const V3CommNameMap &commNames) {
...
void AppendV3GraphvizPositionLine(std::ostringstream &os,
                                  const V3TaskNode *node,
                                  const V3CommNameMap &commNames) {
    if (node == nullptr) {
        return;
    }
...
bool AppendV3GraphvizBoundaryLabel(std::ostringstream &os,
                                   const V3TaskNode *node) {
...
void AppendV3GraphvizTraceSummary(std::ostringstream &os,
                                  const V3TaskNode *node) {
    ...
    os << "\ninstr=" << trace.instrId
       ...
       << ", mission="
       << FormatGraphvizIdValue(trace.missionId, INVALID_MISSION_ID);
}
...
bool AppendV3GraphvizTaskLabel(std::ostringstream &os, const V3TaskNode *node) {
    ...
    case TaskType::TRANS_MEM: {
        ...
        os << "\nsrc" << MakeCompactMemSliceLabel(transMem->GetSrc()) << "\ndst"
           << MakeCompactMemSliceLabel(transMem->GetDst());
        break;
    }
    case TaskType::BATCH_TRANS_MEM: {
    ...
    case TaskType::REDUCE: {
        ...
        os << "\nsrc" << MakeCompactMemSliceLabel(reduce->GetSrc()) << "\ndst"
           << MakeCompactMemSliceLabel(reduce->GetDst())
    ...
    case TaskType::BATCH_REDUCE: {
        ...
        if (!srcGroups.empty() && !srcGroups.front().empty()) {
            os << "\nsrc0"
               << MakeCompactMemSliceLabel(srcGroups.front().front());
        }
        if (!dsts.empty()) {
            os << "\ndst0" << MakeCompactMemSliceLabel(dsts.front());
        }
    ...
    case TaskType::AIV_GRAPH: {
        ...
        os << "\ndevice="
           << FormatGraphvizIdValue(aivGraph->GetDeviceId(), INVALID_DEVICE_ID)
           << ", launch=" << aivGraph->GetLaunchIdx()
           << ", hostStream=" << aivGraph->GetHostStreamId();
        break;
    }
    case TaskType::AIV_SET_FLAG:
    case TaskType::AIV_WAIT_FLAG: {
        const auto *flag = node->GetType() == TaskType::AIV_SET_FLAG
                               ? dynamic_cast<const TaskAivSetFlag *>(node)
                               : nullptr;
        const auto *wait = node->GetType() == TaskType::AIV_WAIT_FLAG
                               ? dynamic_cast<const TaskAivWaitFlag *>(node)
                               : nullptr;
        const AivPipeEvent *event =
            flag == nullptr ? (wait == nullptr ? nullptr : &wait->GetEvent())
                            : &flag->GetEvent();
        if (event == nullptr) {
            return false;
        }
    ...
    case TaskType::AIV_SEND_FLAG:
    case TaskType::AIV_RECV_FLAG: {
        const auto *send = node->GetType() == TaskType::AIV_SEND_FLAG
                               ? dynamic_cast<const TaskAivSendFlag *>(node)
                               : nullptr;
        const auto *recv = node->GetType() == TaskType::AIV_RECV_FLAG
                               ? dynamic_cast<const TaskAivRecvFlag *>(node)
                               : nullptr;
        const AivFlagSync *flag =
            send == nullptr ? (recv == nullptr ? nullptr : &recv->GetFlag())
                            : &send->GetFlag();
        if (flag == nullptr) {
            return false;
        }
        os << "\nflagOwnerDevice="
           << FormatGraphvizIdValue(flag->flagOwnerDevice, INVALID_DEVICE_ID)
           << ", commInfoOffset=" << flag->commInfoOffset
           << ", value=" << flag->value;
        break;
    }
    case TaskType::SYNC_STREAM: {
    ...
    case TaskType::MODEL_EXEC_END: {
        ...
        os << "\nexecSeq=" << modelExec->GetExecSeq()
           << ", peerStreamId=" << modelExec->GetPeerStreamId();
        break;
    }
    default:
        return false;
...
std::string MakeV3GraphvizNodeLabel(const V3TaskNode *node,
    ...
    if (!isBoundaryNode && !AppendV3GraphvizTaskLabel(os, node) &&
        node == nullptr) {
        os << "\n<invalid v3 node>";
    }
    if (!isBoundaryNode) {
...
V3CommNameMap BuildV3CommNameMap() {
    ...
    for (const sim::Communicator &communicator : communicators) {
        const size_t commNameLen =
            ::strnlen(communicator.comm_id, sizeof(communicator.comm_id));
        if (commNameLen == 0U || commNameLen == sizeof(communicator.comm_id)) {
            continue;
        }
        commNames.emplace(communicator.id,
                          std::string(communicator.comm_id, commNameLen));
    }
    return commNames;
}
...
bool PreferRecordCandidate(const std::pair<LaneKey, NodeId> &lhs,
                           const std::pair<LaneKey, NodeId> &rhs) {
...
bool IsNotifyEdge(const V3TaskNode *srcNode, const V3TaskNode *dstNode) {
    return srcNode != nullptr && dstNode != nullptr &&
           srcNode->GetType() == TaskType::RECORD &&
           dstNode->GetType() == TaskType::WAIT;
}

FormalGraphLayout
BuildFormalGraphLayout(const std::vector<const V3TaskNode *> &nodes) {
    ...
    while (!readyNodeIds.empty()) {
        ...
        if (nodeIter == nodeById.end() || nodeIter->second == nullptr) {
            HCCL_VM_WARN("[TaskGraphGeneratorV3][GraphvizDot][v3] Topo node is "
        ...
        for (NodeId childId : childIter->second) {
            ...
            if (indegreeIter == indegreeById.end() ||
                indegreeIter->second == 0U) {
                continue;
            }
            --indegreeIter->second;
    ...
    if (topoNodes.size() != layoutNodes.size()) {
        HCCL_VM_WARN("[TaskGraphGeneratorV3][GraphvizDot][v3] Failed to build "
    ...
    for (const V3TaskNode *node : topoNodes) {
        for (const V3TaskNode *child : node->GetChildren()) {
            ...
            if (layout.nodeLayoutById.count(node->GetNodeId()) == 0 ||
                layout.nodeLayoutById.count(child->GetNodeId()) == 0 ||
                !IsNotifyEdge(node, child)) {
                continue;
            }

            const std::pair<LaneKey, NodeId> candidate(MakeLaneKey(node),
                                                       node->GetNodeId());
            const auto primaryIter =
                primaryRecordByWait.find(child->GetNodeId());
            if (primaryIter == primaryRecordByWait.end() ||
                PreferRecordCandidate(candidate, primaryIter->second)) {
                primaryRecordByWait[child->GetNodeId()] = candidate;
            }
        }
    }

    std::map<NodeId, std::vector<std::pair<NodeId, uint32_t>>>
        incomingLayoutEdges;
    ...
    for (const V3TaskNode *node : topoNodes) {
        ...
        if (lastLaneIter != lastNodeByLaneId.end()) {
            incomingLayoutEdges[nodeId].push_back(
                std::make_pair(lastLaneIter->second, 1U));
        }
        lastNodeByLaneId[laneId] = nodeId;

        for (const V3TaskNode *parent : node->GetParents()) {
            if (parent == nullptr ||
                parent->GetNodeId() == MAIN_START_NODE_ID) {
                continue;
            }
            ...
            if (IsNotifyEdge(parent, node)) {
                ...
                if (primaryIter != primaryRecordByWait.end() &&
                    primaryIter->second.second == parent->GetNodeId()) {
                    colStep = 0U;
                }
            }
            incomingLayoutEdges[nodeId].push_back(
                std::make_pair(parent->GetNodeId(), colStep));
        }
    }
    ...
    for (const V3TaskNode *node : topoNodes) {
        ...
        if (incomingIter != incomingLayoutEdges.end()) {
            for (const auto &entry : incomingIter->second) {
                const auto parentInfoIter =
                    layout.nodeLayoutById.find(entry.first);
...
std::string MakeGenericLaneGroup(const LaneKey &laneKey) {
    std::ostringstream os;
    os << "lane_d" << FormatV3DotGroupValue(laneKey.deviceId, INVALID_DEVICE_ID)
       << "_s" << FormatV3DotGroupValue(laneKey.streamId, INVALID_STREAM_ID)
       << "_q" << FormatV3DotGroupValue(laneKey.queueId, INVALID_QUEUE_ID);
    return os.str();
}

std::string MakeV3DotNodeGroup(const V3TaskNode *node,
    ...
    if (layoutIter != layout.nodeLayoutByPtr.end()) {
        const auto laneIter =
            layout.laneLayoutById.find(layoutIter->second.laneId);
    ...
    const auto &loc = node->GetPosition();
    std::ostringstream os;
    os << "node_d" << FormatV3DotGroupValue(loc.deviceId, INVALID_DEVICE_ID)
       << "_s" << FormatV3DotGroupValue(loc.streamId, INVALID_STREAM_ID) << "_q"
       << FormatV3DotGroupValue(NormalizeQueueId(GetV3GraphvizQueueId(node)),
                                INVALID_QUEUE_ID);
    return os.str();
}
...
double GetDefaultLaneY(const FormalGraphLayout &layout) {
    ...
    return -static_cast<double>(layout.laneLayoutById.size() - 1U) *
           V3_DOT_ROW_GAP / 2.0;
}

std::pair<double, double> GetV3DotNodePosition(const V3TaskNode *node,
                                               const FormalGraphLayout &layout,
    ...
    if (layoutIter == layout.nodeLayoutByPtr.end()) {
        ...
        return {static_cast<double>(fallbackIndex) * V3_DOT_COL_GAP,
                GetDefaultLaneY(layout)};
    }

    const NodeLayoutInfo &nodeInfo = layoutIter->second;
    const auto laneIter = layout.laneLayoutById.find(nodeInfo.laneId);
    const uint32_t row =
        (laneIter == layout.laneLayoutById.end()) ? 0U : laneIter->second.row;
    return {static_cast<double>(nodeInfo.col) * V3_DOT_COL_GAP,
            -static_cast<double>(row) * V3_DOT_ROW_GAP};
}

bool IsV3DotSameLane(const std::vector<const V3TaskNode *> &nodes,
                     const V3DotEdge &edge, const FormalGraphLayout &layout) {
    ...
    return fromIter != layout.nodeLayoutByPtr.end() &&
           toIter != layout.nodeLayoutByPtr.end() &&
           fromIter->second.laneId == toIter->second.laneId;
}

bool IsV3DotSolidEdge(const std::vector<const V3TaskNode *> &nodes,
                      const V3DotEdge &edge, const FormalGraphLayout &layout) {
    if (edge.first >= nodes.size() || edge.second >= nodes.size()) {
        return false;
    }
...
std::string
MakeV3DotSolidEdgeGroupKey(const std::vector<const V3TaskNode *> &nodes,
                           const V3DotEdge &edge,
                           const FormalGraphLayout &layout) {
    const auto laneIter = (edge.first < nodes.size())
                              ? layout.nodeLayoutByPtr.find(nodes[edge.first])
                              : layout.nodeLayoutByPtr.end();
    const auto laneLayoutIter =
        (laneIter == layout.nodeLayoutByPtr.end())
            ? layout.laneLayoutById.end()
...
void EmitV3DotSolidChain(std::ostringstream &os,
...
void EmitV3DotSolidEdgeGroup(std::ostringstream &os, const V3DotEdgeList &edges,
                             const std::string &nodePrefix) {
...
void EmitV3DotDependencyEdges(std::ostringstream &os,
                              const V3DotEdgeSet &edgeIndexes,
                              const std::vector<const V3TaskNode *> &nodes,
                              const FormalGraphLayout &layout,
                              const std::string &nodePrefix) {
    ...
    for (const auto &edge : edgeIndexes) {
        if (IsV3DotSolidEdge(nodes, edge, layout)) {
            solidEdgesByGroup[MakeV3DotSolidEdgeGroupKey(nodes, edge, layout)]
                .push_back(edge);
        } else {
    ...
    for (const auto &edge : dashedEdges) {
        os << "  " << nodePrefix << edge.first << " -> " << nodePrefix
           << edge.second;
...
std::string BuildV3GraphvizDot(const std::vector<const V3TaskNode *> &nodes) {
    ...
    for (const V3TaskNode *node : nodes) {
        ...
        for (const V3TaskNode *childNode : node->GetChildren()) {
            ...
            edgeIndexes.insert(
                std::make_pair(parentIter->second, childIter->second));
        }
    }
    ...
    os << "  edge [fontname=\"Courier\", arrowhead=normal, arrowsize="
       << V3_DOT_EDGE_ARROWSIZE << "];\n\n";
    for (size_t i = 0; i < nodes.size(); ++i) {
        const std::string group = MakeV3DotNodeGroup(nodes[i], layout);
        os << "  v3_" << i << " [label=\""
           << EscapeGraphvizLabel(MakeV3GraphvizNodeLabel(nodes[i], commNames))
           << "\"";
        const auto pos = GetV3DotNodePosition(nodes[i], layout, i);
        os << std::fixed << std::setprecision(3) << ", pos=\"" << pos.first
           << "," << pos.second << "!\"";
        if (!group.empty()) {
            os << ", group=\"" << group << "\"";
        }
...
std::string MakeDagGraphvizTimestamp() {
    ...
    char timeBuffer[32]{};
    (void)std::strftime(timeBuffer, sizeof(timeBuffer), "%Y%m%d%H%M%S",
                        &localTime);
    return timeBuffer;
}
...
std::string ResolveDagGraphvizDumpDir() {
    ...
    if (!rootPath.empty()) {
        return JoinPath(JoinPath(rootPath, DAG_GRAPHVIZ_DUMP_ANCHOR),
                        DAG_GRAPHVIZ_DUMP_SUBDIR);
    }
    return "";
}

std::string MakeDagGraphvizDumpPath() {
    return JoinPath(ResolveDagGraphvizDumpDir(),
                    "TaskGraph_" + MakeDagGraphvizTimestamp() + ".dot");
}

} // namespace

HcclResult DumpDagGraphvizDot(const TaskGraphGeneratorV3::TaskNode *start,
                              std::string *dumpPath) {
    if (start == nullptr) {
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_VM_WARN("[TaskGraphGeneratorV3][GraphvizDot][v3] Failed to create "
                     "dump dir: {}, ret={}",
    ...
    if (!out.is_open()) {
        HCCL_VM_WARN("[TaskGraphGeneratorV3][GraphvizDot][v3] Failed to open "
                     "graph file: {}",
                     path);
    ...
    out.close();
    if (!out) {
        HCCL_VM_WARN("[TaskGraphGeneratorV3][GraphvizDot][v3] Failed to write "
                     "graph file: {}",
                     path);
        return HCCL_E_INTERNAL;
    }
    ...
    HCCL_VM_INFO("[TaskGraphGeneratorV3][GraphvizDot][v3] Graphviz DAG dumped, "
                 "path={}, nodeCount={}",
