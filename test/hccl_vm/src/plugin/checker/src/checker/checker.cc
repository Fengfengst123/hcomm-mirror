...
uint64_t ElapsedV3Ns(V3StageClock::time_point start,
                     V3StageClock::time_point end) {
...
class V3StageTimer {
  public:
    explicit V3StageTimer(const char *stage)
        : stage_(stage), start_(V3StageClock::now()) {}

    ~V3StageTimer() {
        const uint64_t elapsedNs = ElapsedV3Ns(start_, V3StageClock::now());
        if (extraInfo_.empty()) {
            HCCL_VM_INFO(
                "CheckerV3 stage finished, stage={}, status={}, costMs={}",
                stage_, status_, elapsedNs / V3_NS_PER_MS);
            return;
        }
        HCCL_VM_INFO(
            "CheckerV3 stage finished, stage={}, status={}, costMs={}, {}",
            stage_, status_, elapsedNs / V3_NS_PER_MS, extraInfo_);
    }

    void SetStatus(const char *status) { status_ = status; }

    void SetExtraInfo(const std::string &extraInfo) { extraInfo_ = extraInfo; }
...
bool IsV3Boundary(const V3TaskNode *node,
                  TaskGraphGeneratorV3::BoundaryType boundaryType) {
    if (node == nullptr) {
        return false;
    }
    if (node->GetType() == TaskGraphGeneratorV3::TaskType::START) {
        auto *start =
            dynamic_cast<const TaskGraphGeneratorV3::TaskStart *>(node);
...
bool IsAivSyntheticDataNode(const V3TaskNode *node) {
    if (node == nullptr || !IsAivDataTaskType(node->GetType()) ||
        node->HasCcuTrace()) {
        return false;
    }
...
bool IsAivDagLogNode(const V3TaskNode *node) {
    ...
    return IsAivTaskType(node->GetType()) || IsAivSyntheticDataNode(node) ||
           IsV3Boundary(node,
                        TaskGraphGeneratorV3::BoundaryType::AIV_SUB_GRAPH);
}

std::unique_ptr<TaskGraphGeneratorV3::TaskGraphGeneratorV3>
GenGraphV3FromTaskMeta() {
    ...
    // 3. 再生成 V3 图并在日志里记录成图/CCU 展开耗时。
    {
        ...
        HcclResult ret = taskMetaTranslatorV3.Translate(
            storage, TaskGraphGeneratorV3::INVALID_OPERATOR_ID);
        if (ret != HCCL_SUCCESS) {
            HCCL_VM_WARN("Failed to translate one task into the V3 internal "
    ...
    ret = TaskGraphGeneratorV3::CheckSlaveTaskQueue(translatedNodes,
                                                    translatedTaskQueues);
    if (ret != HCCL_SUCCESS) {
        HCCL_VM_WARN(
            "Slave-stream check failed, V3 graph generation is stopped, ret={}",
            static_cast<uint32_t>(ret));
        return nullptr;
    }
    std::unique_ptr<TaskGraphGeneratorV3::TaskGraphGeneratorV3>
        graphGeneratorV3(new (std::nothrow)
                             TaskGraphGeneratorV3::TaskGraphGeneratorV3);
...

    {
        ...
        ret = graphGeneratorV3->GenGraph(std::move(translatedNodes),
                                         std::move(translatedTaskQueues));
        if (ret != HCCL_SUCCESS) {
            HCCL_VM_WARN("Failed to build the graph from translated tasks, "
                         "ret={}, rankCount={}, nodeCount={}",
                         static_cast<uint32_t>(ret),
                         graphGeneratorV3->GetTaskQueues().size(),
    ...
    timerExtraInfo << "taskNodeCount=" << graphGeneratorV3->GetNodes().size()
                   << ", mainStartNodeId="
                   << graphGeneratorV3->GetMainStartNodeId()
                   << ", aivExpandTotalMs="
                   << (aivExpandStats.totalExpandNs / V3_NS_PER_MS)
                   << ", aivGraphCount=" << aivExpandStats.graphCount
                   << ", ccuExpandTotalMs="
                   << (ccuExpandStats.totalExpandNs / V3_NS_PER_MS)
...
const char *TaskTypeKeyName(TaskGraphGeneratorV3::TaskType type) {
    ...
    case TaskGraphGeneratorV3::TaskType::SYNC_STREAM:
        return "SYNC_STREAM";
    case TaskGraphGeneratorV3::TaskType::MODEL_EXEC_START:
        return "MODEL_EXEC_START";
    case TaskGraphGeneratorV3::TaskType::MODEL_EXEC_START_SUB:
        return "MODEL_EXEC_START_SUB";
    case TaskGraphGeneratorV3::TaskType::MODEL_EXEC_END_SUB:
        return "MODEL_EXEC_END_SUB";
    case TaskGraphGeneratorV3::TaskType::MODEL_EXEC_END:
...
std::string MakeAivDagTaskLogLine(const V3TaskNode *node, size_t bfsIndex) {
    ...
    const auto &loc = node->GetPosition();
    std::ostringstream os;
    os << "bfsIndex=" << bfsIndex << ", nodeId=" << node->GetNodeId()
       << ", taskType=" << TaskTypeKeyName(node->GetType())
       << ", rankId=" << loc.rankId << ", streamId=" << loc.streamId
       << ", queueId=" << loc.queueId
...
size_t
LogAivExpandedDagTasks(const std::vector<const V3TaskNode *> &v3BfsNodes) {
    ...
    for (size_t index = 0; index < v3BfsNodes.size(); ++index) {
        ...
        HCCL_VM_DEBUG("Expanded AIV DAG node detail, {}",
                      MakeAivDagTaskLogLine(node, index));
        ++aivTaskCount;
    }
    HCCL_VM_DEBUG(
        "Finished dumping expanded AIV DAG node details, taskCount={}",
        aivTaskCount);
    return aivTaskCount;
}
...
void LogGraphV3Sqe(const TaskGraphGeneratorV3::TaskGraphGeneratorV3 &graph) {
    ...
    for (const auto &iter : translatedTaskQueues) {
        ...
        for (size_t i = 0; i < taskQueue.size(); ++i) {
            ...
            uint32_t taskIdx = 0;
            HCCL_VM_INFO("streamIdx : {:d}, taskNum : {:d}",
            ...
            for (const auto &nodeId : taskQueue[i]) {
                if (nodeId < 0 ||
                    static_cast<size_t>(nodeId) >= translatedNodes.size() ||
                    translatedNodes[static_cast<size_t>(nodeId)] == nullptr) {
                    HCCL_VM_WARN(
                        "rankIdx:{:d}, taskIdx:{:d}, invalid nodeId:{:d}",
                        rankIdx, taskIdx, nodeId);
                    ++taskIdx;
                    continue;
                }
                ...
                HCCL_VM_INFO("rankIdx:{:d}, taskIdx:{:d}, {:s}", rankIdx,
                             taskIdx, tempStr);
                ++taskIdx;
            }
        }
...
HcclResult CheckGraphV3TaskMem(const V3TaskNode *start) {
    ...
    if (TaskGraphGeneratorV3::g_checkerAivUbBufferSize != 0 ||
        ...
        extraInfo << ", aivUbBufferSize="
                  << TaskGraphGeneratorV3::g_checkerAivUbBufferSize
                  << ", aivCommInfoSize="
                  << TaskGraphGeneratorV3::g_checkerAivCommInfoSize;
    }
...
HcclResult CheckGraphV3MemConflict(const V3TaskNode *start) {
    ...
    if (start == nullptr) {
        HCCL_VM_WARN(
            "Skip memory-conflict check because the graph start node is null");
        timer.SetStatus("skipped");
        return HCCL_SUCCESS;
    }
    ...
    HcclResult ret =
        TaskGraphGeneratorV3::CheckDataMoveTaskMemConflict(start, &stats);
    std::ostringstream extraInfo;
    extraInfo << "nodeCount=" << stats.nodeCount
              << ", dataTaskNodeCount=" << stats.dataTaskNodeCount
              << ", processedDataTaskNodeCount="
...
HcclResult CheckGraphV3SyncConflict(const V3TaskNode *start) {
    ...
    if (start == nullptr) {
        HCCL_VM_WARN(
            "Skip sync-conflict check because the graph start node is null");
        timer.SetStatus("skipped");
        return HCCL_SUCCESS;
    }
    ...
    HcclResult ret =
        TaskGraphGeneratorV3::CheckSyncResourceConflict(start, &stats);
    std::ostringstream extraInfo;
...
HcclResult CheckGraphV3Semantic(const V3TaskNode *start) {
    ...
    if (start == nullptr) {
        HCCL_VM_WARN(
            "Skip semantic check because the graph start node is null");
        timer.SetStatus("skipped");
        return HCCL_SUCCESS;
    }
