    ...
    RankId myRank, RankId srcRank, RankId dstRank, VDataDesTagInner vDataDes, All2AllDataDesTagInner all2AllDataDes)
{
    ...
    if (opType == HcclCMDType::HCCL_CMD_ALLREDUCE) {
        ...
        for (u32 j = 0; j < rankSize; j++) {
            ...
            u64 curSendLength = curSendCounts * CHECK_SIZE_TABLE[all2AllDataDes.sendType];
            curSendOffset += curSendLength;
            u64 curRecvCounts = all2AllDataDes.sendCountMatrix[myRank + rankSize * j];
            u64 curRecvLength = curRecvCounts * CHECK_SIZE_TABLE[all2AllDataDes.recvType];
        ...
        outputSize = vDataDes.counts[myRank] * CHECK_SIZE_TABLE[vDataDes.dataType];
        ...
        inputSize = vDataDes.counts[myRank] * CHECK_SIZE_TABLE[vDataDes.dataType];
    } else {
