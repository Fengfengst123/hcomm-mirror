...
HcclResult DumpV3Manager::Initialize(const std::string &dataId) {
    ...
    if (dataId.empty()) {
        HCCL_VM_ERROR("{} dataId is empty, the dump output path cannot be "
    ...
    if (m_pluginRootDir.empty()) {
        HCCL_VM_ERROR("{} Failed to get the current working directory for dump "
                      "output, dataId={}",
    ...
    m_rootDir = HcclSim::JoinPath(HcclSim::JoinPath(m_dataDir, kInsightDirName),
                                  m_dataId);
...
std::string
DumpV3Manager::GetOutputPath(const std::string &relativePath) const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return HcclSim::JoinPath(m_rootDir, relativePath);
}

HcclResult DumpV3Manager::WriteMsgpack(const std::string &relativePath,
    ...
    try {
        ...
        HCCL_VM_ERROR("{} Failed to serialize dump content into msgpack, "
                      "file={}, dataId={}, reason={}",
    ...
    out.flush();
    if (!out.good()) {
        HCCL_VM_ERROR("{} Failed to flush dump file to disk, file={}, "
...
HcclResult DumpV3Manager::WriteJson(const std::string &relativePath,
                                    const nlohmann::json &document) const {
    if (!IsEnabled()) {
        return HcclResult::HCCL_SUCCESS;
    }
    ...
    if (!out.is_open()) {
        HCCL_VM_ERROR(
            "{} Failed to open dump file, file={}, format=json, dataId={}",
            MakeErrorCodeText(ErrorCode::DUMP_FAILED), fullPath, m_dataId);
        return HcclResult::HCCL_E_INTERNAL;
    }
    ...
    try {
        ...
        HCCL_VM_ERROR("{} Failed to serialize dump content into JSON, file={}, "
                      "dataId={}, reason={}",
                      MakeErrorCodeText(ErrorCode::DUMP_FAILED), fullPath,
                      m_dataId, ex.what());
        return HcclResult::HCCL_E_INTERNAL;
    ...
    if (!out.good()) {
        HCCL_VM_ERROR("{} Failed to write dump file content, file={}, "
                      "format=json, dataId={}",
                      MakeErrorCodeText(ErrorCode::DUMP_FAILED), fullPath,
                      m_dataId);
        return HcclResult::HCCL_E_INTERNAL;
...
HcclResult DumpV3Manager::PrepareDirs() const {
    ...
    ret =
        HcclSim::EnsureDirectory(HcclSim::JoinPath(m_dataDir, kInsightDirName));
    if (ret != HcclResult::HCCL_SUCCESS) {
        return ret;
    }
