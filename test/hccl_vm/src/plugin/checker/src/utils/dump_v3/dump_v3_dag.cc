...
std::string GetProtocolName(const TaskNode *node) {
    ...
    case TaskType::TRANS_MEM: {
        ...
        return task->GetProtocol() == ProtocolType::INVALID
                   ? std::string{}
                   : ToString(task->GetProtocol());
    }
    case TaskType::BATCH_TRANS_MEM: {
...
Json ToJson(const MemSlice &slice) {
    ...
    if (slice.memType == MemType::MS_CCU &&
        slice.offset % kMsSliceBytes == 0ULL) {
...
Json MakeNodeJson(const NodeDumpInfo &nodeInfo) {
    ...
    } else if (const auto *batchReduce =
                   dynamic_cast<const TaskBatchReduce *>(node)) {
...
HcclResult
CollectTopoNodes(const TaskGraphGeneratorV3::TaskGraphGeneratorV3 &graph,
                 std::vector<const TaskNode *> &topoNodes) {
    topoNodes.clear();
    const TaskNode *mainStart = graph.GetMainStartNode();
    if (!IsMainGraphStartNode(mainStart)) {
        HCCL_VM_ERROR(
            "{} Main start node of the V3 graph is invalid, mainStartNodeId={}",
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_VM_ERROR("{} Failed to build the V3 graph topological order for "
                      "dump output, "
                      "mainStartNodeId={}, ret={}",
                      MakeErrorCodeText(ErrorCode::DUMP_FAILED),
    ...
    for (NodeId nodeId : topoOrder) {
        ...
        if (node == nullptr) {
            HCCL_VM_ERROR(
                "{} One node in topological order is missing, nodeId={}, "
                "processedTopoNodeCount={}",
                MakeErrorCodeText(ErrorCode::DUMP_FAILED), nodeId,
                topoNodes.size());
            return HCCL_E_PTR;
        }
...
Json MakeStageStats(const std::vector<Json> &laneDocs,
                    const std::vector<Json> &nodeDocs,
...
HcclResult DumpV3Dag(const TaskGraphGeneratorV3::TaskGraphGeneratorV3 &graph,
    ...
    if (stage.empty()) {
        HCCL_VM_ERROR("{} Dump stage name is empty, dataId={}, "
                      "graphOutputPath={}, layoutOutputPath={}",
                      MakeErrorCodeText(ErrorCode::DUMP_FAILED),
                      StorageManager::GetInstance().GetDataId(), kGraphPath,
                      kLayoutPath);
        return HCCL_E_PARA;
    }
    ...
    for (const TaskNode *node : topoNodes) {
        ...
        for (const TaskNode *child : children) {
            ...
            if (srcIter == nodeInfoById.end() ||
                dstIter == nodeInfoById.end()) {
                continue;
            }
            ...
            edgeInfo.kind =
                IsNotifyEdge(node, child)
                    ? "notify"
                    : ((srcIter->second.laneId == dstIter->second.laneId)
                           ? "lane_order"
                           : "dependency");
    ...
    for (const TaskNode *node : topoNodes) {
        ...
        for (const TaskNode *parent : node->GetParents()) {
            ...
            incomingLayoutEdges[nodeId].push_back(
                std::make_pair(parent->GetNodeId(), weight));
        }
    }
    ...
    for (auto &edgeInfo : edgeInfos) {
        if (edgeInfo.kind == "notify") {
            ...
            if (primaryIter != primaryRecordByWait.end() &&
                primaryIter->second.second == edgeInfo.src &&
            ...
            notifyPair["align_mode"] =
                edgeInfo.primaryAlign ? "primary_same_col" : "dependency_only";
    ...
    DumpRunManifest::GetInstance().SetGraphStageStats(
        stage, MakeStageStats(graphDoc["lanes"].get<std::vector<Json>>(),
                              graphDoc["nodes"].get<std::vector<Json>>(),
                              graphDoc["edges"].get<std::vector<Json>>()));

    HCCL_VM_INFO(
        "V3 DAG dump finished, laneCount={}, nodeCount={}, edgeCount={}",
