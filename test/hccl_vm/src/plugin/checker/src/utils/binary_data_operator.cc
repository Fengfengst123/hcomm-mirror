...
HcclResult FileHeaderRead(FILE *fp, FileHeader &header, uint32_t magic) {
    ...
    if (header.magic != magic) {
        HCCL_VM_ERROR("Unmatched magic number:{:#x}≠{:#x}", header.magic,
                      magic);
...
HcclResult HcclVmSynDataRead(FILE *fp, HcclVmSynData &synData, uint32_t magic) {
    ...
    bool isCcuMode =
        synData.model_info.comm.op_expansion_mode == SIM_OP_EXPANSION_MODE_CCU;
...
HcclResult HcclVmInstrDataRead(FILE *fp, HcclVmInstrData &instrData,
                               uint32_t magic) {
...
HcclResult All2AllDataDesTagRead(FILE *fp,
...
HcclResult MicrocodeInstrWrite(FILE *fp,
...
// task读写
HcclResult HcclVmTaskMetaDataWrite(FILE *fp,
...
HcclResult HcclVmTaskMetaDataRead(FILE *fp, HcclVmTaskMetaData &taskMeta,
                                  uint32_t magic) {
...
HcclResult HcclVmFlagDataRead(FILE *fp, HcclVmFlagData &flagData,
                              uint32_t magic) {
