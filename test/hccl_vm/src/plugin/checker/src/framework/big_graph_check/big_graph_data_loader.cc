...
HcclResult BigGraphDataLoader::DecodeTaskMeta(const sim::OpTaskTab &task,
                                              HcclTaskMetaData &taskMeta) {
    if (task.optaskMeta.size() < sizeof(HcclTaskMetaData)) {
        HCCL_VM_ERROR("Cannot load operator task metadata because the payload "
                      "is too small, taskSeq={}, "
                      "actualSize={}, expectedSize={}",
                      task.taskSeq, task.optaskMeta.size(),
...
HcclResult BigGraphDataLoader::Load(loader::Loader &loader, uint32_t syncIter,
                                    BigGraphData &data) const {
    data.Clear();
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_VM_ERROR("Failed to load operator data for a sync window, "
                      "syncIter={}, ret={}",
    ...
    if (operatorCount >
        ...
        HCCL_VM_ERROR("Too many operators in one sync window, syncIter={}, "
                      "operatorCount={}",
    ...
    for (size_t operatorIndex = 0; operatorIndex < operatorCount;
         ++operatorIndex) {
        ...
        for (const auto &rankEntry : compositeData) {
            ...
            const sim::CompositeOpDetail &compositeOp =
                rankEntry.second[operatorIndex];
            if (!hasOp) {
    ...
    HCCL_VM_INFO("Loaded multi-operator data, syncIter={}, operatorCount={}, "
                 "rankCount={}",
