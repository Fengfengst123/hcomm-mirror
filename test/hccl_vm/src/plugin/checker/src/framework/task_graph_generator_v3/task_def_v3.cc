...
const char *ModelExecTypeName(TaskType type) {
    switch (type) {
...
std::string DescribePosition(const TaskPosition &position) {
    ...
    os << "device=" << position.deviceId << ", rank=" << position.rankId
       << ", stream=" << position.streamId;
...
std::string DescribeMemSlice(const MemSlice &slice) {
    std::ostringstream os;
    os << "device " << slice.deviceId << " rank " << slice.rankId << ", "
       << ToString(slice.memType) << ", [0x" << std::hex << slice.offset
       << ",0x" << (slice.offset + slice.len) << "), rawAddr=0x"
       << slice.rawAddr << std::dec;
    return os.str();
}

std::string DescribeAivLocation(const TaskPosition &location) {
    std::ostringstream os;
    os << "device=" << location.deviceId << ", rank=" << location.rankId
       << ", launch=" << location.launchIdx << ", block=" << location.blockId
...
std::string DescribeAicpuNotify(const AicpuNotify &notify) {
    std::ostringstream os;
    os << "{recordDevice=" << notify.recordDeviceId
       << ", waitDevice=" << notify.waitDeviceId
...
std::string DescribeCcuTrace(const CcuTraceInfo &trace) {
    std::ostringstream os;
    os << ", ccuTrace={op=" << trace.opName << ", role=" << trace.nodeRole
...
std::string TaskBatchTransMem::Describe() const {
    ...
    os << "[TaskBatchTransMem] node=" << nodeId_ << ", "
       << DescribePosition(loc_) << ", protocol=" << ToString(protocol_)
       << ", pairCount=" << std::min(srcs_.size(), dsts_.size())
       << ", mergedPairCount="
       << std::min(mergedSrcs_.size(), mergedDsts_.size());
    if (!srcs_.empty()) {
...
std::string TaskRecordAICPU::Describe() const {
    ...
    os << "[TaskRecordAICPU] node=" << nodeId_ << ", " << DescribePosition(loc_)
       << ", protocol=" << ToString(protocol_)
       << ", notify=" << DescribeAicpuNotify(notify_);
    if (HasCcuTrace()) {
        os << DescribeCcuTrace(GetCcuTrace());
    }
    return os.str();
...
std::string TaskRecordCCU::Describe() const {
    ...
    os << "[TaskRecordCCU] node=" << nodeId_ << ", " << DescribePosition(loc_)
       << ", protocol=" << ToString(protocol_)
       << ", channelId=" << notify_.channelId
       << ", recordDevice=" << notify_.recordDeviceId
       << ", waitDevice=" << notify_.waitDeviceId << ", dieId=" << notify_.dieId
       << ", ckeId=" << notify_.ckeId << ", ckeMask=0x" << std::hex
...
std::string TaskAivGraph::Describe() const {
    ...
    os << "[TaskAivGraph] node=" << nodeId_ << ", " << DescribePosition(loc_)
       << ", device=" << deviceId_ << ", launch=" << launchIdx_
       << ", hostStream=" << hostStreamId_;
    return os.str();
}
...
std::string TaskAivPipeBarrier::Describe() const {
    ...
    os << "[TaskAivPipeBarrier] node=" << nodeId_ << ", "
       << DescribePosition(loc_) << ", pipeType=" << info_.pipeType
       << ", merged=" << info_.merged
...
std::string TaskAivSyncAll::Describe() const {
    ...
    os << "[TaskAivSyncAll] node=" << nodeId_ << ", " << DescribePosition(loc_)
       << ", syncRound=" << info_.syncRound << ", merged=" << info_.merged
       << ", memberTaskIds=" << DescribeTaskIds(info_.memberTaskIds);
    return os.str();
}
...
std::string TaskAivSendFlag::Describe() const {
    ...
    os << "[TaskAivSendFlag] node=" << nodeId_ << ", " << DescribePosition(loc_)
       ...
       << ", commInfoOffset=" << flag_.commInfoOffset
       << ", value=" << flag_.value;
    return os.str();
}
...
std::string TaskSyncStream::Describe() const {
    std::ostringstream os;
    os << "[TaskSyncStream] node=" << nodeId_ << ", " << DescribePosition(loc_)
       << ", syncIdx=" << syncIdx_;
    return os.str();
}
...
std::string TaskModelExec::Describe() const {
    std::ostringstream os;
    os << "[TaskModelExec] node=" << nodeId_ << ", " << DescribePosition(loc_)
       << ", type=" << ModelExecTypeName(GetType()) << ", execSeq=" << execSeq_
       << ", peerStreamId=" << peerStreamId_;
    return os.str();
}

std::string TaskModelExec::DescribeShort() const {
    std::ostringstream os;
    os << "[TaskModelExec] node=" << nodeId_ << "\n"
       << DescribePosition(loc_) << "\ntype=" << ModelExecTypeName(GetType())
       << ", execSeq=" << execSeq_ << ", peerStreamId=" << peerStreamId_;
    return os.str();
}
