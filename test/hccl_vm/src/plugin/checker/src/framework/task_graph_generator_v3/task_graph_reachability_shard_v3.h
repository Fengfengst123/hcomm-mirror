...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * A PARTICULAR PURPOSE. Description: 大图可达闭包按卡分块构建器。
 *              - 老文件 task_graph_reachability_v3 继续承载全图一次性校验；
 * 本文件只负责分块路径：按目标节点子集（如单张内存归属卡的候选节点）
 *                构建分片可达闭包，即算即查即释放，降低峰值内存。
 */
...
class ReachabilityShardBuilder {
  public:
    ...
    ReachabilityShardBuilder &
    operator=(const ReachabilityShardBuilder &) = delete;
     ...
     */
    HcclResult BuildClosureForNodes(const std::vector<NodeId> &targetNodeIds,
