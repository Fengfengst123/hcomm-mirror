...
struct SyncResourceKey {
    ...
    bool operator<(const SyncResourceKey &rhs) const {
        return std::tie(kind, notifyId, deviceId, dieId, ckeId, ckeBit,
                        launchIdx, blockId, srcPipe, dstPipe, eventId,
                        commInfoOffset, flagValue) <
               std::tie(rhs.kind, rhs.notifyId, rhs.deviceId, rhs.dieId,
                        rhs.ckeId, rhs.ckeBit, rhs.launchIdx, rhs.blockId,
                        rhs.srcPipe, rhs.dstPipe, rhs.eventId,
                        rhs.commInfoOffset, rhs.flagValue);
    }
...
std::string DescribeResource(const SyncResourceKey &resource) {
    ...
    if (resource.kind == SyncResourceKind::AICPU_NOTIFY) {
        ...
        os << ", waitRankId=" << resource.deviceId
           << ", dieId=" << resource.dieId << ", ckeId=" << resource.ckeId
           << ", bit=" << static_cast<uint32_t>(resource.ckeBit);
    } else if (resource.kind == SyncResourceKind::AIV_EVENT) {
        os << ", rankId=" << resource.deviceId
           << ", launchIdx=" << resource.launchIdx
           << ", blockId=" << resource.blockId
           << ", srcPipe=" << resource.srcPipe
           << ", dstPipe=" << resource.dstPipe
           << ", eventId=" << resource.eventId;
    } else {
        os << ", flagOwnerDevice=" << resource.deviceId
           << ", launchIdx=" << resource.launchIdx << ", commInfoOffset=0x"
           << std::hex << resource.commInfoOffset << std::dec
...
bool IsWaitNode(const TaskNode *node) {
    ...
    return type == TaskType::WAIT || type == TaskType::AIV_WAIT_FLAG ||
           type == TaskType::AIV_RECV_FLAG;
}
...
bool IsPostNode(const TaskNode *node) {
    ...
    const TaskType type = node->GetType();
    return type == TaskType::RECORD || type == TaskType::AIV_SET_FLAG ||
           type == TaskType::AIV_SEND_FLAG;
}
...
bool ContainsResource(const std::vector<SyncResourceKey> &resources,
                      const SyncResourceKey &resource) {
    return std::find(resources.begin(), resources.end(), resource) !=
           resources.end();
}

// 广度优先收集从 `start` 可达的所有节点到 `nodes`，保持 BFS 顺序。
// 同步冲突检查器基于这个扁平化遍历结果工作，而非直接操作原始 DAG。
HcclResult CollectOriginalNodes(const TaskNode *start,
    ...
    while (!pending.empty()) {
        ...
        for (const TaskNode *child : node->GetChildren()) {
            if (child == nullptr) {
                HCCL_VM_ERROR("{} Sync DAG is invalid because a child node is "
                              "null, parent={}",
                              MakeErrorCodeText(ErrorCode::SYNC_DAG_INVALID),
                              node->Describe());
                return HCCL_E_PTR;
            }
...
HcclResult BuildSyncPairs(const std::vector<const TaskNode *> &nodes,
                          SyncBuckets &buckets, SyncParentMap &syncParents) {
    buckets.clear();
    syncParents.clear();
    ...
    for (const TaskNode *wait : nodes) {
        ...
        const std::vector<SyncResourceKey> waitResources =
            GetSyncResources(wait);
        ...
        for (const TaskNode *parent : wait->GetParents()) {
            if (parent == nullptr) {
                HCCL_VM_ERROR("{} Sync DAG is invalid because a parent node is "
                              "null, childWait={}",
                              MakeErrorCodeText(ErrorCode::SYNC_DAG_INVALID),
                              wait->Describe());
                return HCCL_E_PTR;
            ...
            const std::vector<SyncResourceKey> postResources =
                GetSyncResources(parent);
            for (const SyncResourceKey &resource : postResources) {
                if (!ContainsResource(waitResources, resource)) {
                    continue;
                }
                syncParents[wait].insert(parent);
                buckets[resource].pairs.push_back(
                    SyncPair{resource, parent, wait});
            }
        }
    }
    return HCCL_SUCCESS;
...
bool IsSameRankStreamQueue(const TaskNode *lhs, const TaskNode *rhs) {
    ...
    return lhsPosition.deviceId == rhsPosition.deviceId &&
           lhsPosition.streamId == rhsPosition.streamId &&
...
HcclResult BuildCopiedGraph(const std::vector<const TaskNode *> &nodes,
                            const SyncParentMap &syncParents,
                            CopiedGraph &graph) {
    graph = CopiedGraph{};
    ...
    graph.start =
        startIter == graph.normalNodes.end() ? nullptr : startIter->second;
    ...
    for (const TaskNode *parent : nodes) {
        for (const TaskNode *child : parent->GetChildren()) {
            ...
            if (childIsWait) {
                ...
                const bool isSyncParent = syncIter != syncParents.end() &&
                                          syncIter->second.count(parent) != 0;
                const bool isSameRankStreamQueue =
                    IsSameRankStreamQueue(parent, child);
...
HcclResult BuildTopologicalOrder(const CopiedGraph &graph,
    ...
    if (topoOrder.size() != graph.nodes.size()) {
        HCCL_VM_ERROR("{} Sync conflict check failed because the copied sync "
...
class ReachabilityChecker {
  public:
    ReachabilityChecker(size_t nodeCount, const std::vector<size_t> &topoIndex)
        : nodeCount_(nodeCount), topoIndex_(topoIndex),
          visitStamp_(nodeCount, 0) {}
    ...
    bool IsReachable(const CopyNode *from, const CopyNode *to) {
        ...
        if (from == nullptr || to == nullptr || from->index >= nodeCount_ ||
            to->index >= nodeCount_) {
            return false;
        }
        if (from == to) {
        ...
        if (from->index >= topoIndex_.size() ||
            to->index >= topoIndex_.size() ||
        ...
        while (!pending.empty()) {
            ...
            pending.pop_back();
            for (const CopyNode *child : node->children) {
                if (child == nullptr || child->index >= nodeCount_ ||
                    child->index >= topoIndex_.size()) {
                    continue;
                }
                if (child == to) {
                    return true;
                }
                if (topoIndex_[child->index] >= targetTopo ||
...
AivFlagCellKey MakeAivFlagCellKey(const SyncResourceKey &resource) {
    return AivFlagCellKey{resource.deviceId, resource.launchIdx,
                          resource.commInfoOffset};
}

// flag cell 的单行描述，供 AIV 专用错误日志使用。
std::string DescribeAivFlagCell(const AivFlagCellKey &cell) {
    std::ostringstream os;
    os << "flagOwnerDevice=" << cell.deviceId
       << ", launchIdx=" << cell.launchIdx << ", commInfoOffset=0x" << std::hex
       << cell.commInfoOffset << std::dec;
    return os.str();
}

HcclResult CheckAivFlagProducerOrdering(const AivFlagCellKey &cell,
                                        const AivFlagGroup &previousGroup,
                                        const AivFlagGroup &nextGroup,
                                        ReachabilityChecker &reachable,
    ...
    for (const SyncPair *previousPair : previousGroup.pairs) {
        ...
        if (IsReachable(previousPair->receiveWaitCopy, nextGroup.postCopy,
                        reachable)) {
            continue;
        }
        ++stats.conflictCount;
        HCCL_VM_ERROR(
            ...
            DescribeAivFlagCell(cell), previousPair->wait->Describe(),
            previousPair->post->Describe(), nextGroup.post->Describe());
        return HCCL_E_INTERNAL;
    }
    return HCCL_SUCCESS;
}

HcclResult CheckAivFlagFanOutOrdering(const AivFlagCellKey &cell,
                                      const AivFlagGroup &previousGroup,
                                      const AivFlagGroup &nextGroup,
                                      ReachabilityChecker &reachable,
                                      SyncConflictCheckStats &stats) {
    for (const SyncPair *previousPair : previousGroup.pairs) {
        ...
        for (const SyncPair *nextPair : nextGroup.pairs) {
            ...
            if (IsReachable(previousPair->receiveWaitCopy,
                            nextPair->startWaitCopy, reachable)) {
                continue;
            }
            ++stats.conflictCount;
            HCCL_VM_ERROR(
                ...
                "nextRecvFlag={}",
                MakeErrorCodeText(ErrorCode::SYNC_RESOURCE_CONFLICT),
                DescribeAivFlagCell(cell), nextGroup.flagValue,
                previousPair->post->Describe(), previousPair->wait->Describe(),
...
HcclResult CheckAivFlagBuckets(const SyncBuckets &buckets,
                               const std::vector<size_t> &topoIndex,
                               ReachabilityChecker &reachable,
                               SyncConflictCheckStats &stats) {
    ...
    for (const auto &entry : buckets) {
        if (IsAivFlagResource(entry.first.kind)) {
            cellBuckets[MakeAivFlagCellKey(entry.first)].push_back(
                &entry.second);
        }
    }
    ...
    for (const auto &cellEntry : cellBuckets) {
        ...
        for (const SyncBucket *bucket : cellEntry.second) {
            ...
            if (!bucket->originalWaits.empty()) {
                ...
                std::map<const TaskNode *, std::set<const TaskNode *>>
                    waitPosts;
                ...
                for (const TaskNode *wait : bucket->originalWaits) {
                    ...
                    if (iter == waitPosts.end() || iter->second.empty()) {
                        ...
                        HCCL_VM_ERROR(
                            ...
                            MakeErrorCodeText(
                                ErrorCode::SYNC_RESOURCE_CONFLICT),
                            DescribeAivFlagCell(cell), wait->Describe());
                        return HCCL_E_INTERNAL;
                    }
        ...
        for (const SyncBucket *bucket : cellEntry.second) {
            for (const SyncPair &pair : bucket->pairs) {
                ...
                if (iter == groupsByPost.end()) {
                    ...
                    iter =
                        groupsByPost.emplace(pair.post, std::move(group)).first;
                }
        ...
        for (const auto &entry : groupsByPost) {
            ...
            if (group.post == nullptr || group.postCopy == nullptr ||
                group.postCopy->index >= topoIndex.size()) {
                return HCCL_E_INTERNAL;
            }
        ...
        std::sort(
            ...
            [&topoIndex](const AivFlagGroup *lhs, const AivFlagGroup *rhs) {
        ...
        for (const AivFlagGroup *group : groups) {
            ...
            const bool hasWait =
                !group->pairs.empty() && group->pairs.front()->wait != nullptr;
            if (!hasWait) {
                ...
                HcclResult ret = CheckAivFlagProducerOrdering(
                    cell, *lastMatchedGroup, *group, reachable, stats);
                if (ret != HCCL_SUCCESS) {
                    return ret;
                }
                continue;
            ...
            const auto previousSameValue =
                lastMatchedGroupByValue.find(group->flagValue);
...
// 处理）。
HcclResult CheckBuckets(const SyncBuckets &buckets,
                        ReachabilityChecker &reachable,
                        SyncConflictCheckStats &stats) {
    for (const auto &entry : buckets) {
        ...
        if (postCount < waitCount) {
            ++stats.checkedBucketCount;
            ++stats.conflictCount;
            HCCL_VM_ERROR("{} Sync resource has fewer producer tasks than "
                          "consumer tasks, "
                          "producerTaskType={}, consumerTaskType={}, "
                          "resource={}, producerCount={}, consumerCount={}",
                          MakeErrorCodeText(ErrorCode::SYNC_RESOURCE_CONFLICT),
                          SyncPostTaskTypeName(resource.kind),
                          SyncWaitTaskTypeName(resource.kind),
                          DescribeResource(resource), postCount, waitCount);
            return HCCL_E_INTERNAL;
        }
        ...
        if (HasMultiplePeer(pairs, true) || HasMultiplePeer(pairs, false)) {
            ...
            HCCL_VM_ERROR(
                ...
                MakeErrorCodeText(ErrorCode::SYNC_RESOURCE_CONFLICT),
                SyncPostTaskTypeName(resource.kind),
                SyncWaitTaskTypeName(resource.kind), DescribeResource(resource),
        ...
        for (const SyncPair &pair : pairs) {
            if (pair.wait == nullptr) {
                ...
                if (!IsReachable(lastPaired->receiveWaitCopy, pair.postCopy,
                                 reachable)) {
                    ++stats.conflictCount;
                    HCCL_VM_ERROR(
            ...
            if (current.receiveWaitCopy == nullptr) {
                ...
                HCCL_VM_ERROR(
                    ...
                    SyncPostTaskTypeName(resource.kind),
                    SyncWaitTaskTypeName(resource.kind),
                    DescribeResource(resource), current.post->Describe(),
                    current.wait->Describe());
                return HCCL_E_INTERNAL;
            }
            if (!IsReachable(current.receiveWaitCopy, next.postCopy,
                             reachable)) {
                ++stats.conflictCount;
                HCCL_VM_ERROR(
                    "{} Sync resource has a many-to-one ordering conflict, "
                    ...
                    SyncPostTaskTypeName(resource.kind),
                    SyncWaitTaskTypeName(resource.kind),
                    DescribeResource(resource), next.order,
                    current.wait->Describe(), current.post->Describe(),
                    next.post->Describe());
                return HCCL_E_INTERNAL;
            }
            if (!IsReachable(current.receiveWaitCopy, next.startWaitCopy,
                             reachable)) {
                ++stats.conflictCount;
                HCCL_VM_ERROR(
                    ...
                    SyncWaitTaskTypeName(resource.kind),
                    DescribeResource(resource), next.order,
                    current.post->Describe(), current.wait->Describe(),
                    next.wait->Describe());
                return HCCL_E_INTERNAL;
            }
            lastPaired = &pair;
    ...
    for (auto &entry : buckets) {
        ...
        for (SyncPair &pair : bucket.pairs) {
            ...
            if (pair.wait != nullptr) {
                ...
                pair.receiveWaitCopy =
                    FindCompletionCopy(copiedGraph, pair.wait);
                if (pair.startWaitCopy == nullptr ||
                    pair.receiveWaitCopy == nullptr) {
                    return HCCL_E_INTERNAL;
                }
            }
        }
    ...
    for (auto &entry : buckets) {
        ...
        std::sort(pairs.begin(), pairs.end(),
                  [&topoIndex](const SyncPair &lhs, const SyncPair &rhs) {
                      ...
                      const NodeId lhsWaitId = lhs.wait == nullptr
                                                   ? INVALID_NODE_ID
                                                   : lhs.wait->GetNodeId();
                      const NodeId rhsWaitId = rhs.wait == nullptr
                                                   ? INVALID_NODE_ID
                                                   : rhs.wait->GetNodeId();
