...
#define CHK_GET_GSA_V3(curCcuTask, queId, gsaId, gsaValue)                     \
    do {                                                                       \
        ...
        const uint32_t _checkerInstrId =                                       \
            (curCcuTask)->microCodePosInQue[(queId)] +                         \
            (curCcuTask)->startInstrIdInQue[(queId)];                          \
        if (AllRankParamRecorder::Global()->GetGSA(                            \
                (curCcuTask)->GetDeviceId(), _checkerDieId, (gsaId),           \
            ...
            HCCL_VM_ERROR(                                                     \
                ...
                (curCcuTask)->GetCommId(),                                     \
                static_cast<uint32_t>((curCcuTask)->GetDeviceId()),            \
                _checkerDieId, _checkerInstrId, static_cast<uint16_t>(gsaId)); \
            return HCCL_E_PARA;                                                \
        }                                                                      \
    } while (0)
...
HcclResult IsLoopGroupParamNull(LoopGroupParam *loopGroupParam) {
    if (loopGroupParam != nullptr) {
        HCCL_VM_ERROR(
            "{} This A5 instruction cannot be used inside a loop expansion.",
            MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str());
...
HcclResult TransformLoadSqeArgsToGSAInstr(const CcuRep::CcuInstr *instr,
    ...
    CHK_RET(
        AllRankParamRecorder::Global()->SetGSA(deviceId, dieId, gsaId, argVal));
...
HcclResult TransformLoadImdToGSAInstr(const CcuRep::CcuInstr *instr,
                                      CcuGraphStateV3 *curCcuTask,
                                      uint32_t queId, bool &isContinue,
                                      LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
    ...
    CHK_RET(AllRankParamRecorder::Global()->SetGSA(deviceId, dieId, gsaId,
                                                   immediate));
    HCCL_VM_DEBUG("Load immediate[{}] to GSA[{}]", immediate, gsaId);
    return HCCL_SUCCESS;
}

HcclResult TransformLoadImdToXnInstr(const CcuRep::CcuInstr *instr,
                                     CcuGraphStateV3 *curCcuTask,
                                     uint32_t queId, bool &isContinue,
                                     LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
...
HcclResult TransformLoadGSAXnInstr(const CcuRep::CcuInstr *instr,
    ...
    CHK_RET(AllRankParamRecorder::Global()->SetGSA(deviceId, dieId, gsAdId,
                                                   argVal1 + argVal2));
    HCCL_VM_DEBUG("Load GSA[{}]({}) + Xn[{}]({}) to GSA[{}]({})", gsAmId,
                  argVal1, xnId, argVal2, gsAdId, argVal1 + argVal2);
    return HCCL_SUCCESS;
}

HcclResult TransformLoadGSAGSAInstr(const CcuRep::CcuInstr *instr,
                                    CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                    bool &isContinue,
                                    LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
    ...
    CHK_RET(AllRankParamRecorder::Global()->SetGSA(deviceId, dieId, gsAdId,
                                                   argVal1 + argVal2));
    HCCL_VM_DEBUG("Load GSA[{}]({}) + GSA[{}]({}) to GSA[{}]({})", gsAmId,
                  argVal1, gsAnId, argVal2, gsAdId, argVal1 + argVal2);
    return HCCL_SUCCESS;
}

HcclResult TransformLoadXXInstr(const CcuRep::CcuInstr *instr,
                                CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                bool &isContinue,
                                LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
    ...
    CHK_GET_XN_V3(curCcuTask, queId, xnId, argVal2);
    CHK_RET(AllRankParamRecorder::Global()->SetXn(deviceId, dieId, xdId,
                                                  argVal1 + argVal2));
    HCCL_VM_DEBUG("Load Xn[{}]({}) + Xn[{}]({}) to Xn[{}]({})", xmId, argVal1,
                  xnId, argVal2, xdId, argVal1 + argVal2);
    return HCCL_SUCCESS;
}

HcclResult TransformSetCKEInstr(const CcuRep::CcuInstr *instr,
                                CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                bool &isContinue,
                                LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
...
uint16_t UpdateCKEId(uint16_t CKEId, LoopGroupParam *loopGroupParam) {
    ...
    HCCL_VM_DEBUG("Update CKEId[{}], loopIdx[{}], ckOffset[{}]", CKEId,
                  loopGroupParam->curLoopIdx,
                  static_cast<int>(loopGroupParam->loopGroupXm.ckOffset));
    return UpdateId(CKEId, loopGroupParam->curLoopIdx,
                    loopGroupParam->loopGroupXn.expandOffset,
                    loopGroupParam->loopGroupXm.ckOffset,
...
uint16_t UpdateMSId(uint16_t MSId, LoopGroupParam *loopGroupParam) {
    ...
    return UpdateId(MSId, loopGroupParam->curLoopIdx,
                    loopGroupParam->loopGroupXn.expandOffset,
                    loopGroupParam->loopGroupXm.msOffset,
                    loopGroupParam->curExpandCnt);
}
...
uint64_t UpdateGSAValue(uint64_t gsaAddr, LoopGroupParam *loopGroupParam) {
    ...
    return gsaAddr +
           loopGroupParam->curExpandCnt *
               loopGroupParam->loopGroupXm.gsaOffset +
           loopGroupParam->curLoopCnt * xm.gsaStride;
}

// todo: 这边可能支持reduce，需要处理一下
HcclResult TransformTransLocMemToRmtMemInstr(const CcuRep::CcuInstr *instr,
                                             CcuGraphStateV3 *curCcuTask,
                                             uint32_t queId, bool &isContinue,
                                             LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
    ...
    if (g_allRankChannelInfo[deviceId][dieId].count(channelId) == 0) {
        HCCL_VM_ERROR(
            ...
            static_cast<uint32_t>(deviceId), dieId, channelId);
        return HCCL_E_INTERNAL;
    }
    ...
    if (len == 0) {
        HCCL_VM_ERROR(
            ...
            "  size: transferSize={}",
            MakeErrorCodeText(ErrorCode::GRAPH_STRUCTURE_INVALID).c_str(),
            curCcuTask->GetCommId(), curCcuTask->GetDeviceId(),
    ...
    if (len > UB_MAX_SIZE) {
        HCCL_VM_ERROR(
            ...
            MakeErrorCodeText(ErrorCode::GRAPH_STRUCTURE_INVALID).c_str(),
            static_cast<unsigned long long>(len),
            static_cast<unsigned long long>(UB_MAX_SIZE),
            curCcuTask->GetCommId(), curCcuTask->GetDeviceId(),
            static_cast<uint32_t>(deviceId), dieId, queId, instrId);
        return HCCL_E_INTERNAL;
    }
    ...
    HCCL_VM_DEBUG("Preparing local-memory to remote-memory transfer, "
    ...
    if (rId != rmtDeviceId) {
        HCCL_VM_ERROR(
            ...
            "  addr: remoteAddr={}",
            MakeErrorCodeText(ErrorCode::GRAPH_REMOTE_RANK_MISMATCH).c_str(),
            curCcuTask->GetCommId(), curCcuTask->GetDeviceId(),
            static_cast<uint32_t>(deviceId), dieId, queId, instrId, channelId,
            static_cast<uint32_t>(rmtDeviceId), static_cast<uint32_t>(rId),
            static_cast<uint64_t>(rmtAddr));
        return HCCL_E_INTERNAL;
    }
    ...
    if (reduceEn) {
        uint16_t ccuReduceType;
        CHK_RET(GetCheckerReduceInfoFromCcuOpCode(reduceOpCode, checkerReduceOp,
                                                  ccuReduceType));
        DataType hcclDataType;
        CHK_RET(GetHcclDataTypeFromCCUDataType(reduceDataType, ccuReduceType,
                                               hcclDataType));
    ...
    if (rmtDeviceId != deviceId) {
        if (reduceEn) {
            AddWriteReduce(deviceId, rmtDeviceId, queId, curCcuTask, srcSlice,
                           dstSlice, checkerDataType, checkerReduceOp);
        } else {
    ...
    HCCL_VM_DEBUG("Completed local-memory to remote-memory transfer:\n"
                  ...
                  waitCKEId, waitCKEMask, locGSAId, locXnId, rmtGSAId, rmtXnId,
                  lengthXnId, channelId, setCKEId, setCKEMask, clearType,
                  lengthEn, reduceEn, reduceDataType, reduceOpCode);
    return HCCL_SUCCESS;
}
...
HcclResult TransformTransLocMemToLocMemInstr(const CcuRep::CcuInstr *instr,
    ...
    HCCL_VM_DEBUG("Built local-memory to local-memory copy, deviceId={}, "
                  "srcSlice={}, dstSlice={}, "
                  "transferSize={}",
    ...
    HCCL_VM_DEBUG("Completed local-memory to local-memory transfer:\n"
                  ...
                  waitCKEId, waitCKEMask, srcGSAId, srcXnId, dstGSAId, dstXnId,
                  lengthXnId, channelId, setCKEId, setCKEMask, clearType,
                  lengthEn);
    return HCCL_SUCCESS;
}

HcclResult TransformSyncCKEInstr(const CcuRep::CcuInstr *instr,
                                 CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                 bool &isContinue,
                                 LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
    ...
    DeviceId rmtDeviceId =
        g_allRankChannelInfo[deviceId][dieId][channelId].dstDeviceId;
    uint32_t rmtDieId =
        g_allRankChannelInfo[deviceId][dieId][channelId].remoteDieId;

    if (locCKEMask != 0x0000) {
        AddPost(deviceId, rmtDeviceId, queId, curCcuTask, rmtDieId, rmtCKEId,
                locCKEMask);

        // 将对应的bit位置一
        uint16_t ckeValue = 0;
        uint16_t localCkeValue = 0;
        CHK_RET(AllRankParamRecorder::Global()->GetCKE(
            deviceId, dieId, locCKEId, localCkeValue));
        const uint16_t maskedLocalCkeValue =
            static_cast<uint16_t>(localCkeValue & locCKEMask);
        HCCL_VM_DEBUG("CKE[{}]({}) & mask[0x{:04x}] -> {}", locCKEId,
                      localCkeValue, locCKEMask, maskedLocalCkeValue);
        CHK_RET(AllRankParamRecorder::Global()->GetCKE(rmtDeviceId, rmtDieId,
                                                       rmtCKEId, ckeValue));
        const uint16_t mergedCkeValue =
            static_cast<uint16_t>(ckeValue | maskedLocalCkeValue);
        CHK_RET(AllRankParamRecorder::Global()->SetCKE(
            rmtDeviceId, rmtDieId, rmtCKEId, mergedCkeValue));
        HCCL_VM_DEBUG("CKE[{}]({}) | {} -> CKE[{}]({})", rmtCKEId, ckeValue,
                      maskedLocalCkeValue, rmtCKEId, mergedCkeValue);
    }
    ...
    HCCL_VM_DEBUG(
        ...
        "  set: Set CKE[{}:0x{:04x}]\n"
        "  ctrl: clearType[{}]",
        waitCKEId, waitCKEMask, locCKEId, locCKEMask, rmtCKEId, locCKEMask,
        channelId, setCKEId, setCKEMask, clearType);
    return HCCL_SUCCESS;
}

HcclResult TransformSyncXnInstr(const CcuRep::CcuInstr *instr,
                                CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                bool &isContinue,
                                LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
    ...
    CHK_RET(AllRankParamRecorder::Global()->SetXn(rmtDeviceId, rmtDieId,
                                                  rmtXnId, localXnValue));

    if (setRmtCKEMask != 0x0000) {
        AddPost(deviceId, rmtDeviceId, queId, curCcuTask, rmtDieId, setRmtCKEId,
                setRmtCKEMask);

        // 将对应的bit位置一
        uint16_t ckeValue = 0;
        CHK_RET(AllRankParamRecorder::Global()->GetCKE(rmtDeviceId, rmtDieId,
                                                       setRmtCKEId, ckeValue));
        CHK_RET(AllRankParamRecorder::Global()->SetCKE(
            rmtDeviceId, rmtDieId, setRmtCKEId, ckeValue | setRmtCKEMask));
    }
    ...
    HCCL_VM_DEBUG("Wait CKE[{}:0x{:04x}]:\n"
                  ...
                  "  ctrl: clearType[{}]",
                  waitCKEId, waitCKEMask, locXnId, localXnValue, rmtXnId,
                  channelId, setRmtCKEId, setRmtCKEMask, setCKEId, setCKEMask,
                  clearType);
    return HCCL_SUCCESS;
}
...
HcclResult TransformTransLocMemToLocMSInstr(const CcuRep::CcuInstr *instr,
                                            ...
                                            LoopGroupParam *loopGroupParam) {
    uint16_t locMSId =
        UpdateMSId(instr->v1.transLocMemToLocMS.locMSId, loopGroupParam);
    ...
    uint16_t setCKEId =
        UpdateCKEId(instr->v1.transLocMemToLocMS.setCKEId, loopGroupParam);
    uint16_t setCKEMask = instr->v1.transLocMemToLocMS.setCKEMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v1.transLocMemToLocMS.waitCKEId, loopGroupParam);
    ...
    HCCL_VM_DEBUG("Completed local-memory to local-MS transfer:\n"
                  ...
                  "  set: setCkeId={}, setMask=0x{:04x}\n"
                  "  ctrl: clearType={}, lengthEnabled={}",
                  waitCKEId, waitCKEMask, locGSAId, locXnId, locMSId / 0x8000,
                  locMSId % 0x8000, lengthXnId, channelId, setCKEId, setCKEMask,
                  clearType, lengthEn);
    return HCCL_SUCCESS;
}
...
HcclResult TransformTransLocMSToLocMemInstr(const CcuRep::CcuInstr *instr,
    ...
    uint16_t locMSId =
        UpdateMSId(instr->v1.transLocMSToLocMem.locMSId, loopGroupParam);
    ...
    uint16_t setCKEId =
        UpdateCKEId(instr->v1.transLocMSToLocMem.setCKEId, loopGroupParam);
    uint16_t setCKEMask = instr->v1.transLocMSToLocMem.setCKEMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v1.transLocMSToLocMem.waitCKEId, loopGroupParam);
    ...
    HCCL_VM_DEBUG("Completed local-MS to local-memory transfer:\n"
                  ...
                  "  set: setCkeId={}, setMask=0x{:04x}\n"
                  "  ctrl: clearType={}, lengthEnabled={}",
                  waitCKEId, waitCKEMask, locMSId / 0x8000, locMSId % 0x8000,
                  locGSAId, locXnId, lengthXnId, channelId, setCKEId,
...
HcclResult TransformTransLocMSToLocMSInstr(const CcuRep::CcuInstr *instr,
                                           ...
                                           LoopGroupParam *loopGroupParam) {
    uint16_t dstMSId =
        UpdateMSId(instr->v1.transLocMSToLocMS.dstMSId, loopGroupParam);
    uint16_t srcMSId =
        UpdateMSId(instr->v1.transLocMSToLocMS.srcMSId, loopGroupParam);
    ...
    uint16_t setCKEId =
        UpdateCKEId(instr->v1.transLocMSToLocMS.setCKEId, loopGroupParam);
    uint16_t setCKEMask = instr->v1.transLocMSToLocMS.setCKEMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v1.transLocMSToLocMS.waitCKEId, loopGroupParam);
    ...
    HCCL_VM_DEBUG("Completed local-MS to local-MS transfer:\n"
                  ...
                  "  set: setCkeId={}, setMask=0x{:04x}\n"
                  "  ctrl: clearType={}, lengthEnabled={}",
                  waitCKEId, waitCKEMask, srcMSId / 0x8000, srcMSId % 0x8000,
                  dstMSId / 0x8000, dstMSId % 0x8000, lengthXnId, channelId,
                  setCKEId, setCKEMask, clearType, lengthEn);
    return HCCL_SUCCESS;
}

HcclResult TransformTransLocMSToRmtMemInstr(const CcuRep::CcuInstr *instr,
                                            CcuGraphStateV3 *curCcuTask,
                                            uint32_t queId, bool &isContinue,
                                            LoopGroupParam *loopGroupParam) {
    ...
    uint16_t locMSId =
        UpdateMSId(instr->v1.transLocMSToRmtMem.locMSId, loopGroupParam);
    ...
    uint16_t setCKEId =
        UpdateCKEId(instr->v1.transLocMSToRmtMem.setCKEId, loopGroupParam);
    uint16_t setCKEMask = instr->v1.transLocMSToRmtMem.setCKEMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v1.transLocMSToRmtMem.waitCKEId, loopGroupParam);
    ...
    HCCL_VM_DEBUG("Completed local-MS to remote-memory transfer:\n"
                  ...
                  "  ctrl: clearType={}, lengthEnabled={}",
                  waitCKEId, waitCKEMask, locMSId / 0x8000, locMSId % 0x8000,
                  rmtGSAId, rmtXnId, lengthXnId, channelId, setCKEId,
                  setCKEMask, clearType, lengthEn);
    return HCCL_SUCCESS;
}

HcclResult TransformTransLocMSToRmtMSInstr(const CcuRep::CcuInstr *instr,
                                           CcuGraphStateV3 *curCcuTask,
                                           uint32_t queId, bool &isContinue,
                                           LoopGroupParam *loopGroupParam) {
    uint16_t rmtMSId =
        UpdateMSId(instr->v1.transLocMSToRmtMS.rmtMSId, loopGroupParam);
    uint16_t locMSId =
        UpdateMSId(instr->v1.transLocMSToRmtMS.locMSId, loopGroupParam);
    ...
    uint16_t setRmtCKEId =
        UpdateCKEId(instr->v1.transLocMSToRmtMS.setRmtCKEId, loopGroupParam);
    ...
    uint16_t setCKEId =
        UpdateCKEId(instr->v1.transLocMSToRmtMS.setCKEId, loopGroupParam);
    uint16_t setCKEMask = instr->v1.transLocMSToRmtMS.setCKEMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v1.transLocMSToRmtMS.waitCKEId, loopGroupParam);
    ...
    HCCL_VM_DEBUG("Completed local-MS to remote-MS transfer:\n"
                  ...
                  "  ctrl: clearType={}, lengthEnabled={}",
                  waitCKEId, waitCKEMask, locMSId / 0x8000, locMSId % 0x8000,
                  rmtMSId / 0x8000, rmtMSId % 0x8000, lengthXnId, channelId,
                  setRmtCKEId, setRmtCKEMask, setCKEId, setCKEMask, clearType,
                  lengthEn);
    return HCCL_SUCCESS;
}

HcclResult TransformTransRmtMemToLocMSInstr(const CcuRep::CcuInstr *instr,
                                            CcuGraphStateV3 *curCcuTask,
                                            uint32_t queId, bool &isContinue,
                                            LoopGroupParam *loopGroupParam) {
    uint16_t locMSId =
        UpdateMSId(instr->v1.transRmtMemToLocMS.locMSId, loopGroupParam);
    ...
    uint16_t setCKEId =
        UpdateCKEId(instr->v1.transRmtMemToLocMS.setCKEId, loopGroupParam);
    uint16_t setCKEMask = instr->v1.transRmtMemToLocMS.setCKEMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v1.transRmtMemToLocMS.waitCKEId, loopGroupParam);
    ...
    HCCL_VM_DEBUG("Completed remote-memory to local-MS transfer:\n"
                  ...
                  "  set: setCkeId={}, setMask=0x{:04x}\n"
                  "  ctrl: clearType={}, lengthEnabled={}",
                  waitCKEId, waitCKEMask, rmtGSAId, rmtXnId, locMSId / 0x8000,
                  locMSId % 0x8000, lengthXnId, channelId, setCKEId, setCKEMask,
                  clearType, lengthEn);
    return HCCL_SUCCESS;
}

HcclResult TransformTransRmtMSToLocMemInstr(const CcuRep::CcuInstr *instr,
                                            CcuGraphStateV3 *curCcuTask,
                                            uint32_t queId, bool &isContinue,
                                            LoopGroupParam *loopGroupParam) {
    ...
    uint16_t rmtMSId =
        UpdateMSId(instr->v1.transRmtMSToLocMem.rmtMSId, loopGroupParam);
    ...
    uint16_t setCKEId =
        UpdateCKEId(instr->v1.transRmtMSToLocMem.setCKEId, loopGroupParam);
    uint16_t setCKEMask = instr->v1.transRmtMSToLocMem.setCKEMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v1.transRmtMSToLocMem.waitCKEId, loopGroupParam);
    ...
    HCCL_VM_DEBUG("Completed remote-MS to local-memory transfer:\n"
                  ...
                  "  set: setCkeId={}, setMask=0x{:04x}\n"
                  "  ctrl: clearType={}, lengthEnabled={}",
                  waitCKEId, waitCKEMask, rmtMSId / 0x8000, rmtMSId % 0x8000,
                  locGSAId, locXnId, lengthXnId, channelId, setCKEId,
                  setCKEMask, clearType, lengthEn);
    return HCCL_SUCCESS;
}

HcclResult TransformTransRmtMSToLocMSInstr(const CcuRep::CcuInstr *instr,
                                           CcuGraphStateV3 *curCcuTask,
                                           uint32_t queId, bool &isContinue,
                                           LoopGroupParam *loopGroupParam) {
    uint16_t locMSId =
        UpdateMSId(instr->v1.transRmtMSToLocMS.locMSId, loopGroupParam);
    uint16_t rmtMSId =
        UpdateMSId(instr->v1.transRmtMSToLocMS.rmtMSId, loopGroupParam);
    ...
    uint16_t setCKEId =
        UpdateCKEId(instr->v1.transRmtMSToLocMS.setCKEId, loopGroupParam);
    uint16_t setCKEMask = instr->v1.transRmtMSToLocMS.setCKEMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v1.transRmtMSToLocMS.waitCKEId, loopGroupParam);
    ...
    HCCL_VM_DEBUG("Completed remote-MS to local-MS transfer:\n"
                  ...
                  "  ctrl: clearType={}, lengthEnabled={}",
                  waitCKEId, waitCKEMask, rmtMSId / 0x8000, rmtMSId % 0x8000,
                  locMSId / 0x8000, locMSId % 0x8000, lengthXnId, channelId,
                  setCKEId, setCKEMask, clearType, lengthEn);
    return HCCL_SUCCESS;
}

// todo：这边待处理reduce信息
HcclResult TransformTransRmtMemToLocMemInstr(const CcuRep::CcuInstr *instr,
                                             CcuGraphStateV3 *curCcuTask,
                                             uint32_t queId, bool &isContinue,
                                             LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
    ...
    DeviceId rmtDeviceId =
        g_allRankChannelInfo[deviceId][dieId][channelId].dstDeviceId;
    HCCL_VM_DEBUG("Preparing remote-memory to local-memory transfer, "
                  "localDeviceId={}, remoteDeviceId={}",
                  deviceId, rmtDeviceId);
    // 说明transportId写错了
    if (rId != rmtDeviceId) {
    ...
    if (rmtDeviceId != deviceId) {
        if (reduceEn) {
            AddReadReduce(deviceId, rmtDeviceId, queId, curCcuTask, srcSlice,
                          dstSlice, checkerDataType, checkerReduceOp);
        } else {
    ...
    HCCL_VM_DEBUG("Completed remote-memory to local-memory transfer:\n"
                  ...
                  "  ctrl: clearType={}, lengthEnabled={}, reduceEnabled={}, "
                  "reduceDataType={}, reduceOpCode={}",
                  waitCKEId, waitCKEMask, rmtGSAId, rmtXnId, locGSAId, locXnId,
                  lengthXnId, channelId, setCKEId, setCKEMask, clearType,
                  lengthEn, reduceEn, reduceDataType, reduceOpCode);
    return HCCL_SUCCESS;
}
...
static HcclResult GetRemoteRankByChannel(DeviceId deviceId, uint32_t dieId,
                                         uint16_t channelId,
                                         DeviceId &rmtDeviceId,
                                         uint32_t &rmtDieId) {
    if (g_allRankChannelInfo[deviceId][dieId].count(channelId) == 0) {
...
static void SetLoopParamAtIteration(const LoopGroupParam &sampleParam,
                                    uint32_t curLoopCnt,
...
static HcclResult CollectLoopCkeOpsA5(const CcuRep::CcuInstr *instr,
                                      const LoopGroupParam &sampleParam,
                                      CcuLoopCkeOpV3 &waitOp,
                                      CcuLoopCkeOpV3 &setOp) {
    switch (instr->header.type) {
    case CTRL_TYPE:
        if (instr->header.code == CLEARCKE_CODE) {
            waitOp = {UpdateCKEId(instr->v1.clearCKE.waitCKEId,
                                  const_cast<LoopGroupParam *>(&sampleParam)),
    ...
    case TRANS_TYPE:
        ...
        case TRANSLOCMSTORMTMS_CODE:
            ...
            waitOp = {UpdateCKEId(instr->v1.transLocMSToRmtMS.waitCKEId,
                                  const_cast<LoopGroupParam *>(&sampleParam)),
                      instr->v1.transLocMSToRmtMS.waitCKEMask};
            setOp = {UpdateCKEId(instr->v1.transLocMSToRmtMS.setCKEId,
                                 const_cast<LoopGroupParam *>(&sampleParam)),
                     instr->v1.transLocMSToRmtMS.setCKEMask};
            return HCCL_SUCCESS;
        default:
            return HCCL_E_NOT_SUPPORT;
        }
    case REDUCE_TYPE:
        if (instr->header.code == ADD_CODE) {
            waitOp = {UpdateCKEId(instr->v1.add.waitCKEId,
                                  const_cast<LoopGroupParam *>(&sampleParam)),
                      instr->v1.add.waitCKEMask};
            setOp = {UpdateCKEId(instr->v1.add.setCKEId,
                                 const_cast<LoopGroupParam *>(&sampleParam)),
                     instr->v1.add.setCKEMask};
            return HCCL_SUCCESS;
        }
        if (instr->header.code == MAX_CODE) {
            waitOp = {UpdateCKEId(instr->v1.max.waitCKEId,
                                  const_cast<LoopGroupParam *>(&sampleParam)),
                      instr->v1.max.waitCKEMask};
            setOp = {UpdateCKEId(instr->v1.max.setCKEId,
                                 const_cast<LoopGroupParam *>(&sampleParam)),
                     instr->v1.max.setCKEMask};
            return HCCL_SUCCESS;
        }
        if (instr->header.code == MIN_CODE) {
            waitOp = {UpdateCKEId(instr->v1.min.waitCKEId,
                                  const_cast<LoopGroupParam *>(&sampleParam)),
                      instr->v1.min.waitCKEMask};
            setOp = {UpdateCKEId(instr->v1.min.setCKEId,
                                 const_cast<LoopGroupParam *>(&sampleParam)),
...
static HcclResult CollectTransLoopInstrA5(
    ...
    for (uint64_t iter = 0; iter < loopCnt; ++iter) {
        ...
        case TRANSRMTMEMTOLOCMS_CODE: {
            ...
            CHK_RET(GetRemoteRankByChannel(deviceId, dieId, op.channelId,
                                           rmtDeviceId, rmtDieId));
            uint64_t rmtAddr = 0;
...
static HcclResult CollectReduceLoopInstrA5(
    const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    uint16_t instrId, const LoopGroupParam &sampleParam, uint64_t loopCnt,
    std::shared_ptr<CcuLoopInstrV3> &instrInLoop) {
    DeviceId deviceId = curCcuTask->GetDeviceId();
...
static HcclResult
CollectClearCkeLoopInstrA5(const CcuRep::CcuInstr *instr,
                           CcuGraphStateV3 *curCcuTask, uint32_t queId,
                           uint16_t instrId, const LoopGroupParam &sampleParam,
                           std::shared_ptr<CcuLoopInstrV3> &instrInLoop) {
    if (instr->header.code != CLEARCKE_CODE ||
        instr->v1.clearCKE.clearMask != 0) {
        return HCCL_E_NOT_SUPPORT;
    }
    DeviceId deviceId = curCcuTask->GetDeviceId();
    uint32_t dieId = INVALID_DIE_ID;
    ...
    clearInstr->clearCKEId =
        UpdateCKEId(instr->v1.clearCKE.clearCKEId,
                    const_cast<LoopGroupParam *>(&sampleParam));
...
static HcclResult
CollectMergedLoopInstrsA5(CcuGraphStateV3 *curCcuTask, uint32_t queId,
                          uint16_t startInstrId, uint16_t endInstrId,
                          const LoopGroupParam &sampleParam, uint64_t loopCnt,
    ...
    for (uint16_t insId = startInstrId; insId <= endInstrId; ++insId) {
        ...
        CcuLoopCkeOpV3 setOp;
        HcclResult ret =
            CollectLoopCkeOpsA5(curInstr, sampleParam, waitOp, setOp);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        ...
        if (curInstr->header.type == CTRL_TYPE) {
            ret = CollectClearCkeLoopInstrA5(curInstr, curCcuTask, queId, insId,
                                             sampleParam, instrInLoop);
        } else if (curInstr->header.type == TRANS_TYPE) {
            ret = CollectTransLoopInstrA5(curInstr, curCcuTask, queId, insId,
                                          sampleParam, loopCnt, instrInLoop);
        } else if (curInstr->header.type == REDUCE_TYPE) {
            ret = CollectReduceLoopInstrA5(curInstr, curCcuTask, queId, insId,
                                           sampleParam, loopCnt, instrInLoop);
        } else {
...
static HcclResult CollectLoopExpandA5(const CcuRep::CcuInstr *loopInstr,
                                      CcuGraphStateV3 *curCcuTask,
                                      uint32_t queId,
                                      const LoopGroupParam &loopParam,
                                      CcuLoopInstrsV3 &mergedInstrs,
                                      uint64_t &loopCnt) {
    if (loopInstr->header.type != CTRL_TYPE ||
        loopInstr->header.code != LOOP_CODE) {
        return HCCL_E_NOT_SUPPORT;
    }
    DeviceId deviceId = curCcuTask->GetDeviceId();
    uint32_t dieId = INVALID_DIE_ID;
    ...
    return CollectMergedLoopInstrsA5(
        curCcuTask, queId, loopInstr->v1.loop.startInstrId,
        loopInstr->v1.loop.endInstrId, sampleParam, loopCnt, mergedInstrs);
}

static HcclResult RecordMergedLoopParamA5(const CcuRep::CcuInstr *loopInstr,
                                          CcuGraphStateV3 *curCcuTask,
                                          uint32_t queId,
                                          LoopGroupParam &loopParam) {
    DeviceId deviceId = curCcuTask->GetDeviceId();
    uint32_t dieId = INVALID_DIE_ID;
    curCcuTask->GetDieId(queId, dieId);
    uint64_t value = 0;
...
static HcclResult TryProcessMergedLoopA5(const CcuRep::CcuInstr *loopInstr,
                                         CcuGraphStateV3 *curCcuTask,
                                         uint32_t queId, bool &isContinue,
                                         LoopGroupParam &loopParam,
    ...
    for (uint32_t expandCnt = 0; expandCnt < expandTimes; ++expandCnt) {
        ...
        uint64_t loopCnt = 0;
        HcclResult ret = CollectLoopExpandA5(
            loopInstr, curCcuTask, queId, expandParam, expandInstrs, loopCnt);
        if (ret != HCCL_SUCCESS) {
    ...
    TaskNode *loopStart = AddLoopStartTask(
        queId, loopIdx, loopGroupIdx, curCcuTask,
    ...
    const uint16_t loopInstructionId =
        static_cast<uint16_t>(loopInstr->v1.loop.startInstrId);
    HCCL_VM_INFO("Merged LoopGroup body successfully, loopInstructionId={}, "
                 "loopCount={}, "
                 "expandTimes={}, mergedInstructionCount={}",
                 loopInstructionId, mergedLoopCnt, expandTimes,
...
HcclResult TransformLoopGroupInstr(const CcuRep::CcuInstr *instr,
    ...
    for (uint32_t curLoopIdx = 0; curLoopIdx < loopCnt; curLoopIdx++) {
        ...
        const uint32_t expandTimes =
            (curLoopIdx < loopGroupParam.loopGroupXn.expandOffset)
                ? 1U
                : static_cast<uint32_t>(loopGroupParam.loopGroupXn.expandCnt +
                                        1U);
        bool merged = false;
        CHK_RET(TryProcessMergedLoopA5(
        ...
        if (curLoopIdx < loopGroupParam.loopGroupXn.expandOffset) {
            ...
            // 需要进行loop展开
            for (uint32_t expandCnt = 0;
                 expandCnt <= loopGroupParam.loopGroupXn.expandCnt;
                 expandCnt++) {
    ...
    HCCL_VM_DEBUG("Finished LoopGroup expansion, startLoopInstructionId={}, "
                  "loopGroupXnId={}, "
...
HcclResult TransformClearCKEInstr(const CcuRep::CcuInstr *instr,
    ...
    uint16_t clearCKEId =
        UpdateCKEId(instr->v1.clearCKE.clearCKEId, loopGroupParam);
    uint16_t clearMask = instr->v1.clearCKE.clearMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v1.clearCKE.waitCKEId, loopGroupParam);
...
HcclResult TransformSyncGSAInstr(const CcuRep::CcuInstr *instr,
                                 CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                 bool &isContinue,
                                 LoopGroupParam *loopGroupParam) {
    CHK_RET(IsLoopGroupParamNull(loopGroupParam));
    ...
    CHK_RET(AllRankParamRecorder::Global()->SetGSA(rmtDeviceId, rmtDieId,
                                                   rmtGSAId, localGSAValue));
    ...
    HCCL_VM_DEBUG("Wait CKE[{}:0x{:04x}]:\n"
                  ...
                  "  set: Set rmtCKE[{}:0x{:04x}], Set CKE[{}:0x{:04x}]\n"
                  "  ctrl: clearType[{}]",
                  waitCKEId, waitCKEMask, locGSAId, localGSAValue, rmtGSAId,
                  channelId, setRmtCKEId, setRmtCKEMask, setCKEId, setCKEMask,
                  clearType);
    return HCCL_SUCCESS;
}

HcclResult GetHcclDataTypeFromCCUDataType(uint16_t ccuDataType,
                                          uint16_t ccuReduceType,
                                          DataType &dataType) {
    static std::map<uint16_t, DataType> ccuSumDataTypeMap = {
        {0, DataType::FP32},  {1, DataType::FP16},    {2, DataType::BFP16},
        {3, DataType::HIF8},  {4, DataType::FP8E4M3}, {5, DataType::FP8E5M2},
        {6, DataType::INT8},  {7, DataType::UINT8},   {8, DataType::INT16},
        {9, DataType::INT32},
    };
    ...
    if (ccuReduceType == CcuRep::CCU_REDUCE_SUM) {
    ...
    } else if (ccuReduceType == CcuRep::CCU_REDUCE_MAX ||
               ccuReduceType == CcuRep::CCU_REDUCE_MIN) {
        if (ccuMaxMinDataTypeMap.find(ccuDataType) ==
            ccuMaxMinDataTypeMap.end()) {
            HCCL_VM_ERROR(
                "{} This CCU reduce data type is not supported for MAX or MIN, "
                "ccuDataType={}",
                MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
                ccuDataType);
            return HCCL_E_INTERNAL;
        ...
        HCCL_VM_ERROR(
            "{} This CCU reduce opcode is not supported, ccuReduceType={}",
            MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
            ccuReduceType);
        return HCCL_E_INTERNAL;
    }
...
HcclResult TransformAddInstr(const CcuRep::CcuInstr *instr,
    ...
    HCCL_VM_INFO(
        ...
        "  set: setCkeId={}, setMask=0x{:04x}\n"
        "  ctrl: clearType={}",
        waitCKEId, waitCKEMask, ParseMSList(instr), count, dataType, castEn,
        setCKEId, setCKEMask, clearType);
    return HCCL_SUCCESS;
}

HcclResult TransformMaxInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue, LoopGroupParam *loopGroupParam) {
    ...
    HCCL_VM_DEBUG("Completed local MAX reduce:\n"
                  ...
                  "  set: setCkeId={}, setMask=0x{:04x}\n"
                  "  ctrl: clearType={}",
                  waitCKEId, waitCKEMask, ParseMSList(instr), count, dataType,
                  setCKEId, setCKEMask, clearType);
    return HCCL_SUCCESS;
}

HcclResult TransformMinInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue, LoopGroupParam *loopGroupParam) {
...
HcclResult TransformLoadSqeArgsToGSAInstr(const CcuRep::CcuInstr *instr,
    ...
    return TransformLoadSqeArgsToGSAInstr(instr, curCcuTask, queId, isContinue,
                                          loopGroupParam);
}
...
HcclResult TransformLoadImdToGSAInstr(const CcuRep::CcuInstr *instr,
                                      ...
                                      void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformLoadImdToGSAInstr(instr, curCcuTask, queId, isContinue,
                                      loopGroupParam);
}
...
HcclResult TransformLoadImdToXnInstr(const CcuRep::CcuInstr *instr,
                                     ...
                                     void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformLoadImdToXnInstr(instr, curCcuTask, queId, isContinue,
                                     loopGroupParam);
}
...
HcclResult TransformLoadGSAXnInstr(const CcuRep::CcuInstr *instr,
    ...
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformLoadGSAXnInstr(instr, curCcuTask, queId, isContinue,
                                   loopGroupParam);
}
...
HcclResult TransformLoadGSAGSAInstr(const CcuRep::CcuInstr *instr,
                                    ...
                                    bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformLoadGSAGSAInstr(instr, curCcuTask, queId, isContinue,
                                    loopGroupParam);
}
...
HcclResult TransformLoadXXInstr(const CcuRep::CcuInstr *instr,
                                ...
                                bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformLoadXXInstr(instr, curCcuTask, queId, isContinue,
                                loopGroupParam);
}
...
HcclResult TransformSetCKEInstr(const CcuRep::CcuInstr *instr,
                                ...
                                bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformSetCKEInstr(instr, curCcuTask, queId, isContinue,
                                loopGroupParam);
}
...
HcclResult TransformTransRmtMemToLocMSInstr(const CcuRep::CcuInstr *instr,
                                            ...
                                            void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformTransRmtMemToLocMSInstr(instr, curCcuTask, queId,
                                            isContinue, loopGroupParam);
}
...
HcclResult TransformTransLocMSToRmtMemInstr(const CcuRep::CcuInstr *instr,
                                            ...
                                            void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformTransLocMSToRmtMemInstr(instr, curCcuTask, queId,
                                            isContinue, loopGroupParam);
}
HcclResult TransformTransRmtMSToLocMemInstr(const CcuRep::CcuInstr *instr,
                                            CcuGraphStateV3 *curCcuTask,
                                            ...
                                            void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformTransRmtMSToLocMemInstr(instr, curCcuTask, queId,
                                            isContinue, loopGroupParam);
}
...
HcclResult TransformTransRmtMSToLocMSInstr(const CcuRep::CcuInstr *instr,
                                           ...
                                           void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformTransRmtMSToLocMSInstr(instr, curCcuTask, queId, isContinue,
                                           loopGroupParam);
}
...
HcclResult TransformTransLocMSToRmtMSInstr(const CcuRep::CcuInstr *instr,
                                           ...
                                           void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformTransLocMSToRmtMSInstr(instr, curCcuTask, queId, isContinue,
                                           loopGroupParam);
}
...
HcclResult TransformTransRmtMemToLocMemInstr(const CcuRep::CcuInstr *instr,
                                             ...
                                             void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformTransRmtMemToLocMemInstr(instr, curCcuTask, queId,
                                             isContinue, loopGroupParam);
}
...
HcclResult TransformTransLocMemToRmtMemInstr(const CcuRep::CcuInstr *instr,
                                             ...
                                             void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformTransLocMemToRmtMemInstr(instr, curCcuTask, queId,
                                             isContinue, loopGroupParam);
}
...
HcclResult TransformSyncCKEInstr(const CcuRep::CcuInstr *instr,
                                 ...
                                 bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformSyncCKEInstr(instr, curCcuTask, queId, isContinue,
                                 loopGroupParam);
}
...
HcclResult TransformSyncGSAInstr(const CcuRep::CcuInstr *instr,
                                 ...
                                 bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformSyncGSAInstr(instr, curCcuTask, queId, isContinue,
                                 loopGroupParam);
}
...
HcclResult TransformSyncXnInstr(const CcuRep::CcuInstr *instr,
                                ...
                                bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformSyncXnInstr(instr, curCcuTask, queId, isContinue,
                                loopGroupParam);
}
...
HcclResult TransformMaxInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformMaxInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformMinInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParam *>(loopParam);
    return TransformMinInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}

std::unordered_map<uint16_t, TransformInstrFunc> transformA5InstrSqeMap{
    {CcuRep::InstrHeader(LOAD_TYPE, LOADSQEARGSTOGSA_CODE).header,
     &TransformLoadSqeArgsToGSAInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, LOADSQEARGSTOXN_CODE).header,
     &TransformLoadSqeArgsToXnInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, LOADIMDTOGSA_CODE).header,
     &TransformLoadImdToGSAInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, LOADIMDTOXN_CODE).header,
     &TransformLoadImdToXnInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, LOADGSAXN_CODE).header,
     &TransformLoadGSAXnInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, LOADGSAGSA_CODE).header,
     &TransformLoadGSAGSAInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, LOADXX_CODE).header, &TransformLoadXXInstr},
    {CcuRep::InstrHeader(CTRL_TYPE, LOOPGROUP_CODE).header,
    ...
    {CcuRep::InstrHeader(CTRL_TYPE, CLEARCKE_CODE).header,
     &TransformClearCKEInstr},
    {CcuRep::InstrHeader(CTRL_TYPE, JMP_CODE).header, &TransformJumpInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSLOCMEMTOLOCMS_CODE).header,
     &TransformTransLocMemToLocMSInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSRMTMEMTOLOCMS_CODE).header,
     &TransformTransRmtMemToLocMSInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSLOCMSTOLOCMEM_CODE).header,
     &TransformTransLocMSToLocMemInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSLOCMSTORMTMEM_CODE).header,
     &TransformTransLocMSToRmtMemInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSRMTMSTOLOCMEM_CODE).header,
     &TransformTransRmtMSToLocMemInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSLOCMSTOLOCMS_CODE).header,
     &TransformTransLocMSToLocMSInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSRMTMSTOLOCMS_CODE).header,
     &TransformTransRmtMSToLocMSInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSLOCMSTORMTMS_CODE).header,
     &TransformTransLocMSToRmtMSInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSRMTMEMTOLOCMEM_CODE).header,
     &TransformTransRmtMemToLocMemInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSLOCMEMTORMTMEM_CODE).header,
     &TransformTransLocMemToRmtMemInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, TRANSLOCMEMTOLOCMEM_CODE).header,
     &TransformTransLocMemToLocMemInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, SYNCCKE_CODE).header,
     &TransformSyncCKEInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, SYNCGSA_CODE).header,
     &TransformSyncGSAInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, SYNCXN_CODE).header,
     &TransformSyncXnInstr},
...
HcclResult ProcessLoopIns(const CcuRep::CcuInstr *instr,
    ...
    auto loopStart = AddLoopStartTask(
        queId, loopIdx, loopGroupIdx, curCcuTask, startInstrId, endInstrId,
        static_cast<uint64_t>(xm.loopCnt), 1ULL); // loop前新增loopStart标记节点

    HCCL_VM_DEBUG("Entered loop execution, loopCount={}",
                  static_cast<uint64_t>(xm.loopCnt));
    hcomm::CcuRep::CcuInstrInfo &microCodeQue = curCcuTask->instrInfo[queId];
    uint32_t &pos = curCcuTask->microCodePosInQue[queId];
    const uint32_t savedPos = pos;
    for (uint32_t curLoopCnt = 0; curLoopCnt < xm.loopCnt; curLoopCnt++) {
        ...
        for (uint16_t insId = startInstrId; insId <= endInstrId; insId++) {
            ...
            const uint16_t instructionHeader =
                static_cast<uint16_t>(instr->header.header);
            HCCL_VM_DEBUG("Expanding loop instruction, instructionId={}",
                          insId);
            if (transformA5InstrSqeMap.count(instructionHeader) == 0) {
                HCCL_VM_ERROR(
                    ...
                    MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
                    curCcuTask->GetDeviceId(), queId, insId, instructionHeader);
                pos = savedPos;
                return HCCL_E_INTERNAL;
            }
            CHK_RET(transformA5InstrSqeMap[instructionHeader](
                instr, curCcuTask, queId, isContinue, loopGroupParam));
        }
    }
    pos = savedPos;
...
HcclResult InstructMapA5::Transform(const CcuRep::CcuInstr *instr,
                                    CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                    bool &isContinue, void *loopParam) {
    const uint16_t instructionHeader =
        static_cast<uint16_t>(instr->header.header);
    ...
    if (it == transformInstrSqeMap.end()) {
        HCCL_VM_ERROR("{} This A5 CCU instruction type is not supported by "
