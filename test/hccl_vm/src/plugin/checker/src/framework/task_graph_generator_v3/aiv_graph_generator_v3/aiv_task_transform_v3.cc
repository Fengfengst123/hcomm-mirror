...
uint64_t ElapsedNs(AivExpandClock::time_point start,
                   AivExpandClock::time_point end) {
...
TaskPosition MakeAivPosition(DeviceId deviceId, RankId rankId,
                             uint64_t launchIdx, uint32_t blockId,
                             uint32_t pipe, uint32_t taskId,
                             OperatorId operatorId, StorageManager *storage,
                             const TaskPosition &sourcePosition) {
    ...
    const CheckerParam param = storage == nullptr
                                   ? CheckerParam{}
                                   : storage->GetCheckerParam(operatorId);
    ...
    const uint32_t queueBlockId =
        blockId == std::numeric_limits<uint32_t>::max() ? 0U : blockId;
    const uint32_t queuePipe = IsValidPipe(pipe) ? pipe : 0U;
    position.queueId = static_cast<QueueId>(
        (launchIdx & 0xFFFFULL) * AIV_PIPE_NUM * 1024ULL +
...
FlagCellKey MakeFlagCellKey(const TaskNode &node, const AivFlagSync &flag) {
    return FlagCellKey{flag.flagOwnerDevice, node.GetPosition().launchIdx,
                       flag.commInfoOffset};
}

std::unique_ptr<TaskNode>
TranslateAivRuntimeTask(const AivRuntimeTaskV3 &task) {
    ...
    case AivRuntimeTaskTypeV3::REDUCE: {
        ...
        auto node = std::make_unique<TaskReduce>(
            src, dst, static_cast<uint8_t>(task.dataType),
...
HcclResult RewireGroupMember(TaskGraphGeneratorV3 *graph, NodeId memberNodeId,
                             NodeId mergeNodeId) {
    if (graph == nullptr) {
        return HCCL_E_PTR;
    }
...
HcclResult ValidateSnapshot(const AivLaunchContext &ctx) {
    ...
    const CheckerParam param =
        ctx.storage->GetCheckerParam(ctx.placeholder->GetOperatorId());
    if (ctx.snapshot.rankSize != 0 && param.rankSize != 0 &&
        ctx.snapshot.rankSize != param.rankSize) {
        HCCL_VM_ERROR(
            ...
            MakeErrorCodeText(ErrorCode::GRAPH_SNAPSHOT_MISMATCH),
            ctx.placeholder->GetDeviceId(), ctx.placeholder->GetLaunchIdx(),
            ctx.snapshot.rankSize, param.rankSize, ctx.snapshot.filePath);
        return HCCL_E_PARA;
    }
    return HCCL_SUCCESS;
}

HcclResult UpdateAivBufferSize(uint64_t snapshotSize, const char *fieldName,
                               const AivLaunchContext &ctx,
                               uint64_t &bufferSize) {
    if (snapshotSize == 0) {
        return HCCL_SUCCESS;
    }
    ...
    HCCL_VM_ERROR("{} AIV buffer size is inconsistent across snapshots, "
                  "field={}, rankId={}, "
                  "launchId={}, expectedSize={}, actualSize={}, "
                  "expectedSource=firstNonZeroSnapshot, snapshotFile={}",
                  MakeErrorCodeText(ErrorCode::GRAPH_SNAPSHOT_MISMATCH),
                  fieldName, ctx.placeholder->GetDeviceId(),
                  ctx.placeholder->GetLaunchIdx(), bufferSize, snapshotSize,
                  ctx.snapshot.filePath);
    return HCCL_E_PARA;
}

HcclResult AppendRuntimeTask(AivLaunchContext &ctx,
                             const AivRuntimeTaskV3 &task, uint64_t order,
                             NodeId &nodeId) {
    ...
    if (node == nullptr) {
        HCCL_VM_ERROR(
            ...
            MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED),
            ctx.placeholder->GetDeviceId(), ctx.placeholder->GetLaunchIdx(),
            task.taskId, static_cast<uint32_t>(task.taskType),
            ctx.snapshot.filePath);
        return HCCL_E_NOT_SUPPORT;
    }

    const DeviceId taskDeviceId = task.deviceId == INVALID_DEVICE_ID
                                      ? ctx.snapshot.deviceId
                                      : task.deviceId;
    const TaskPosition position = MakeAivPosition(
        taskDeviceId, task.rankId, ctx.placeholder->GetLaunchIdx(),
        task.blockId, task.curPipe, task.taskId,
        ctx.placeholder->GetOperatorId(), ctx.storage,
        ctx.placeholder->GetPosition());
    HcclResult ret =
        ctx.graph->AppendGeneratedNode(std::move(node), position, nodeId);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
...
HcclResult AppendPipeTasks(AivLaunchContext &ctx,
                           const std::vector<AivRuntimeTaskV3> &tasks,
                           NodeId headNodeId, uint64_t &order,
    ...
    for (const auto &task : tasks) {
        ...
        ret = (prevNodeId == INVALID_NODE_ID)
                  ? AddEdge(ctx.graph, headNodeId, currNodeId)
...
HcclResult BuildPipeSkeleton(AivLaunchContext &ctx,
                             std::vector<NodeId> &tailNodes) {
    uint64_t order = 0;
    for (const auto &block : ctx.snapshot.blocks) {
        HcclResult ret = AppendPipeTasks(ctx, block.scalarTasks, ctx.headNodeId,
                                         order, tailNodes);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        ret = AppendPipeTasks(ctx, block.mte2Tasks, ctx.headNodeId, order,
                              tailNodes);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        ret = AppendPipeTasks(ctx, block.mte3Tasks, ctx.headNodeId, order,
                              tailNodes);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
...
HcclResult AddSetWaitFlagEdges(AivLaunchContext &ctx) {
    std::set<NodeId> internalSet(ctx.internalNodeIds.begin(),
                                 ctx.internalNodeIds.end());
    ...
    auto enqueueChildren = [&ctx, &internalSet, &queued,
                            &readyQueue](const TaskNode *node) -> HcclResult {
        if (node == nullptr) {
            return HCCL_E_PTR;
        }
    ...
    while (!readyQueue.empty()) {
        if (unmatchedCnt >= readyQueue.size()) {
            ...
            if (node != nullptr && node->GetType() == TaskType::AIV_WAIT_FLAG) {
                const auto *waitFlag =
                    dynamic_cast<const TaskAivWaitFlag *>(node);
                if (waitFlag != nullptr) {
                    const SetWaitKey key =
                        MakeSetWaitKey(*node, waitFlag->GetEvent());
                    const auto iter = seenSetFlags.find(key);
                    seenMatchingSetFlagCount =
                        (iter == seenSetFlags.end()) ? 0U : iter->second.size();
                }
            }
            HCCL_VM_ERROR(
                ...
                readyQueue.size(), seenMatchingSetFlagCount,
                ctx.snapshot.filePath);
        ...
        if (node->GetType() == TaskType::AIV_SET_FLAG) {
            ...
            seenSetFlags[MakeSetWaitKey(*node, setFlag->GetEvent())].push_back(
                nodeId);
    ...
    for (const auto &entry : seenSetFlags) {
        if (!entry.second.empty()) {
            ...
            HCCL_VM_WARN("{} Found SetFlag tasks that were never consumed by "
                         "any WaitFlag task, "
...
bool IsCpGmExternalMemType(MemType memType) {
    return memType == MemType::INPUT || memType == MemType::CCL ||
           memType == MemType::OUTPUT || memType == MemType::AIV_COMM;
}
...
bool IsContinuousAfter(const MemSlice &prev, const MemSlice &next) {
    return SameSliceIdentity(prev, next) &&
           next.offset == prev.offset + prev.len &&
...
bool SliceExactEqual(const MemSlice &lhs, const MemSlice &rhs) {
    return SameSliceIdentity(lhs, rhs) && lhs.offset == rhs.offset &&
           lhs.len == rhs.len && lhs.rawAddr == rhs.rawAddr;
}
...
bool IntervalsOverlap(const MemSlice &lhs, const MemSlice &rhs) {
    ...
    return lhs.offset < rhs.offset + rhs.len &&
           rhs.offset < lhs.offset + lhs.len;
}
...
std::vector<MemSlice> MergeMemSliceIntervals(std::vector<MemSlice> slices) {
    ...
    std::sort(slices.begin(), slices.end(),
              [](const MemSlice &lhs, const MemSlice &rhs) {
                  if (lhs.deviceId != rhs.deviceId) {
                      return lhs.deviceId < rhs.deviceId;
                  }
                  if (lhs.memType != rhs.memType) {
                      return static_cast<uint32_t>(lhs.memType) <
...
bool BuildContinuousMergedSlices(const std::vector<MemSlice> &slices,
                                 std::vector<MemSlice> &merged) {
    merged.clear();
    if (slices.empty()) {
        return false;
...
bool GetDataSlices(const TaskNode *node, MemSlice &src, MemSlice &dst,
...
bool IsValidCpGmDataNode(const TaskNode *node, uint32_t pipe, bool ubAsDst) {
    ...
    return ubSlice.memType == MemType::AIV_UB &&
           IsCpGmExternalMemType(externalSlice.memType);
}

bool MatchPipeEventNode(const TaskNode *node, TaskType type, uint32_t curPipe,
                        uint32_t srcPipe, uint32_t dstPipe) {
    if (node == nullptr || node->GetType() != type ||
        node->GetPosition().pipe != curPipe) {
        return false;
    }
    ...
    return event != nullptr && event->srcPipe == srcPipe &&
...
bool SamePositionScope(const TaskPosition &lhs, const TaskPosition &rhs) {
    return lhs.deviceId == rhs.deviceId && lhs.rankId == rhs.rankId &&
           lhs.launchIdx == rhs.launchIdx && lhs.blockId == rhs.blockId;
}

bool SameEventExceptTaskId(const TaskNode *lhsNode, const TaskNode *rhsNode) {
    if (lhsNode == nullptr || rhsNode == nullptr ||
        lhsNode->GetType() != rhsNode->GetType()) {
        return false;
    }
    ...
    return SamePositionScope(lhsNode->GetPosition(), rhsNode->GetPosition()) &&
           lhsNode->GetPosition().pipe == rhsNode->GetPosition().pipe &&
           lhs->srcPipe == rhs->srcPipe && lhs->dstPipe == rhs->dstPipe &&
...
bool SameSetWaitEvent(const TaskNode *setNode, const TaskNode *waitNode) {
    ...
    return SamePositionScope(setNode->GetPosition(), waitNode->GetPosition()) &&
           setEvent->srcPipe == waitEvent->srcPipe &&
           setEvent->dstPipe == waitEvent->dstPipe &&
...
bool DataSliceShapeValid(const CpGmIter &iter) {
    ...
    if (!IsCpGmExternalMemType(t0Src.memType) ||
        !IsCpGmExternalMemType(t3Dst.memType)) {
        return false;
    }
...
bool SameExternalIdentityAsFirst(const CpGmIter &first, const CpGmIter &iter) {
    ...
    return SameSliceIdentity(firstT0Src, curT0Src) &&
           SameSliceIdentity(firstT3Dst, curT3Dst);
}
...
bool ExternalSlicesContinuous(const CpGmLoopGather &gather) {
    ...
    std::vector<MemSlice> merged;
    return BuildContinuousMergedSlices(t0Srcs, merged) &&
           BuildContinuousMergedSlices(t3Dsts, merged);
}
...
bool CanMergeCpGmRun(const AivLaunchContext &ctx,
                     const CpGmLoopGather &gather) {
...
TaskNode *FindOnlyChildByType(const AivLaunchContext &ctx, const TaskNode *node,
                              TaskType type) {
    if (node == nullptr) {
        return nullptr;
    }
    ...
    for (TaskNode *child : node->GetChildren()) {
        if (child == nullptr || IsInactive(ctx, child->GetNodeId()) ||
            child->GetType() != type) {
            continue;
        }
        if (matched != nullptr) {
...
TaskNode *FindOnlyWaitChildForSet(const AivLaunchContext &ctx,
                                  const TaskNode *setNode) {
    if (setNode == nullptr) {
        return nullptr;
    }
    TaskNode *matched = nullptr;
    for (TaskNode *child : setNode->GetChildren()) {
        if (child == nullptr || IsInactive(ctx, child->GetNodeId()) ||
            child->GetType() != TaskType::AIV_WAIT_FLAG ||
...
TaskNode *FindOnlySetParentForWait(const AivLaunchContext &ctx,
                                   const TaskNode *waitNode) {
    if (waitNode == nullptr) {
        return nullptr;
    }
    TaskNode *matched = nullptr;
    for (TaskNode *parent : waitNode->GetParents()) {
        if (parent == nullptr || IsInactive(ctx, parent->GetNodeId()) ||
            parent->GetType() != TaskType::AIV_SET_FLAG ||
...
bool HasActiveChild(const AivLaunchContext &ctx, const TaskNode *parent,
    ...
    for (const TaskNode *child : parent->GetChildren()) {
        if (child != nullptr && child->GetNodeId() == targetId &&
            !IsInactive(ctx, child->GetNodeId())) {
            return true;
        }
    }
    return false;
...
bool MatchCpGmTemplateFromNode(TaskNode *start, const AivLaunchContext &ctx,
                               CpGmIter &iter) {
    ...
    if (!IsValidCpGmDataNode(iter[0], PIPE_MTE2, true) ||
        !MatchPipeEventNode(iter[1], TaskType::AIV_SET_FLAG, PIPE_MTE2,
                            PIPE_MTE2, PIPE_MTE3) ||
        !MatchPipeEventNode(iter[2], TaskType::AIV_WAIT_FLAG, PIPE_MTE3,
                            PIPE_MTE2, PIPE_MTE3) ||
        !IsValidCpGmDataNode(iter[3], PIPE_MTE3, false) ||
        !MatchPipeEventNode(iter[4], TaskType::AIV_SET_FLAG, PIPE_MTE3,
                            PIPE_MTE3, PIPE_MTE2) ||
        !MatchPipeEventNode(iter[5], TaskType::AIV_WAIT_FLAG, PIPE_MTE2,
                            PIPE_MTE3, PIPE_MTE2)) {
        return false;
    }
    if (!SameSetWaitEvent(iter[1], iter[2]) ||
        !SameSetWaitEvent(iter[4], iter[5])) {
        return false;
    }
...
bool ValidateCpGmRun(const CpGmLoopGather &gather) {
    ...
    for (const CpGmIter &iter : gather.templateLoop) {
        if (iter[0]->GetType() != first[0]->GetType() ||
            iter[3]->GetType() != first[3]->GetType()) {
            return false;
        }
        if (!DataSliceShapeValid(iter) || !SameUbLayout(iter) ||
            !SameExternalIdentityAsFirst(first, iter)) {
            return false;
        }
...
bool CollectCpGmBlockTopo(const AivLaunchContext &ctx, uint32_t blockId,
    ...
    auto lessByTaskId = [&ctx](NodeId lhs, NodeId rhs) {
        ...
        const uint32_t lhsTaskId = lhsNode == nullptr
                                       ? std::numeric_limits<uint32_t>::max()
                                       : lhsNode->GetPosition().taskId;
        const uint32_t rhsTaskId = rhsNode == nullptr
                                       ? std::numeric_limits<uint32_t>::max()
    ...
    std::priority_queue<NodeId, std::vector<NodeId>, decltype(lessByTaskId)>
        ready(lessByTaskId);
...
TaskPosition MakeSyntheticPosition(AivLaunchContext &ctx,
                                   const TaskNode *source) {
    TaskPosition position =
        source == nullptr ? TaskPosition{} : source->GetPosition();
    position.taskId = AllocateSyntheticTaskId(ctx);
    return position;
}

std::unique_ptr<TaskNode> CloneSyntheticSyncNode(const TaskNode *source,
...
std::vector<uint32_t> CollectSourceTaskIds(const CpGmLoopGather &gather,
                                           size_t column) {
...
std::unique_ptr<TaskNode> BuildBatchNodeForColumn(const CpGmLoopGather &gather,
                                                  size_t column) {
    ...
    const bool srcIsUb =
        !srcs.empty() && srcs.front().memType == MemType::AIV_UB;
    const bool dstIsUb =
        !dsts.empty() && dsts.front().memType == MemType::AIV_UB;
    if (srcIsUb) {
    ...
    if (taskType == TaskType::REDUCE) {
        auto batch = std::make_unique<TaskBatchReduce>(dataType, reduceOp,
                                                       ProtocolType::SDMA);
...
HcclResult AppendCpGmMergedNode(AivLaunchContext &ctx, CpGmLoopGather &gather,
                                size_t column, std::unique_ptr<TaskNode> node,
                                const TaskPosition &position) {
    if (node == nullptr) {
        return HCCL_E_INTERNAL;
    }
...
bool CollectCpGmBoundaryNodes(
    const CpGmLoopGather &gather, std::set<NodeId> &runNodes,
...
HcclResult RemoveRunEdges(AivLaunchContext &ctx,
                          const std::set<NodeId> &runNodes) {
...
HcclResult ReplaceCpGmRun(AivLaunchContext &ctx, CpGmLoopGather &gather) {
    ...
    std::set<std::pair<NodeId, size_t>> externalParentEdges;
    std::set<std::pair<size_t, NodeId>> externalChildEdges;
    if (!CollectCpGmBoundaryNodes(gather, runNodes, externalParentEdges,
                                  externalChildEdges)) {
        ++ctx.cpGmLoopSkipCount;
        return HCCL_SUCCESS;
    }

    HcclResult ret = AppendCpGmMergedNode(
        ctx, gather, 0, BuildBatchNodeForColumn(gather, 0),
    ...
    for (size_t column : {1U, 2U}) {
        const TaskPosition position =
            MakeSyntheticPosition(ctx, gather.templateLoop.front()[column]);
        ret = AppendCpGmMergedNode(
            ctx, gather, column,
            CloneSyntheticSyncNode(gather.templateLoop.front()[column],
                                   position),
            position);
        if (ret != HCCL_SUCCESS) {
            return ret;
    ...
    ret = AppendCpGmMergedNode(
        ctx, gather, 3, BuildBatchNodeForColumn(gather, 3),
    ...
    const std::array<std::pair<size_t, size_t>, 6> internalEdges{
        {{0U, 1U}, {1U, 5U}, {1U, 2U}, {2U, 3U}, {3U, 4U}, {4U, 5U}}};
...
HcclResult MergeCpGM2GMLoops(AivLaunchContext &ctx) {
    ...
    for (const auto &block : ctx.snapshot.blocks) {
        ...
        if (!CollectCpGmBlockTopo(ctx, block.blockIdx, topo)) {
            HCCL_VM_WARN("Skip CpGM-to-GM merge for one block because the "
                         "block DAG order could not "
                         "be determined, rankId={}, launchId={}, blockId={}, "
                         "snapshotFile={}",
                         ctx.placeholder->GetDeviceId(),
                         ctx.placeholder->GetLaunchIdx(), block.blockIdx,
                         ctx.snapshot.filePath);
            continue;
        }
        ...
        std::set<NodeId>
            consumed; // 记录节点已经作为某个完整模板的一部分被识别过
    ...
    HCCL_VM_INFO("Finished CpGM-to-GM merge analysis:\n"
                 ...
                 ctx.placeholder->GetDeviceId(),
                 ctx.placeholder->GetLaunchIdx(), ctx.taskJsonTotalTaskCount,
                 ctx.dagNodeCountBeforeCpGmMerge,
                 ctx.dagNodeCountAfterCpGmMerge, ctx.cpGmLoopMergeCount,
                 ctx.cpGmMergedIterationCount, ctx.cpGmMergedOriginalNodeCount,
...
std::vector<MergeGroup> BuildPipeBarrierGroups(const AivLaunchContext &ctx) {
    ...
    for (const auto &entry : ctx.taskIdToNode) {
        if (entry.second.task != nullptr &&
            entry.second.task->taskType == AivRuntimeTaskTypeV3::PIPE_BARRIER) {
    ...
    for (size_t i = 0; i < barriers.size(); ++i) {
        ...
        group.topoOrder = group.memberNodeIds.size() == 1
                              ? barriers[i].order
                              : std::min(group.topoOrder, barriers[i].order);
    ...
    for (auto &entry : groupsByRoot) {
        std::sort(entry.second.memberTaskIds.begin(),
                  entry.second.memberTaskIds.end());
        groups.push_back(std::move(entry.second));
    }
    std::sort(groups.begin(), groups.end(),
              [](const MergeGroup &lhs, const MergeGroup &rhs) {
...
std::vector<MergeGroup> BuildSyncAllGroups(const AivLaunchContext &ctx) {
    ...
    for (const auto &entry : ctx.taskIdToNode) {
        ...
        if (record.task == nullptr ||
            record.task->taskType != AivRuntimeTaskTypeV3::SYNC_ALL) {
            continue;
        }
        ...
        group.topoOrder = group.memberNodeIds.size() == 1
                              ? record.order
                              : std::min(group.topoOrder, record.order);
...
HcclResult MergePipeBarrierGroups(AivLaunchContext &ctx) {
    ...
    for (const MergeGroup &group : groups) {
        ...
        for (NodeId memberNodeId : group.memberNodeIds) {
            ...
            const auto *barrier =
                dynamic_cast<const TaskAivPipeBarrier *>(member);
            if (barrier == nullptr) {
                HCCL_VM_ERROR(
                    ...
                    ctx.placeholder->GetDeviceId(),
                    ctx.placeholder->GetLaunchIdx(), memberNodeId,
                    ctx.snapshot.filePath);
                return HCCL_E_INTERNAL;
            }
            ...
            if (memberPosition.blockId != group.blockId) {
                HCCL_VM_ERROR(
                    ...
                    MakeErrorCodeText(ErrorCode::GRAPH_STRUCTURE_INVALID),
                    ctx.placeholder->GetDeviceId(),
                    ctx.placeholder->GetLaunchIdx(), group.blockId,
                    memberPosition.blockId, memberPosition.taskId,
                    ctx.snapshot.filePath);
                return HCCL_E_INTERNAL;
            }
            if (!memberPipes.insert(memberPosition.pipe).second) {
                HCCL_VM_ERROR(
                    ...
                    ctx.placeholder->GetDeviceId(),
                    ctx.placeholder->GetLaunchIdx(), group.blockId,
                    memberPosition.pipe, ctx.snapshot.filePath);
                return HCCL_E_INTERNAL;
            }
        }
        if (group.pipeType == PIPE_ALL) {
            for (uint32_t pipe = 0; pipe < AIV_PIPE_NUM; ++pipe) {
                ...
                HCCL_VM_ERROR(
                    ...
                    ctx.placeholder->GetDeviceId(),
                    ctx.placeholder->GetLaunchIdx(), group.blockId, pipe,
                    existingPipeIds.str(), memberTaskIds.str(),
                    ctx.snapshot.filePath);
                return HCCL_E_INTERNAL;
            }
        }
        AivBarrierInfo info;
        const uint32_t mergedTaskId = group.memberTaskIds.empty()
                                          ? std::numeric_limits<uint32_t>::max()
        ...
        const TaskPosition position =
            MakeAivPosition(ctx.snapshot.deviceId, ctx.snapshot.rankId,
                            ctx.placeholder->GetLaunchIdx(), group.blockId,
                            std::numeric_limits<uint32_t>::max(), mergedTaskId,
                            ctx.placeholder->GetOperatorId(), ctx.storage,
                            ctx.placeholder->GetPosition());
        HcclResult ret = ctx.graph->AppendGeneratedNode(
            std::make_unique<TaskAivPipeBarrier>(std::move(info)), position,
            mergeNodeId);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
...
HcclResult MergeSyncAllGroups(AivLaunchContext &ctx) {
    ...
    const size_t expectedMemberCount =
        ctx.snapshot.blocks.size() * AIV_PIPE_NUM;
    for (const MergeGroup &group : groups) {
        if (group.memberNodeIds.empty()) {
            continue;
        }
        if (expectedMemberCount != 0 &&
            group.memberNodeIds.size() != expectedMemberCount) {
            HCCL_VM_ERROR("{} One SyncAll group is incomplete because it does "
                          ...
                          MakeErrorCodeText(ErrorCode::GRAPH_MEMBER_MISSING),
                          ctx.placeholder->GetDeviceId(),
                          ctx.placeholder->GetLaunchIdx(), group.syncRound,
                          group.memberNodeIds.size(), expectedMemberCount,
                          ctx.snapshot.filePath);
            return HCCL_E_INTERNAL;
        }
        ...
        for (NodeId memberNodeId : group.memberNodeIds) {
            ...
            if (!members.insert({position.blockId, position.pipe}).second) {
                HCCL_VM_ERROR(
                    ...
                    ctx.placeholder->GetDeviceId(),
                    ctx.placeholder->GetLaunchIdx(), group.syncRound,
                    position.blockId, position.pipe, ctx.snapshot.filePath);
                return HCCL_E_INTERNAL;
            }
        }
        for (const auto &block : ctx.snapshot.blocks) {
        ...
        const TaskPosition position =
            MakeAivPosition(ctx.snapshot.deviceId, ctx.snapshot.rankId,
                            ...
                            ctx.placeholder->GetPosition());
        HcclResult ret = ctx.graph->AppendGeneratedNode(
            std::make_unique<TaskAivSyncAll>(std::move(info)), position,
            mergeNodeId);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
...
HcclResult ConnectCurrentTailsToEnd(AivLaunchContext &ctx) {
    ...
    for (NodeId nodeId : reachableInternalNodes) {
        ...
        if (node->GetType() == TaskType::START ||
            node->GetType() == TaskType::END) {
            continue;
        }
        bool hasInternalChild = false;
        for (TaskNode *child : node->GetChildren()) {
            if (child != nullptr &&
                internalSet.count(child->GetNodeId()) != 0) {
...
HcclResult ReconnectPlaceholder(AivLaunchContext &ctx) {
    ...
    HcclResult ret =
        AddEdge(ctx.graph, placeholder->GetNodeId(), ctx.headNodeId);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
    ...
    for (TaskNode *child : originalChildren) {
        ...
        ret =
            ctx.graph->RemoveEdge(placeholder->GetNodeId(), child->GetNodeId());
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
...
HcclResult ExpandOneAivGraph(TaskGraphGeneratorV3 *graph,
                             StorageManager *storage, TaskAivGraph *aivGraph,
    ...
    HcclResult ret = loader.LoadByDeviceAndLaunch(
        deviceId, aivGraph->GetLaunchIdx(), ctx.snapshot, errorMessage);
    if (ret != HCCL_SUCCESS) {
        HCCL_VM_ERROR("{} Failed to load the AIV runtime snapshot file, "
                      "deviceId={}, launchId={}, "
                      "ret={}, loaderMessage={}",
                      MakeErrorCodeText(ErrorCode::GRAPH_RESOURCE_NOT_FOUND),
                      deviceId, aivGraph->GetLaunchIdx(),
    ...
    ret = UpdateAivBufferSize(ctx.snapshot.ubBufferSize, "ubBufferSize", ctx,
                              g_checkerAivUbBufferSize);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
    ret = UpdateAivBufferSize(ctx.snapshot.aivCommInfoSize, "aivCommInfoSize",
                              ctx, g_checkerAivCommInfoSize);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
    ...
    ret = graph->AppendGeneratedNode(
        std::make_unique<TaskStart>(BoundaryType::AIV_SUB_GRAPH), basePosition,
        ctx.headNodeId);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
    ...
    ret = graph->AppendGeneratedNode(
        std::make_unique<TaskEnd>(BoundaryType::AIV_SUB_GRAPH), basePosition,
        ctx.endNodeId);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
...
bool IsExecutableForFlagMatch(const TaskGraphGeneratorV3 *graph, NodeId nodeId,
                              const std::set<NodeId> &executed) {
    if (graph == nullptr) {
        return false;
    }
...
HcclResult EnqueueChildrenForFlagMatch(const TaskGraphGeneratorV3 *graph,
                                       NodeId nodeId,
                                       std::deque<NodeId> &readyQueue,
                                       std::set<NodeId> &queued) {
    const TaskNode *node = graph->GetNode(nodeId);
...
HcclResult MatchSendRecvFlagEdges(TaskGraphGeneratorV3 *graph,
    ...
    while (!readyQueue.empty()) {
        if (unmatchedCnt >= readyQueue.size()) {
            ...
            if (node != nullptr && node->GetType() == TaskType::AIV_RECV_FLAG) {
                const auto *recvFlag =
                    dynamic_cast<const TaskAivRecvFlag *>(node);
                if (recvFlag != nullptr) {
                    const auto stateIter = flagStates.find(
                        MakeFlagCellKey(*node, recvFlag->GetFlag()));
                    if (stateIter != flagStates.end()) {
                        std::ostringstream os;
                        os << "{currentValue=" << stateIter->second.currentValue
                           << ", producerCount="
                           << stateIter->second.producerNodes.size()
                           << ", producerNodeIds=[";
                        for (size_t i = 0;
                             i < stateIter->second.producerNodes.size(); ++i) {
                            if (i != 0) {
                                os << ",";
                            }
            ...
            HCCL_VM_ERROR("{} AIV SendFlag/RecvFlag matching is stuck. Some "
        ...
        if (node->GetType() == TaskType::AIV_SEND_FLAG) {
            ...
            FlagCellState &state =
                flagStates[MakeFlagCellKey(*node, sendFlag->GetFlag())];
            if (state.currentValue == sendFlag->GetFlag().value &&
                !state.producerNodes.empty()) {
                HCCL_VM_WARN("The same flag cell received the same SendFlag "
            ...
            FlagCellState &state =
                flagStates[MakeFlagCellKey(*node, recvFlag->GetFlag())];
        ...
        executed.insert(nodeId);
        HcclResult ret =
            EnqueueChildrenForFlagMatch(graph, nodeId, readyQueue, queued);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        unmatchedCnt = 0;
...
HcclResult ExpandAivGraphsV3(const AivGraphsGenerateInputV3 &input,
    ...
    for (TaskAivGraph *aivGraph : input.aivGraphs) {
        HcclResult ret =
            ExpandOneAivGraph(input.graph, input.storage, aivGraph, output);
        if (ret != HCCL_SUCCESS) {
    ...
    const std::string snapshotFileHint = input.aivGraphs.empty()
                                             ? "aiv_snapshot_unavailable"
                                             : "multiple_aiv_snapshots";
    HcclResult ret = MatchSendRecvFlagEdges(input.graph, snapshotFileHint,
                                            output.sendRecvEdgeCount);
    output.expandNs = ElapsedNs(start, AivExpandClock::now());
    return ret;
}
