...
const TaskNode *
GetQueueNode(const std::vector<std::unique_ptr<TaskNode>> &nodes,
             NodeId nodeId) {
...
std::string DescribePosition(const TaskPosition &loc) {
    ...
    os << "deviceId=";
    if (loc.deviceId == INVALID_DEVICE_ID) {
        os << "invalid";
    } else {
        os << loc.deviceId;
    }
...
std::string DescribeMemSlice(const MemSlice &slice) {
    ...
    os << ", memType=" << DescribeMemType(slice.memType) << ", rawAddr=0x"
...
bool IsEmptyLocalCopy(const TaskNode *node) {
    ...
    return src.deviceId == dst.deviceId && src.len == 0 && dst.len == 0;
}
...
bool StreamHasAivGraph(const std::vector<std::unique_ptr<TaskNode>> &nodes,
...
GetMainStreamIds(const std::vector<std::unique_ptr<TaskNode>> &nodes,
                 DeviceId deviceId, const RankNodeQueues &taskQueue) {
    std::set<StreamId> mainStreamIds;
    for (const auto &stream : taskQueue) {
        for (const NodeId nodeId : stream) {
            const TaskNode *node = GetQueueNode(nodes, nodeId);
            if (node == nullptr) {
                continue;
...
HcclResult CheckTwoSliceOverlap(const TaskNode *node, const MemSlice &lhs,
                                const MemSlice &rhs);
...
struct MemoryKey {
    ...
    bool operator==(const MemoryKey &rhs) const {
        return deviceId == rhs.deviceId && memType == rhs.memType;
    }
...
HcclResult AccumulateCoveredSizeByKey(const TaskNode *node,
                                      const std::vector<MemSlice> &slices,
    ...
    for (const auto &slice : slices) {
        ...
        if (!slice.HasMemoryAddress() ||
            slice.len >
                std::numeric_limits<uint64_t>::max() - slice.MemoryBegin()) {
            HCCL_VM_ERROR(
                "{} One memory slice is invalid because its end address "
                "overflows "
                "while total coverage is being calculated, task={}, "
                "memorySlice={}",
                MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                node->Describe(), DescribeMemSlice(slice));
            return HCCL_E_PARA;
        }
        const MemoryKey key{slice.deviceId, slice.memType};
        if (slice.memType == MemType::MS_CCU) {
            repeatedSizeByKey[key] += slice.len;
            continue;
        }
        rangesByKey[key].push_back(
            {slice.MemoryBegin(), slice.MemoryBegin() + slice.len});
    }
...
HcclResult BuildCoverageSummary(const TaskNode *node,
                                const std::vector<MemSlice> &slices,
                                SliceCoverageSummary &summary) {
...
HcclResult
BuildCoverageSummary(const TaskNode *node,
                     const std::vector<std::vector<MemSlice>> &sliceGroups,
                     SliceCoverageSummary &summary) {
    summary = SliceCoverageSummary{};
...
HcclResult CheckCoverageMultiple(const TaskNode *node, uint64_t srcSize,
                                 uint64_t dstSize, const std::string &group) {
    if (node == nullptr) {
        return HCCL_E_PTR;
    }
    if (dstSize == 0) {
        ...
        HCCL_VM_ERROR(
            "{} Source data exists but the target range is empty, task={}, "
            "srcDataSize={}, dstDataSize={}, group={}",
            MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
            node->Describe(), srcSize, dstSize, group);
    ...
    HCCL_VM_ERROR(
        "{} Source data size is not an integer multiple of target data size, "
...
HcclResult CheckSliceLenEqual(const TaskNode *node, const MemSlice &src,
                              const MemSlice &dst, const std::string &label,
                              size_t index) {
    if (node == nullptr) {
        return HCCL_E_PTR;
    }
    ...
    HCCL_VM_ERROR(
        "{} In one batch transfer pair, source length and target length are "
        "different, task={}, group={}, pairIndex={}, sourceMemorySlice={}, "
        "targetMemorySlice={}",
        MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
        node->Describe(), label, index, DescribeMemSlice(src),
        DescribeMemSlice(dst));
    return HCCL_E_INTERNAL;
}
...
bool MemSliceLess(const MemSlice &lhs, const MemSlice &rhs) {
    ...
    if (lhs.MemoryBegin() != rhs.MemoryBegin()) {
        return lhs.MemoryBegin() < rhs.MemoryBegin();
    }
    return lhs.len < rhs.len;
}

// merged slices 只用于更快地做 batch
// 节点内部重叠检查，因此这里先按完整四元组去重。
...
HcclResult CheckBatchTransSlices(const TaskNode *node,
                                 const std::vector<MemSlice> &srcs,
                                 const std::vector<MemSlice> &dsts,
                                 const std::vector<MemSlice> &mergedSrcs,
                                 const std::vector<MemSlice> &mergedDsts,
    ...
    if (srcs.size() != dsts.size()) {
        HCCL_VM_ERROR("{} Batch transfer has different counts of source memory "
                      "slices and "
                      "target memory slices, task={}, group={}, "
                      "sourceMemorySliceCount={}, targetMemorySliceCount={}",
                      MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                      node->Describe(), label, srcs.size(), dsts.size());
        return HCCL_E_PARA;
    }
    ...
    HCCL_VM_DEBUG(
        "Batch transfer merged-slice check finished, "
        "taskId={}, group={}, originalSourceMemorySliceCount={}, "
        "originalTargetMemorySliceCount={}, "
        "mergedSourceMemorySliceCount={}, mergedTargetMemorySliceCount={}, "
        "uniqueMergedSourceMemorySliceCount={}, "
        "uniqueMergedTargetMemorySliceCount={}",
        node->GetNodeId(), label, srcs.size(), dsts.size(), mergedSrcs.size(),
        mergedDsts.size(), dedupMergedSrcs.size(), dedupMergedDsts.size());
    return HCCL_SUCCESS;
}

HcclResult CheckBatchReduceSlices(
    const TaskNode *node, const std::vector<std::vector<MemSlice>> &srcGroups,
    const std::vector<MemSlice> &dsts, const std::string &label,
    SliceCoverageSummary &srcSummary, SliceCoverageSummary &dstSummary) {
    if (node == nullptr) {
        return HCCL_E_PTR;
    }
    ...
    if (srcGroups.empty() || srcGroups.size() != dsts.size()) {
        HCCL_VM_ERROR(
            "{} Batch reduce has different counts of source groups and target "
            "memory slices, task={}, group={}, sourceGroupCount={}, "
            "targetMemorySliceCount={}",
            MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
            node->Describe(), label, srcGroups.size(), dsts.size());
        return HCCL_E_PARA;
    }
    for (size_t groupIndex = 0; groupIndex < srcGroups.size(); ++groupIndex) {
        ...
        if (srcGroups[groupIndex].empty()) {
            HCCL_VM_ERROR(
                ...
                "groupIndex={}",
                MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                node->Describe(), label, groupIndex);
            return HCCL_E_PARA;
        }
    ...
    return CheckCoverageMultiple(node, srcSummary.totalCoveredSize,
                                 dstSummary.totalCoveredSize, label);
}
...
HcclResult CheckSingleSlice(const TaskNode *node, const MemSlice &slice) {
    ...
    if (slice.deviceId == INVALID_DEVICE_ID ||
        ...
        HCCL_VM_ERROR("{} One memory slice is missing a valid rank or memory "
                      "type, task={}, "
                      "memorySlice={}",
                      MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                      node->Describe(), DescribeMemSlice(slice));
    ...
    if (!slice.HasMemoryAddress()) {
        HCCL_VM_ERROR("{} Memory slice has no address, task={}, memorySlice={}",
                      MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                      node->Describe(), DescribeMemSlice(slice));
        return HCCL_E_PARA;
    }
    if (slice.len >
        std::numeric_limits<uint64_t>::max() - slice.MemoryBegin()) {
        HCCL_VM_ERROR("{} One memory slice is invalid because address + length "
                      "exceeds the "
    ...
    if (slice.memType == MemType::AIV_UB ||
        slice.memType == MemType::AIV_COMM) {
        ...
        if (boundSize != 0 && slice.offset + slice.len > boundSize) {
            HCCL_VM_ERROR(
                "{} One AIV memory slice goes past the valid AIV buffer "
                "boundary, "
    ...
    if (IsGlobalPhysicalMemoryType(slice.memType)) {
        // 绝对地址包含校验：按设备上注册的全部布局块判定，
        // 不依赖可能被 inplace 首命中误判的 memType 与对应错误的块尺寸。
        if (slice.len == 0) {
            // 零长度切片不占用内存，无需包含校验（与旧相对路径对零长度切片放行的行为一致）。
            return HCCL_SUCCESS;
        }
        const auto &physPosition = node->GetPosition();
        if (!StorageManager::GetInstance().IsMemRangeRegistered(
                physPosition.commName, physPosition.commHash,
                physPosition.opIter, slice.deviceId, slice.MemoryBegin(),
                slice.len)) {
            HCCL_VM_ERROR(
                "{} One memory slice goes outside the registered buffers of "
                "its device, "
                "task={}, memorySlice={}, position={}",
                MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                node->Describe(), DescribeMemSlice(slice),
                DescribePosition(physPosition));
            return HCCL_E_INTERNAL;
        }
        return HCCL_SUCCESS;
    }
    ...
    HCCL_VM_ERROR(
        "{} This task uses a memory type that the checker does not support, "
        "task={}, memorySlice={}",
        MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
        node->Describe(), DescribeMemSlice(slice));
...
HcclResult CheckTwoSliceOverlap(const TaskNode *node, const MemSlice &lhs,
    ...
    if (lhs.deviceId != rhs.deviceId || lhs.len == 0 || rhs.len == 0) {
        return HCCL_SUCCESS;
    }
    // INPUT/OUTPUT/CCL 可能是同一物理 buffer
    // 的不同逻辑视图，因此先按绝对地址比较。
    if (!lhs.HasMemoryAddress() || !rhs.HasMemoryAddress()) {
        HCCL_VM_ERROR("{} Memory slice overlap check requires valid addresses, "
                      "task={}, memorySlice1={}, memorySlice2={}",
                      MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                      node->Describe(), DescribeMemSlice(lhs),
                      DescribeMemSlice(rhs));
        return HCCL_E_PARA;
    }
    const bool comparePhysicalRanges =
        IsGlobalPhysicalMemoryType(lhs.memType) &&
        IsGlobalPhysicalMemoryType(rhs.memType);
    if (comparePhysicalRanges) {
        if (ArePhysicalRangesEqual(lhs, rhs)) {
            HCCL_VM_WARN("Source and destination use the same physical memory "
                         "range (inplace alias), "
                         "which may hurt performance, position={}, task={}, "
                         "memorySlice1={}, memorySlice2={}",
                         DescribePosition(node->GetPosition()),
                         node->Describe(), DescribeMemSlice(lhs),
                         DescribeMemSlice(rhs));
            return HCCL_SUCCESS;
        }
        if (DoPhysicalRangesOverlap(lhs, rhs)) {
            HCCL_VM_ERROR(
                "{} Two global memory slices partially overlap inside the same "
                "task, "
                "task={}, memorySlice1={}, memorySlice2={}, position={}",
                MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_CONFLICT),
                node->Describe(), DescribeMemSlice(lhs), DescribeMemSlice(rhs),
                DescribePosition(node->GetPosition()));
            return HCCL_E_INTERNAL;
        }
        // 两个全局切片的逻辑类型可能不同，绝对区间不相交时无需再比较类型。
        return HCCL_SUCCESS;
    }
    if (!IsSameMemoryType(lhs, rhs)) {
        return HCCL_SUCCESS;
    }
    if (lhs.len > std::numeric_limits<uint64_t>::max() - lhs.MemoryBegin() ||
        rhs.len > std::numeric_limits<uint64_t>::max() - rhs.MemoryBegin()) {
        HCCL_VM_ERROR("{} Two memory slices cannot be compared because one "
                      "absolute range already "
    ...
    if (node->HasCcuTrace()) {
        // CCU 模式允许同一独立地址空间内 src 与 dst 的区间完全相同。
        if (lhs.deviceId == rhs.deviceId && IsSameMemoryType(lhs, rhs) &&
            lhs.MemoryBegin() == rhs.MemoryBegin() && lhs.len == rhs.len) {
            HCCL_VM_WARN("Source and destination use the same memory slice, "
                         "which may hurt "
                         "performance, position={}, task={}, memorySlice1={}, "
                         "memorySlice2={}",
                         DescribePosition(node->GetPosition()),
                         node->Describe(), DescribeMemSlice(lhs),
                         DescribeMemSlice(rhs));
    ...
    const bool conflictCase1 =
        lhs.MemoryBegin() >= rhs.MemoryBegin() &&
        lhs.MemoryBegin() < (rhs.MemoryBegin() + rhs.len);
    const bool conflictCase2 =
        rhs.MemoryBegin() >= lhs.MemoryBegin() &&
        rhs.MemoryBegin() < (lhs.MemoryBegin() + lhs.len);
    if (conflictCase1 || conflictCase2) {
        HCCL_VM_ERROR(
            "{} Two memory slices overlap inside the same task, task={}, "
...
HcclResult CheckAivCommInfoSyncNodeMem(const TaskNode *node) {
    ...
    if (flag->commInfoOffset % AIV_COMM_INFO_SYNC_CELL_BYTES != 0) {
        HCCL_VM_ERROR(
            "{} AIV commInfo synchronization address is not cell aligned, "
            "task={}, commInfoOffset={}, cellSize={}",
            MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
            node->Describe(), flag->commInfoOffset,
            AIV_COMM_INFO_SYNC_CELL_BYTES);
        return HCCL_E_PARA;
    }
    const uint64_t aivCommInfoSize = GetAivSliceBoundSize(MemType::AIV_COMM);
    if (aivCommInfoSize != 0 &&
        (aivCommInfoSize < sizeof(int32_t) ||
         flag->commInfoOffset > aivCommInfoSize - sizeof(int32_t))) {
        HCCL_VM_ERROR("{} AIV commInfo synchronization address goes past the "
                      "valid buffer boundary, task={}, "
                      "commInfoOffset={}, aivCommInfoSize={}",
                      MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                      node->Describe(), flag->commInfoOffset, aivCommInfoSize);
        return HCCL_E_INTERNAL;
    }
    return HCCL_SUCCESS;
}
...
HcclResult CheckDataMoveTaskMem(const TaskNode *node) {
    ...
    if (node->GetType() == TaskType::TRANS_MEM) {
        ...
        return CheckTwoSliceOverlap(node, transMem->GetSrc(),
                                    transMem->GetDst());
    }
    ...
    if (node->GetType() == TaskType::REDUCE) {
        ...
        if (srcs.empty()) {
            HCCL_VM_ERROR(
                "{} This reduce task has no source data at all, task={}",
                MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                node->Describe());
            return HCCL_E_PARA;
        }
    ...
    if (node->GetType() == TaskType::BATCH_TRANS_MEM) {
        ...
        HcclResult ret = CheckBatchTransSlices(
            node, transMem->GetSrcs(), transMem->GetDsts(),
    ...
    if (node->GetType() == TaskType::BATCH_REDUCE) {
        ...
        HcclResult ret = CheckBatchReduceSlices(
            node, reduce->GetSrcs(), reduce->GetDsts(), "original",
    ...
    if (node->GetType() == TaskType::AIV_SEND_FLAG ||
        node->GetType() == TaskType::AIV_RECV_FLAG) {
...
HcclResult
CheckSlaveTaskQueue(const std::vector<std::unique_ptr<TaskNode>> &nodes,
                    const AllRankNodeQueues &allRankTaskQueues) {
    for (const auto &rankEntry : allRankTaskQueues) {
        const DeviceId deviceId = rankEntry.first;
        const RankNodeQueues &taskQueue = rankEntry.second;
        const size_t queueNum = taskQueue.size();
        const std::set<StreamId> mainStreamIds =
            GetMainStreamIds(nodes, deviceId, taskQueue);
        for (size_t streamId = 0; streamId < queueNum; ++streamId) {
            if (mainStreamIds.find(static_cast<StreamId>(streamId)) !=
                mainStreamIds.end()) {
                continue;
            }
            ...
            if (StreamHasAivGraph(nodes, stream)) {
                HCCL_VM_DEBUG("Skip this slave-stream structure check because "
                              "the stream contains "
                              "an AIV graph task, deviceId={}, streamId={}",
                              deviceId, streamId);
                continue;
            }
            if (taskSize < 2) {
            ...
            if (firstTask == nullptr || lastTask == nullptr) {
                HCCL_VM_ERROR(
                    "{} This slave stream is missing its start node "
                    "or end node, deviceId={}, streamId={}, taskCount={}, "
                    "startNode={}, endNode={}",
                    MakeErrorCodeText(
                        ErrorCode::SINGLETASK_SLAVE_STREAM_INVALID),
                    deviceId, streamId, taskSize,
            ...
            while (IsEmptyLocalCopy(lastTask) && backStep < taskSize) {
                ...
                if (lastTask == nullptr) {
                    HCCL_VM_ERROR(
                        "{} This slave stream still has no valid end "
                        "node after empty local-copy tasks are skipped, "
                        "deviceId={}, streamId={}, "
                        "skippedEmptyLocalCopyCount={}, currentTailNode={}",
                        MakeErrorCodeText(
                            ErrorCode::SINGLETASK_SLAVE_STREAM_INVALID),
                        deviceId, streamId, skippedEmptyLocalCopyCount, "null");
                    return HCCL_E_PTR;
                }
            ...
            if (hcclvmTopoType != nullptr &&
                std::string(hcclvmTopoType) == "HF") {
                continue;
            }

            if (!IsLocalAicpuWait(firstTask)) {
                HCCL_VM_ERROR("{} The first task in this slave stream is not a "
                              "local WAIT task, deviceId={}, streamId={}, "
                              "actualFirstTaskType={}, firstTask={}",
                              MakeErrorCodeText(
                                  ErrorCode::SINGLETASK_SLAVE_STREAM_INVALID),
                              deviceId, streamId,
                              TaskTypeName(firstTask->GetType()),
                              firstTask->Describe());
                return HCCL_E_INTERNAL;
            }
            ...
            if (!IsLocalAicpuRecord(lastTask)) {
                HCCL_VM_ERROR("{} The last task in this slave stream is not a "
                              "local RECORD task, deviceId={}, streamId={}, "
                              "actualLastTaskType={}, lastTask={}",
                              MakeErrorCodeText(
                                  ErrorCode::SINGLETASK_SLAVE_STREAM_INVALID),
                              deviceId, streamId,
                              TaskTypeName(lastTask->GetType()),
                              lastTask->Describe());
                return HCCL_E_INTERNAL;
            }
        }
...
HcclResult CheckTaskMem(const TaskNode *dummyStart,
                        SingleTaskCheckStats *stats) {
