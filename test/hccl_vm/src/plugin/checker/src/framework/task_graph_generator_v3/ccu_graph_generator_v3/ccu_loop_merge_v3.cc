...
void AppendCkeOps(std::vector<CcuLoopCkeOpV3> &dst,
                  const std::vector<CcuLoopCkeOpV3> &src) {
    for (const auto &op : src) {
        if (op.mask == 0) {
            continue;
        }
        auto iter = std::find_if(dst.begin(), dst.end(),
                                 [&op](const CcuLoopCkeOpV3 &exist) {
...
bool SameRankGroup(const std::vector<MemSlice> &slices, DeviceId deviceId) {
    return std::all_of(slices.begin(), slices.end(),
                       [deviceId](const MemSlice &slice) {
                           return slice.deviceId == deviceId;
                       });
}

// 判断所有切片是否都属于同一个对端rank（非本端），并输出该对端rankId
bool SamePeerGroup(const std::vector<MemSlice> &slices, DeviceId localDeviceId,
...
HcclResult ResolveTransRole(DeviceId deviceId,
    ...
    for (size_t index = 0; index < srcs.size(); ++index) {
        ...
        if (srcs[index].deviceId == deviceId &&
            dsts[index].deviceId == deviceId) {
            curRole = CcuNodeRoleV3::LOCAL_COPY;
        } else if (srcs[index].deviceId == deviceId &&
                   dsts[index].deviceId != deviceId) {
...
HcclResult
ResolveReduceRole(DeviceId deviceId,
                  const std::vector<std::vector<MemSlice>> &srcGroups,
                  const std::vector<MemSlice> &dsts, CcuNodeRoleV3 &role,
                  DeviceId &peerDeviceId) {
    ...
    for (size_t index = 0; index < srcGroups.size(); ++index) {
        CcuNodeRoleV3 curRole = CcuNodeRoleV3::UNKNOWN;
        DeviceId curPeerDeviceId = INVALID_DEVICE_ID;
        if (dsts[index].deviceId == deviceId &&
            SameRankGroup(srcGroups[index], deviceId)) {
            curRole = CcuNodeRoleV3::LOCAL_BATCH_REDUCE;
        } else if (dsts[index].deviceId != deviceId &&
                   SameRankGroup(srcGroups[index], deviceId)) {
            curRole = CcuNodeRoleV3::WRITE_REDUCE;
            curPeerDeviceId = dsts[index].deviceId;
        } else if (dsts[index].deviceId == deviceId &&
                   SamePeerGroup(srcGroups[index], deviceId, curPeerDeviceId)) {
...
HcclResult ProcessWaitOps(CcuGraphStateV3 *curCcuTask, uint32_t queId,
    ...
    for (const auto &wait : waitOps) {
        CHK_RET(ProcessWaitMask(deviceId, dieId, curCcuTask, queId, wait.ckeId,
                                wait.mask, isContinue));
        if (!isContinue) {
            return HCCL_SUCCESS;
        }
    }
...
HcclResult ProcessSetOps(CcuGraphStateV3 *curCcuTask, uint32_t queId,
    ...
    for (const auto &set : setOps) {
        CHK_RET(ProcessSetMask(deviceId, dieId, curCcuTask, queId, set.ckeId,
                               set.mask));
    }
    return HCCL_SUCCESS;
}
...
std::set<uint16_t> CcuLoopInstrV3::GetUsedMS() const { return {}; }
...
std::string CcuLoopInstrV3::Describe() const {
    ...
    os << "{deviceId=" << deviceId << ", dieId=" << dieId
       << ", instrId=" << instrId << "}";
    return os.str();
}
...
bool CcuLoopInstrV3::MergeMemSlices(std::vector<MemSlice> &slices,
    ...
    for (size_t index = 1; index < slices.size(); ++index) {
        ...
        if (last.deviceId == cur.deviceId && last.memType == cur.memType &&
            cur.memType != MemType::MS_CCU) {
            const uint64_t lastEnd = last.offset + last.len;
            const uint64_t curEnd = cur.offset + cur.len;
            if (lastEnd > cur.offset && !allowOverlap) {
                HCCL_VM_WARN("Skip merged-loop optimization because memory "
...
std::set<uint16_t>
CcuLoopInstrV3::GetUsedCKEFromOps(const std::vector<CcuLoopCkeOpV3> &ops) {
...
bool CcuLoopTransMemV3::MemMerge(bool allowOverlap) {
    mergedSrcs = srcs;
    mergedDsts = dsts;
    return MergeMemSlices(mergedSrcs, allowOverlap) &&
           MergeMemSlices(mergedDsts, allowOverlap);
}
...
std::shared_ptr<CcuLoopInstrV3> CcuLoopTransMemV3::InstrMerge(
    ...
    for (const auto &instr : instrs) {
        ...
        result->srcs.insert(result->srcs.end(), trans->srcs.begin(),
                            trans->srcs.end());
        result->dsts.insert(result->dsts.end(), trans->dsts.begin(),
                            trans->dsts.end());
...
std::set<uint16_t> CcuLoopTransMemV3::GetUsedMS() const { return msIds; }
...
std::string CcuLoopTransMemV3::Describe() const {
    std::ostringstream os;
    os << "{deviceId=" << deviceId << ", dieId=" << dieId
       << ", instrId=" << instrId << ", srcs=" << srcs.size()
       << ", dsts=" << dsts.size() << ", waitOps=" << waitOps.size()
       << ", setOps=" << setOps.size() << "}";
    return os.str();
}
...
std::shared_ptr<CcuLoopInstrV3> CcuLoopReduceV3::InstrMerge(
    ...
    for (const auto &instr : instrs) {
        ...
        if (reduce == nullptr || reduce->dataType != dataType ||
            reduce->reduceOp != reduceOp) {
            return nullptr;
        }
        result->srcs.insert(result->srcs.end(), reduce->srcs.begin(),
                            reduce->srcs.end());
        result->dsts.insert(result->dsts.end(), reduce->dsts.begin(),
                            reduce->dsts.end());
...
std::set<uint16_t> CcuLoopReduceV3::GetUsedMS() const { return msIds; }
...
std::string CcuLoopReduceV3::Describe() const {
    std::ostringstream os;
    os << "{deviceId=" << deviceId << ", dieId=" << dieId
       << ", instrId=" << instrId << ", srcGroups=" << srcs.size()
       << ", dsts=" << dsts.size()
...
std::string CcuLoopClearCkeV3::Describe() const {
    std::ostringstream os;
    os << "{deviceId=" << deviceId << ", dieId=" << dieId
       << ", instrId=" << instrId << ", clearCKEId=" << clearCKEId
       << ", clearMask=0x" << std::hex << clearMask << std::dec
...
HcclResult CcuLoopWaitCkeChecker::Check(size_t instrIndex,
    ...
    for (const auto &loopInstrs : loopExpands) {
        for (uint16_t ckeId : loopInstrs.GetUsedCKE()) {
            if (!allCKEs.insert(ckeId).second) {
                HCCL_VM_WARN("Skip merged-loop optimization because different "
...
bool CcuLoopV3::LoopIterationMerge() {
    for (auto &loopInstrs : loopExpands) {
        if (!loopInstrs.MemMerge(true)) {
            HCCL_VM_WARN("Skip merged-loop optimization because memory slices "
                         "inside one loop "
...
bool CcuLoopV3::LoopExpandMerge() {
    ...
    for (const auto &expand : loopExpands) {
        if (expand.instrs.size() != instrCnt) {
            HCCL_VM_WARN("Skip merged-loop optimization because loop "
    ...
    for (size_t instrIdx = 0; instrIdx < instrCnt; ++instrIdx) {
        ...
        for (size_t expandIdx = 1; expandIdx < loopExpands.size();
             ++expandIdx) {
            parallelInstrs.push_back(loopExpands[expandIdx].instrs[instrIdx]);
        }
        auto mergedInstr =
            loopExpands.front().instrs[instrIdx]->InstrMerge(parallelInstrs);
        if (mergedInstr == nullptr) {
            HCCL_VM_WARN("Skip merged-loop optimization because one "
    ...
    if (!merged.MemMerge(false)) {
        HCCL_VM_WARN("Skip merged-loop optimization because merged instruction "
                     "memory ranges still "
                     "overlap, loopInstrId={}",
                     instrId);
        return false;
...
HcclResult EmitMergedLoopInstrsV3(CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                  const CcuLoopInstrsV3 &mergedInstrs,
                                  bool &isContinue) {
    ...
    for (const auto &instr : mergedInstrs.instrs) {
        if (instr == nullptr) {
            HCCL_VM_ERROR(
                ...
                MakeErrorCodeText(ErrorCode::GRAPH_LOOP_MERGE_ERROR).c_str(),
                static_cast<uint32_t>(deviceId), queId);
            return HCCL_E_PTR;
        }
        HcclResult ret =
            ProcessWaitOps(curCcuTask, queId, instr->waitOps, isContinue);
        if (ret != HCCL_SUCCESS) {
            HCCL_VM_ERROR(
                ...
                MakeErrorCodeText(ErrorCode::GRAPH_LOOP_MERGE_ERROR).c_str(),
                static_cast<uint32_t>(deviceId), queId, instr->Describe());
            return ret;
        }
        if (!isContinue) {
            return HCCL_SUCCESS;
        ...
        if (auto trans = std::dynamic_pointer_cast<CcuLoopTransMemV3>(instr)) {
            ...
            ret = ResolveTransRole(deviceId, trans->srcs, trans->dsts, role,
                                   peerDeviceId);
            if (ret != HCCL_SUCCESS) {
                HCCL_VM_ERROR(
                    ...
                    "merged loop "
                    "instruction, deviceId={}, queueId={}, mergedLoopInstr={}",
                    MakeErrorCodeText(ErrorCode::GRAPH_LOOP_MERGE_ERROR)
                        .c_str(),
                    static_cast<uint32_t>(deviceId), queId, trans->Describe());
                return ret;
            }
            ret = AddBatchTransMem(deviceId, queId, curCcuTask, trans->srcs,
                                   trans->dsts, trans->mergedSrcs,
                                   trans->mergedDsts, role, peerDeviceId);
            if (ret != HCCL_SUCCESS) {
                HCCL_VM_ERROR(
                ...
                return ret;
                       ...
                       std::dynamic_pointer_cast<CcuLoopReduceV3>(instr)) {
            CcuNodeRoleV3 role = CcuNodeRoleV3::UNKNOWN;
            DeviceId peerDeviceId = INVALID_DEVICE_ID;
            ret = ResolveReduceRole(deviceId, reduce->srcs, reduce->dsts, role,
                                    peerDeviceId);
            if (ret != HCCL_SUCCESS) {
                HCCL_VM_ERROR(
                    ...
                    MakeErrorCodeText(ErrorCode::GRAPH_LOOP_MERGE_ERROR)
                        .c_str(),
                    static_cast<uint32_t>(deviceId), queId, reduce->Describe());
                return ret;
            }
            ret = AddBatchReduce(deviceId, queId, curCcuTask, reduce->srcs,
                                 reduce->dsts, reduce->mergedSrcs,
                                 reduce->mergedDsts, reduce->dataType,
                                 reduce->reduceOp, role, peerDeviceId);
            if (ret != HCCL_SUCCESS) {
                HCCL_VM_ERROR(
                ...
                return ret;
                       ...
                       std::dynamic_pointer_cast<CcuLoopClearCkeV3>(instr)) {
            if (clear->clearMask != 0) {
                HCCL_VM_WARN("{} Skip merged-loop optimization because "
