...
class CcuLoopInstrV3 {
  ...
  protected:
    ...
    GetUsedCKEFromOps(const std::vector<CcuLoopCkeOpV3> &ops);
...
class CcuLoopTransMemV3 : public CcuLoopInstrV3 {
  public:
    ...
    InstrMerge(const std::vector<std::shared_ptr<CcuLoopInstrV3>> &instrs)
        const override;
...
template <typename T>
std::shared_ptr<T>
EnsureCcuLoopInstr(std::shared_ptr<CcuLoopInstrV3> &instrInLoop,
                   DeviceId deviceId, uint32_t dieId, uint16_t instrId) {
