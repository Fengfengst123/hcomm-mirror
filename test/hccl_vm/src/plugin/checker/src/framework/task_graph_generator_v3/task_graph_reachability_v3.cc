...
HcclResult AddReachableTaskNode(const TaskNode *node,
                                ReachableTaskNodes &reachable) {
    if (node == nullptr) {
        return HCCL_E_PTR;
    }
...
HcclResult BuildDataMoveNodeIndexes(const ReachableTaskNodes &reachable,
                                    DataMoveNodeIndexes &dataIndexes) {
...
HcclResult CollectReachableTaskNodes(const TaskNode *start,
    ...
    if (!IsMainGraphStartNode(start)) {
        HCCL_VM_ERROR("{} Reachability analysis cannot start because the main "
                      "start node is "
...
HcclResult GetValidChildIndex(const TaskNode *parent, const TaskNode *child,
                              const ReachableTaskNodes &reachable,
                              size_t &childIndex) {
    if (parent == nullptr) {
        return HCCL_E_PTR;
    }
    if (child == nullptr) {
        HCCL_VM_ERROR("{} Graph structure is broken because one child node is "
                      "null, parent={}",
                      MakeErrorCodeText(ErrorCode::MEMCONFLICT_DAG_INVALID),
                      parent->Describe());
        return HCCL_E_PTR;
    ...
    if (iter == reachable.indexByNode.end()) {
        HCCL_VM_ERROR("{} One graph edge is invalid, so reachability analysis "
                      "cannot continue, "
                      "parent={}, child={}",
...
HcclResult AddChildrenIndegree(const TaskNode *parent,
                               const ReachableTaskNodes &reachable,
                               std::vector<uint32_t> &indegree) {
    for (const TaskNode *childNode : parent->GetChildren()) {
        size_t childIndex = 0;
        HcclResult ret =
            GetValidChildIndex(parent, childNode, reachable, childIndex);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        ++indegree[childIndex];
...
size_t CountEdges(const TaskNode *start,
                  const std::vector<const TaskNode *> &nodes) {
...
HcclResult BuildReachabilityContext(const TaskNode *start,
    ...
    while (!topoQue.empty()) {
        ...
        for (const TaskNode *childNode : node->GetChildren()) {
            size_t childIndex = 0;
            ret = GetValidChildIndex(node, childNode, context.reachable,
                                     childIndex);
            if (ret != HCCL_SUCCESS) {
                return ret;
            }
    ...
    if (context.taskTopoNodes.size() != nodeCount) {
        ...
        for (const TaskNode *node : context.reachable.nodes) {
            ...
            const bool found = std::find(context.taskTopoNodes.begin(),
                                         context.taskTopoNodes.end(),
                                         node) != context.taskTopoNodes.end();
            if (!found) {
        ...
        HCCL_VM_ERROR(
            ...
            MakeErrorCodeText(ErrorCode::MEMCONFLICT_DAG_INVALID),
            context.fullTopoOrder.size(), nodeCount + 1U,
...
BuildDenseReachabilityClosure(const ReachabilityBuildContext &context,
    ...
    for (auto iter = context.taskTopoNodes.rbegin();
        ...
        for (const TaskNode *childNode : node->GetChildren()) {
            size_t childIndex = 0;
            HcclResult ret = GetValidChildIndex(node, childNode,
                                                context.reachable, childIndex);
            if (ret != HCCL_SUCCESS) {
                return ret;
            }
            const auto childDataIter =
                context.dataIndexes.dataIndexByNode.find(childNode);
            ...
            for (size_t wordIndex = 0; wordIndex < wordCount; ++wordIndex) {
                reachableDataByNode[nodeIndex][wordIndex] |=
                    reachableDataByNode[childIndex][wordIndex];
            }
        }
    }
    ...
    for (const auto &dataItem : context.dataIndexes.dataIndexByNode) {
        ...
        const auto nodeIter = context.reachable.indexByNode.find(node);
        if (nodeIter == context.reachable.indexByNode.end() ||
            dataIndex >= closure.matrix.size()) {
            HCCL_VM_ERROR("{} This data-move node is missing its reachability "
                          "index, node={}",
                          MakeErrorCodeText(ErrorCode::MEMCONFLICT_DAG_INVALID),
                          node->Describe());
            return HCCL_E_INTERNAL;
...
HcclResult GenReachabilityClosure(const TaskNode *start,
                                  ReachabilityClosure &closure,
...
HcclResult IsReachable(const ReachabilityClosure &closure, NodeId fromNodeId,
                       NodeId toNodeId, bool &isReachable) {
    isReachable = false;
    const auto fromIter = closure.dataIndexByNodeId.find(fromNodeId);
    if (fromIter == closure.dataIndexByNodeId.end()) {
        HCCL_VM_ERROR("{} Task dependency query only supports memory-copy or "
                      "reduce tasks, "
    ...
    if (toIter == closure.dataIndexByNodeId.end()) {
        HCCL_VM_ERROR(
            ...
            MakeErrorCodeText(ErrorCode::CHECKER_RUNTIME_ERROR), fromNodeId,
            toNodeId, fromIter->second);
        return HCCL_E_PARA;
    }
    ...
    if (fromIndex >= closure.matrix.size() ||
        toIndex / BITS_PER_WORD >= closure.matrix[fromIndex].size()) {
        HCCL_VM_ERROR(
            ...
            MakeErrorCodeText(ErrorCode::MEMCONFLICT_DAG_INVALID), fromNodeId,
            toNodeId, fromIndex, toIndex, closure.matrix.size(),
            fromIndex < closure.matrix.size() ? closure.matrix[fromIndex].size()
                                              : 0U);
        return HCCL_E_INTERNAL;
    }
...
HcclResult CheckDataMoveTaskReachability(const TaskNode *start,
                                         DataTaskReachabilityStats *stats) {
    ReachabilityClosure closure;
    std::vector<NodeId> topoOrder;
    ...
    for (size_t i = 0; i < dataTaskNodes.size(); ++i) {
        ...
        for (size_t j = i + 1; j < dataTaskNodes.size(); ++j) {
            ...
            ret = IsReachable(closure, lhsNode->GetNodeId(),
                              rhsNode->GetNodeId(), lhsReachRhs);
            if (ret != HCCL_SUCCESS) {
                return ret;
            }
            bool rhsReachLhs = false;
            ret = IsReachable(closure, rhsNode->GetNodeId(),
                              lhsNode->GetNodeId(), rhsReachLhs);
            if (ret != HCCL_SUCCESS) {
                return ret;
            }
    ...
    HCCL_VM_INFO(
        ...
        "parallelTaskPairs={}",
        localStats.nodeCount, localStats.edgeCount,
        localStats.dataTaskNodeCount, localStats.dataTaskPairCount,
