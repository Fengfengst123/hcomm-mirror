...
class TaskGraphGeneratorV3 {
  ...
  private:
    ...
    HcclResult GenGraph4Rank(DeviceId deviceId,
                             const RankNodeQueues &rankTaskQueues);
    HcclResult AddLocalNotifyEdges(DeviceId deviceId,
                                   const RankNodeQueues &rankTaskQueues);
    HcclResult AddInterRankNotifyEdges();
    HcclResult AddModelExecEdges();
    HcclResult ExpandAivSubGraphs();
    HcclResult ExpandCcuSubGraphs();
    HcclResult PushNextNode(const RankNodeQueues &rankTaskQueues,
                            NodeId currNodeId,
                            std::vector<NodeId> &nodeQue) const;
    bool IsExecutable(NodeId nodeId,
                      const std::vector<uint8_t> &execFlags) const;
    HcclResult ExecuteNode(NodeId nodeId, std::vector<NodeId> &graphNodeQue,
