...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: A6 ccu instruction transform
 * to checker task Author: caiyifan、zhanhaifeng Create: 2025-06-19
 */
...
constexpr uint16_t MAX_LOADX_STOREX_ID_NUM =
    16383; // loadx/storeX 指令S使用，最大寄存器ID个数
...
#define CHK_GET_XN_V3(curCcuTask, queId, xnId, xnValue)                        \
    do {                                                                       \
        ...
        const uint32_t _checkerInstrId =                                       \
            (curCcuTask)->microCodePosInQue[(queId)] +                         \
            (curCcuTask)->startInstrIdInQue[(queId)];                          \
        if (AllRankParamRecorder::Global()->GetXn(                             \
                (curCcuTask)->GetDeviceId(), _checkerDieId, (xnId),            \
                (xnValue)) != HCCL_SUCCESS) {                                  \
            HCCL_VM_ERROR(                                                     \
                "{} Failed to read XN register before it was initialized, "    \
                "commId={}, deviceId={}, dieId={}, instrId={}, xnId={}",       \
                MakeErrorCodeText(ErrorCode::GRAPH_REGISTER_UNINITIALIZED)     \
                    .c_str(),                                                  \
                (curCcuTask)->GetCommId(),                                     \
                static_cast<uint32_t>((curCcuTask)->GetDeviceId()),            \
                _checkerDieId, _checkerInstrId, static_cast<uint16_t>(xnId));  \
            return HCCL_E_PARA;                                                \
        }                                                                      \
    } while (0)
...
static HcclResult
GetCheckerReduceInfoFromCcuOpCode(uint16_t reduceOpCode,
                                  HcclReduceOp &checkerReduceOp,
                                  uint16_t &ccuReduceType) {
    const auto opIt = g_ReduceOp2CheckerReduceOp_ccu.find(reduceOpCode);
    const auto typeIt = ccuReduceTypeMap.find(reduceOpCode);
    if (opIt == g_ReduceOp2CheckerReduceOp_ccu.end() ||
        typeIt == ccuReduceTypeMap.end()) {
        HCCL_VM_ERROR("{} Unsupported CCU reduce opcode, reduceOpCode={}",
                      MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
                      reduceOpCode);
        return HCCL_E_NOT_SUPPORT;
    }
    checkerReduceOp = opIt->second;
    ccuReduceType = typeIt->second;
    return HCCL_SUCCESS;
}
...
struct LoopGroupParamA6 {
    ...
    uint32_t curLoopIdx = 0;   // 表示当前处理第几个loop
    uint32_t curExpandCnt = 0; // 该loop被第几次展开
    uint32_t curLoopCnt = 0;   // 表示当前Loop第几次循环
};
...
uint16_t UpdateCKEId(uint16_t CKEId, LoopGroupParamA6 *loopGroupParam) {
    ...
    const uint32_t expandOffset =
        static_cast<uint32_t>(loopGroupParam->loopGroupXn.expandOffset);
    const uint32_t ckOffset =
        static_cast<uint32_t>(loopGroupParam->loopGroupXm.ckOffset);
    return UpdateId(CKEId, loopGroupParam->curLoopIdx, expandOffset, ckOffset,
...
uint16_t UpdateXnId(uint16_t xnId, LoopGroupParamA6 *loopGroupParam) {
    ...
    const uint32_t expandOffset =
        static_cast<uint32_t>(loopGroupParam->loopGroupXn.expandOffset);
    const uint32_t xnIdOffset =
        static_cast<uint32_t>(loopGroupParam->loopGroupXp.xnIdOffset);
    return UpdateId(xnId, loopGroupParam->curLoopIdx, expandOffset, xnIdOffset,
                    loopGroupParam->curExpandCnt);
}
...
uint16_t UpdateMSId(uint16_t MSId, LoopGroupParamA6 *loopGroupParam) {
    ...
    const uint32_t expandOffset =
        static_cast<uint32_t>(loopGroupParam->loopGroupXn.expandOffset);
    const uint32_t msOffset =
        static_cast<uint32_t>(loopGroupParam->loopGroupXm.msOffset);
    return UpdateId(MSId, loopGroupParam->curLoopIdx, expandOffset, msOffset,
                    loopGroupParam->curExpandCnt);
}
...
uint64_t UpdateAddress(uint64_t addr, LoopGroupParamA6 *loopGroupParam,
    ...
    const uint64_t gsaOffset =
        static_cast<uint64_t>(loopGroupParam->loopGroupXm.gsaOffset);
    return addr +
           ((loopGroupParam->curExpandCnt * gsaOffset) << addrExpandCoef) +
...
bool IsExistRemoteDieInfo(DeviceId deviceId, uint32_t dieId, uint32_t channel) {
    auto rankIt = g_allRankChannelInfo.find(deviceId);
...
HcclResult TransformLoadSqeArgsToXnInstr(const CcuRep::CcuInstr *instr,
                                         CcuGraphStateV3 *curCcuTask,
                                         uint32_t queId, bool &isContinue,
                                         LoopGroupParamA6 *loopGroupParam) {
    (void)isContinue;
    ...
    CHK_RET(
        AllRankParamRecorder::Global()->SetXn(deviceId, dieId, xnId, argVal));
    uint16_t ckeId =
        UpdateCKEId(instr->v2.loadSqeArgsToX.setCKEId, loopGroupParam);
    uint16_t ckeMask = instr->v2.loadSqeArgsToX.setCKEMask;
    CHK_RET(ProcessSetMask(deviceId, dieId, curCcuTask, queId, ckeId, ckeMask));
    HCCL_VM_DEBUG("Load SqeArg[{}]({}) to Xn[{}]", sqeArgsId, argVal, xnId);
    return HCCL_SUCCESS;
}

// 将立即数 写到第n个X寄存器中
HcclResult TransformLoadImdToXInstr(const CcuRep::CcuInstr *instr,
    ...
    CHK_RET(AllRankParamRecorder::Global()->SetXn(deviceId, dieId, xnId,
                                                  immediate));
    // cke设置
...
HcclResult CheckXnValid(uint16_t xnId, uint16_t xnIdMin, uint16_t xnIdMax) {
    if (xnId < xnIdMin || xnId > xnIdMax) {
        HCCL_VM_ERROR("{} Xn register id is out of the valid range, xnId={}, "
                      "validMin={}, validMax={}",
                      MakeErrorCodeText(ErrorCode::GRAPH_OUT_OF_RANGE).c_str(),
                      xnId, xnIdMin, xnIdMax);
        return HCCL_E_PARA;
    }
    return HCCL_SUCCESS;
}

HcclResult GetXnValueAndCheck(CcuGraphStateV3 *curCcuTask, uint32_t queId,
                              uint16_t xnId, uint64_t &xnValue) {
    CHK_RET(CheckXnValid(xnId, 0, MAX_LOADX_STOREX_ID_NUM));
    CHK_GET_XN_V3(curCcuTask, queId, xnId, xnValue);
    return HCCL_SUCCESS;
}

HcclResult TransformLoadXInstr(const CcuRep::CcuInstr *instr,
                               CcuGraphStateV3 *curCcuTask, uint32_t queId,
                               bool &isContinue,
                               LoopGroupParamA6 *loopGroupParam) {
    (void)isContinue;
    ...
    if (mode == 0) {
        ...
        HCCL_VM_DEBUG("LoadX Xn[{}]({}) + immediate[{}] -> Xn[{}]({}), + "
                      "immediate[{}] -> Xn[{}]({})",
                      xsId, xsValue, immedataSo, xsTmpId, xsTmpValue,
                      immedataDo, xdId, xdValue);
    } else if (mode == 1) {
        ...
        HCCL_VM_DEBUG("LoadX Xn[{}]({}) + Xn[{}]({}) -> Xn[{}]({}), + "
                      "Xn[{}]({}) -> Xn[{}]({})",
                      xsId, xsValue, xsSoId, xsSoValue, xsTmpId, xsTmpValue,
                      xsDoId, xsDoValue, xdId, xdValue);
    } else {
        HCCL_VM_ERROR("{} This instruction mode is not supported by CheckerV3 "
                      "graph expansion, "
    ...
    CHK_RET(
        AllRankParamRecorder::Global()->SetXn(deviceId, dieId, xdId, xdValue));
    // cke设置
...
// StoreX指令
HcclResult TransformStoreXInstr(const CcuRep::CcuInstr *instr,
                                CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                bool &isContinue,
                                LoopGroupParamA6 *loopGroupParam) {
    (void)isContinue;
    ...
    if (mode == 0) {
        ...
        HCCL_VM_DEBUG("StoreX Xn[{}]({}) + immediate[{}] -> Xn[{}], Xn[{}]({}) "
                      "+ immediate[{}] -> Xn[{}]({})",
                      xdId, xdValue, immedataDo, newId, xsId, xsValue,
                      immedataSo, newId, newValue);
        ...
        HCCL_VM_DEBUG("StoreX Xn[{}]({}) + Xn[{}]({}) -> Xn[{}], Xn[{}]({}) + "
                      "Xn[{}]({}) -> Xn[{}]({})",
                      xdId, xdValue, xsDoId, xsDoValue, newId, xsId, xsValue,
                      xsSoId, xsSoValue, newId, newValue);
    } else {
        HCCL_VM_ERROR("{} This instruction mode is not supported by CheckerV3 "
                      "graph expansion, "
    ...
    CHK_RET(AllRankParamRecorder::Global()->SetXn(deviceId, dieId, newId,
                                                  newValue));
    // cke设置
    uint16_t ckeId = UpdateCKEId(instr->v2.loadStoreX.setCKEId, loopGroupParam);
    uint16_t ckeMask = instr->v2.loadStoreX.setCKEMask;
...
HcclResult TransformClearXInstr(const CcuRep::CcuInstr *instr,
                                CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                bool &isContinue,
                                LoopGroupParamA6 *loopGroupParam) {
    (void)isContinue;
    ...
    uint16_t xnId = (instr->v2.clearX.xnIdMode == 0)
                        ? xnOriId
                        : UpdateXnId(xnOriId, loopGroupParam);
    uint16_t xmOriId = instr->v2.clearX.xmId;
    uint16_t xmId = (instr->v2.clearX.xmIdMode == 0)
                        ? xmOriId
                        : UpdateXnId(xmOriId, loopGroupParam);
    ...
    for (uint16_t i = xnId; i <= xmId; i++) {
        CHK_RET(AllRankParamRecorder::Global()->SetXn(deviceId, dieId, i, 0));
    }
    // cke设置
...
// Nop指令，空指令，用于排指令流水
HcclResult TransformNopInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue,
                             LoopGroupParamA6 *loopGroupParam) {
    (void)isContinue;
...
HcclResult TransformLoadInstr(const CcuRep::CcuInstr *instr,
                              CcuGraphStateV3 *curCcuTask, uint32_t queId,
                              bool &isContinue,
                              LoopGroupParamA6 *loopGroupParam) {
    (void)isContinue;
    ...
    if ((xsStartAddr & 0x7) != 0) {
        HCCL_VM_ERROR(
            ...
            MakeErrorCodeText(ErrorCode::GRAPH_ADDRESS_INVALID).c_str(),
            xsStartAddr);
        return HCCL_E_PARA;
    }
    ...
    if (AllRankParamRecorder::Global()->GetHBM(deviceId, dieId, xsStartAddr,
                                               entry) != HCCL_SUCCESS) {
        const uint32_t instrId = curCcuTask->microCodePosInQue[queId] +
                                 curCcuTask->startInstrIdInQue[queId];
        HCCL_VM_ERROR(
            "{} Failed to read HBM content before it was initialized, "
            "deviceId={}, dieId={}, "
            "instrId={}, hbmAddr={}",
            MakeErrorCodeText(ErrorCode::GRAPH_REGISTER_UNINITIALIZED).c_str(),
            static_cast<uint32_t>(deviceId), dieId, instrId, xsStartAddr);
        return HCCL_E_PARA;
    }
    if (length / 8 > entry.size()) {
        HCCL_VM_ERROR(
            "{} Load length exceeds the number of 64-bit entries available in "
            "HBM, "
    ...
    for (size_t i = 0; i < length / 8; i++) {
        CHK_RET(AllRankParamRecorder::Global()->SetXn(deviceId, dieId,
                                                      xdValue++, entry[i]));
    }
    // cke设置
...
HcclResult TransformStoreInstr(const CcuRep::CcuInstr *instr,
                               CcuGraphStateV3 *curCcuTask, uint32_t queId,
                               bool &isContinue,
                               LoopGroupParamA6 *loopGroupParam) {
    (void)isContinue;
    ...
    if (srcType != 0) {
        // 目前只支持Xn寄存器
        HCCL_VM_ERROR("{} Store only supports reading from Xn registers, "
                      "actualSrcType={}",
                      MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
                      srcType);
        return HCCL_E_PARA;
    ...
    if ((entry.size() % 8) != 0) {
        const uint32_t instrId = curCcuTask->microCodePosInQue[queId] +
                                 curCcuTask->startInstrIdInQue[queId];
        HCCL_VM_ERROR(
            "{} Store data length must be a multiple of 64 bytes before "
            "writing HBM, "
            "deviceId={}, dieId={}, queueId={}, instrId={}, hbmAddr={}, "
            "dataEntryCount={}, dataLengthBytes={}",
            MakeErrorCodeText(ErrorCode::GRAPH_ADDRESS_INVALID).c_str(),
            static_cast<uint32_t>(deviceId), dieId, queId, instrId, xdValue,
            entry.size(), entry.size() * sizeof(uint64_t));
        return HCCL_E_PARA;
    }
    CHK_RET(AllRankParamRecorder::Global()->SetHBM(deviceId, dieId, xdValue,
                                                   entry));
...
// Add指令，实现算数“+”
HcclResult TransformAddInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue,
                             LoopGroupParamA6 *loopGroupParam) {
    ...
    if (parMode == 0) {
        ...
        HCCL_VM_DEBUG("Xn[{}]({}) + immediate[{}] -> Xn[{}]({})", xnId, xnValue,
        ...
        HCCL_VM_DEBUG("Xn[{}]({}) + Xn[{}]({}) -> Xn[{}]({})", xnId, xnValue,
                      xmId, xmValue, xdId, xdValue);
    }
...
// Sub指令，实现算数“-”
HcclResult TransformSubInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue,
                             LoopGroupParamA6 *loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
...
// Mul指令，实现算数“*”
HcclResult TransformMulInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue,
                             LoopGroupParamA6 *loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    ...
    if (parMode == 0) {
        ...
        HCCL_VM_DEBUG("low32(Xn[{}]({})) * immediate[{}] -> Xn[{}]({})", xnId,
                      xnValue & 0xFFFFFFFF, instr->v2.operate.xmId, xdId,
                      xdValue);
    } else {
        uint16_t xmId = GetXnId(instr->v2.operate.xmId, loopGroupParam);
        uint64_t xmValue = 0;
        ...
        HCCL_VM_DEBUG("low32(Xn[{}]({})) * low32(Xn[{}]({})) -> Xn[{}]({})",
                      xnId, xnValue & 0xFFFFFFFF, xmId, xmValue & 0xFFFFFFFF,
                      xdId, xdValue);
    }
...
// And指令，实现逻辑“&”
HcclResult TransformANDInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue,
                             LoopGroupParamA6 *loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    ...
    HCCL_VM_DEBUG("Xn[{}]({}) & Xn[{}]({}) -> Xn[{}]({})", xnId, xnValue, xmId,
                  xmValue, xdId, xdValue);
...
// Or指令，实现逻辑“|”
HcclResult TransformORInstr(const CcuRep::CcuInstr *instr,
                            CcuGraphStateV3 *curCcuTask, uint32_t queId,
                            bool &isContinue,
                            LoopGroupParamA6 *loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
...
// Not指令，实现逻辑“~”
HcclResult TransformNOTInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue,
                             LoopGroupParamA6 *loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
...
// Xor指令，实现逻辑“^”
HcclResult TransformXORInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue,
                             LoopGroupParamA6 *loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
...
// SHL指令，实现逻辑“<<”
HcclResult TransformSHLInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue,
                             LoopGroupParamA6 *loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    ...
    case 0: // 逻辑移位
        xdValue = xnValue << xmValue;
        HCCL_VM_DEBUG("Xn[{}]({}) << Xn[{}]({}) [logical] -> Xn[{}]({})", xnId,
                      xnValue, xmId, xmValue, xdId, xdValue);
        break;
    case 1: // 算术移位
        xdValue = static_cast<int64_t>(xnValue) << xmValue;
        HCCL_VM_DEBUG("Xn[{}]({}) << Xn[{}]({}) [arithmetic] -> Xn[{}]({})",
                      xnId, xnValue, xmId, xmValue, xdId, xdValue);
        break;
    case 2: // 循环移位
        xdValue = (xnValue << (xmValue & 0x3F)) |
                  (xnValue >> (64 - (xmValue & 0x3F)));
        HCCL_VM_DEBUG("Xn[{}]({}) << Xn[{}]({}) [rotate] -> Xn[{}]({})", xnId,
                      xnValue, xmId, xmValue, xdId, xdValue);
        break;
    default:
        HCCL_VM_ERROR("{} Shift-left type is not supported, shiftType={}",
                      MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
                      shiftType);
        return HCCL_E_INTERNAL;
    }
...
// SHR指令，实现逻辑“>>”
HcclResult TransformSHRInstr(const CcuRep::CcuInstr *instr,
                             CcuGraphStateV3 *curCcuTask, uint32_t queId,
                             bool &isContinue,
                             LoopGroupParamA6 *loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    ...
    case 2: // 循环移位
        ...
        break;
    default:
        HCCL_VM_ERROR("{} Shift-right type is not supported, shiftType={}",
                      MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
                      shiftType);
        return HCCL_E_INTERNAL;
    }
...
HcclResult TransformPopcntInstr(const CcuRep::CcuInstr *instr,
                                CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                bool &isContinue,
                                LoopGroupParamA6 *loopGroupParam) {
    DeviceId deviceId = curCcuTask->GetDeviceId();
...
// 处理Loop指令
HcclResult ProcessLoopIns(const CcuRep::CcuInstr *instr,
                          CcuGraphStateV3 *curCcuTask, uint32_t queId,
                          bool &isContinue, LoopGroupParamA6 *loopGroupParam,
                          uint32_t loopGroupIdx);

HcclResult TransformLoopInstr(const CcuRep::CcuInstr *instr,
                              CcuGraphStateV3 *curCcuTask, uint32_t queId,
                              bool &isContinue,
                              LoopGroupParamA6 *loopGroupParam) {
    ...
    HCCL_VM_ERROR("{} A loop instruction cannot be executed by itself; it must "
                  "be triggered from a "
...
static bool ConsumeProducedMask(std::map<uint16_t, uint16_t> &producedMasks,
                                uint16_t ckeId, uint16_t waitMask) {
    if (waitMask == 0) {
        return true;
    }
...
static void ProduceMask(std::map<uint16_t, uint16_t> &producedMasks,
                        uint16_t ckeId, uint16_t setMask) {
    if (setMask == 0) {
        return;
    }
    producedMasks[ckeId] =
        static_cast<uint16_t>(producedMasks[ckeId] | setMask);
}
...
static void SetLoopParamAtIteration(const LoopGroupParamA6 &sampleParam,
                                    uint32_t curLoopCnt,
...
static HcclResult CollectLoopCkeOpsA6(const CcuRep::CcuInstr *instr,
                                      const LoopGroupParamA6 &sampleParam,
                                      CcuLoopCkeOpV3 &waitOp,
                                      CcuLoopCkeOpV3 &setOp) {
    switch (instr->header.type) {
    case CTRL_TYPE:
        if (instr->header.code == A6_SETCKBIT_CODE) {
            waitOp = {UpdateCKEId(instr->v2.setCKE.waitCKEId,
                                  const_cast<LoopGroupParamA6 *>(&sampleParam)),
                      instr->v2.setCKE.waitCKEMask};
            return HCCL_SUCCESS;
        }
        if (instr->header.code == A6_CLEARCKE_CODE) {
            waitOp = {UpdateCKEId(instr->v2.clearCKE.waitCKEId,
                                  const_cast<LoopGroupParamA6 *>(&sampleParam)),
    ...
    case TRANS_TYPE:
        ...
        if (instr->header.code == A6_TRANSMEM_CODE) {
            ...
            setOp = {UpdateCKEId(instr->v2.transMem.setCKEId,
                                 const_cast<LoopGroupParamA6 *>(&sampleParam)),
    ...
    case REDUCE_TYPE:
        if (instr->header.code == A6_REDUCEADD_CODE ||
            instr->header.code == A6_REDUCEMAX_CODE ||
            instr->header.code == A6_REDUCEMIN_CODE) {
            setOp = {UpdateCKEId(instr->v2.reduce.setCKEId,
                                 const_cast<LoopGroupParamA6 *>(&sampleParam)),
...
// A6硬件要求一个Loop内waitCKE不能重复使用同一个CKE
// ID；在Loop收集前校验原始微码，
static HcclResult
CheckA6LoopWaitCkeIdUnique(const CcuRep::CcuInstr *loopInstr,
                           CcuGraphStateV3 *curCcuTask, uint32_t queId,
                           const LoopGroupParamA6 &loopParam) {
    CcuLoopWaitCkeChecker checker;
    hcomm::CcuRep::CcuInstrInfo &microCodeQue = curCcuTask->instrInfo[queId];
    const uint16_t startInstrId = loopInstr->v2.loop.startInstrId;
    const uint16_t endInstrId = loopInstr->v2.loop.endInstrId;
    for (uint32_t instrId = startInstrId; instrId <= endInstrId; ++instrId) {
        const CcuRep::CcuInstr *instr =
            &microCodeQue
                 .instrVec[instrId - curCcuTask->startInstrIdInQue[queId]];
        CcuLoopCkeOpV3 waitOp;
        CcuLoopCkeOpV3 setOp;
        HcclResult ret = CollectLoopCkeOpsA6(instr, loopParam, waitOp, setOp);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        // 同一个wait CKE ID在一个Loop内只能出现一次，mask不同也不能复用。
        CHK_RET(checker.Check(instrId, waitOp));
    }
    return HCCL_SUCCESS;
}

static HcclResult CollectTransMemGenericLoopInstrA6(
    const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    uint16_t instrId, const LoopGroupParamA6 &sampleParam, uint64_t loopCnt,
    ...
    curCcuTask->GetDieId(queId, dieId);
    const uint16_t xlId =
        GetXnId(op.xlId, const_cast<LoopGroupParamA6 *>(&sampleParam));
    uint64_t len = 0;
    CHK_GET_XN_V3(curCcuTask, queId, xlId, len);
    CHK_RET(ValidateLoopLen(len));

    const uint16_t xcId =
        GetXnId(op.xcId, const_cast<LoopGroupParamA6 *>(&sampleParam));
    uint64_t channelId = 0;
    CHK_GET_XN_V3(curCcuTask, queId, xcId, channelId);
    if (!IsExistRemoteDieInfo(deviceId, dieId, channelId)) {
        return HCCL_E_NOT_SUPPORT;
    }
    DeviceId rmtDeviceId =
        g_allRankChannelInfo[deviceId][dieId][channelId].dstDeviceId;
    ...
    if (reduceEn) {
        reduceInstr = EnsureCcuLoopInstr<CcuLoopReduceV3>(instrInLoop, deviceId,
                                                          dieId, instrId);
        ...
        HcclReduceOp checkerReduceOp;
        uint16_t ccuReduceType;
        CHK_RET(GetCheckerReduceInfoFromCcuOpCode(
            op.reduceOpCode, checkerReduceOp, ccuReduceType));
        DataType hcclDataType;
        CHK_RET(GetHcclDataTypeFromCCUDataType(op.reduceDataType, ccuReduceType,
                                               hcclDataType));
        ...
        transInstr = EnsureCcuLoopInstr<CcuLoopTransMemV3>(
            instrInLoop, deviceId, dieId, instrId);
    ...
    for (uint64_t iter = 0; iter < loopCnt; ++iter) {
        LoopGroupParamA6 iterParam;
        SetLoopParamAtIteration(sampleParam, static_cast<uint32_t>(iter),
                                iterParam);
        ...
        if (op.dmaOpCode == 3) {
            if (op.msIdMode == 1) {
                ...
                CHK_GET_XN_V3(curCcuTask, queId, xsId, xsValue);
                xsValue = (op.src_mode == 1)
                              ? UpdateAddress(xsValue, &iterParam)
                              : UpdateAddressWithoutStride(xsValue, &iterParam);
                CHK_RET(curCcuTask->GetSlice(xsValue, len, srcSlice));
            }
            CHK_GET_XN_V3(curCcuTask, queId, xdId, xdValue);
            xdValue = (op.dst_mode == 1)
                          ? UpdateAddress(xdValue, &iterParam)
                          : UpdateAddressWithoutStride(xdValue, &iterParam);
            CHK_RET(curCcuTask->GetSlice(xdValue, len, dstSlice));
            if (reduceInstr != nullptr) {
                reduceInstr->srcs.push_back(
                    {MakeCcuMemSlice(deviceId, srcSlice)});
                reduceInstr->dsts.push_back(
                    MakeCcuMemSlice(rmtDeviceId, dstSlice));
            } else {
                transInstr->srcs.push_back(MakeCcuMemSlice(deviceId, srcSlice));
                transInstr->dsts.push_back(
                    MakeCcuMemSlice(rmtDeviceId, dstSlice));
            }
            ...
            if (reduceInstr != nullptr) {
                reduceInstr->srcs.push_back(
                    {MakeCcuMemSlice(rmtDeviceId, srcSlice)});
                reduceInstr->dsts.push_back(
                    MakeCcuMemSlice(deviceId, dstSlice));
            } else {
                transInstr->srcs.push_back(
                    MakeCcuMemSlice(rmtDeviceId, srcSlice));
...
static HcclResult CollectTransLoopInstrA6(
    ...
    std::shared_ptr<CcuLoopInstrV3> &instrInLoop) {
    if (instr->header.code == A6_TRANSMEM_CODE) {
        return CollectTransMemGenericLoopInstrA6(instr, curCcuTask, queId,
                                                 instrId, sampleParam, loopCnt,
                                                 instrInLoop);
    }
    ...
    uint32_t dieId = INVALID_DIE_ID;
    curCcuTask->GetDieId(queId, dieId);
    auto transInstr = EnsureCcuLoopInstr<CcuLoopTransMemV3>(
        instrInLoop, deviceId, dieId, instrId);
    if (transInstr == nullptr) {
        return HCCL_E_MEMORY;
    }
    ...
    for (uint64_t iter = 0; iter < loopCnt; ++iter) {
        ...
        case A6_TRANSLOCMEMTOLOCMEM_CODE: {
            ...
            CHK_GET_XN_V3(curCcuTask, queId, xlId, len);
            CHK_RET(ValidateLoopLen(len));
            CHK_RET(curCcuTask->GetSlice(srcMemAddr, len, srcSlice));
            CHK_RET(curCcuTask->GetSlice(dstMemAddr, len, dstSlice));
            transInstr->srcs.push_back(MakeCcuMemSlice(deviceId, srcSlice));
            transInstr->dsts.push_back(MakeCcuMemSlice(deviceId, dstSlice));
            break;
        }
...
static HcclResult CollectReduceLoopInstrA6(
    ...
    uint32_t dieId = INVALID_DIE_ID;
    curCcuTask->GetDieId(queId, dieId);
    auto reduceInstr = EnsureCcuLoopInstr<CcuLoopReduceV3>(
        instrInLoop, deviceId, dieId, instrId);
    if (reduceInstr == nullptr) {
        return HCCL_E_MEMORY;
    }
    ...
    DataType hcclDataType;
    CHK_RET(GetHcclDataTypeFromCCUDataType(instr->v2.reduce.dataType,
                                           reduceType, hcclDataType));
    reduceInstr->dataType = g_DataType2CheckerDataType_aicpu[hcclDataType];
    reduceInstr->reduceOp = checkerReduceOp;

    const uint16_t lengthId =
        GetXnId(instr->v2.reduce.XnIdLength,
                const_cast<LoopGroupParamA6 *>(&sampleParam));
    uint64_t len = 0;
    ...
    for (uint64_t iter = 0; iter < loopCnt; ++iter) {
        ...
        for (uint16_t i = 1; i < instr->v2.reduce.count + 2; ++i) {
            DataSlice srcSlice;
            const uint16_t srcMSId =
                UpdateMSId(instr->v2.reduce.msId[i], &iterParam);
            CHK_RET(GenSliceFromMs(srcMSId, len, srcSlice));
            srcGroup.push_back(MakeCcuMemSlice(deviceId, srcSlice));
            reduceInstr->msIds.insert(srcMSId);
        }
        DataSlice dstSlice;
        const uint16_t dstMSId =
            UpdateMSId(instr->v2.reduce.msId[0], &iterParam);
        CHK_RET(GenSliceFromMs(dstMSId, len, dstSlice));
...
static HcclResult CollectClearCkeLoopInstrA6(
    const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    uint16_t instrId, const LoopGroupParamA6 &sampleParam,
    std::shared_ptr<CcuLoopInstrV3> &instrInLoop) {
    if (instr->header.code != A6_CLEARCKE_CODE ||
        instr->v2.clearCKE.clearMask != 0) {
        return HCCL_E_NOT_SUPPORT;
    }
    DeviceId deviceId = curCcuTask->GetDeviceId();
    uint32_t dieId = INVALID_DIE_ID;
    curCcuTask->GetDieId(queId, dieId);
    auto clearInstr = EnsureCcuLoopInstr<CcuLoopClearCkeV3>(
        instrInLoop, deviceId, dieId, instrId);
    ...
    clearInstr->clearCKEId =
        UpdateCKEId(instr->v2.clearCKE.clearCKEId,
                    const_cast<LoopGroupParamA6 *>(&sampleParam));
...
static HcclResult
CollectMergedLoopInstrsA6(CcuGraphStateV3 *curCcuTask, uint32_t queId,
                          uint16_t startInstrId, uint16_t endInstrId,
                          const LoopGroupParamA6 &sampleParam, uint64_t loopCnt,
                          CcuLoopInstrsV3 &mergedInstrs) {
    ...
    for (uint16_t insId = startInstrId; insId <= endInstrId; ++insId) {
        const CcuRep::CcuInstr *curInstr =
            &microCodeQue
                 .instrVec[insId - curCcuTask->startInstrIdInQue[queId]];
        CcuLoopCkeOpV3 waitOp;
        CcuLoopCkeOpV3 setOp;
        HcclResult ret =
            CollectLoopCkeOpsA6(curInstr, sampleParam, waitOp, setOp);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        ...
        if (curInstr->header.type == CTRL_TYPE) {
            ret = CollectClearCkeLoopInstrA6(curInstr, curCcuTask, queId, insId,
                                             sampleParam, instrInLoop);
        } else if (curInstr->header.type == TRANS_TYPE) {
            ret = CollectTransLoopInstrA6(curInstr, curCcuTask, queId, insId,
                                          sampleParam, loopCnt, instrInLoop);
        } else if (curInstr->header.type == REDUCE_TYPE) {
            ret = CollectReduceLoopInstrA6(curInstr, curCcuTask, queId, insId,
                                           sampleParam, loopCnt, instrInLoop);
        } else {
...
static HcclResult CollectLoopExpandA6(const CcuRep::CcuInstr *loopInstr,
                                      CcuGraphStateV3 *curCcuTask,
                                      uint32_t queId,
                                      const LoopGroupParamA6 &loopParam,
                                      CcuLoopInstrsV3 &mergedInstrs,
                                      uint64_t &loopCnt) {
    if (loopInstr->header.type != CTRL_TYPE ||
        loopInstr->header.code != A6_LOOP_CODE ||
    ...
    curCcuTask->GetDieId(queId, dieId);
    LoopA6 loop;
    uint16_t xpId = GetXnId(loopInstr->v2.loop.xpId,
                            const_cast<LoopGroupParamA6 *>(&loopParam));
    CHK_GET_XN_V3(curCcuTask, queId, xpId, loop.loopXp.value);
    uint16_t xnId = GetXnId(loopInstr->v2.loop.xnId,
                            const_cast<LoopGroupParamA6 *>(&loopParam));
    CHK_GET_XN_V3(curCcuTask, queId, xnId, loop.loopXn.value);
    uint16_t xmId = GetXnId(loopInstr->v2.loop.xmId,
                            const_cast<LoopGroupParamA6 *>(&loopParam));
    ...
    return CollectMergedLoopInstrsA6(
        curCcuTask, queId, loopInstr->v2.loop.startInstrId,
        loopInstr->v2.loop.endInstrId, sampleParam, loopCnt, mergedInstrs);
}

static HcclResult RecordMergedLoopParamA6(const CcuRep::CcuInstr *loopInstr,
                                          CcuGraphStateV3 *curCcuTask,
                                          uint32_t queId,
                                          LoopGroupParamA6 &loopParam) {
    DeviceId deviceId = curCcuTask->GetDeviceId();
    uint32_t dieId = INVALID_DIE_ID;
    curCcuTask->GetDieId(queId, dieId);
...
static HcclResult TryProcessMergedLoopA6(const CcuRep::CcuInstr *loopInstr,
                                         CcuGraphStateV3 *curCcuTask,
                                         uint32_t queId, bool &isContinue,
                                         LoopGroupParamA6 &loopParam,
                                         uint32_t loopGroupIdx,
                                         uint32_t expandTimes, bool &merged) {
    merged = false;
    ...
    for (uint32_t expandCnt = 0; expandCnt < expandTimes; ++expandCnt) {
        ...
        HcclResult ret = CheckA6LoopWaitCkeIdUnique(loopInstr, curCcuTask,
                                                    queId, expandParam);
        if (ret != HCCL_SUCCESS) {
        ...
        uint64_t loopCnt = 0;
        ret = CollectLoopExpandA6(loopInstr, curCcuTask, queId, expandParam,
                                  expandInstrs, loopCnt);
        if (ret != HCCL_SUCCESS) {
            if (ret == HCCL_E_NOT_SUPPORT) {
                return HCCL_SUCCESS;
            }
    ...
    if (mergedLoop.loopExpands.empty() || !mergedLoop.LoopIterationMerge() ||
        !mergedLoop.LoopExpandMerge()) {
        return HCCL_SUCCESS;
    }
    ...
    TaskNode *loopStart =
        AddLoopStartTask(queId, loopIdx, loopGroupIdx, curCcuTask);
    if (loopStart == nullptr) {
        return HCCL_E_INTERNAL;
    }
    CHK_RET(EmitMergedLoopInstrsV3(curCcuTask, queId,
                                   mergedLoop.loopExpands.front(), isContinue));
    if (!isContinue) {
        merged = true;
        return HCCL_SUCCESS;
    }
    TaskNode *loopEnd =
        AddLoopEndTask(queId, loopIdx, loopGroupIdx, curCcuTask);
    if (loopEnd == nullptr) {
        return HCCL_E_INTERNAL;
    }
    HCCL_VM_INFO("Merged LoopGroup body successfully, loopInstructionId={}, "
                 "loopCount={}, "
...
// LoopGroup指令
HcclResult TransformLoopGroupInstr(const CcuRep::CcuInstr *instr,
                                   CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                   bool &isContinue,
    ...
    // 采集 LoopGroup 处 xpId/xmId 寄存器运行时 value，供 convert-ccu-microcode
    // 规则三使用
    {
        const uint32_t pc = curCcuTask->microCodePosInQue[queId] +
                            curCcuTask->startInstrIdInQue[queId];
        AllRankParamRecorder::Global()->RecordLoopGroupRegSnapshot(
            deviceId, dieId, pc, loopGroupParam.loopGroupXp.value,
            loopGroupParam.loopGroupXm.value);
    }
    const uint32_t loopInsCnt =
        static_cast<uint32_t>(loopGroupParam.loopGroupXn.loopInsCnt);
    const uint32_t expandCnt =
        static_cast<uint32_t>(loopGroupParam.loopGroupXn.expandCnt);
    const uint32_t expandOffset =
        static_cast<uint32_t>(loopGroupParam.loopGroupXn.expandOffset);
    // 数值校验
    if (loopInsCnt > 512 || expandCnt > 511 || expandOffset > 511 ||
        expandOffset > loopInsCnt) {
        HCCL_VM_ERROR(
            "{} LoopGroup configuration is invalid, loopGroupXnValue={}, "
            "loopCount={}, "
            "expandCount={}, expandOffset={}",
            MakeErrorCodeText(ErrorCode::GRAPH_STRUCTURE_INVALID).c_str(),
            loopGroupParam.loopGroupXn.value, loopInsCnt, expandCnt,
            expandOffset);
        return HCCL_E_INTERNAL;
    }
    ...
    HCCL_VM_INFO("Parsed LoopGroup configuration, loopCount={}, "
                 "expandOffset={}, expandCount={}",
                 loopCnt, static_cast<uint64_t>(expandOffset),
                 static_cast<uint64_t>(expandCnt));

    for (uint32_t curLoopIdx = 0; curLoopIdx < loopCnt; curLoopIdx++) {
        loopGroupParam.curLoopIdx = curLoopIdx;
        // 计算当前Loop指令的位置
        uint32_t insPos = startLoopInstrId + curLoopIdx -
                          curCcuTask->startInstrIdInQue[queId];
        const uint32_t expandTimes =
            (curLoopIdx < expandOffset) ? 1U : (expandCnt + 1U);
        bool merged = false;
        CHK_RET(TryProcessMergedLoopA6(
            &microCodeQue.instrVec[insPos], curCcuTask, queId, isContinue,
        ...
        if (curLoopIdx < expandOffset) {
            // 不用进行Loop展开
            CHK_RET(ProcessLoopIns(&microCodeQue.instrVec[insPos], curCcuTask,
                                   queId, isContinue, &loopGroupParam,
                                   loopGroupIdx));
        } else {
            // 需要进行loop展开
            for (uint32_t expandIdx = 0; expandIdx <= expandCnt; expandIdx++) {
                loopGroupParam.curExpandCnt = expandIdx;
                CHK_RET(ProcessLoopIns(&microCodeQue.instrVec[insPos],
                                       curCcuTask, queId, isContinue,
                                       &loopGroupParam, loopGroupIdx));
            }
        }
    }

    HCCL_VM_INFO("Finished LoopGroup expansion, startLoopInstructionId={}, "
                 "loopGroupXnId={}, "
                 "loopGroupXmId={}",
...
// SetCKBit指令
HcclResult TransformSetCKBitInstr(const CcuRep::CcuInstr *instr,
                                  CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                  bool &isContinue,
                                  LoopGroupParamA6 *loopGroupParam) {
    ...
    uint16_t waitCKEId =
        UpdateCKEId(instr->v2.setCKE.waitCKEId, loopGroupParam);
    ...
    CHK_RET(ProcessWaitMask(deviceId, dieId, curCcuTask, queId, waitCKEId,
                            waitCKEMask, isContinue));
    if (!isContinue) {
        return HCCL_SUCCESS;
    }
    CHK_RET(ProcessSetMask(deviceId, dieId, curCcuTask, queId, setCKEId,
                           setCKEMask, true));
...
// ClearCKE指令
HcclResult TransformClearCKEInstr(const CcuRep::CcuInstr *instr,
                                  CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                  bool &isContinue,
                                  LoopGroupParamA6 *loopGroupParam) {
    uint16_t clearType = instr->v2.clearCKE.clearType;
    uint16_t clearCKEId =
        UpdateCKEId(instr->v2.clearCKE.clearCKEId, loopGroupParam);
    uint16_t clearMask = instr->v2.clearCKE.clearMask;
    uint16_t waitCKEId =
        UpdateCKEId(instr->v2.clearCKE.waitCKEId, loopGroupParam);
    ...
    if (clearMask != 0x0000) {
        HCCL_VM_ERROR(
            ...
            MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(), clearCKEId,
            clearMask);
        return HCCL_E_INTERNAL;
    }
    ...
    HCCL_VM_DEBUG(
        "Wait CKE[{}:0x{:04x}], Clear CKE[{}:0x{:04x}], clearType[{}]",
        waitCKEId, waitCKEMask, clearCKEId, clearMask, clearType);
    return HCCL_SUCCESS;
}

HcclResult TransformJumpInstr(const CcuRep::CcuInstr *instr,
                              CcuGraphStateV3 *curCcuTask, uint32_t queId,
                              bool &isContinue,
                              LoopGroupParamA6 *loopGroupParam) {
    uint16_t expectedXnId = GetXnId(instr->v2.jmp.expectedXnId, loopGroupParam);
    uint16_t conditionXnId =
        GetXnId(instr->v2.jmp.conditionXnId, loopGroupParam);
    uint16_t relTarInstrXnId =
        GetXnId(instr->v2.jmp.relTarInstrXnId, loopGroupParam);
    ...
    auto updateNextInsIdx = [jumpMode, curInstrId, relTarInstrValue, queId,
                             curCcuTask, &nextInsIdx](bool isJump) -> void {
        if (isJump) {
            if (jumpMode == 0) {
                // 相对跳转
            ...
            curCcuTask->microCodePosInQue[queId] =
                nextInsIdx - curCcuTask->startInstrIdInQue[queId];
        }
    };
    ...
    case 0: // 等于
        ...
        HCCL_VM_DEBUG(
            "Jump if Xn[{}]({}) == Xn[{}]({}), jumpMode[{}], targetXn[{}]({})",
            conditionXnId, conditionValue, expectedXnId, expectedValue,
    ...
    default: // 无条件跳转
        ...
        HCCL_VM_DEBUG("Jump always, Xn[{}]({}), jumpMode[{}], targetXn[{}]({})",
                      conditionXnId, conditionValue, jumpMode, relTarInstrXnId,
                      relTarInstrValue);
        break;
    }
    return HCCL_SUCCESS;
...
// Wait指令
HcclResult TransformWaitInstr(const CcuRep::CcuInstr *instr,
                              CcuGraphStateV3 *curCcuTask, uint32_t queId,
                              bool &isContinue,
                              LoopGroupParamA6 *loopGroupParam) {

    uint16_t expectedXnId =
        GetXnId(instr->v2.wait.expectedXnId, loopGroupParam);
    uint16_t conditionXnId =
        GetXnId(instr->v2.wait.conditionXnId, loopGroupParam);
    ...
    case 0: // 等于
        updateNextInsIdx(conditionValue == expectedValue);
        HCCL_VM_DEBUG("Wait Xn[{}]({}) == Xn[{}]({}), satisfied[{}]",
                      conditionXnId, conditionValue, expectedXnId,
...
// Fence指令
HcclResult TransformFenceInstr(const CcuRep::CcuInstr *instr,
                               CcuGraphStateV3 *curCcuTask, uint32_t queId,
                               bool &isContinue,
                               LoopGroupParamA6 *loopGroupParam) {
    HCCL_VM_ERROR(
...
HcclResult TransformTransLocMemToLocMSInstr(const CcuRep::CcuInstr *instr,
                                            CcuGraphStateV3 *curCcuTask,
                                            uint32_t queId, bool &isContinue,
                                            LoopGroupParamA6 *loopGroupParam) {
    ...
    uint16_t xstId =
        GetXnId(instr->v2.transLocMemToLocMS.xstId, loopGroupParam);
    ...
    uint16_t setCkeId =
        UpdateCKEId(instr->v2.transLocMemToLocMS.setCKEId, loopGroupParam);
    ...
    if (len == 0) {
        ...
        HCCL_VM_ERROR(
            ...
            MakeErrorCodeText(ErrorCode::GRAPH_STRUCTURE_INVALID).c_str(),
            static_cast<uint32_t>(deviceId), dieId, queId, instrId, len);
        return HCCL_E_INTERNAL;
    }
...
HcclResult TransformTransLocMSToLocMemInstr(const CcuRep::CcuInstr *instr,
                                            CcuGraphStateV3 *curCcuTask,
                                            uint32_t queId, bool &isContinue,
                                            LoopGroupParamA6 *loopGroupParam) {
    ...
    uint16_t xdtId =
        GetXnId(instr->v2.transLocMSToLocMem.xdtId, loopGroupParam);
    ...
    // allocHint和victimHint用于L2 Cache功能，暂不支持
    uint16_t setCkeId =
        UpdateCKEId(instr->v2.transLocMSToLocMem.setCKEId, loopGroupParam);
...
HcclResult TransformTransLocMSToLocMSInstr(const CcuRep::CcuInstr *instr,
                                           CcuGraphStateV3 *curCcuTask,
                                           uint32_t queId, bool &isContinue,
                                           LoopGroupParamA6 *loopGroupParam) {
    ...
    // allocHint和victimHint用于L2 Cache功能，暂不支持
    uint16_t setCkeId =
        UpdateCKEId(instr->v2.transLocMSToLocMS.setCKEId, loopGroupParam);
...
HcclResult TransformTransLocMemToLocMemInstr(const CcuRep::CcuInstr *instr,
                                             CcuGraphStateV3 *curCcuTask,
                                             uint32_t queId, bool &isContinue,
                                             LoopGroupParamA6 *loopGroupParam) {
    ...
    uint16_t xdtId =
        GetXnId(instr->v2.transLocMemToLocMem.xdtId, loopGroupParam);
    uint16_t xsId = GetXnId(instr->v2.transLocMemToLocMem.xsId, loopGroupParam);
    uint16_t xstId =
        GetXnId(instr->v2.transLocMemToLocMem.xstId, loopGroupParam);
    ...
    // allocHint和victimHint用于L2 Cache功能，暂不支持
    uint16_t setCkeId =
        UpdateCKEId(instr->v2.transLocMemToLocMem.setCKEId, loopGroupParam);
...
HcclResult TransformTransMemInstr(const CcuRep::CcuInstr *instr,
                                  CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                  bool &isContinue,
                                  LoopGroupParamA6 *loopGroupParam) {
    // 参数校验
    ...
    if (udfType != 0) {
        HCCL_VM_ERROR("{} This TransMem udfType is not supported by CheckerV3 "
                      "graph expansion, "
                      "udfType={}",
                      MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
                      udfType);
    ...
    // 需要找对端的rank信息
    if (!IsExistRemoteDieInfo(deviceId, dieId, xcValue)) {
        HCCL_VM_ERROR(
            "{} Cannot find remote rank/die mapping for this channel, "
            "instruction=TransMem, "
            "localRankId={}, localDieId={}, channelValue={}",
            MakeErrorCodeText(ErrorCode::GRAPH_RESOURCE_NOT_FOUND).c_str(),
            static_cast<uint32_t>(deviceId), dieId,
            static_cast<unsigned long long>(xcValue));
        return HCCL_E_PARA;
    }
    uint16_t srcMode = instr->v2.transMem.src_mode;
    uint16_t dstMode = instr->v2.transMem.dst_mode;
    DeviceId rmtDeviceId =
        g_allRankChannelInfo[deviceId][dieId][xcValue].dstDeviceId;
    uint32_t rmtDieId =
        g_allRankChannelInfo[deviceId][dieId][xcValue].remoteDieId;
    ...
    HcclReduceOp checkerReduceOp = HCCL_REDUCE_RESERVED;
    HcclDataType checkerDataType = HCCL_DATA_TYPE_RESERVED;
    if (reduceEn) {
        uint16_t ccuReduceType;
        CHK_RET(GetCheckerReduceInfoFromCcuOpCode(
            instr->v2.transMem.reduceOpCode, checkerReduceOp, ccuReduceType));
        DataType hcclDataType;
        CHK_RET(GetHcclDataTypeFromCCUDataType(
            instr->v2.transMem.reduceDataType, ccuReduceType, hcclDataType));
        checkerDataType = g_DataType2CheckerDataType_aicpu[hcclDataType];
    }
    uint64_t xnAddr = 0;
    uint32_t notifyOrAtomicValue = instr->v2.transMem.value;

    if (dmaOpCode == 3 || dmaOpCode == 5) {
        // 写数据 dst为对端 src为本端
        if (msMode == 1) {
            // XsId寄存器中存的就是MSId
            CHK_RET(GenSliceFromMs(
                UpdateMSId(instr->v2.transMem.xsId, loopGroupParam), len,
                srcSlice));
        } else {
            CHK_GET_XN_V3(curCcuTask, queId, xsId, xsValue);
            xsValue = (srcMode == 1)
                          ? UpdateAddress(xsValue, loopGroupParam)
                          : UpdateAddressWithoutStride(xsValue, loopGroupParam);
            CHK_RET(curCcuTask->GetSlice(xsValue, len, srcSlice));
        }
        CHK_GET_XN_V3(curCcuTask, queId, xdId, xdValue);
        xdValue = (dstMode == 1)
                      ? UpdateAddress(xdValue, loopGroupParam)
                      : UpdateAddressWithoutStride(xdValue, loopGroupParam);
        CHK_RET(curCcuTask->GetSlice(xdValue, len, dstSlice));
        if (rmtDeviceId != deviceId) {
            if (reduceEn) {
                AddWriteReduce(deviceId, rmtDeviceId, queId, curCcuTask,
                               srcSlice, dstSlice, checkerDataType,
                               checkerReduceOp);
            } else {
                AddWrite(deviceId, rmtDeviceId, queId, curCcuTask, srcSlice,
                         dstSlice);
            }
        } else {
            if (reduceEn) {
                AddLocalReduce(deviceId, queId, curCcuTask, srcSlice, dstSlice,
        ...
        // 写notify信息
        if (dmaOpCode == 5) {
            uint16_t xnId = GetXnId(instr->v2.transMem.xnId, loopGroupParam);
            CHK_GET_XN_V3(curCcuTask, queId, xnId, xnAddr);
            uint16_t xnIdRmt = 0;
            CcuComponerntType ccuType{CcuComponerntType::UNKNOWN};
            CHK_RET(AllRankParamRecorder::Global()->GetXnAndTypeIdByAddr(
                dieId, xnAddr, ccuType, xnIdRmt));
            if (ccuType == CcuComponerntType::XN_A6) {
                CHK_RET(AllRankParamRecorder::Global()->SetXn(
                    rmtDeviceId, rmtDieId, xnIdRmt,
                    static_cast<uint64_t>(notifyOrAtomicValue)));
            } else if (ccuType == CcuComponerntType::CKE_A6) {
                CHK_RET(AllRankParamRecorder::Global()->SetCKE(
                    rmtDeviceId, rmtDieId, xnIdRmt,
                    static_cast<uint64_t>(notifyOrAtomicValue)));
            } else {
                HCCL_VM_ERROR("Not support ccu type({}) for dmaOpCode(5) mode.",
                              static_cast<uint32_t>(ccuType));
                return HCCL_E_PARA;
            }
        }
    } else if (dmaOpCode == 6) {
        ...
        if (msMode == 1) {
            CHK_RET(GenSliceFromMs(
                UpdateMSId(instr->v2.transMem.xdId, loopGroupParam), len,
                dstSlice));
        } else {
            CHK_GET_XN_V3(curCcuTask, queId, xdId, xdValue);
            xdValue = (dstMode == 1)
        ...
        if (rmtDeviceId != deviceId) {
            if (reduceEn) {
                AddReadReduce(deviceId, rmtDeviceId, queId, curCcuTask,
                              srcSlice, dstSlice, checkerDataType,
                              checkerReduceOp);
            } else {
                AddRead(deviceId, rmtDeviceId, queId, curCcuTask, srcSlice,
                        dstSlice);
            }
        } else {
            if (reduceEn) {
    ...
    } else if (dmaOpCode == 0x19) {
        HCCL_VM_ERROR("Not support dmaOpCode(0x19) mode, which is compressed "
                      "write with atomic store add.");
        return HCCL_E_PARA;
    } else if (dmaOpCode == 0x1A) {
        // 真正接收的字节数在xn的高32bit ———— atomicValue 左移 32bit
        uint64_t atomicValue = static_cast<uint64_t>(notifyOrAtomicValue);
        uint16_t xnIdRmt = 0;
        uint16_t xnId = GetXnId(instr->v2.transMem.xnId, loopGroupParam);
        CHK_GET_XN_V3(curCcuTask, queId, xnId, xnAddr);
        CHK_RET(AllRankParamRecorder::Global()->GetXnIdByAddr(
            dieId, CcuComponerntType::XN_A6, xnAddr, xnIdRmt));
        uint64_t oldValue = 0;
        CHK_GET_XN_V3(curCcuTask, queId, xnIdRmt, oldValue);
        // 1. 首先判断最高位vld
        constexpr uint64_t LOW_31_MASK = 0x000000007FFFFFFFULL;
        constexpr uint64_t LOW_32_MASK = 0x00000000FFFFFFFFULL;
        constexpr uint64_t MSB = 1ULL << 63;

        uint64_t result;
        bool isNotFirstWrite = oldValue & MSB;
        if (isNotFirstWrite) {
            // 最高位为1时，低32bit不变；bit62..32 = 旧bit62..32 +
            // atomicValue低31bit（31bit内截断）
            uint64_t high31 = (oldValue >> 32) & LOW_31_MASK;
            uint64_t delta = atomicValue & LOW_31_MASK;
            result = MSB | (((high31 + delta) & LOW_31_MASK) << 32) |
                     (oldValue & LOW_32_MASK);
        } else {
            // 最高位为0时，全部bit清0，置低32bit=0；bit62..32 = 0 +
            // atomicValue低31bit，最高位为1
            result = MSB | ((atomicValue & LOW_31_MASK) << 32);
        }
        CHK_RET(AllRankParamRecorder::Global()->SetXn(rmtDeviceId, rmtDieId,
                                                      xnIdRmt, result));
        // 比较 wt count 和 wish
        // count：即除了vld位外，result的高32bit和低32bit数据是否相等
        if (isNotFirstWrite &&
            (((result >> 32) & LOW_31_MASK) == (result & LOW_32_MASK))) {
            for (auto &rtt : g_allDeviceHalfRTTInfo[deviceId][dieId]) {
                if (rtt.wishCntXnBegin <= xnIdRmt &&
                    xnIdRmt <= rtt.wishCntXnEnd) {
                    // 给对应total cnt xn的value加1
                    uint64_t totalCntXnValue = 0;
                    CHK_GET_XN_V3(curCcuTask, queId, rtt.totalCntXn,
                                  totalCntXnValue);
                    totalCntXnValue++;
                    CHK_RET(AllRankParamRecorder::Global()->SetXn(
                        rmtDeviceId, rmtDieId, rtt.totalCntXn,
                        totalCntXnValue));
                    HCCL_VM_INFO("Add total cnt xn value once ({}:{}), wish "
                                 "cnt xn id={}",
                                 rtt.totalCntXn, totalCntXnValue, xnIdRmt);
                }
            }
        }
    } else {
        HCCL_VM_ERROR("{} This TransMem dmaOpCode is not supported by "
    ...
    uint16_t setCkeId =
        UpdateCKEId(instr->v2.transMem.setCKEId, loopGroupParam);
...
HcclResult TransformSyncXnWtxInstr(const CcuRep::CcuInstr *instr,
                                   CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                   bool &isContinue,
                                   LoopGroupParamA6 *loopGroupParam) {
    DeviceId deviceId = curCcuTask->GetDeviceId();
    ...
    CcuComponerntType ccuType{CcuComponerntType::UNKNOWN};
    CHK_RET(AllRankParamRecorder::Global()->GetXnAndTypeIdByAddr(
        dieId, xdValue, ccuType, xdIdRmt));
    if (ccuType == CcuComponerntType::XN_A6) {
        CHK_RET(AllRankParamRecorder::Global()->SetXn(rmtDeviceId, rmtDieId,
                                                      xdIdRmt, xsValue));
    } else if (ccuType == CcuComponerntType::CKE_A6) {
        CHK_RET(ProcessSetMask(rmtDeviceId, rmtDieId, curCcuTask, queId,
                               xdIdRmt, xsValue));
    } else {
        HCCL_VM_ERROR("{} SyncXnWtx destination address does not map to a "
                      "supported remote target; "
    ...
    if (notifyValid == 1) {
        ...
        CHK_RET(ProcessSetMask(rmtDeviceId, rmtDieId, curCcuTask, queId,
                               cekIdRmtId, static_cast<uint64_t>(notifyValue)));
    }
    return HCCL_SUCCESS;
}

// TransformSyncAtx指令：同步指令，将本端寄存器的值以atomic操作同步到远端，用于同步ATX
HcclResult TransformSyncAtxInstr(const CcuRep::CcuInstr *instr,
                                 CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                 bool &isContinue,
                                 LoopGroupParamA6 *loopGroupParam) {
    DeviceId deviceId = curCcuTask->GetDeviceId();
    uint32_t dieId{0};
    curCcuTask->GetDieId(queId, dieId);
    uint16_t xdId = GetXnId(instr->v2.syncAtX.xdId, loopGroupParam);
    uint64_t xdAddr = 0;
    CHK_GET_XN_V3(curCcuTask, queId, xdId, xdAddr);

    uint16_t xcId = GetXnId(instr->v2.syncAtX.xcId, loopGroupParam);
    uint64_t xcChannelId = 0;
    CHK_GET_XN_V3(curCcuTask, queId, xcId, xcChannelId);
    if (!IsExistRemoteDieInfo(deviceId, dieId, xcChannelId)) {
        HCCL_VM_ERROR(
            "{} Cannot find remote rank/die mapping for this channel, "
            "instruction=SyncAtx, "
            "localRankId={}, localDieId={}, channelValue={}",
            MakeErrorCodeText(ErrorCode::GRAPH_RESOURCE_NOT_FOUND).c_str(),
            static_cast<uint32_t>(deviceId), dieId,
            static_cast<unsigned long long>(xcChannelId));
        return HCCL_E_PARA;
    }
    // 需要找对端的rank信息
    DeviceId rmtDeviceId =
        g_allRankChannelInfo[deviceId][dieId][xcChannelId].dstDeviceId;
    uint32_t rmtDieId =
        g_allRankChannelInfo[deviceId][dieId][xcChannelId].remoteDieId;

    uint16_t parMode = instr->v2.syncAtX.parMode;
    uint64_t xsValue = instr->v2.syncAtX.xsId;
    if (parMode == 1) {
        uint16_t xsId = GetXnId(instr->v2.syncAtX.xsId, loopGroupParam);
        CHK_GET_XN_V3(curCcuTask, queId, xsId, xsValue);
    }
    ...
    CcuComponerntType ccuType{CcuComponerntType::UNKNOWN};
    CHK_RET(AllRankParamRecorder::Global()->GetXnIdByAddr(
        dieId, CcuComponerntType::XN_A6, xdAddr, xdIdRmt));
    // write with atomic操作处理
    uint64_t oldValue = 0;
    CHK_RET(AllRankParamRecorder::Global()->GetXn(rmtDeviceId, rmtDieId,
                                                  xdIdRmt, oldValue));
    constexpr uint64_t LOW_MASK = 0x00000000FFFFFFFFULL;
    constexpr uint64_t HIGH_MASK = 0xFFFFFFFF00000000ULL;
    constexpr uint64_t MSB = 1ULL << 63;

    uint64_t result;
    if (oldValue & MSB) {
        // 最高位vld == 1时，保持高32bit保持不变，低32bit替换
        result = (oldValue & HIGH_MASK) | (xsValue & LOW_MASK);
    } else {
        // 最高位vld == 0时，全部bit清0，置最高位为1，低32bit替换
        result = MSB | (xsValue & LOW_MASK);
    }
    CHK_RET(AllRankParamRecorder::Global()->SetXn(rmtDeviceId, rmtDieId,
                                                  xdIdRmt, result));

    uint16_t setCkeId = UpdateCKEId(instr->v2.syncAtX.setCKEId, loopGroupParam);
...
// ReduceAdd指令
HcclResult TransformReduceAddInstr(const CcuRep::CcuInstr *instr,
                                   CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                   bool &isContinue,
                                   LoopGroupParamA6 *loopGroupParam) {
    ...
    DataType hcclDataType;
    CHK_RET(GetHcclDataTypeFromCCUDataType(dataType, CcuRep::CCU_REDUCE_SUM,
                                           hcclDataType));
    AddLocalBatchReduce(deviceId, queId, curCcuTask, srcSlices, dstSlice,
                        hcclDataType, HCCL_REDUCE_SUM);
    ...
    HCCL_VM_INFO(
        ...
        ParseMSList(instr).c_str(), count, dataType, castEn, setCKEId,
        setCKEMask);
    return HCCL_SUCCESS;
}

HcclResult TransformReduceMaxInstr(const CcuRep::CcuInstr *instr,
                                   CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                   bool &isContinue,
                                   LoopGroupParamA6 *loopGroupParam) {
    uint16_t count = instr->v2.reduce.count;
    ...
    DataType hcclDataType;
    CHK_RET(GetHcclDataTypeFromCCUDataType(dataType, CcuRep::CCU_REDUCE_MAX,
                                           hcclDataType));
    AddLocalBatchReduce(deviceId, queId, curCcuTask, srcSlices, dstSlice,
                        hcclDataType, HCCL_REDUCE_MAX);
...
// ReduceMin指令
HcclResult TransformReduceMinInstr(const CcuRep::CcuInstr *instr,
                                   CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                   bool &isContinue,
                                   LoopGroupParamA6 *loopGroupParam) {
    uint16_t count = instr->v2.reduce.count;
    ...
    DataType hcclDataType;
    CHK_RET(GetHcclDataTypeFromCCUDataType(dataType, CcuRep::CCU_REDUCE_MIN,
                                           hcclDataType));
    AddLocalBatchReduce(deviceId, queId, curCcuTask, srcSlices, dstSlice,
                        hcclDataType, HCCL_REDUCE_MIN);
...
HcclResult TransformLoadSqeArgsToXnInstr(const CcuRep::CcuInstr *instr,
    ...
    return TransformLoadSqeArgsToXnInstr(instr, curCcuTask, queId, isContinue,
                                         loopGroupParam);
}
...
HcclResult TransformLoadImdToXInstr(const CcuRep::CcuInstr *instr,
    ...
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformLoadImdToXInstr(instr, curCcuTask, queId, isContinue,
                                    loopGroupParam);
}
...
HcclResult TransformLoadXInstr(const CcuRep::CcuInstr *instr,
                               ...
                               bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformLoadXInstr(instr, curCcuTask, queId, isContinue,
                               loopGroupParam);
}
...
HcclResult TransformStoreXInstr(const CcuRep::CcuInstr *instr,
                                ...
                                bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformStoreXInstr(instr, curCcuTask, queId, isContinue,
                                loopGroupParam);
}
...
HcclResult TransformClearXInstr(const CcuRep::CcuInstr *instr,
                                ...
                                bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformClearXInstr(instr, curCcuTask, queId, isContinue,
                                loopGroupParam);
}
...
HcclResult TransformNopInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformNopInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformLoadInstr(const CcuRep::CcuInstr *instr,
                              ...
                              bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformLoadInstr(instr, curCcuTask, queId, isContinue,
                              loopGroupParam);
}
...
HcclResult TransformStoreInstr(const CcuRep::CcuInstr *instr,
                               ...
                               bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformStoreInstr(instr, curCcuTask, queId, isContinue,
                               loopGroupParam);
}
...
HcclResult TransformAddInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformAddInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformSubInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformSubInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformMulInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformMulInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformANDInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformANDInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformORInstr(const CcuRep::CcuInstr *instr,
                            ...
                            bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformORInstr(instr, curCcuTask, queId, isContinue,
                            loopGroupParam);
}
...
HcclResult TransformNOTInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformNOTInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformXORInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformXORInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformSHLInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformSHLInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformSHRInstr(const CcuRep::CcuInstr *instr,
                             ...
                             bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformSHRInstr(instr, curCcuTask, queId, isContinue,
                             loopGroupParam);
}
...
HcclResult TransformPopcntInstr(const CcuRep::CcuInstr *instr,
                                ...
                                bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformPopcntInstr(instr, curCcuTask, queId, isContinue,
                                loopGroupParam);
}
...
HcclResult TransformLoopInstr(const CcuRep::CcuInstr *instr,
                              ...
                              bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformLoopInstr(instr, curCcuTask, queId, isContinue,
                              loopGroupParam);
}
...
HcclResult TransformLoopGroupInstr(const CcuRep::CcuInstr *instr,
                                   ...
                                   bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformLoopGroupInstr(instr, curCcuTask, queId, isContinue,
                                   loopGroupParam);
}
...
HcclResult TransformSetCKBitInstr(const CcuRep::CcuInstr *instr,
                                  ...
                                  bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformSetCKBitInstr(instr, curCcuTask, queId, isContinue,
                                  loopGroupParam);
}
...
HcclResult TransformClearCKEInstr(const CcuRep::CcuInstr *instr,
                                  ...
                                  bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformClearCKEInstr(instr, curCcuTask, queId, isContinue,
                                  loopGroupParam);
}
...
HcclResult TransformJumpInstr(const CcuRep::CcuInstr *instr,
                              ...
                              bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformJumpInstr(instr, curCcuTask, queId, isContinue,
                              loopGroupParam);
}
...
HcclResult TransformWaitInstr(const CcuRep::CcuInstr *instr,
                              ...
                              bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformWaitInstr(instr, curCcuTask, queId, isContinue,
                              loopGroupParam);
}
...
HcclResult TransformFenceInstr(const CcuRep::CcuInstr *instr,
                               ...
                               bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformFenceInstr(instr, curCcuTask, queId, isContinue,
                               loopGroupParam);
}
...
HcclResult TransformTransLocMemToLocMSInstr(const CcuRep::CcuInstr *instr,
                                            ...
                                            void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformTransLocMemToLocMSInstr(instr, curCcuTask, queId,
                                            isContinue, loopGroupParam);
}
...
HcclResult TransformTransLocMSToLocMemInstr(const CcuRep::CcuInstr *instr,
                                            ...
                                            void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformTransLocMSToLocMemInstr(instr, curCcuTask, queId,
                                            isContinue, loopGroupParam);
}
...
HcclResult TransformTransLocMSToLocMSInstr(const CcuRep::CcuInstr *instr,
                                           ...
                                           void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformTransLocMSToLocMSInstr(instr, curCcuTask, queId, isContinue,
                                           loopGroupParam);
}
...
HcclResult TransformTransLocMemToLocMemInstr(const CcuRep::CcuInstr *instr,
                                             ...
                                             void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformTransLocMemToLocMemInstr(instr, curCcuTask, queId,
                                             isContinue, loopGroupParam);
}
...
HcclResult TransformTransMemInstr(const CcuRep::CcuInstr *instr,
                                  ...
                                  bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformTransMemInstr(instr, curCcuTask, queId, isContinue,
                                  loopGroupParam);
}
...
HcclResult TransformSyncXnWtxInstr(const CcuRep::CcuInstr *instr,
                                   ...
                                   bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformSyncXnWtxInstr(instr, curCcuTask, queId, isContinue,
                                   loopGroupParam);
}
...
HcclResult TransformSyncAtxInstr(const CcuRep::CcuInstr *instr,
                                 ...
                                 bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformSyncAtxInstr(instr, curCcuTask, queId, isContinue,
                                 loopGroupParam);
}
...
HcclResult TransformReduceAddInstr(const CcuRep::CcuInstr *instr,
                                   ...
                                   bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformReduceAddInstr(instr, curCcuTask, queId, isContinue,
                                   loopGroupParam);
}
...
HcclResult TransformReduceMaxInstr(const CcuRep::CcuInstr *instr,
                                   ...
                                   bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformReduceMaxInstr(instr, curCcuTask, queId, isContinue,
                                   loopGroupParam);
}
...
HcclResult TransformReduceMinInstr(const CcuRep::CcuInstr *instr,
                                   ...
                                   bool &isContinue, void *loopParam) {
    auto *loopGroupParam = static_cast<LoopGroupParamA6 *>(loopParam);
    return TransformReduceMinInstr(instr, curCcuTask, queId, isContinue,
                                   loopGroupParam);
}

std::unordered_map<uint16_t, TransformInstrFunc> transformA6InstrSqeMap{
    {CcuRep::InstrHeader(LOAD_TYPE, A6_LOADSQEARGSTOXN_CODE).header,
     &TransformLoadSqeArgsToXnInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, A6_LOADIMDTOX_CODE).header,
     &TransformLoadImdToXInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, A6_LOADX_CODE).header,
     &TransformLoadXInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, A6_STOREX_CODE).header,
     &TransformStoreXInstr},
    {CcuRep::InstrHeader(LOAD_TYPE, A6_CLEARX_CODE).header,
     &TransformClearXInstr},
    ...
    {CcuRep::InstrHeader(LOAD_TYPE, A6_STORE_CODE).header,
     &TransformStoreInstr},
    ...
    {CcuRep::InstrHeader(LOAD_TYPE, A6_POPCNT_CODE).header,
     &TransformPopcntInstr},
    {CcuRep::InstrHeader(CTRL_TYPE, A6_LOOP_CODE).header, &TransformLoopInstr},
    {CcuRep::InstrHeader(CTRL_TYPE, A6_LOOPGROUP_CODE).header,
     &TransformLoopGroupInstr},
    {CcuRep::InstrHeader(CTRL_TYPE, A6_SETCKBIT_CODE).header,
     &TransformSetCKBitInstr},
    {CcuRep::InstrHeader(CTRL_TYPE, A6_CLEARCKE_CODE).header,
     &TransformClearCKEInstr},
    ...
    {CcuRep::InstrHeader(CTRL_TYPE, A6_FENCE_CODE).header,
     &TransformFenceInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, A6_TRANSLOCMEMTOLOCMS_CODE).header,
     &TransformTransLocMemToLocMSInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, A6_TRANSLOCMSTOLOCMEM_CODE).header,
     &TransformTransLocMSToLocMemInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, A6_TRANSLOCMSTOLOCMS_CODE).header,
     &TransformTransLocMSToLocMSInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, A6_TRANSLOCMEMTOLOCMEM_CODE).header,
     &TransformTransLocMemToLocMemInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, A6_TRANSMEM_CODE).header,
     &TransformTransMemInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, A6_SYNCXNWTX_CODE).header,
     &TransformSyncXnWtxInstr},
    {CcuRep::InstrHeader(TRANS_TYPE, A6_SYNCATX_CODE).header,
     &TransformSyncAtxInstr},
    {CcuRep::InstrHeader(REDUCE_TYPE, A6_REDUCEADD_CODE).header,
     &TransformReduceAddInstr},
    {CcuRep::InstrHeader(REDUCE_TYPE, A6_REDUCEMAX_CODE).header,
     &TransformReduceMaxInstr},
    {CcuRep::InstrHeader(REDUCE_TYPE, A6_REDUCEMIN_CODE).header,
     &TransformReduceMinInstr}};
...
HcclResult ProcessLoopIns(const CcuRep::CcuInstr *instr,
    ...
    if (mode == 1) {
        // Xp中存储的是Loop Context
        // ID，当mode为1时，在loop与自身预期值checkEntry相等时才进行展开。
        // 否则，直接跳过该loop。
    ...
    const uint32_t loopIterCnt = static_cast<uint32_t>(loop.loopXm.loopIterCnt);

    auto loopIdx = curCcuTask->loopIdx[loopGroupIdx]++;
    auto loopStart =
        AddLoopStartTask(queId, loopIdx, loopGroupIdx, curCcuTask, startInstrId,
                         endInstrId, static_cast<uint64_t>(loopIterCnt),
                         1ULL); // loop前新增loopStart标记节点

    HCCL_VM_INFO("Entered loop execution, loopCount={}",
                 static_cast<uint64_t>(loopIterCnt));
    hcomm::CcuRep::CcuInstrInfo &microCodeQue = curCcuTask->instrInfo[queId];
    uint32_t &pos = curCcuTask->microCodePosInQue[queId];
    const uint32_t savedPos = pos;
    for (uint32_t curLoopCnt = 0; curLoopCnt < loopIterCnt; curLoopCnt++) {
        loopGroupParam->curLoopCnt = curLoopCnt;
        HCCL_VM_INFO(
            "Running loop iteration, loopCount={}, currentIteration={}",
            static_cast<uint64_t>(loopIterCnt), curLoopCnt);
        for (uint16_t insId = startInstrId; insId <= endInstrId; insId++) {
            pos = insId - curCcuTask->startInstrIdInQue[queId];
            // 获取当前要处理的指令
            const CcuRep::CcuInstr *instrVec = &microCodeQue.instrVec[pos];
            HCCL_VM_DEBUG("Expanding loop instruction, deviceId={}, "
                          "queueId={}, instructionId={}",
                          curCcuTask->GetDeviceId(), queId, insId);
            if (transformA6InstrSqeMap.count(instrVec->header.header) == 0) {
                HCCL_VM_ERROR(
                    "{} This A6 CCU instruction type is not supported during "
                    "loop "
                    "expansion, deviceId={}, queueId={}, instructionId={}, "
                    "instructionHeader=0x{:04x}",
                    MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
                    curCcuTask->GetDeviceId(), queId, insId,
                    instrVec->header.header);
                pos = savedPos;
                return HCCL_E_INTERNAL;
            }
...
HcclResult InstructMapA6::Transform(const CcuRep::CcuInstr *instr,
                                    CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                    bool &isContinue, void *loopParam) {
    auto it = transformInstrSqeMap.find(instr->header.header);
    if (it == transformInstrSqeMap.end()) {
        HCCL_VM_ERROR(
            "{} This A6 CCU instruction type is not supported by CheckerV3 "
            "graph "
            "expansion, deviceId={}, queueId={}, instructionHeader=0x{:04x}",
            MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(),
            curCcuTask == nullptr ? std::string("null")
                                  : std::to_string(curCcuTask->GetDeviceId()),
            queId, instr->header.header);
        return HCCL_E_INTERNAL;
    }
