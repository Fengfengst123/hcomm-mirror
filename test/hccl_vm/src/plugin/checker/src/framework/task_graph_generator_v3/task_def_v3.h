...
constexpr OperatorId INVALID_OPERATOR_ID =
    std::numeric_limits<OperatorId>::max();
constexpr size_t MAX_NODE_COUNT =
    static_cast<size_t>(std::numeric_limits<NodeId>::max());
...
constexpr uint64_t INVALID_NOTIFY_ID = std::numeric_limits<uint64_t>::max();
...
enum class MemType : uint8_t {
    ...
    AIV_COMM,
    MS_CCU,
    // 冲突检查内部的物理内存桶标记：INPUT/OUTPUT/CCL
    // 三种逻辑视图按绝对地址统一归桶。 仅用于冲突检查的 MemoryKey，不会写入
    // MemSlice.memType。
    PHYSICAL_GLOBAL,
    INVALID = UINT8_MAX,
};
...
inline const char *DescribeMemType(MemType memType) {
    ...
    case MemType::MS_CCU:
        return "MS_CCU";
    case MemType::PHYSICAL_GLOBAL:
        return "PHYSICAL_GLOBAL";
    default:
        return "invalid";
    }
}
...
enum class TaskType : uint8_t {
    ...
    SYNC_STREAM,
    MODEL_EXEC_START,
    MODEL_EXEC_START_SUB,
    MODEL_EXEC_END_SUB,
    MODEL_EXEC_END,
    INVALID = UINT8_MAX
};

enum class ProtocolType : uint8_t { SDMA = 0, RDMA, CCU, INVALID = UINT8_MAX };
...
struct MemSlice {
    ...
    // 绝对地址身份：rawAddr 非零视为具备绝对地址。
    inline bool HasAbsoluteAddress() const { return rawAddr != 0; }
    inline uint64_t AbsoluteBegin() const { return rawAddr; }
    inline uint64_t AbsoluteEnd() const { return rawAddr + len; }

    // CCU MS 使用独立槽位地址空间，生产端仅提供 offset，0 号槽位合法。
    // 其余切片必须携带绝对地址，不能用逻辑偏移掩盖全局地址缺失。
    inline bool HasMemoryAddress() const {
        return memType == MemType::MS_CCU || HasAbsoluteAddress();
    }
    inline uint64_t MemoryBegin() const {
        return memType == MemType::MS_CCU ? offset : rawAddr;
    }
    inline uint64_t MemoryEnd() const { return MemoryBegin() + len; }
};

// 全局物理内存（INPUT/OUTPUT/CCL）：同一设备上三种逻辑视图可能互为别名（inplace），
// 冲突/别名判定必须按绝对地址归桶；MS_CCU/AIV_UB/AIV_COMM
// 属于独立地址空间，维持逻辑身份。
inline bool IsGlobalPhysicalMemoryType(MemType memType) {
    return memType == MemType::INPUT || memType == MemType::OUTPUT ||
           memType == MemType::CCL;
}

// 两个切片的物理区间是否完全相同（inplace src==dst 的判定依据）。
inline bool ArePhysicalRangesEqual(const MemSlice &lhs, const MemSlice &rhs) {
    return lhs.HasAbsoluteAddress() && rhs.HasAbsoluteAddress() &&
           lhs.deviceId == rhs.deviceId &&
           lhs.AbsoluteBegin() == rhs.AbsoluteBegin() && lhs.len == rhs.len;
}

// 两个切片的物理区间是否相交（任一方无物理身份时返回
// false；全局切片缺失地址由边界校验报错）。 前提：rawAddr + len
// 无溢出；该前提由切片边界校验（CheckSingleSlice）在比较前先行保证。
inline bool DoPhysicalRangesOverlap(const MemSlice &lhs, const MemSlice &rhs) {
    return lhs.HasAbsoluteAddress() && rhs.HasAbsoluteAddress() &&
           lhs.deviceId == rhs.deviceId &&
           lhs.AbsoluteBegin() < rhs.AbsoluteEnd() &&
           rhs.AbsoluteBegin() < lhs.AbsoluteEnd();
}
...
struct AicpuNotify {
    ...
    uint64_t notifyId{INVALID_NOTIFY_ID};
};
...
class TaskNode {
  public:
    ...
    TaskNode(TaskType type, const TaskPosition &position)
        : type_(type), loc_(position) {}
    ...
    void SetCcuTrace(CcuTraceInfo trace) {
        if (trace.position.rankId == INVALID_RANK_ID &&
            trace.taskLoc.rankId != INVALID_RANK_ID) {
            trace.position = trace.taskLoc;
        }
        if (trace.taskLoc.rankId == INVALID_RANK_ID &&
            trace.position.rankId != INVALID_RANK_ID) {
            trace.taskLoc = trace.position;
        }
        if (trace.position.queueId == INVALID_QUEUE_ID &&
            trace.queueId != INVALID_QUEUE_ID) {
...
class TaskTransMem : public TaskNode {
  public:
    TaskTransMem(const MemSlice &src, const MemSlice &dst,
                 ProtocolType protocol = ProtocolType::INVALID)
        : TaskNode(TaskType::TRANS_MEM), src_(src), dst_(dst),
          protocol_(protocol) {}
...
class TaskBatchTransMem : public TaskNode {
  public:
    ...
    void AddMergedSrcMemSlice(const MemSlice &slice) {
        mergedSrcs_.push_back(slice);
    }
    void AddMergedDstMemSlice(const MemSlice &slice) {
        mergedDsts_.push_back(slice);
    }
    void SetSrcMemSlices(const std::vector<MemSlice> &srcs) { srcs_ = srcs; }
    void SetSrcMemSlices(std::vector<MemSlice> &&srcs) {
        srcs_ = std::move(srcs);
    }
    void SetDstMemSlices(const std::vector<MemSlice> &dsts) { dsts_ = dsts; }
    void SetDstMemSlices(std::vector<MemSlice> &&dsts) {
        dsts_ = std::move(dsts);
    }
    void SetMergedSrcMemSlices(const std::vector<MemSlice> &mergedSrcs) {
        mergedSrcs_ = mergedSrcs;
    }
    void SetMergedSrcMemSlices(std::vector<MemSlice> &&mergedSrcs) {
        mergedSrcs_ = std::move(mergedSrcs);
    }
    void SetMergedDstMemSlices(const std::vector<MemSlice> &mergedDsts) {
        mergedDsts_ = mergedDsts;
    }
    void SetMergedDstMemSlices(std::vector<MemSlice> &&mergedDsts) {
        mergedDsts_ = std::move(mergedDsts);
    }
    ProtocolType GetProtocol() const { return protocol_; }
...
class TaskReduce : public TaskNode {
  public:
    TaskReduce(const MemSlice &src, const MemSlice &dst, uint8_t dataType,
               uint8_t reduceOp, ProtocolType protocol = ProtocolType::INVALID)
        : TaskNode(TaskType::REDUCE), src_(src), srcs_{src}, dst_(dst),
          dataType_(dataType), reduceOp_(reduceOp), protocol_(protocol) {}
    TaskReduce(const std::vector<MemSlice> &srcs, const MemSlice &dst,
               uint8_t dataType, uint8_t reduceOp,
               ProtocolType protocol = ProtocolType::INVALID)
        : TaskNode(TaskType::REDUCE),
          src_(srcs.empty() ? MemSlice{} : srcs.front()), srcs_(srcs),
          dst_(dst), dataType_(dataType), reduceOp_(reduceOp),
          protocol_(protocol) {}
...
class TaskBatchReduce : public TaskNode {
  public:
    TaskBatchReduce(uint8_t dataType, uint8_t reduceOp,
                    ProtocolType protocol = ProtocolType::INVALID)
        : TaskNode(TaskType::BATCH_REDUCE), dataType_(dataType),
          reduceOp_(reduceOp), protocol_(protocol) {}
    ...
    const std::vector<MemSlice> &GetDsts() const { return dsts_; }
    const std::vector<std::vector<MemSlice>> &GetMergedSrcs() const {
        return mergedSrcs_;
    }
    const std::vector<MemSlice> &GetMergedDsts() const { return mergedDsts_; }
    void AddSrcMemSlice(const std::vector<MemSlice> &slices) {
        srcs_.push_back(slices);
    }
    void AddDstMemSlice(const MemSlice &slice) { dsts_.push_back(slice); }
    void AddMergedSrcMemSlice(const std::vector<MemSlice> &slices) {
        mergedSrcs_.push_back(slices);
    }
    void AddMergedDstMemSlice(const MemSlice &slice) {
        mergedDsts_.push_back(slice);
    }
    void SetSrcMemSlices(const std::vector<std::vector<MemSlice>> &srcs) {
        srcs_ = srcs;
    }
    void SetSrcMemSlices(std::vector<std::vector<MemSlice>> &&srcs) {
        srcs_ = std::move(srcs);
    }
    void SetDstMemSlices(const std::vector<MemSlice> &dsts) { dsts_ = dsts; }
    void SetDstMemSlices(std::vector<MemSlice> &&dsts) {
    ...
    void SetMergedSrcMemSlices(
        const std::vector<std::vector<MemSlice>> &mergedSrcs) {
        mergedSrcs_ = mergedSrcs;
    }
    void
    SetMergedSrcMemSlices(std::vector<std::vector<MemSlice>> &&mergedSrcs) {
        mergedSrcs_ = std::move(mergedSrcs);
    }
    void SetMergedDstMemSlices(const std::vector<MemSlice> &mergedDsts) {
        mergedDsts_ = mergedDsts;
...
class TaskRecordAICPU : public TaskNode {
  public:
    explicit TaskRecordAICPU(const AicpuNotify &notify,
                             ProtocolType protocol = ProtocolType::INVALID)
...
class TaskWaitAICPU : public TaskNode {
  public:
    explicit TaskWaitAICPU(const AicpuNotify &notify,
                           ProtocolType protocol = ProtocolType::INVALID)
...
class TaskRecordCCU : public TaskNode {
  public:
    explicit TaskRecordCCU(const CcuNotify &notify,
                           ProtocolType protocol = ProtocolType::INVALID)
        : TaskNode(TaskType::RECORD), notify_(notify), protocol_(protocol) {}
...
class TaskWaitCCU : public TaskNode {
  public:
    explicit TaskWaitCCU(const CcuNotify &notify,
                         ProtocolType protocol = ProtocolType::INVALID)
        : TaskNode(TaskType::WAIT), notify_(notify), protocol_(protocol) {}
...
class TaskCcuGraph : public TaskNode {
  public:
    explicit TaskCcuGraph(const CcuSubGraphDesc &ccuDesc)
        : TaskNode(TaskType::CCU_GRAPH), ccuDesc_(ccuDesc) {}
    explicit TaskCcuGraph(CcuSubGraphDesc &&ccuDesc)
        : TaskNode(TaskType::CCU_GRAPH), ccuDesc_(std::move(ccuDesc)) {}
    ...
    uint64_t GetSqeArg(size_t queueId, size_t sqeIndex,
                       size_t sqeArgsId) const {
...
class TaskAivGraph : public TaskNode {
  public:
    TaskAivGraph(DeviceId deviceId, uint64_t launchIdx, uint64_t hostStreamId)
        : TaskNode(TaskType::AIV_GRAPH), deviceId_(deviceId),
          launchIdx_(launchIdx), hostStreamId_(hostStreamId) {}
...
class TaskAivSetFlag : public TaskNode {
  public:
    explicit TaskAivSetFlag(const AivPipeEvent &event)
        : TaskNode(TaskType::AIV_SET_FLAG), event_(event) {}
...
class TaskAivWaitFlag : public TaskNode {
  public:
    explicit TaskAivWaitFlag(const AivPipeEvent &event)
        : TaskNode(TaskType::AIV_WAIT_FLAG), event_(event) {}
...
class TaskAivPipeBarrier : public TaskNode {
  public:
    explicit TaskAivPipeBarrier(AivBarrierInfo info)
        : TaskNode(TaskType::AIV_PIPE_BARRIER), info_(std::move(info)) {}
...
class TaskAivSyncAll : public TaskNode {
  public:
    explicit TaskAivSyncAll(AivSyncAllInfo info)
        : TaskNode(TaskType::AIV_SYNC_ALL), info_(std::move(info)) {}
...
class TaskAivSendFlag : public TaskNode {
  public:
    explicit TaskAivSendFlag(const AivFlagSync &flag)
        : TaskNode(TaskType::AIV_SEND_FLAG), flag_(flag) {}
...
class TaskAivRecvFlag : public TaskNode {
  public:
    explicit TaskAivRecvFlag(const AivFlagSync &flag)
        : TaskNode(TaskType::AIV_RECV_FLAG), flag_(flag) {}
...
class TaskSyncStream : public TaskNode {
  public:
    explicit TaskSyncStream(uint64_t syncIdx)
        : TaskNode(TaskType::SYNC_STREAM), syncIdx_(syncIdx) {}
...
class TaskModelExec : public TaskNode {
  public:
    TaskModelExec(TaskType type, uint32_t execSeq, uint64_t peerStreamId)
        : TaskNode(type), execSeq_(execSeq), peerStreamId_(peerStreamId) {}
    ~TaskModelExec() override = default;
    std::string Describe() const override;
    std::string DescribeShort() const override;

    uint32_t GetExecSeq() const { return execSeq_; }
    uint64_t GetPeerStreamId() const { return peerStreamId_; }

  private:
    uint32_t execSeq_{0};
    uint64_t peerStreamId_{0};
};

class TaskStart : public TaskNode {
  public:
    explicit TaskStart(BoundaryType boundaryType)
        : TaskNode(TaskType::START), boundaryType_(boundaryType) {}
...
class TaskEnd : public TaskNode {
  public:
    explicit TaskEnd(BoundaryType boundaryType)
        : TaskNode(TaskType::END), boundaryType_(boundaryType) {}
