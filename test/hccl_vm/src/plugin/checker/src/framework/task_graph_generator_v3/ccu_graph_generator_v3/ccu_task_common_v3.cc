...
void AddSameQueueChildrenForPrint(TaskNode *node, DeviceId deviceId,
                                  QueueId queueId,
...
void PrintCcuSingleQueueV3(TaskNode *head) {
    ...
    HCCL_VM_INFO("streamId={}, queueId={}, head[{:p}] {}",
                 static_cast<uint32_t>(streamId),
                 static_cast<uint32_t>(queueId), static_cast<void *>(head),
                 head->Describe());
    printedNodes.insert(head);
    AddSameQueueChildrenForPrint(head, deviceId, queueId, candNodes,
                                 visitedNodes);

    while (!candNodes.empty()) {
        TaskNode *curNode = candNodes.front();
        candNodes.pop_front();
        AddSameQueueChildrenForPrint(curNode, deviceId, queueId, candNodes,
                                     visitedNodes);

        bool parentsAllPrinted = true;
        for (TaskNode *parent : curNode->GetParents()) {
            if (parent == nullptr ||
                parent->GetPosition().deviceId != deviceId ||
        ...
        if (parentsAllPrinted) {
            HCCL_VM_INFO("[{:p}] {}", static_cast<void *>(curNode),
                         curNode->Describe());
...
std::vector<MemSlice> MakeMemSlices(DeviceId deviceId, RankId rankId,
...
const CcuSqeParam *FindTraceSqeParam(const CcuGraphStateV3 &state,
                                     QueueId queueId) {
...
void PopulateTraceContext(const CcuGraphStateV3 &state, CcuTraceInfo &trace) {
    ...
    if (trace.position.deviceId == INVALID_DEVICE_ID ||
        trace.dieId == INVALID_DIE_ID) {
        return;
    }

    auto *recorder = AllRankParamRecorder::Global();
    trace.registerState.xn =
        recorder->GetXnSnapshot(trace.position.deviceId, trace.dieId);
    trace.registerState.gsa =
        recorder->GetGSASnapshot(trace.position.deviceId, trace.dieId);
    trace.registerState.cke =
        recorder->GetCKESnapshot(trace.position.deviceId, trace.dieId);
}
...
HcclResult FindInstrForQueue(StorageManager &storage, DeviceId deviceId,
                             uint32_t dieId,
    ...
    HCCL_VM_ERROR("{} Missing CCU instruction data for this device/die, "
                  "deviceId={}, dieId={}",
...
CcuGraphStateV3::CcuGraphStateV3(TaskGraphGeneratorV3 &graphIn,
                                 TaskCcuGraph &ccuGraphIn)
    : graph(graphIn), ccuGraph(ccuGraphIn), commId(ccuGraphIn.GetCommId()),
      deviceId(ccuGraphIn.GetDeviceId()),
      storage_(graphIn.GetStorageManager()) {}

HcclResult CcuGraphStateV3::GetSlice(uint64_t addr, uint64_t len,
                                     DataSlice &dataSlice,
                                     DeviceId *deviceId) const {
    const TaskPosition &position = ccuGraph.GetPosition();
    return storage_.GetSlice(position.commName, position.commHash,
                             position.opIter, addr, len, dataSlice, deviceId);
}
...
HcclResult CcuGraphStateV3::InitInstrInfo(StorageManager &storage) {
    ...
    for (uint32_t queId = 0; queId < queueNum_; ++queId) {
        ...
        HcclResult ret = FindInstrForQueue(storage, deviceId, firstSqe.dieId,
                                           instrInfo[queId]);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        instrInfo[queId].missionStartInstrId =
            static_cast<uint16_t>(firstSqe.instStartId);
        instrInfo[queId].missionInstrCount = static_cast<uint16_t>(
            GetMissionEndInstrId(queId) - firstSqe.instStartId);
        startInstrIdInQue[queId] = instrInfo[queId].startInstrId;
        microCodePosInQue[queId] =
            firstSqe.instStartId - instrInfo[queId].startInstrId;
    }
    return HCCL_SUCCESS;
}
...
HcclResult CcuGraphStateV3::CreateHeadNode() {
    ...
    HcclResult ret = AppendGeneratedNode(
        std::move(node), ccuGraph.GetPosition(), CcuNodeRoleV3::HEAD, rawNode);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
...
HcclResult CcuGraphStateV3::AppendGeneratedNode(
    std::unique_ptr<TaskNode> node, DeviceId devId, uint32_t queId,
    CcuNodeRoleV3 role, TaskNode *&outNode, DeviceId peerDeviceId,
    uint16_t remainingCkeMask, uint32_t dieId, uint16_t ckeId,
    bool invalidPost) {
    ...
    return AppendGeneratedNode(std::move(node), position, role, outNode,
                               peerDeviceId, remainingCkeMask, dieId, ckeId,
                               invalidPost);
}
...
HcclResult CcuGraphStateV3::AppendGeneratedNode(
    ...
    if (position.queueId < microCodePosInQue.size() &&
        position.queueId < startInstrIdInQue.size()) {
        trace.instrId = microCodePosInQue[position.queueId] +
                        startInstrIdInQue[position.queueId];
    }
    ...
    HcclResult ret =
        graph.AppendGeneratedNode(std::move(node), position, nodeId);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
...
void CcuGraphStateV3::SetNodeRemainingCkeMask(TaskNode *node,
                                              uint16_t remainingCkeMask) {
    auto iter = nodeMetas_.find(node);
    if (iter != nodeMetas_.end()) {
...
void CcuGraphStateV3::SetNodePeerDeviceId(TaskNode *node,
...
std::string CcuGraphStateV3::DescribeNextInstructionByQueue() const {
    ...
    for (size_t queId = 0; queId < microCodePosInQue.size(); ++queId) {
        ...
        if (queId < startInstrIdInQue.size()) {
            os << ", nextInstructionId="
               << (startInstrIdInQue[queId] + microCodePosInQue[queId]);
        }
        os << "}";
    }
    os << "]";
...
void CcuGraphStateV3::GetSqe(uint32_t queId, uint16_t sqeArgsId,
                             uint64_t &argVal) {
    argVal = 0;
    if (queId >= ccuParams.size() || queId >= ccuParamIndexs.size() ||
        sqeArgsId >= CCU_SQE_ARGS_LEN ||
...
void PrintCcuGraph(TaskNode *dummyStart) {
    ...
    for (TaskNode *child : dummyStart->GetChildren()) {
        ...
        if (queueId == INVALID_QUEUE_ID ||
            !printedQueues.insert(queueId).second) {
            continue;
        }

        HCCL_VM_INFO("-------------------------------------------------------");
        HCCL_VM_INFO("deviceId={}, rankId={}, streamId={}, queueId={}",
                     child->GetPosition().deviceId, child->GetPosition().rankId,
                     static_cast<uint32_t>(child->GetPosition().streamId),
...
HcclResult CollectBilateralWaitInfo(CcuGraphStateV3 *curCcuTask, uint32_t queId,
    ...
    if (curCcuTask->GetNodeRole(node) == CcuNodeRoleV3::WAIT) {
        ...
        if (peerDeviceId == INVALID_DEVICE_ID ||
            peerDeviceId >= curCcuTask->waitInfoTmp_.size()) {
            return HCCL_SUCCESS;
        }
        ...
        if (waitSize > 0) {
            waitNode =
                curCcuTask->waitInfoTmp_[peerDeviceId].waitNodes[waitSize - 1];
        }
        curCcuTask->bilateralPart1_[queId].insert(
            std::make_pair(node, waitNode));
    }
    return HCCL_SUCCESS;
}

HcclResult CollectBilateralPostInfo(CcuGraphStateV3 *curCcuTask, uint32_t queId,
                                    TaskNode *node, bool isLast) {
    if (curCcuTask == nullptr) {
        return HCCL_E_PTR;
    }
    if (isLast) {
        for (uint32_t devId = 0; devId < curCcuTask->postInfoTmp_.size();
             devId++) {
            if (devId == curCcuTask->deviceId) {
                continue;
            }
            for (auto *asynNode : curCcuTask->postInfoTmp_[devId].asyncNodes) {
                curCcuTask->bilateralPart2_[queId].insert(
                    std::make_pair(asynNode, nullptr));
            }
    ...
    if (curCcuTask->GetNodeRole(node) == CcuNodeRoleV3::POST) {
        DeviceId peerDeviceId = curCcuTask->GetNodePeerDeviceId(node);
        if (peerDeviceId == INVALID_DEVICE_ID ||
            peerDeviceId >= curCcuTask->postInfoTmp_.size()) {
            return HCCL_SUCCESS;
        }
        auto asynNodeSize =
            curCcuTask->postInfoTmp_[peerDeviceId].asyncNodes.size();
        if (asynNodeSize > 0) {
            for (auto *asynNode :
                 curCcuTask->postInfoTmp_[peerDeviceId].asyncNodes) {
                curCcuTask->bilateralPart2_[queId].insert(
                    std::make_pair(asynNode, node));
            }
...
HcclResult AppendTailNode(CcuGraphStateV3 *curCcuTask, uint32_t queId,
                          TaskNode *node) {
    if (curCcuTask == nullptr || node == nullptr ||
        queId >= curCcuTask->tailNodes.size()) {
        return HCCL_E_PTR;
    }
    TaskNode *preNode = (curCcuTask->tailNodes[queId] == nullptr)
                            ? curCcuTask->ccuHeadTaskNode
...
HcclResult AddBatchTransMem(DeviceId deviceId, uint32_t queId,
    ...
    HcclResult ret = curCcuTask->AppendGeneratedNode(
        std::move(task), deviceId, queId, role, node, peerDeviceId);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
...
HcclResult AddBatchReduce(
    DeviceId deviceId, uint32_t queId, CcuGraphStateV3 *curCcuTask,
    std::vector<std::vector<MemSlice>> srcGroups, std::vector<MemSlice> dsts,
    std::vector<std::vector<MemSlice>> mergedSrcGroups,
    std::vector<MemSlice> mergedDsts, HcclDataType checkerDataType,
    HcclReduceOp checkerReduceOp, CcuNodeRoleV3 role, DeviceId peerDeviceId) {
    auto task = std::make_unique<TaskBatchReduce>(
        ToTaskDataType(checkerDataType), ToTaskReduceOp(checkerReduceOp),
        ProtocolType::CCU);
...
void AddLocalCopy(DeviceId deviceId, uint32_t queId,
    ...
    auto task = std::make_unique<TaskTransMem>(
        ...
        MakeCcuMemSlice(deviceId, dstSlice), ProtocolType::CCU);
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(std::move(task), deviceId, queId,
                                        CcuNodeRoleV3::LOCAL_COPY,
                                        node) == HCCL_SUCCESS) {
...
void AddLocalReduce(DeviceId deviceId, uint32_t queId,
    ...
    auto task = std::make_unique<TaskReduce>(
        MakeCcuMemSlice(deviceId, srcSlice),
        MakeCcuMemSlice(deviceId, dstSlice), ToTaskDataType(checkerDataType),
        ToTaskReduceOp(checkerReduceOp), ProtocolType::CCU);
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(std::move(task), deviceId, queId,
                                        CcuNodeRoleV3::LOCAL_REDUCE,
                                        node) == HCCL_SUCCESS) {
        (void)AppendTailNode(curCcuTask, queId, node);
    }
}
...
void AddLocalBatchReduce(DeviceId deviceId, uint32_t queId,
    ...
    HcclDataType checkerDataType =
        g_DataType2CheckerDataType_aicpu[hcclDataType];
    auto task = std::make_unique<TaskBatchReduce>(
        ToTaskDataType(checkerDataType), ToTaskReduceOp(checkerReduceOp),
        ProtocolType::CCU);
    const std::vector<MemSlice> srcMemSlices =
        MakeCcuMemSlices(deviceId, srcSlices);
    ...
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(std::move(task), deviceId, queId,
                                        CcuNodeRoleV3::LOCAL_BATCH_REDUCE,
                                        node) == HCCL_SUCCESS) {
        (void)AppendTailNode(curCcuTask, queId, node);
    }
}
...
void AddWrite(DeviceId deviceId, DeviceId rmtDeviceId, uint32_t queId,
    ...
    auto task = std::make_unique<TaskTransMem>(
        MakeCcuMemSlice(deviceId, srcSlice),
        MakeCcuMemSlice(rmtDeviceId, dstSlice), ProtocolType::CCU);
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(std::move(task), deviceId, queId,
                                        CcuNodeRoleV3::WRITE, node,
                                        rmtDeviceId) == HCCL_SUCCESS) {
        (void)AppendTailNode(curCcuTask, queId, node);
    }
}
...
void AddWriteReduce(DeviceId deviceId, DeviceId rmtDeviceId, uint32_t queId,
    ...
    auto task = std::make_unique<TaskReduce>(
        MakeCcuMemSlice(deviceId, srcSlice),
        MakeCcuMemSlice(rmtDeviceId, dstSlice), ToTaskDataType(checkerDataType),
        ToTaskReduceOp(checkerReduceOp), ProtocolType::CCU);
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(std::move(task), deviceId, queId,
                                        CcuNodeRoleV3::WRITE_REDUCE, node,
                                        rmtDeviceId) == HCCL_SUCCESS) {
        (void)AppendTailNode(curCcuTask, queId, node);
    }
}
...
void AddReadReduce(DeviceId deviceId, DeviceId rmtDeviceId, uint32_t queId,
    ...
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(std::move(task), deviceId, queId,
                                        CcuNodeRoleV3::READ_REDUCE, node,
                                        rmtDeviceId) == HCCL_SUCCESS) {
        (void)AppendTailNode(curCcuTask, queId, node);
    }
}
...
TaskNode *AddLoopStartTask(uint32_t queId, uint32_t loopIdx,
                           ...
                           uint16_t instrIdStart, uint16_t instrIdEnd,
    ...
    if (curCcuTask->AppendGeneratedNode(
            std::move(task), curCcuTask->GetDeviceId(), queId,
...
TaskNode *AddLoopEndTask(uint32_t queId, uint32_t loopIdx,
    ...
    if (node != nullptr) {
        curCcuTask->loopGroupInfo_[loopGroupIdx][loopIdx].endNodeId =
            node->GetNodeId();
    }
    ...
    TaskNode *startNode =
        curCcuTask->loopGroupInfo_[loopGroupIdx][loopIdx].loopStart;
...
void AddPost(DeviceId deviceId, DeviceId rmtDeviceId, uint32_t queId,
    ...
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(
            std::move(task), deviceId, queId, CcuNodeRoleV3::POST, node,
            rmtDeviceId, setRmtCKEMask, rmtDieId, rmtCKEId) != HCCL_SUCCESS) {
        return;
    }
    (void)AppendTailNode(curCcuTask, queId, node);
    AllRankParamRecorder::Global()
        ->seenPost[rmtDeviceId][rmtDieId][rmtCKEId]
        .insert(node);
...
TaskNode *AddWait(DeviceId deviceId, uint32_t queId,
    ...
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(std::move(task), deviceId, queId,
                                        CcuNodeRoleV3::WAIT, node,
                                        INVALID_DEVICE_ID, remoteWaitMask,
                                        dieId, waitCKEId) != HCCL_SUCCESS) {
        return nullptr;
    }
    TaskNode *preNode = (curCcuTask->tailNodes[queId] == nullptr)
                            ? curCcuTask->ccuHeadTaskNode
...
void AddLocalPost(DeviceId deviceId, uint32_t queId, uint32_t dieId,
    ...
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(std::move(task), deviceId, queId,
                                        CcuNodeRoleV3::LOCAL_POST_TO, node,
                                        deviceId, setCKEMask, dieId, setCKEId,
                                        invalidPost) != HCCL_SUCCESS) {
        return;
    }
    (void)AppendTailNode(curCcuTask, queId, node);
    AllRankParamRecorder::Global()->seenPost[deviceId][dieId][setCKEId].insert(
        node);
    CcuPostNodeMetaV3 meta;
    meta.recordDeviceId = deviceId;
...
TaskNode *AddLocalWait(DeviceId deviceId, uint32_t queId,
    ...
    TaskNode *node = nullptr;
    if (curCcuTask->AppendGeneratedNode(
            std::move(task), deviceId, queId, CcuNodeRoleV3::LOCAL_WAIT_FROM,
            node, deviceId, localWaitMask, dieId, waitCKEId) == HCCL_SUCCESS) {
        (void)AppendTailNode(curCcuTask, queId, node);
    }
    return node;
}
...
void AddCcuSubGraphEnd(TaskNode *node, CcuGraphStateV3 *curCcuTask) {
    ...
    if (curCcuTask->AppendGeneratedNode(std::move(task), position,
                                        CcuNodeRoleV3::SUB_GRAPH_END,
                                        endNode) != HCCL_SUCCESS) {
        return;
    }
...
HcclResult ClearWaitMask(DeviceId deviceId, uint32_t dieId, uint16_t waitCKEId,
    ...
    uint16_t ckeValue = 0;
    CHK_RET(AllRankParamRecorder::Global()->GetCKE(deviceId, dieId, waitCKEId,
                                                   ckeValue));
    CHK_RET(AllRankParamRecorder::Global()->SetCKE(deviceId, dieId, waitCKEId,
                                                   ckeValue & (~waitCKEMask)));
    return HCCL_SUCCESS;
}
...
HcclResult ProcessSetMask(DeviceId deviceId, uint32_t dieId,
    ...
    if (setCKEMask != 0x0000) {
        AddLocalPost(deviceId, queId, dieId, curCcuTask, setCKEId, setCKEMask,
                     invalidPost);
        uint16_t ckeValue = 0;
        CHK_RET(AllRankParamRecorder::Global()->GetCKE(deviceId, dieId,
                                                       setCKEId, ckeValue));
        CHK_RET(AllRankParamRecorder::Global()->SetCKE(
            deviceId, dieId, setCKEId, ckeValue | setCKEMask));
    }
...
HcclResult GenSliceFromMs(uint16_t msId, uint64_t len, DataSlice &slice) {
    constexpr uint32_t MS_LEN = 4 * 1024;
