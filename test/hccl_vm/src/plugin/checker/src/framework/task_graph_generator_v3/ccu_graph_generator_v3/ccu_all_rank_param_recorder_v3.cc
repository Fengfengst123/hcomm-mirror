...
AllRankParamRecorder *AllRankParamRecorder::Global() {
    static AllRankParamRecorder *allRankParamRecorder =
        new AllRankParamRecorder;
...
void AllRankParamRecorder::RegisterPostNode(TaskNode *node,
                                            const CcuPostNodeMetaV3 &meta) {
    if (node == nullptr) {
        return;
    }
...
uint32_t
AllRankParamRecorder::GetPostRemainingCkeMask(const TaskNode *node) const {
...
void AllRankParamRecorder::SetPostRemainingCkeMask(TaskNode *node,
                                                   uint32_t remainingCkeMask) {
    auto it = postNodeMeta.find(node);
    if (it == postNodeMeta.end()) {
        return;
    }
...
const CcuPostNodeMetaV3 *
AllRankParamRecorder::GetPostNodeMeta(const TaskNode *node) const {
    auto it = postNodeMeta.find(node);
    if (it == postNodeMeta.end()) {
        return nullptr;
    }
...
HcclResult AllRankParamRecorder::SetXn(DeviceId deviceId, uint32_t dieId,
                                       uint16_t xnId, uint64_t xnValue) {
    curXn[deviceId][dieId][xnId] = xnValue;
    return HCCL_SUCCESS;
}

HcclResult AllRankParamRecorder::SetGSA(DeviceId deviceId, uint32_t dieId,
                                        uint16_t gsaId, uint64_t gsaValue) {
    curGSA[deviceId][dieId][gsaId] = gsaValue;
    return HCCL_SUCCESS;
}

HcclResult AllRankParamRecorder::SetCKE(DeviceId deviceId, uint32_t dieId,
                                        uint16_t ckeId, uint16_t ckeValue) {
    curCKE[deviceId][dieId][ckeId] = ckeValue;
    return HCCL_SUCCESS;
}

HcclResult AllRankParamRecorder::GetXn(DeviceId deviceId, uint32_t dieId,
                                       uint16_t xnId, uint64_t &xnValue) {
    if (curXn.find(deviceId) != curXn.end()) {
        if (curXn[deviceId].find(dieId) != curXn[deviceId].end()) {
            if (curXn[deviceId][dieId].find(xnId) !=
                curXn[deviceId][dieId].end()) {
...
HcclResult AllRankParamRecorder::GetGSA(DeviceId deviceId, uint32_t dieId,
                                        uint16_t gsaId, uint64_t &gsaValue) {
    if (curGSA.find(deviceId) != curGSA.end()) {
        if (curGSA[deviceId].find(dieId) != curGSA[deviceId].end()) {
            if (curGSA[deviceId][dieId].find(gsaId) !=
                curGSA[deviceId][dieId].end()) {
...
HcclResult AllRankParamRecorder::GetCKE(DeviceId deviceId, uint32_t dieId,
                                        uint16_t ckeId, uint16_t &ckeValue) {
    if (curCKE.find(deviceId) != curCKE.end()) {
        if (curCKE[deviceId].find(dieId) != curCKE[deviceId].end()) {
            if (curCKE[deviceId][dieId].find(ckeId) !=
                curCKE[deviceId][dieId].end()) {
...
AllRankParamRecorder::GetXnSnapshot(DeviceId deviceId, uint32_t dieId) const {
...
std::map<uint16_t, uint64_t>
AllRankParamRecorder::GetGSASnapshot(DeviceId deviceId, uint32_t dieId) const {
...
AllRankParamRecorder::GetCKESnapshot(DeviceId deviceId, uint32_t dieId) const {
...
void AllRankParamRecorder::RecordLoopGroupRegSnapshot(DeviceId deviceId,
                                                      uint32_t dieId,
                                                      uint32_t pc,
                                                      uint64_t xpValue,
                                                      uint64_t xmValue) {
    loopGroupRegSnapshot[deviceId][dieId][pc] =
        std::make_pair(xpValue, xmValue);
}

uint16_t AllRankParamRecorder::GetMaxUsedXnId(DeviceId deviceId,
                    ...
                    if (meta != nullptr) {
                        HCCL_VM_WARN(
                            ...
                            MakeErrorCodeText(ErrorCode::GRAPH_UNMATCHED)
                                .c_str(),
                            regPair.second.size(), meta->remainingCkeMask,
                            meta->isLocal,
                            post == nullptr ? "node=null"
                                            : post->Describe().c_str());
                        continue;
                    }
                    HCCL_VM_WARN(
...
HcclResult AllRankParamRecorder::SetHBM(DeviceId deviceId, uint32_t dieId,
                                        uint64_t hbmAddr,
...
HcclResult AllRankParamRecorder::GetHBM(DeviceId deviceId, uint32_t dieId,
                                        uint64_t hbmAddr,
                                        std::vector<uint64_t> &data) {
    if (curHBM.find(deviceId) != curHBM.end()) {
        if (curHBM[deviceId].find(dieId) != curHBM[deviceId].end()) {
            if (curHBM[deviceId][dieId].find(hbmAddr) !=
                curHBM[deviceId][dieId].end()) {
...
HcclResult AllRankParamRecorder::GetMSIdByAddr(uint32_t dieId, uint64_t addr,
                                               uint16_t &msId) {
    if (dieId >= ccu_resource_base_addr_.size()) {
        const std::string maxDieId =
            ccu_resource_base_addr_.empty()
                ? "null"
                : std::to_string(static_cast<uint32_t>(
                      ccu_resource_base_addr_.size() - 1));
        HCCL_VM_ERROR("{} dieId is out of range when converting address to MS "
                      "id, dieId={}, "
                      "maxDieId={}",
    ...
    if (ccuIndex == UINT64_MAX) {
        HCCL_VM_ERROR(
            ...
            "localMsAddr={}, "
            "rawAddr={}",
            MakeErrorCodeText(ErrorCode::GRAPH_ADDRESS_INVALID).c_str(), msAddr,
            addr);
        return HCCL_E_PARA;
    }
...
// 通过XnId所在的地址值来找到XnId
HcclResult AllRankParamRecorder::GetXnAndTypeIdByAddr(uint32_t dieId,
                                                      uint64_t xnAddr,
    ...
    if (dieId >= ccu_resource_base_addr_.size()) {
        const std::string maxDieId =
            ccu_resource_base_addr_.empty()
                ...
                : std::to_string(static_cast<uint32_t>(
                      ccu_resource_base_addr_.size() - 1));
        HCCL_VM_ERROR("{} dieId is out of range when converting address to "
                      "register id, dieId={}, "
                      "maxDieId={}",
                      MakeErrorCodeText(ErrorCode::GRAPH_OUT_OF_RANGE).c_str(),
                      dieId, maxDieId);
    ...
    if (type == CcuComponerntType::UNKNOWN) {
        HCCL_VM_ERROR(
            ...
            "addr={}, "
            "dieId={}, dieBaseAddr={}",
            MakeErrorCodeText(ErrorCode::GRAPH_ADDRESS_INVALID).c_str(), xnAddr,
            dieId, ccu_resource_base_addr_[dieId]);
        return HCCL_E_PARA;
    }
...
// 通过XnId所在的地址值来找到XnId
HcclResult AllRankParamRecorder::GetXnIdByAddr(uint32_t dieId,
                                               CcuComponerntType type,
                                               uint64_t xnAddr,
                                               uint16_t &xnId) {
    if (dieId >= ccu_resource_base_addr_.size()) {
...
// 通过XnId所在的地址值来找到XnId
HcclResult AllRankParamRecorder::GetAddrByXnId(uint32_t dieId,
                                               CcuComponerntType type,
                                               uint16_t xnId,
                                               uint64_t &xnAddr) {
    // 需要判断界限  todo
    if (dieId >= ccu_resource_base_addr_.size()) {
        const std::string maxDieId =
            ccu_resource_base_addr_.empty()
                ...
                : std::to_string(static_cast<uint32_t>(
                      ccu_resource_base_addr_.size() - 1));
        HCCL_VM_ERROR("{} dieId is out of range when converting register id to "
                      "address, dieId={}, "
                      "maxDieId={}",
                      MakeErrorCodeText(ErrorCode::GRAPH_OUT_OF_RANGE).c_str(),
                      dieId, maxDieId);
