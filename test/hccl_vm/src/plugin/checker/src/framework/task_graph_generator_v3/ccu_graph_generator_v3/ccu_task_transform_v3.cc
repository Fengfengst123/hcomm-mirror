...
// 作用：读取 post 节点当前还未被 wait 消费掉的 CKE mask。
uint32_t GetPostRemainingCkeMask(TaskNode *post) {
    return AllRankParamRecorder::Global()->GetPostRemainingCkeMask(post);
}

// 作用：更新 post 节点当前还未被 wait 消费掉的 CKE mask。
void SetPostRemainingCkeMask(TaskNode *post, uint32_t remainingCkeMask) {
    AllRankParamRecorder::Global()->SetPostRemainingCkeMask(post,
                                                            remainingCkeMask);
}

// 作用：按 wait mask 生成本地/远端 wait 节点，并补齐 post 到 wait 的依赖边。
void GenWaitNode(CcuGraphStateV3 *curCcuTask, uint32_t queId,
                 uint16_t waitCKEId, uint16_t waitCKEMask) {
    ...
    std::set<TaskNode *> &seenPosts =
        AllRankParamRecorder::Global()->seenPost[deviceId][dieId][waitCKEId];
    std::vector<TaskNode *> localPosts;
    for (auto *post : seenPosts) {
        uint32_t remainingCkeMask = GetPostRemainingCkeMask(post);
        uint16_t curPostMask = remainingCkeMask & waitCKEMask;
        if (curPostMask == 0) {
            continue;
        }
    ...
    if (remoteWaitMask != 0) {
        remoteWaitNode = AddWait(deviceId, queId, curCcuTask, dieId, waitCKEId,
                                 remoteWaitMask);
    }

    if (localWaitMask != 0) {
        localWaitNode = AddLocalWait(deviceId, queId, curCcuTask, dieId,
                                     waitCKEId, localWaitMask);
    ...
    for (auto it = seenPosts.begin(); it != seenPosts.end();) {
        ...
        if (postMeta != nullptr && postMeta->isLocal) {
            ...
            if (postMeta != nullptr) {
                curCcuTask->SetNodePeerDeviceId(remoteWaitNode,
                                                postMeta->recordDeviceId);
            } else {
                curCcuTask->SetNodePeerDeviceId(remoteWaitNode,
                                                post->GetPosition().deviceId);
            }
            (void)CollectBilateralWaitInfo(curCcuTask, queId, remoteWaitNode);
        }
        remainingCkeMask = remainingCkeMask & (~waitCKEMask);
        SetPostRemainingCkeMask(post, remainingCkeMask);
        if (remainingCkeMask == 0) {
...
// queue。
HcclResult ProcessWaitMask(DeviceId deviceId, uint32_t dieId,
                           CcuGraphStateV3 *curCcuTask, uint32_t queId,
                           uint16_t waitCKEId, uint16_t waitCKEMask,
                           bool &isContinue) {
    HCCL_VM_DEBUG("Evaluate CCU wait condition, deviceId={}, dieId={}, "
                  "queueId={}, waitCkeId={}, "
                  "requiredMask=0x{:04x}",
                  deviceId, static_cast<uint32_t>(dieId), queId, waitCKEId,
                  waitCKEMask);
    if (waitCKEMask != 0x0000) {
        uint16_t ckeValue = 0;
        CHK_RET(AllRankParamRecorder::Global()->GetCKE(deviceId, dieId,
                                                       waitCKEId, ckeValue));
        if ((ckeValue & waitCKEMask) != waitCKEMask) {
            // 当前 queue 在这里暂停，直到生产侧把所需的 wait bit 全部 post
            // 完成。
            isContinue = false;
            HCCL_VM_DEBUG("Pause this CCU queue because the wait condition is "
                          "not satisfied yet, "
                          "deviceId={}, dieId={}, queueId={}, waitCkeId={}, "
                          "currentCkeValue=0x{:04x}, requiredMask=0x{:04x}",
                          deviceId, static_cast<uint32_t>(dieId), queId,
                          waitCKEId, ckeValue, waitCKEMask);
            return HCCL_SUCCESS;
        }
...
HcclResult TransformInstr(const CcuRep::CcuInstr *instr,
                          CcuGraphStateV3 *curCcuTask, uint32_t queId,
                          bool &isContinue) {
    ...
    if (!g_instrMap->IsSupported(instructionHeader)) {
        HCCL_VM_ERROR("{} This CCU instruction type is not supported by "
    ...
    return g_instrMap.get()->Transform(instr, curCcuTask, queId, isContinue,
                                       nullptr);
}
...
HcclResult TransformInstrQue(TaskNode *node, CcuGraphStateV3 *curCcuTask,
                             uint32_t queId) {
    ...
    while (pos < endInstrId) {
        ...
        const uint16_t instructionHeader =
            static_cast<uint16_t>(microCodeQue.instrVec[pos].header.header);
        HCCL_VM_DEBUG(
            "Translating one CCU instruction, graph={}, instructionId={}, "
            "instructionHeader=0x{:04x}",
            curCcuTask->Describe(), instrId, instructionHeader);
        HcclResult ret = TransformInstr(&microCodeQue.instrVec[pos], curCcuTask,
                                        queId, isContinue);
        if (ret != HCCL_SUCCESS) {
            HCCL_VM_ERROR(
                ...
                "graph={}",
                MakeErrorCodeText(ErrorCode::GRAPH_TRANSLATE_FAILED).c_str(),
                curCcuTask->GetDeviceId(), queId, instrId, instructionHeader,
                static_cast<uint32_t>(ret), curCcuTask->Describe().c_str());
            return ret;
        }
        if (!isContinue) {
            return HCCL_SUCCESS;
    ...
    curCcuTask->isGenGraphed =
        std::all_of(curCcuTask->instrQueStatus.begin(),
                    curCcuTask->instrQueStatus.end(), [](bool b) { return b; });

    HCCL_VM_DEBUG("Finished translating one CCU queue, deviceId={}, queueId={}",
                  curCcuTask->GetDeviceId(), queId);
    return HCCL_SUCCESS;
}
...
class QueueIdReuseAllocatorV3 {
  public:
    ...
    explicit QueueIdReuseAllocatorV3(uint32_t baseQueueCount) {
        curMaxQueueId_ = (baseQueueCount == 0U)
                             ? INVALID_QUEUE_ID
                             : static_cast<QueueId>(baseQueueCount - 1U);
    }
    ...
    QueueId Acquire() {
        ...
        curMaxQueueId_ = (curMaxQueueId_ == INVALID_QUEUE_ID)
                             ? 0U
                             : static_cast<QueueId>(curMaxQueueId_ + 1U);
    ...
    QueueId NextQueueId() const {
        return (curMaxQueueId_ == INVALID_QUEUE_ID)
                   ? 0U
                   : static_cast<QueueId>(curMaxQueueId_ + 1U);
    }
...
void AddReachableChildren(std::deque<TaskNode *> &queue,
...
bool AreParentsCompleted(const TaskNode *node,
                         const std::set<TaskNode *> &completedNodes) {
    if (node == nullptr) {
        return false;
    }
...
bool IsAsyncNodeV3(const CcuGraphStateV3 &state, const TaskNode *node) {
    ...
    return role == CcuNodeRoleV3::LOCAL_COPY ||
           role == CcuNodeRoleV3::LOCAL_REDUCE ||
           role == CcuNodeRoleV3::LOCAL_BATCH_REDUCE ||
           role == CcuNodeRoleV3::READ || role == CcuNodeRoleV3::READ_REDUCE ||
           role == CcuNodeRoleV3::WRITE || role == CcuNodeRoleV3::WRITE_REDUCE;
}

// 作用：根据 CCU 通知参数判断 RECORD 是否对应
// WAIT；这里只判断同步语义，不负责判断图上是否已有边。
bool IsCcuRecordWaitPairV3(const TaskNode *recordNode,
                           const TaskNode *waitNode) {
    if (recordNode == nullptr || waitNode == nullptr ||
        recordNode->GetType() != TaskType::RECORD ||
    ...
    return recordNotify.waitDeviceId == waitNotify.waitDeviceId &&
           recordNotify.dieId == waitNotify.dieId &&
           recordNotify.ckeId == waitNotify.ckeId &&
           (recordNotify.ckeMask & waitNotify.ckeMask) != 0;
}
...
bool HasNodeEdgeV3(const TaskNode *parentNode, const TaskNode *childNode) {
    ...
    return std::find(children.begin(), children.end(), childNode) !=
           children.end();
}
...
std::vector<TaskNode *> GetSameQueueParentsV3(const TaskNode *node,
                                              QueueId queueId) {
...
std::vector<TaskNode *>
GetParentsOutsideNodesV3(const TaskNode *node,
                         const std::set<TaskNode *> &excludedNodes) {
    std::vector<TaskNode *> parents;
    if (node == nullptr) {
        return parents;
    }
...
std::vector<TaskNode *> GetSameQueueChildrenV3(const TaskNode *node,
                                               QueueId queueId) {
...
std::vector<TaskNode *>
GetChildrenOutsideNodesV3(const TaskNode *node,
                          const std::set<TaskNode *> &excludedNodes) {
    std::vector<TaskNode *> children;
    if (node == nullptr) {
        return children;
...
bool IsCcuSubGraphEndNodeV3(const TaskNode *node) {
    ...
    return endNode != nullptr &&
           endNode->GetBoundaryType() == BoundaryType::CCU_SUB_GRAPH;
}
...
TaskNode *GetSerialSameQueueChildV3(const TaskNode *node, QueueId queueId) {
    ...
    for (TaskNode *child : node->GetChildren()) {
        if (child == nullptr || child->GetLocation().queueId != queueId ||
            child->GetNodeId() <= node->GetNodeId()) {
            continue;
        }
        if (serialChild == nullptr ||
            child->GetNodeId() < serialChild->GetNodeId()) {
            serialChild = child;
        }
    }
    return serialChild;
...
NodeId GetReleaseNodeIdV3(const std::vector<TaskNode *> &afterNodes) {
    ...
    return (releaseNodeId == INVALID_NODE_ID)
               ? std::numeric_limits<NodeId>::max()
               : releaseNodeId;
}

// 作用：收集同一 queue 内从 loopStart 到 loopEnd 的 loop body 节点集合。
void CollectLoopBodyNodesSameQueueV3(TaskNode *loopStart, TaskNode *loopEnd,
    ...
    while (!candidateNodes.empty()) {
        ...
        if (curNode == nullptr ||
            curNode->GetLocation().queueId != originalQueueId) {
            continue;
        }
        ...
        for (TaskNode *child : curNode->GetChildren()) {
            if (child == nullptr ||
                child->GetLocation().queueId != originalQueueId) {
                continue;
            }
...
void CollectAsyncBoundRecordNodesV3(TaskNode *asyncNode,
                                    QueueId originalQueueId,
                                    std::set<TaskNode *> &nodes,
                                    TaskNode *&tailNode) {
...
// 已被压缩成边，则沿用压缩后的等价边界。
std::vector<TaskNode *>
GetLoopBeforeNodesV3(TaskNode *firstHead, QueueId originalQueueId,
...
HcclResult
BuildLoopParallelGroupV3(const std::vector<LoopInfoV3> &loopGroup,
                         const CompressedLoopBoundaryV3 &compressedBoundary,
                         LoopParallelGroupV3 &group) {
    group = LoopParallelGroupV3{};
    if (loopGroup.empty() || loopGroup.front().loopStart == nullptr ||
        loopGroup.back().loopEnd == nullptr) {
        return HCCL_SUCCESS;
    }

    const QueueId originalQueueId =
        loopGroup.front().loopStart->GetLocation().queueId;
    ...
    group.beforeNodes = GetLoopBeforeNodesV3(
        loopGroup.front().loopStart, originalQueueId, compressedBoundary);
    group.afterNodes =
        GetSameQueueChildrenV3(loopGroup.back().loopEnd, originalQueueId);
    ...
    for (const auto &loopInfo : loopGroup) {
        ...
        CollectLoopBodyNodesSameQueueV3(loopInfo.loopStart, loopInfo.loopEnd,
                                        originalQueueId, unit.nodes);
        if (!unit.nodes.empty()) {
...
void PopulateAsyncGroupBoundaryNodesV3(AsyncParallelGroupV3 &group) {
    ...
    group.beforeNodes =
        GetParentsOutsideNodesV3(group.units.front().head, groupNodes);
    for (auto &unit : group.units) {
        ...
        unit.afterNodes.erase(
            std::remove_if(
                unit.afterNodes.begin(), unit.afterNodes.end(),
                [](TaskNode *node) { return IsCcuSubGraphEndNodeV3(node); }),
            unit.afterNodes.end());
    }
    // 分组级 afterNodes 用于原 queue 前后节点之间的桥接边，以及 queue
    // 释放时机计算。
    group.afterNodes =
        GetChildrenOutsideNodesV3(group.units.back().tail, groupNodes);
    group.afterNodes.erase(
        std::remove_if(
            group.afterNodes.begin(), group.afterNodes.end(),
            [](TaskNode *node) { return IsCcuSubGraphEndNodeV3(node); }),
        group.afterNodes.end());
}

// 作用：从候选 async 节点中构造可并行化的 async 分组。
void BuildAsyncParallelGroupsV3(const CcuGraphStateV3 &state,
                                std::vector<AsyncParallelGroupV3> &groups) {
    ...
    std::sort(asyncNodes.begin(), asyncNodes.end(),
    ...
    for (TaskNode *node : asyncNodes) {
        ...
        while (curHead != nullptr &&
               curHead->GetLocation().queueId == originalQueueId &&
               processedHeads.count(curHead) == 0 &&
               IsAsyncNodeV3(state, curHead)) {
            QueueReuseUnitV3 unit;
            ...
            CollectAsyncBoundRecordNodesV3(curHead, originalQueueId, unit.nodes,
                                           unit.tail);
            group.units.push_back(std::move(unit));
            processedHeads.insert(curHead);

            TaskNode *nextHead = GetSerialSameQueueChildV3(
                group.units.back().tail, originalQueueId);
            if (nextHead == nullptr ||
                nextHead->GetLocation().queueId != originalQueueId ||
                processedHeads.count(nextHead) != 0 ||
                !IsAsyncNodeV3(state, nextHead)) {
                break;
            }
            curHead = nextHead;
        }
...
void CollectQueueUnitsV3(std::vector<LoopParallelGroupV3> &groups,
                         std::vector<QueueReuseUnitV3 *> &units) {
...
void CollectQueueUnitsV3(std::vector<AsyncParallelGroupV3> &groups,
                         std::vector<QueueReuseUnitV3 *> &units) {
    for (auto &group : groups) {
        for (auto &unit : group.units) {
            units.push_back(&unit);
...
void AssignQueueIdsWithReuseV3(std::vector<QueueReuseUnitV3 *> &units,
                               QueueIdReuseAllocatorV3 &allocator) {
    std::sort(units.begin(), units.end(),
              [](const QueueReuseUnitV3 *lhs, const QueueReuseUnitV3 *rhs) {
    ...
    for (QueueReuseUnitV3 *unit : units) {
        ...
        while (!activeUnits.empty() &&
               activeUnits.begin()->first < unit->startNodeId) {
...
HcclResult RewireLoopParallelGroupV3(TaskGraphGeneratorV3 &graph,
                                     const LoopParallelGroupV3 &group) {
    // 等价于老 checker 的 ProcLoopNode：先拆相邻 loop 的原串行边。
    for (size_t index = 1; index < group.units.size(); ++index) {
        if (group.units[index - 1].tail == nullptr ||
            group.units[index].head == nullptr) {
            continue;
        }
        CHK_RET(graph.RemoveEdge(group.units[index - 1].tail->GetNodeId(),
                                 group.units[index].head->GetNodeId()));
    }
    ...
    if (!group.units.empty()) {
        ...
        for (TaskNode *beforeNode : group.beforeNodes) {
            ...
            CHK_RET(graph.RemoveEdge(beforeNode->GetNodeId(),
                                     firstHead->GetNodeId()));
        }
        for (TaskNode *afterNode : group.afterNodes) {
            ...
            CHK_RET(graph.RemoveEdge(lastTail->GetNodeId(),
                                     afterNode->GetNodeId()));
        }
    }
    ...
    for (TaskNode *beforeNode : group.beforeNodes) {
        ...
        for (const auto &unit : group.units) {
            ...
            CHK_RET(
                graph.AddEdge(beforeNode->GetNodeId(), unit.head->GetNodeId()));
        }
    }
    ...
    for (TaskNode *afterNode : group.afterNodes) {
        ...
        for (const auto &unit : group.units) {
            ...
            CHK_RET(
                graph.AddEdge(unit.tail->GetNodeId(), afterNode->GetNodeId()));
        }
    }
    ...
    for (TaskNode *beforeNode : group.beforeNodes) {
        ...
        for (TaskNode *afterNode : group.afterNodes) {
            ...
            CHK_RET(
                graph.AddEdge(beforeNode->GetNodeId(), afterNode->GetNodeId()));
        }
    }
    return HCCL_SUCCESS;
}
...
void UpdateCompressedLoopBoundaryV3(
    const LoopParallelGroupV3 &group,
    CompressedLoopBoundaryV3 &compressedBoundary) {
...
HcclResult
RewireAsyncParallelGroupsV3(TaskGraphGeneratorV3 &graph,
                            const std::vector<AsyncParallelGroupV3> &groups) {
    for (const auto &group : groups) {
        ...
        for (TaskNode *beforeNode : group.beforeNodes) {
            ...
            for (const auto &unit : group.units) {
                ...
                CHK_RET(graph.AddEdge(beforeNode->GetNodeId(),
                                      unit.head->GetNodeId()));
            }
        }
        ...
        for (const auto &unit : group.units) {
            ...
            if (tailNode->GetType() == TaskType::RECORD) {
                for (TaskNode *afterNode : unit.afterNodes) {
                    if (HasNodeEdgeV3(tailNode, afterNode) &&
                        IsCcuRecordWaitPairV3(tailNode, afterNode)) {
                ...
                HCCL_VM_WARN("Skip async parallel exit edge because the tail "
            ...
            for (TaskNode *afterNode : unit.afterNodes) {
                ...
                CHK_RET(graph.RemoveEdge(tailNode->GetNodeId(),
                                         afterNode->GetNodeId()));
                ...
                // 顺序边保持删除。
                CHK_RET(graph.AddEdge(tailNode->GetNodeId(),
                                      afterNode->GetNodeId()));
            }
        }
...
HcclResult ApplyCcuQueueParallelizationV3(CcuGraphStateV3 &state) {
    ...
    for (const auto &loopGroupInfo : state.loopGroupInfo_) {
        LoopParallelGroupV3 group;
        CHK_RET(BuildLoopParallelGroupV3(loopGroupInfo, compressedLoopBoundary,
                                         group));
        if (group.units.empty()) {
            continue;
        }
...
void InitCcuGeneratorContextV3(StorageManager &storage) {
    ...
    storage.InitCcuInfo(
        AllRankParamRecorder::Global()->devType_,
        AllRankParamRecorder::Global()->ccu_resource_base_addr_);
}

// 作用：准备单个 CCU 图 raw 成图所需的初始状态。
HcclResult PrepareRawCcuGraphStateV3(StorageManager &storage,
                                     CcuGraphStateV3 &state) {
...
HcclResult InitCcuGraphRuntimeV3(TaskGraphGeneratorV3 &graph,
                                 StorageManager &storage,
                                 TaskCcuGraph *ccuGraph,
                                 CcuGraphRuntimeV3 &runtime) {
    if (ccuGraph == nullptr) {
        return HCCL_E_PTR;
    }
...
HcclResult GetOrCreateCcuGraphRuntimeV3(
    TaskGraphGeneratorV3 &graph, StorageManager &storage,
    TaskCcuGraph *ccuGraph,
...
HcclResult GenerateRawCcuGraphStateV3(TaskNode *node, CcuGraphStateV3 &state) {
    ...
    while (!state.isGenGraphed) {
        std::vector<uint32_t> microCodePosInQuePre = state.microCodePosInQue;
        ...
        if (unmatchedCnt > state.queueNum_) {
            HCCL_VM_ERROR(
                ...
                "blockedGraphSummary={}",
                MakeErrorCodeText(ErrorCode::GRAPH_DEADLOCK).c_str(),
                state.GetDeviceId(), state.queueNum_,
                state.DescribeNextInstructionByQueue().c_str(),
...
void PrintGeneratedCcuGraphsV3(
    const std::vector<std::unique_ptr<CcuGraphRuntimeV3>> &runtimes) {
    ...
    for (const auto &runtime : runtimes) {
        if (runtime == nullptr || !runtime->generated ||
            ...
            runtime->state->ccuHeadTaskNode == nullptr ||
            runtime->ccuGraph == nullptr) {
            continue;
        }
    ...
    std::sort(orderedRuntimes.begin(), orderedRuntimes.end(),
              [](const CcuGraphRuntimeV3 *lhs, const CcuGraphRuntimeV3 *rhs) {
                  ...
                  return lhs->ccuGraph->GetNodeId() <
                         rhs->ccuGraph->GetNodeId();
              });

    HCCL_VM_INFO("Printing all generated CCU graphs before queue "
    ...
    for (const CcuGraphRuntimeV3 *runtime : orderedRuntimes) {
        ...
        HCCL_VM_INFO("CCU graph detail, ccuGraphNodeId={}, deviceId={}",
                     runtime->ccuGraph->GetNodeId(),
...
HcclResult
AttachExpandedCcuGraphV3(TaskGraphGeneratorV3 &graph, TaskCcuGraph &ccuGraph,
                         const std::vector<TaskNode *> &originalChildren,
                         CcuGraphStateV3 &state,
    ...
    CHK_RET(graph.AddEdge(ccuGraph.GetNodeId(),
                          state.ccuHeadTaskNode->GetNodeId()));
    for (TaskNode *childNode : originalChildren) {
        if (childNode == nullptr) {
            return HCCL_E_PTR;
        }
        CHK_RET(graph.RemoveEdge(ccuGraph.GetNodeId(), childNode->GetNodeId()));
        CHK_RET(graph.AddEdge(state.ccuSubGraphEndNode->GetNodeId(),
                              childNode->GetNodeId()));
    }
...
HcclResult
FinalizeGeneratedCcuGraphRuntimeV3(TaskGraphGeneratorV3 &graph,
                                   CcuGraphRuntimeV3 &runtime,
                                   CcuGraphGenerateOutputV3 &output) {
    ...
    CHK_RET(AttachExpandedCcuGraphV3(graph, *runtime.ccuGraph,
                                     runtime.originalChildren, state, output));
    runtime.finalized = true;
    return HCCL_SUCCESS;
}

// 作用：联合推进多个 CCU 图的 raw 成图阶段，处理跨图依赖解锁。
HcclResult GenerateRawCcuGraphsV3(
    const CcuGraphsGenerateInputV3 &input,
    std::vector<std::unique_ptr<CcuGraphRuntimeV3>> &runtimes,
    CcuGraphGenerateOutputV3 &output) {
    ...
    while (!candNode.empty()) {
        if (unmatchedCnt >= candNode.size()) {
            TaskNode *firstBlockedGraph = candNode.front();
            const CcuGraphRuntimeV3 *blockedRuntime = nullptr;
            auto runtimeIter = runtimeByNodeId.find(
                firstBlockedGraph == nullptr ? INVALID_NODE_ID
                                             : firstBlockedGraph->GetNodeId());
            if (runtimeIter != runtimeByNodeId.end()) {
            ...
            HCCL_VM_ERROR(
                ...
                blockedRuntime == nullptr || blockedRuntime->state == nullptr
                    ? "[]"
                    : blockedRuntime->state->DescribeNextInstructionByQueue()
                          .c_str(),
                firstBlockedGraph == nullptr
                    ? "node=null"
                    : firstBlockedGraph->Describe().c_str());
            return HCCL_E_INTERNAL;
        }
        ...
        if (ccuGraphIter == ccuGraphByNodeId.end()) {
            ...
            AddReachableChildren(candNode, visitedNodes,
                                 curNode->GetChildren());
            unmatchedCnt = 0;
            continue;
        }
        ...
        CHK_RET(GetOrCreateCcuGraphRuntimeV3(graph, storage,
                                             ccuGraphIter->second, runtimes,
                                             runtimeByNodeId, runtime));
        if (runtime == nullptr || runtime->state == nullptr ||
            runtime->ccuGraph == nullptr) {
            return HCCL_E_PTR;
        }
        if (output.startNodeId == INVALID_NODE_ID &&
            runtime->state->ccuHeadTaskNode != nullptr) {
    ...
    if (generatedCount != input.ccuGraphs.size()) {
        const CcuGraphRuntimeV3 *firstUnfinishedRuntime = nullptr;
        for (const auto &runtime : runtimes) {
            if (runtime != nullptr && !runtime->generated) {
                firstUnfinishedRuntime = runtime.get();
                break;
            }
        }
        HCCL_VM_ERROR(
            ...
            firstUnfinishedRuntime == nullptr ||
                    firstUnfinishedRuntime->ccuGraph == nullptr
                ? "node=null"
                : firstUnfinishedRuntime->ccuGraph->Describe().c_str());
        return HCCL_E_INTERNAL;
    }
    return HCCL_SUCCESS;
}
...
HcclResult FinalizeGeneratedCcuGraphsV3(
    ...
    if (finalizedCount != runtimes.size()) {
        const CcuGraphRuntimeV3 *firstUnfinishedRuntime = nullptr;
        for (const auto &runtime : runtimes) {
            if (runtime != nullptr && !runtime->finalized) {
                firstUnfinishedRuntime = runtime.get();
                break;
            }
        }
...
HcclResult ExpandCcuGraphsV3(const CcuGraphsGenerateInputV3 &input,
                             CcuGraphGenerateOutputV3 &output) {
...
std::unique_ptr<InstructMapBase>
InstructMapFactory::Create(CcuInstrVersion version) {
    switch (version) {
