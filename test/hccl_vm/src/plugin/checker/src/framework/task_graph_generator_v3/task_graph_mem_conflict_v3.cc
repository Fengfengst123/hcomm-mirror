...
struct AccessStartLess {
    bool operator()(const MemoryAccess &lhs, const MemoryAccess &rhs) const {
        ...
        return static_cast<uint32_t>(lhs.kind) <
               static_cast<uint32_t>(rhs.kind);
    }
};
...
struct AccessExactLess {
    bool operator()(const MemoryAccess &lhs, const MemoryAccess &rhs) const {
        ...
        if (lhs.key.memType != rhs.key.memType) {
            return static_cast<uint32_t>(lhs.key.memType) <
                   static_cast<uint32_t>(rhs.key.memType);
        }
...
uint64_t ElapsedMemConflictMs(MemConflictClock::time_point start,
                              MemConflictClock::time_point end) {
...
SelfStatusMemoryKb ReadSelfStatusMemoryKb() {
    ...
    while (std::getline(statusFile, line)) {
        ...
        if (line.compare(0, 6, "VmRSS:") == 0) {
            memory.rssKb = static_cast<size_t>(
                std::strtoull(line.c_str() + valueBegin, nullptr, 10));
        } else if (line.compare(0, 6, "VmHWM:") == 0) {
            memory.hwmKb = static_cast<size_t>(
                std::strtoull(line.c_str() + valueBegin, nullptr, 10));
        }
    }
    return memory;
...
void LogMemConflictStageTime(const char *stage, const char *status,
                             MemConflictClock::time_point start) {
    ...
    HCCL_VM_INFO(
        "Stage finished, stage={}, status={}, costMs={}, rssKb={}, hwmKb={}",
        stage, status, ElapsedMemConflictMs(start, MemConflictClock::now()),
        memory.rssKb, memory.hwmKb);
}
...
std::string DescribeAccess(const MemoryAccess &access) {
    ...
    os << "    node " << access.nodeId << ", action="
       ...
       << "\n    access range : [0x" << std::hex << access.start << ",0x"
       << access.end << ")" << std::dec;
...
void LogCountBreakdown(const char *prefix,
                       const std::map<std::string, size_t> &counts) {
    (void)prefix;
    (void)counts;
}

HcclResult CollectReachableTaskNodes(const TaskNode *start,
...
HcclResult AddAccess(const MemSlice &slice, MemoryAccessKind kind,
                     const TaskNode *node, TaskMemoryAccesses &accesses) {
    if (node == nullptr) {
        return HCCL_E_PTR;
    }
    if (node->GetNodeId() < 0) {
    ...
    if (slice.deviceId == INVALID_DEVICE_ID ||
        ...
        HCCL_VM_ERROR(
            ...
            node->Describe(),
            slice.deviceId == INVALID_DEVICE_ID
                ? "invalid"
                : std::to_string(slice.deviceId),
    ...
    if (!slice.HasMemoryAddress() ||
        ...
        HCCL_VM_ERROR("{} Memory slice address is missing or its end "
                      "overflows, task={}, memType={}, "
                      "rawAddr=0x{:x}, offset=0x{:x}, length=0x{:x}",
                      MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                      node->Describe(), DescribeMemType(slice.memType),
                      slice.rawAddr, slice.offset, slice.len);
        return HCCL_E_PARA;
    }
    // 全局逻辑视图按物理地址合桶；MS 和 AIV 保持各自独立的地址空间。
    const bool isGlobal = IsGlobalPhysicalMemoryType(slice.memType);
    access.key.memType = isGlobal ? MemType::PHYSICAL_GLOBAL : slice.memType;
    access.key.aivUbIdx = isGlobal ? 0 : GetAivUbIdx(slice, node);
    access.start = slice.MemoryBegin();
    access.end = slice.MemoryEnd();
    access.kind = kind;
...
HcclResult BuildTaskMemoryAccesses(const TaskNode *node,
                                   TaskMemoryAccesses &accesses) {
    accesses.clear();
    if (!IsDataMoveTaskNode(node)) {
        return HCCL_SUCCESS;
    }
    ...
    if (node->GetType() == TaskType::TRANS_MEM) {
        ...
        HcclResult ret = AddAccess(transMem->GetSrc(), MemoryAccessKind::READ,
                                   node, accesses);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        return AddAccess(transMem->GetDst(), MemoryAccessKind::WRITE, node,
                         accesses);
    }
    ...
    if (node->GetType() == TaskType::REDUCE) {
        ...
        for (size_t index = 0; index < srcs.size(); ++index) {
            HcclResult ret =
                AddAccess(srcs[index], MemoryAccessKind::READ, node, accesses);
            if (ret != HCCL_SUCCESS) {
                return ret;
            }
        }
        return AddAccess(reduce->GetDst(), MemoryAccessKind::WRITE, node,
                         accesses);
    }
    ...
    if (node->GetType() == TaskType::BATCH_TRANS_MEM) {
        ...
        for (const auto &src : srcs) {
            HcclResult ret =
                AddAccess(src, MemoryAccessKind::READ, node, accesses);
            if (ret != HCCL_SUCCESS) {
                return ret;
            }
        }
        for (const auto &dst : dsts) {
            HcclResult ret =
                AddAccess(dst, MemoryAccessKind::WRITE, node, accesses);
            if (ret != HCCL_SUCCESS) {
                return ret;
            }
        }
    ...
    if (node->GetType() == TaskType::BATCH_REDUCE) {
        ...
        if (srcGroups.empty() || srcGroups.size() != dsts.size()) {
            HCCL_VM_ERROR(
                ...
                MakeErrorCodeText(ErrorCode::SINGLETASK_SLICE_INVALID),
                node->Describe(), srcGroups.size(), dsts.size());
            return HCCL_E_PARA;
        }
        for (size_t groupIndex = 0; groupIndex < srcGroups.size();
             ++groupIndex) {
            ...
            HcclResult ret = AddAccess(dsts[groupIndex],
                                       MemoryAccessKind::WRITE, node, accesses);
            if (ret != HCCL_SUCCESS) {
                return ret;
            }
        }
...
void DedupTaskMemoryAccesses(TaskMemoryAccesses &accesses) {
    ...
    accesses.erase(
        std::unique(accesses.begin(), accesses.end(),
                    [](const MemoryAccess &lhs, const MemoryAccess &rhs) {
                        return lhs.key.deviceId == rhs.key.deviceId &&
                               lhs.key.memType == rhs.key.memType &&
...
bool HasWriteAccess(const MemoryAccess &lhs, const MemoryAccess &rhs) {
    return lhs.kind == MemoryAccessKind::WRITE ||
           rhs.kind == MemoryAccessKind::WRITE;
}

void RefreshOverlapCandidateStats(
    const OverlapCandidateStatsCollector &collector,
    MemConflictCheckStats &stats) {
    stats.overlapCandidateTaskNodeCount = collector.nodeIds.size();
}

void RecordOverlapCandidateTaskNodes(
    NodeId lhs, NodeId rhs, OverlapCandidateStatsCollector &collector) {
    collector.nodeIds.insert(lhs);
    collector.nodeIds.insert(rhs);
}

HcclResult CanRunInParallel(const ReachabilityClosure &closure, NodeId lhs,
                            NodeId rhs, bool &canRunInParallel) {
...
HcclResult CollectConflictCandidate(const MemoryAccess &current,
                                    const MemoryAccess &history,
                                    bool &needReachabilityClosure,
    ...
    if (current.node != nullptr && history.node != nullptr) {
        collector.pairCountByTaskTypePair[MakeTaskTypePairKey(
            current.node->GetType(), history.node->GetType())]++;
    }
...
void RemoveInactiveAccesses(ActiveAccesses &activeAccesses,
                            uint64_t sweepStart) {
...
HcclResult CheckActiveAccesses(const MemoryAccess &current,
                               const ActiveAccesses &activeAccesses,
                               bool &needReachabilityClosure,
                               ConflictCandidates &candidates,
                               MemConflictCheckStats &stats,
                               OverlapCandidateStatsCollector &collector) {
    for (const MemoryAccess *activeAccess : activeAccesses) {
        ...
        HcclResult ret = CollectConflictCandidate(current, *activeAccess,
                                                  needReachabilityClosure,
                                                  candidates, stats, collector);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
    }
...
HcclResult ScanMemoryBucket(TaskMemoryAccesses &bucketAccesses,
                            bool &needReachabilityClosure,
                            ConflictCandidates &candidates,
                            MemConflictCheckStats &stats,
                            OverlapCandidateStatsCollector &collector) {
    ...
    for (const MemoryAccess &current : bucketAccesses) {
        ...
        if (current.kind == MemoryAccessKind::READ) {
            // 读只需要和当前仍活跃的写比较。
            HcclResult ret = CheckActiveAccesses(current, activeWrites,
                                                 needReachabilityClosure,
                                                 candidates, stats, collector);
            if (ret != HCCL_SUCCESS) {
                return ret;
        ...
        HcclResult ret =
            CheckActiveAccesses(current, activeReads, needReachabilityClosure,
                                candidates, stats, collector);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        ret =
            CheckActiveAccesses(current, activeWrites, needReachabilityClosure,
                                candidates, stats, collector);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
...
HcclResult ScanMemoryBuckets(MemoryAccessBuckets &accessBuckets,
                             bool &needReachabilityClosure,
                             ConflictCandidates &candidates,
                             MemConflictCheckStats &stats,
                             OverlapCandidateStatsCollector &collector) {
    for (auto &bucketItem : accessBuckets) {
        HcclResult ret =
            ScanMemoryBucket(bucketItem.second, needReachabilityClosure,
                             candidates, stats, collector);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
...
HcclResult
CheckCollectedConflictCandidates(const ConflictCandidates &candidates,
                                 const ReachabilityClosureT &closure,
                                 MemConflictCheckStats &stats) {
    for (const ConflictCandidate &candidate : candidates) {
        ...
        HcclResult ret =
            CanRunInParallel(closure, candidate.current->nodeId,
                             candidate.history->nodeId, canRunInParallel);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
        ...
        const uint64_t overlapStart =
            std::max(candidate.current->start, candidate.history->start);
        const uint64_t overlapEnd =
            std::min(candidate.current->end, candidate.history->end);
        HCCL_VM_ERROR("{} Two tasks may access the same memory range in "
                      "parallel, and at least one "
                      "access is a write.\n  Conflict memory : {}\n  Overlap "
                      "range    : [0x{:x},0x{:x})\n"
                      "  Conflict task 1:\n{}\n  Conflict task 2:\n{}",
                      MakeErrorCodeText(ErrorCode::MEMCONFLICT_DETECTED),
                      DescribeKey(candidate.current->key), overlapStart,
                      overlapEnd, DescribeAccess(*candidate.current),
...
HcclResult
CheckCandidatesWithDenseReachability(const TaskNode *start,
    ...
    LogMemConflictStageTime("GenReachabilityClosure(Dense)",
                            ret == HCCL_SUCCESS ? "success" : "failed",
                            closureStart);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
    ...
    LogMemConflictStageTime("CheckCollectedConflictCandidates(Dense)",
                            ret == HCCL_SUCCESS ? "success" : "failed",
                            checkStart);
    return ret;
}
...
size_t GetIdleThreadCount() {
    ...
    return static_cast<size_t>(hardwareThreadCount) -
           static_cast<size_t>(busyThreadCount);
}
...
struct ShardRankResult {
    HcclResult ret{
        HCCL_SUCCESS}; /**< 本卡校验返回码，HCCL_E_MEMORY 表示发现并发冲突。 */
    bool processed{
        false}; /**< 本卡是否真正执行过分片校验（失败早退时部分卡会被跳过）。 */
 ...
 */
void ProcessShardRank(const ReachabilityShardBuilder &builder,
                      size_t shardIndex, size_t shardCount,
                      const ShardWorkItem &item, ShardRankResult &result) {
    ...
    HCCL_VM_INFO(
        ...
        shardIndex, shardCount, item.deviceId, item.targetNodeIds.size(),
    ...
    LogMemConflictStageTime("GenReachabilityClosure(Sharded)",
                            ret == HCCL_SUCCESS ? "success" : "failed",
                            closureStart);
    ...
    const auto checkStart = MemConflictClock::now();
    ret = CheckCollectedConflictCandidates(*item.candidates, closure,
                                           result.stats);
    LogMemConflictStageTime("CheckCollectedConflictCandidates(Sharded)",
                            ret == HCCL_SUCCESS ? "success" : "failed",
                            checkStart);
    result.ret = ret;
 ...
 */
HcclResult
CheckCandidatesWithShardedReachability(const TaskNode *start,
                                       const ConflictCandidates &candidates,
                                       MemConflictCheckStats &stats) {
    ...
    HCCL_VM_INFO(
        ...
        candidates.size(), REACHABILITY_SHARD_NODE_THRESHOLD, beginMemory.rssKb,
        beginMemory.hwmKb);
    ...
    LogMemConflictStageTime("ReachabilityShardBuilderInit(Sharded)",
                            ret == HCCL_SUCCESS ? "success" : "failed",
                            builderStart);
    if (ret != HCCL_SUCCESS) {
        return ret;
    ...
    HCCL_VM_INFO(
        "Mem conflict shard plan, graphNodeCount={}, matrixGenerateCount={}",
        builder.GetNodeCount(), candidatesByDevice.size());
    ...
    for (const auto &deviceItem : candidatesByDevice) {
        const auto targetSetIter =
            targetNodeIdSetByDevice.find(deviceItem.first);
        if (targetSetIter == targetNodeIdSetByDevice.end()) {
            return HCCL_E_INTERNAL;
        }
        ShardWorkItem item;
        item.deviceId = deviceItem.first;
        item.targetNodeIds.assign(targetSetIter->second.begin(),
                                  targetSetIter->second.end());
    ...
    if (shardCount > SHARDED_REACHABILITY_PARALLEL_RANK_THRESHOLD) {
        ...
        workerThreadCount = std::min(std::min(idleThreadCount, shardCount),
                                     SHARDED_REACHABILITY_MAX_WORKER_THREADS);
    }
    // 线程数日志：workerThreadCount
    // 为参与干活的线程总数（含主线程），实际额外创建线程数为其减一，
    // 硬件线程数与空余线程数用于核对线程数是如何动态决定的。
    HCCL_VM_INFO("Mem conflict sharded reachability thread plan, rankCount={}, "
                 "workerThreadCount={}, "
                 "createdThreadCount={}, hardwareThreadCount={}, "
                 "idleThreadCount={}, parallelRankThreshold={}, "
                 "maxWorkerThreadCount={}",
                 shardCount, workerThreadCount, workerThreadCount - 1U,
                 std::thread::hardware_concurrency(), idleThreadCount,
                 SHARDED_REACHABILITY_PARALLEL_RANK_THRESHOLD,
                 SHARDED_REACHABILITY_MAX_WORKER_THREADS);
    ...
    if (workerThreadCount > 1U) {
        ...
        const auto bindCpuForSlot = [&bindCpuList](size_t slotIndex) -> int {
            return bindCpuList.empty()
                       ? -1
                       : bindCpuList[slotIndex % bindCpuList.size()];
        };
        if (!bindCpuList.empty()) {
            // 绑核计划日志：bindCpuCount 即可绑定的 CPU 数（物理核优先排序），
            // preferredCpuList 前若干个即各线程槽位实际使用的 CPU。
            HCCL_VM_INFO("Mem conflict sharded reachability core bind plan, "
                         "workerThreadCount={}, "
                         "bindCpuCount={}, preferredCpuList=[{}]",
                         workerThreadCount, bindCpuList.size(),
                         DescribeCpuList(bindCpuList));
        ...
        auto workerFunc = [&builder, &workItems, &shardResults, &nextShardIndex,
                           &hasFailure, shardCount]() {
            while (!hasFailure.load(std::memory_order_relaxed)) {
                const size_t shardIndex =
                    nextShardIndex.fetch_add(1U, std::memory_order_relaxed);
                if (shardIndex >= shardCount) {
                    return;
                }
                ProcessShardRank(builder, shardIndex + 1U, shardCount,
                                 workItems[shardIndex],
                                 shardResults[shardIndex]);
        ...
        for (size_t threadIndex = 1U; threadIndex < workerThreadCount;
             ++threadIndex) {
        ...
        for (size_t shardIndex = 0; shardIndex < shardCount; ++shardIndex) {
            ProcessShardRank(builder, shardIndex + 1U, shardCount,
                             workItems[shardIndex], shardResults[shardIndex]);
            if (shardResults[shardIndex].ret != HCCL_SUCCESS) {
                break;
            }
        }
    ...
    for (const ShardRankResult &shardResult : shardResults) {
        ...
        ++processedShardCount;
        stats.orderedCandidatePairCount +=
            shardResult.stats.orderedCandidatePairCount;
        stats.parallelCandidatePairCount +=
            shardResult.stats.parallelCandidatePairCount;
    ...
    HCCL_VM_INFO("Mem conflict sharded reachability path finished, "
                 "matrixGenerateCount={}, "
                 "candidatePairCount={}, rssKb={}, hwmKb={}",
                 processedShardCount, candidates.size(), finishedMemory.rssKb,
                 finishedMemory.hwmKb);
    return HCCL_SUCCESS;
}

HcclResult
CollectMemoryAccessBuckets(const std::vector<const TaskNode *> &nodes,
    ...
    for (const TaskNode *node : nodes) {
        ...
        for (const MemoryAccess &access : currentAccesses) {
            ...
            accessCountByTaskTypeAndKind[std::string(
                                             TaskTypeName(node->GetType())) +
                                         ":" + AccessKindName(access.kind)]++;
            accessCountByMemType[DescribeMemType(access.key.memType)]++;
            accessCountByMemTypeAndKind[std::string(DescribeMemType(
                                            access.key.memType)) +
                                        ":" + AccessKindName(access.kind)]++;
    ...
    LogCountBreakdown("AccessCountByTaskTypeAndKind",
                      accessCountByTaskTypeAndKind);
    LogCountBreakdown("AccessCountByMemType", accessCountByMemType);
    LogCountBreakdown("AccessCountByMemTypeAndKind",
...
HcclResult CheckDataMoveTaskMemConflict(const TaskNode *start,
                                        MemConflictCheckStats *stats) {
    ...
    ret = ScanMemoryBuckets(accessBuckets, needReachabilityClosure, candidates,
                            localStats, overlapCandidateStats);
    RefreshOverlapCandidateStats(overlapCandidateStats, localStats);
    LogCountBreakdown("OverlapCandidatePairsByBucket",
                      overlapCandidateStats.pairCountByBucket);
    LogCountBreakdown("OverlapCandidatePairsByTaskTypePair",
                      overlapCandidateStats.pairCountByTaskTypePair);
    LogMemConflictStageTime("CollectReachableTaskNodesToScanMemoryBuckets",
                            ret == HCCL_SUCCESS ? "success" : "failed",
                            scanLineStart);
    if (ret != HCCL_SUCCESS) {
    ...
    if (needReachabilityClosure) {
        if (localStats.nodeCount > REACHABILITY_SHARD_NODE_THRESHOLD) {
            ...
            ret = CheckCandidatesWithShardedReachability(start, candidates,
                                                         localStats);
        } else {
            ret = CheckCandidatesWithDenseReachability(start, candidates,
                                                       localStats);
        ...
        LogMemConflictStageTime("GenReachabilityClosure", "skipped",
                                MemConflictClock::now());
    }
