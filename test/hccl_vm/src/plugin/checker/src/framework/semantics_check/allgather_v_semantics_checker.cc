...
namespace HcclSim {
HcclResult
TaskCheckAllGatherVSemantics(const PhysicalMemorySemantics &memorySemantics,
    ...
    for (uint32_t i = 0; i < rankSize; i++) {
        ...
        uint64_t curLength = curCounts * CHECK_SIZE_TABLE[vDataDes.dataType];
    ...
    for (const auto &[deviceId, mem] : memorySemantics) {
        ...
        for (const auto &[eleAddr, original] : mem) {
            ...
            if (ele.startAddr >= outputBase + outputSize) {
                ...
                if (outputBufferSize != 0 && ele.size != 0) {
                    HCCL_VM_ERROR(
                        "{} AllGatherV output for device {} contains "
                        "unexpected extra data: range "
            ...
            const uint64_t inputSize = vDataDes.counts[curRankIdx] *
        ...
        if (totalSize != outputSize) {
            HCCL_VM_ERROR("{} AllGatherV output for device {} ends too early: "
