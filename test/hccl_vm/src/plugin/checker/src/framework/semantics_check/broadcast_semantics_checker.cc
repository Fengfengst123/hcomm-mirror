...
namespace HcclSim {
HcclResult
TaskCheckBroadcastSemantics(const PhysicalMemorySemantics &memorySemantics,
    ...
    for (const auto &[deviceId, mem] : memorySemantics) {
        // Broadcast 是就地语义：最终结果落在各设备的 INPUT 缓冲区，结果窗口以
        // INPUT 布局为准。
        uint64_t resultBase = 0;
        uint64_t resultBufferSize = 0;
        GetSemanticBufferExtent(bufferLayouts, deviceId, BufferType::INPUT,
                                resultBase, resultBufferSize);
        uint64_t totalSize = 0;
        for (const auto &[eleAddr, original] : mem) {
            BufferSemantic ele = original;
            if (!ClipSemanticToBuffer(ele, resultBase, resultBufferSize)) {
                continue;
            }
            if (ele.startAddr >= resultBase + dataSize) {
                // 已裁剪到注册缓冲区；超出期望结果大小的非空语义段仍应报错。
                // 注册大小未知时保留原有兼容行为。
                if (resultBufferSize != 0 && ele.size != 0) {
                    HCCL_VM_ERROR(
                        "{} Broadcast output for device {} contains unexpected "
                        "extra data: range "
                        "[0x{:x},0x{:x}) starts beyond the expected result "
                        "size 0x{:x} but inside the "
                        "result buffer (size 0x{:x}).\nCurrent result range "
                        "detail:\n{}",
                        MakeErrorCodeText(ErrorCode::SEMANTIC_FINAL_SIZE_ERROR),
                        deviceId, ele.startAddr, ele.startAddr + ele.size,
                        dataSize, resultBufferSize, ele.Describe());
                    return HcclResult::HCCL_E_PARA;
                }
                break;
            }
            const uint64_t rangeEnd = ele.startAddr + ele.size;
            if (ele.startAddr != resultBase + totalSize) {
                HCCL_VM_ERROR(
                    "{} Broadcast output for device {} should continue at "
                    "0x{:x}, "
                    "but the next actual range starts at 0x{:x} (actual range: "
                    "[0x{:x},0x{:x}))."
                    "\nCurrent result range detail:\n{}",
                    MakeErrorCodeText(ErrorCode::SEMANTIC_FINAL_MISSING),
                    deviceId, resultBase + totalSize, ele.startAddr,
            ...
            if (ele.srcBufs.size() != 1) {
                HCCL_VM_ERROR(
                    "{} Broadcast output range [0x{:x},0x{:x}) for device {} "
                    "should "
        ...
        if (totalSize != dataSize) {
            HCCL_VM_ERROR("{} Broadcast output for device {} ends too early. "
                          "The checker has "
