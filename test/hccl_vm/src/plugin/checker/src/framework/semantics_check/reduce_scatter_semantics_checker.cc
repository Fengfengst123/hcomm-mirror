...
namespace HcclSim {
HcclResult
TaskCheckReduceScatterSemantics(const PhysicalMemorySemantics &memorySemantics,
    ...
    for (const auto &[deviceId, mem] : memorySemantics) {
        ...
        for (const auto &[eleAddr, original] : mem) {
            ...
            if (ele.startAddr >= outputBase + dataSize) {
                ...
                if (outputBufferSize != 0 && ele.size != 0) {
                    HCCL_VM_ERROR(
                        "{} ReduceScatter output for device {} contains "
        ...
        if (totalSize != dataSize) {
            HCCL_VM_ERROR("{} ReduceScatter output for device {} ends too "
