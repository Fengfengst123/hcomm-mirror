 ...
 * for the full text of the License. Description: ccu representation base header
 * file Create: 2025-02-18
 */
...
namespace CcuRep {
    struct TransDep {
        ...
        uint16_t reserveChannalId[2]; //  0: selfLoopBack; 1: inter die,
                                      //  0xffff为无效值，rep翻译时检查
        uint64_t xnBaseAddr;
    ...
    public:
        explicit CcuRepBase();
        virtual ~CcuRepBase();
        virtual bool Translate(hcomm::CcuRep::CcuInstr*& instr, uint16_t& instrId, const TransDep& dep) = 0;
