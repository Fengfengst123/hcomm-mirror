...
#define HCCL_LOG_MASK                                                          \
    (static_cast<int32_t>(HCCL) | static_cast<int32_t>(RUN_LOG_MASK))

#define HCCL_LOG_PRINT(moduleId, logType, format, ...)                         \
    do {                                                                       \
        LOG_FUNC(moduleId, logType, "[%s:%d] [%u]" format, __FILE__, __LINE__, \
...
#define HCCL_ERROR_LOG_PRINT(format, ...)                                      \
    do {                                                                       \
        if (IsErrorToWarn()) {                                                 \
            LOG_FUNC(HCCL_LOG_MASK, HCCL_LOG_WARN,                             \
                     "[%s:%d] [%u]ErrToWarn: " format, __FILE__, __LINE__,     \
                     syscall(SYS_gettid), ##__VA_ARGS__);                      \
        } else {                                                               \
            LOG_FUNC(HCCL, HCCL_LOG_ERROR, "[%s:%d] [%u]" format, __FILE__,    \
                     __LINE__, syscall(SYS_gettid), ##__VA_ARGS__);            \
        }                                                                      \
    } while (0)
...
#define HCCL_RUN_WARNING(format, ...)                                          \
    do {                                                                       \
        if (LIKELY(HcclCheckLogLevel(HCCL_LOG_WARN, HCCL_LOG_MASK))) {         \
            HCCL_LOG_PRINT(HCCL_LOG_MASK, HCCL_LOG_WARN, format,               \
                           ##__VA_ARGS__);                                     \
        }                                                                      \
    } while (0)
...
#define HCCL_ERROR_CODE(error)                                                 \
    ((SYSTEM_RESERVE_ERROR << 32) + (HCCL_MODULE_ID << 24) +                   \
     ((static_cast<u64>(HcclSubModuleID::LOG_SUB_MODULE_ID_HCCL)) << 16) +     \
     static_cast<u64>(error))
#define HCOM_ERROR_CODE(error)                                                 \
    ((SYSTEM_RESERVE_ERROR << 32) + (HCCL_MODULE_ID << 24) +                   \
     ((static_cast<u64>(HcclSubModuleID::LOG_SUB_MODULE_ID_HCOM)) << 16) +     \
     static_cast<u64>(error))
#endif
...
#define CHK_SMART_PTR_NULL(smart_ptr)                                          \
    do {                                                                       \
        if (UNLIKELY(!(smart_ptr))) {                                          \
            HCCL_ERROR(                                                        \
                "[%s]errNo[0x%016llx] ptr [%s] is nullptr, return HCCL_E_PTR", \
                __func__, HCCL_ERROR_CODE(HCCL_E_PTR), #smart_ptr);            \
            return HCCL_E_PTR;                                                 \
        }                                                                      \
    } while (0)
...
#define CHK_SMART_PTR_RET_NULL(smart_ptr)                                      \
    do {                                                                       \
        if (UNLIKELY(!(smart_ptr))) {                                          \
            HCCL_ERROR("[%s]errNo[0x%016llx]smart_ptr is nullptr.", __func__,  \
    ...
    do {                                                                       \
        ...
        if (UNLIKELY(hcclRet != HCCL_SUCCESS)) {                               \
            if (hcclRet == HCCL_E_AGAIN) {                                     \
                HCCL_WARNING("[%s]call trace: hcclRet -> %d", __func__,        \
                             hcclRet);                                         \
            } else {                                                           \
                HCCL_ERROR("[%s]call trace: hcclRet -> %d", __func__,          \
                           hcclRet);                                           \
            }                                                                  \
            return hcclRet;                                                    \
        }                                                                      \
...
#define CHK_RET_AND_PRINT_IDE(call, identifier)                                \
    do {                                                                       \
        HcclResult hcclRet = call;                                             \
        if (UNLIKELY(hcclRet != HCCL_SUCCESS)) {                               \
            HCCL_RUN_INFO("[HCCL_TRACE]%s identifier[%s]", __func__,           \
                          identifier);                                         \
            if (hcclRet == HCCL_E_AGAIN) {                                     \
                HCCL_WARNING("[%s]call trace: hcclRet -> %d", __func__,        \
                             hcclRet);                                         \
...
#define CHK_SAFETY_FUNC_RET(call)                                              \
    do {                                                                       \
        s32 ret = call;                                                        \
        if (UNLIKELY(ret != EOK)) {                                            \
            HCCL_ERROR("[%s]call trace: safety func err ret -> %d", __func__,  \
                       ret);                                                   \
