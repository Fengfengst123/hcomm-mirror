...
std::string CcuExecutorBase::ParseMSList() {
    ...
    for (uint16_t index = 0; index < hcomm::CcuRep::CCU_REDUCE_MAX_MS;
         index++) {
    ...
    for (uint16_t i = 0; i < count + 2; i++) { // 循环范围 0~count + 2
        if (i == count + 1) {
            res += std::to_string(msId[i] / 0x8000) + ":" +
                   std::to_string(msId[i] % 0x8000) + "]";
        } else {
            res += std::to_string(msId[i] / 0x8000) + ":" +
                   std::to_string(msId[i] % 0x8000) + ", ";
        }
...
void CcuExecutorBase::SetCkeSignal(CcuResourceManager &ccuResMgr,
    ...
    HCCL_VM_DEBUG("success, locCcu[{}:{}], SetCKE[{}:{:04x}], value[{} --> {}]",
                  rankId_, dieId_, setCKEId, setCKEMask, setCKE, newSetCKE);
}

void CcuExecutorBase::SetRmtCKESignal(CcuResourceManager &ccuResMgr,
                                      int rmtRank, int rmtDie,
    ...
    HCCL_VM_DEBUG(
        "success, ccu[{}:{} --> {}:{}], SetRmtCKE[{}:{:04x}], value[{} --> {}]",
        rankId_, dieId_, rmtRank, rmtDie, setRmtCKEId, setRmtCKEMask, rmtCKE,
        newRmtCKE);
}

void CcuExecutorBase::ClearCkeSignal(CcuResourceManager &ccuResMgr,
                                     uint16_t clearCKEId, uint16_t clearMask) {
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        HCCL_VM_DEBUG("locCcu[{}:{}], Get cke id offset = [{}]", rankId_,
                      dieId_, ckeOffset);
    }
    ...
    HCCL_VM_DEBUG(
        "success, locCcu[{}:{}], ClearCKE[{}:{:04x}], value[{} --> {}]",
        rankId_, dieId_, clearCKEId, clearMask, setCKE, newSetCKE);
}

uint16_t CcuExecutorBase::UpdateXnId(uint16_t xnIdField) {
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        auto xnIdOffset = ccuSimulator_->GetLoopXnIdOffset();
        HCCL_VM_DEBUG(", locCcu[{}:{}], Get cke id offset = [{:x}]", rankId_,
                      dieId_, xnIdOffset);
...
uint64_t CcuExecutorBase::UpdateAddressWithoutStride(uint64_t addr) {
    return addr +
           ccuSimulator_->GetCurLoopCnt() * ccuSimulator_->GetLoopIterStepGSA();
}

uint64_t CcuExecutorBase::UpdateAddress(uint64_t addr,
                                        uint16_t addrExpandCoef) {
    return addr +
           ((ccuSimulator_->GetLoopExtendNum() * ccuSimulator_->GetGSAOffset())
            << addrExpandCoef) +
           ((ccuSimulator_->GetCurLoopCnt() *
             ccuSimulator_->GetLoopIterStepGSA())
            << addrExpandCoef);
}

void CcuExecutorBase::WaitCkeProcess(uint16_t waitCKEId, uint16_t waitCKEMask,
                                     uint16_t clearType,
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        HCCL_VM_DEBUG("instr[{}], locCcu[{}:{}], Get cke id offset = [{:x}]",
                      instrName, rankId_, dieId_, ckeOffset);
    }
        ...
        if ((waitCKE & waitCKEMask) == waitCKEMask) {
            ...
            if (duration.count() >= 1000) {
                HCCL_VM_DEBUG(
                    ...
                    instrName, rankId_, dieId_, newWaitCKEId, waitCKEMask,
                    waitCKEMask, (waitCKE & waitCKEMask), waitCKE);
                oldTime = currentTime;
            }
