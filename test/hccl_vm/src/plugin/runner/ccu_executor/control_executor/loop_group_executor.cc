...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::CTRL_TYPE, SimCcuV1::LOOPGROUP_CODE,
                             LoopGroupExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::CTRL_TYPE, SimCcuV2::LOOPGROUP_CODE,
                                LoopGroupExecutor);
...
void LoopGroupExecutor::RunV2() {
    ...
    ccuSimulator_->InitLoopGroupInfoV2(startLoopInstrId_, xnValue, xmValue,
                                       xpValue);
...
std::string LoopGroupExecutor::Describe() {
    return HcclSim::StringFormat("[Simulation Execute] LoopGroup From "
                                 "startLoopInstrId[%u] with loopGroupXn[%u], "
                                 "offsetXn[%u] and highPerfModeEn[%u]\n",
                                 startLoopInstrId_, xnId_, xmId_,
                                 highPerfModeEn_);
}
...
CcuTrace::CcuInstrTraceDetail LoopGroupExecutor::CollectTraceDetail() {
    ...
    auto &ccuResMgr = CcuResourceManager::GetInstance();
    if (version_ == RunnerCcuVersion::CCU_V1) {
        detail.args["offsetCfg"] =
            std::to_string(ccuResMgr.GetXnValue(rankId_, dieId_, xmId_));
        detail.args["repeatCfg"] =
            std::to_string(ccuResMgr.GetXnValue(rankId_, dieId_, xnId_));
    } else {
        detail.args["xnValue"] = std::to_string(
            ccuResMgr.GetXnValue(rankId_, dieId_, GetXnId(xnId_)));
        detail.args["xmValue"] = std::to_string(
            ccuResMgr.GetXnValue(rankId_, dieId_, GetXnId(xmId_)));
        detail.args["xpValue"] = std::to_string(
            ccuResMgr.GetXnValue(rankId_, dieId_, GetXnId(xpId_)));
    }
    return detail;
}
