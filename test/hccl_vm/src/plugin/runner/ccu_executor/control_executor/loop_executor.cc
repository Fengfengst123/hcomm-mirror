...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::CTRL_TYPE, SimCcuV1::LOOP_CODE,
                             LoopExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::CTRL_TYPE, SimCcuV2::LOOP_CODE,
                                LoopExecutor);
...
void LoopExecutor::RunV1() {
    uint64_t xnValue =
        CcuResourceManager::GetInstance().GetXnValue(rankId_, dieId_, xnId_);
    uint16_t loopCnt = xnValue & 0x1FFF; // 0x1FFF: 取低13位
    // [搬运任务]用到的GSA地址的偏移步长
    uint32_t gsaAddrStep =
        (xnValue >> 13) & 0xFFFFFFFF; // 13: 右移13位 0xFFFFFFFF: 取低32位

    ccuSimulator_->InitLoopInfo(startInstrId_, endInstrId_, loopCnt,
                                gsaAddrStep);
...
void LoopExecutor::RunV2() {
    ...
    ccuSimulator_->InitLoopInfoV2(startInstrId_, endInstrId_, xnValue, xmValue,
                                  xpValue);
    ccuSimulator_->ExecuteLoop();
}
...
CcuTrace::CcuInstrTraceDetail LoopExecutor::CollectTraceDetail() {
    ...
    if (version_ == RunnerCcuVersion::CCU_V1) {
        ...
        detail.args["gsaAddrStep"] =
            std::to_string((xnValue >> 13) & 0xFFFFFFFF);
    } else {
        uint16_t xnId = GetXnId(xnId_);
        uint16_t xmId = GetXnId(xmId_);
        uint16_t xpId = GetXnId(xpId_);
        detail.args["xnValue"] =
            std::to_string(ccuResMgr.GetXnValue(rankId_, dieId_, xnId));
        detail.args["xmValue"] =
            std::to_string(ccuResMgr.GetXnValue(rankId_, dieId_, xmId));
        detail.args["xpValue"] =
            std::to_string(ccuResMgr.GetXnValue(rankId_, dieId_, xpId));
    }
    return detail;
}
