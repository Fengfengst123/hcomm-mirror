...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::CTRL_TYPE, SimCcuV1::JMP_CODE,
                             JumpExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::CTRL_TYPE, SimCcuV2::JMP_CODE,
                                JumpExecutor);
...
void JumpExecutor::RunV2() {
    ...
    uint64_t expectedValue =
        ccuResMgr.GetXnValue(rankId_, dieId_, expectedXnId);
    uint64_t conditionValue =
        ccuResMgr.GetXnValue(rankId_, dieId_, conditionXnId);
    uint64_t relTarInstrValue =
        ccuResMgr.GetXnValue(rankId_, dieId_, relTarInstrXnId);
    uint64_t nextInsIdx = 0;
    auto curInstrId = ccuSimulator_->GetCurInstrId();
    auto updateNextInsIdx = [this, curInstrId, relTarInstrValue, &nextInsIdx,
                             &ccuResMgr](bool isJump) -> void {
        if (isJump) {
            if (jumpMode_ == 0) {
                // 相对跳转
    ...
    case 0: // 等于
        ...
        HCCL_VM_INFO("When conditionX{}{} equal to expectData{}{}, Jump "
    ...
    case 1: // 不等于
        updateNextInsIdx(conditionValue != expectedValue);
        HCCL_VM_INFO("When conditionX{}{} not equal to expectData{}{}, Jump "
                     "Mode{}, Jump instruction offset{}{}",
                     conditionXnId, conditionValue, expectedXnId, expectedValue,
                     jumpMode_, relTarInstrXnId, relTarInstrValue);
        break;
    case 2: // 大于
        updateNextInsIdx(conditionValue > expectedValue);
        HCCL_VM_INFO("When conditionX{}{} greater than expectData{}{}, Jump "
                     "Mode{}, Jump instruction offset{}{}",
                     conditionXnId, conditionValue, expectedXnId, expectedValue,
                     jumpMode_, relTarInstrXnId, relTarInstrValue);
        break;
    case 3: // 大于/等于
        updateNextInsIdx(conditionValue >= expectedValue);
        HCCL_VM_INFO("When conditionX{}{} greater than or equal to "
    ...
    case 4: // 小于
        updateNextInsIdx(conditionValue < expectedValue);
        HCCL_VM_INFO("When conditionX{}{} less than expectData{}{}, Jump "
                     "Mode{}, Jump instruction offset{}{}",
                     conditionXnId, conditionValue, expectedXnId, expectedValue,
                     jumpMode_, relTarInstrXnId, relTarInstrValue);
        break;
    case 5: // 小于/等于
        updateNextInsIdx(conditionValue <= expectedValue);
        HCCL_VM_INFO("When conditionX{}{} less than or equal to "
                     "expectData{}{}, Jump Mode{}, Jump instruction offset{}{}",
                     conditionXnId, conditionValue, expectedXnId, expectedValue,
                     jumpMode_, relTarInstrXnId, relTarInstrValue);
        break;
    default: // 无条件跳转
        ...
        HCCL_VM_INFO("When conditionX{}{} is true, Jump Mode{}, Jump "
                     "instruction offset{}{}",
                     conditionXnId, conditionValue, jumpMode_, relTarInstrXnId,
                     relTarInstrValue);
        break;
    }
}
...
CcuTrace::CcuInstrTraceDetail JumpExecutor::CollectTraceDetail() {
    ...
    auto &ccuResMgr = CcuResourceManager::GetInstance();
    if (version_ == RunnerCcuVersion::CCU_V1) {
        detail.args["targetInstrId"] = std::to_string(
            ccuResMgr.GetXnValue(rankId_, dieId_, dstInstrXnId_));
        detail.args["conditionValue"] = std::to_string(
            ccuResMgr.GetXnValue(rankId_, dieId_, conditionXnId_));
    } else {
        detail.args["expectedValue"] = std::to_string(
            ccuResMgr.GetXnValue(rankId_, dieId_, GetXnId(expectedXnId_)));
        detail.args["conditionValue"] = std::to_string(
            ccuResMgr.GetXnValue(rankId_, dieId_, GetXnId(conditionXnId_)));
        detail.args["relTarInstrValue"] = std::to_string(
            ccuResMgr.GetXnValue(rankId_, dieId_, GetXnId(relTarInstrXnId_)));
    }
    return detail;
}
