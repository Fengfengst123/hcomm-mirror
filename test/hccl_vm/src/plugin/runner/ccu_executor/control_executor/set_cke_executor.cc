...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::CTRL_TYPE, SimCcuV1::SETCKE_CODE,
                             SetCkeExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::CTRL_TYPE, SimCcuV2::SETCKBIT_CODE,
                                SetCkeExecutor);
...
std::string SetCkeExecutor::Describe() {
    return HcclSim::StringFormat("[Simulation Execute] Wait CKE[%u:%04x], Set "
                                 "CKE[%u:%04x], clearType[%u]\n",
                                 waitCKEId_, waitCKEMask_, setCKEId_,
