...
#ifndef HCCL_SIM_CCU_RESOURCE_V2_H
#define HCCL_SIM_CCU_RESOURCE_V2_H
...
class CcuResourceV2 {
  public:
    CcuResourceV2(int rankId, uint32_t rankSize);
    ~CcuResourceV2() = default;
    void Reset();
  ...
  private:
    ...
    std::array<uint64_t, SimCcuV2::CCU_RESOURCE_XN_MAX> xn_[HcclSim::DIE_NUM];
    std::array<uint64_t, SimCcuV2::CCU_RESOURCE_GSA_MAX> gsa_[HcclSim::DIE_NUM];
    std::array<uint64_t, SimCcuV2::CCU_RESOURCE_CKE_MAX> cke_[HcclSim::DIE_NUM];
...
#endif // HCCL_SIM_CCU_RESOURCE_V2_H
