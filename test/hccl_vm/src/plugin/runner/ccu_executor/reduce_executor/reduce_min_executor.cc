...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor -- reduce min
 * Author: caiyifan
 */
...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::REDUCE_TYPE, SimCcuV1::MIN_CODE,
                             ReduceMinExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::REDUCE_TYPE, SimCcuV2::REDUCEMIN_CODE,
                                ReduceMinExecutor);
...
void ReduceMinExecutor::Parser() {
    if (version_ == RunnerCcuVersion::CCU_V1) {
        ...
        (void)memcpy(msId_, instr_.v1.min.msId,
...
std::string ReduceMinExecutor::Describe() {
    return HcclSim::StringFormat(
        "[Simulation Execute] Wait CKE[%u:%04x], Min %s with Count[%u], "
        "DataType[%u] and "
