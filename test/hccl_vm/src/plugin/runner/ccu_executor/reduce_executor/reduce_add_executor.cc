...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::REDUCE_TYPE, SimCcuV1::ADD_CODE,
                             ReduceAddExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::REDUCE_TYPE, SimCcuV2::REDUCEADD_CODE,
                                ReduceAddExecutor);
...
void ReduceAddExecutor::Parser() {
    if (version_ == RunnerCcuVersion::CCU_V1) {
        ...
        (void)memcpy(msId_, instr_.v1.add.msId,
                     sizeof(uint16_t) * CCU_REDUCE_MAX_MS);
        ...
        (void)memcpy(msId_, instr_.v2.reduce.msId,
                     sizeof(uint16_t) * CCU_REDUCE_MAX_MS);
    } else {
...
void ReduceAddExecutor::Process(CcuResourceManager &ccuResMgr) {
    HCCL_VM_DEBUG("Reduce Add info, locCcu[{}:{}], count=[{}], castEn=[{}], "
                  "dataType=[{}]",
                  rankId_, dieId_, count_, castEn_, dataType_);
    for (uint32_t i = 0; i < CCU_REDUCE_MAX_MS; i++) {
        HCCL_VM_TRACE("msId_[{}]:dieId[{}], msId[{}]", i, msId_[i] >> 15,
                      msId_[i] & 0x7FFF);
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        HCCL_VM_DEBUG("locCcu[{}:{}], ms offset=[{}], ckeOffset[{}]", rankId_,
                      dieId_, msOffset, ckeOffset);
    }
    // 2. reduce操作
    ReduceAddDataType type = static_cast<ReduceAddDataType>(dataType_);
    HCCL_VM_INFO("ReduceAddExecutor::Process type {} count {} castEn {}",
                 static_cast<uint16_t>(type), count_, castEn_);
...
std::string ReduceAddExecutor::Describe() {
    return HcclSim::StringFormat(
        ...
        waitCKEId_, waitCKEMask_, ParseMSList().c_str(), count_, dataType_,
        castEn_, setCKEId_, setCKEMask_, clearType_);
}

CcuTrace::CcuInstrTraceDetail ReduceAddExecutor::CollectTraceDetail() {
    CcuTrace::CcuInstrTraceDetail detail;
    detail.typeName = "ReduceAdd";
    detail.args["reduceOp"] = "Add";
    return detail;
}
