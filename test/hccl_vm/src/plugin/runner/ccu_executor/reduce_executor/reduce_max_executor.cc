...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::REDUCE_TYPE, SimCcuV1::MAX_CODE,
                             ReduceMaxExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::REDUCE_TYPE, SimCcuV2::REDUCEMAX_CODE,
                                ReduceMaxExecutor);
...
void ReduceMaxExecutor::Parser() {
    if (version_ == RunnerCcuVersion::CCU_V1) {
        ...
        (void)memcpy(msId_, instr_.v1.max.msId,
...
void ReduceMaxExecutor::Process(CcuResourceManager &ccuResMgr) {
    HCCL_VM_DEBUG("Reduce Max info, locCcu[{}:{}], count:[{}], dataType:[{}]",
                  rankId_, dieId_, count_, dataType_);
    if (dataType_ >= ReduceMaxMinDataType::MAX_MIN_RESERVED4 ||
        dataType_ == ReduceMaxMinDataType::MAX_MIN_RESERVED1 ||
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        HCCL_VM_DEBUG("locCcu[{}:{}], ms offset=[{}]", rankId_, dieId_,
                      msOffset);
    }
    // 2. reduce操作
...
CcuTrace::CcuInstrTraceDetail ReduceMaxExecutor::CollectTraceDetail() {
    CcuTrace::CcuInstrTraceDetail detail;
    detail.typeName = "ReduceMax";
    detail.args["reduceOp"] = "Max";
    return detail;
}
