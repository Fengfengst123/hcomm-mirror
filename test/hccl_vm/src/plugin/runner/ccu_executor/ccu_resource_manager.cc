...
void CcuResourceManager::Init(
    int rankId, int rankSize, RunnerCcuVersion version,
    const std::vector<uint64_t> &ccuResourceBaseAddr) {
    ...
    if (ccuResData_.version == RunnerCcuVersion::CCU_V1) {
        ...
        ccuResData_.v1Res[rankId] =
            std::make_unique<CcuResourceV1>(rankId, rankSize);
        ...
        ccuResData_.v2Res[rankId] =
            std::make_unique<CcuResourceV2>(rankId, rankSize);
    } else {
        HCCL_VM_ERROR("ccu version {} not supported",
                      static_cast<int>(ccuResData_.version));
    }
    ccuResourceBaseAddr_ = ccuResourceBaseAddr;
}

void CcuResourceManager::InitInstrInfo(int rankId, int dieId,
                                       const CcuInstrData &ccuInstrInfo) {
...
bool CcuResourceManager::HasResource(int index) const {
    if (index < 0) {
        return false;
    }
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
        return static_cast<size_t>(index) < ccuResData_.v1Res.size() &&
               ccuResData_.v1Res[index] != nullptr;
    }
    if (version == RunnerCcuVersion::CCU_V2) {
        return static_cast<size_t>(index) < ccuResData_.v2Res.size() &&
               ccuResData_.v2Res[index] != nullptr;
    }
    return false;
}

void CcuResourceManager::InitChannelInfo(int rankId,
                                         const RankChannelInfo &channelInfo) {
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
...
uint64_t CcuResourceManager::GetSqeArgValue(int rankId, int dieId,
                                            uint16_t argId) const {
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
...
uint64_t CcuResourceManager::GetXnValue(int rankId, int dieId,
                                        uint16_t xnId) const {
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
...
uint64_t CcuResourceManager::GetGsaValue(int rankId, int dieId,
                                         uint16_t gsaId) const {
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
...
uint16_t CcuResourceManager::GetCkeValue(int rankId, int dieId,
                                         uint16_t ckeId) const {
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
...
std::pair<int, int> CcuResourceManager::GetRmtCcu(int rankId, int dieId,
                                                  uint16_t channelId) const {
    int rmtRank = S32_INVALID;
    int rmtDie = S32_INVALID;

    if (channelId >= SimCcuV1::MAX_CCU_CHANNEL_NUM) {
        HCCL_VM_ERROR("invalid channelId[{}], max={}", channelId,
                      SimCcuV1::MAX_CCU_CHANNEL_NUM);
        return std::make_pair(S32_INVALID, S32_INVALID);
    }

    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
        rmtRank = ccuResData_.v1Res[rankId]
                      ->channelId2RmtRankMap_[dieId][channelId]
                      .rankId;
        rmtDie = ccuResData_.v1Res[rankId]
                     ->channelId2RmtRankMap_[dieId][channelId]
                     .dieId;
    } else if (ccuResData_.version == RunnerCcuVersion::CCU_V2) {
        rmtRank = ccuResData_.v2Res[rankId]
                      ->channelId2RmtRankMap_[dieId][channelId]
                      .rankId;
        rmtDie = ccuResData_.v2Res[rankId]
                     ->channelId2RmtRankMap_[dieId][channelId]
                     .dieId;
    } else {
        HCCL_VM_ERROR("ccu version {} not supported",
    ...
    if (dieId >= HcclSim::DIE_NUM || rmtRank == S32_INVALID ||
        rmtDie == S32_INVALID) {
        HCCL_VM_ERROR("get invalid value, dieId{}, rmtRank{}, rmtDie{}", dieId,
                      rmtRank, rmtDie);
        return std::make_pair(S32_INVALID, S32_INVALID);
    }
...
void CcuResourceManager::UpdateXnValue(int rankId, int dieId, uint16_t xnId,
                                       uint64_t value) {
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
...
void CcuResourceManager::UpdateGsaValue(int rankId, int dieId, uint16_t gsaId,
                                        uint64_t value) {
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
...
void CcuResourceManager::UpdateCkeValue(int rankId, int dieId, uint16_t ckeId,
                                        uint16_t value) {
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
...
void CcuResourceManager::TransMemToMem(void *srcBuf, void *dstBuf,
                                       uint64_t length, bool reduceEn,
                                       uint16_t reduceOp, uint16_t dataType) {
    ...
    if (srcAddr == nullptr) {
        HCCL_VM_ERROR("无法获取srcBuf的设备地址(addr= {:x})！",
                      reinterpret_cast<uintptr_t>(srcBuf));
        return;
    }
    ...
    if (dstAddr == nullptr) {
        HCCL_VM_ERROR("无法获取dstBuf的设备地址(addr= {:x})！",
                      reinterpret_cast<uintptr_t>(dstBuf));
        return;
    }

    if (reduceEn) {
        ReduceProcess(srcAddr, dstAddr, length, dataType, reduceOp,
                      ccuResData_.version);
    } else {
...
void CcuResourceManager::TransMSToMS(int srcRank, int srcDie, int dstRank,
                                     int dstDie, uint16_t srcMsId,
                                     uint16_t dstMsId, uint16_t length) {
    memcpy(GetMsAddr(dstRank, dstDie, dstMsId),
           GetMsAddr(srcRank, srcDie, srcMsId), length);
}

bool CcuResourceManager::TransMSToMem(int rankId, int dieId, uint16_t msId,
    ...
    if (dstAddr == nullptr) {
        HCCL_VM_ERROR("无法获取buf的设备地址(addr= {:x})！",
...
bool CcuResourceManager::TransMemToXn(int rankId, int dieId, uint16_t xnId,
                                      uint64_t buf, uint16_t length) {
    sim::PhyMemBlock srcPhyMem{};
...
bool CcuResourceManager::TransXnToMem(int rankId, int dieId, uint16_t xnId,
                                      uint64_t buf, uint16_t length) {
    sim::PhyMemBlock dstPhyMem{};
    auto dstAddr = sim::AcquireDevPtrInNoHostProcess((void *)buf, dstPhyMem);
    if (dstAddr == nullptr) {
...
bool CcuResourceManager::TransMemToMS(int rankId, int dieId, uint16_t msId,
                                      void *buf, uint16_t length) {
    if (buf == nullptr) {
        HCCL_VM_ERROR("param invalid...");
        return false;
...
char *CcuResourceManager::GetMsAddr(int rankId, int dieId,
                                    uint16_t msId) const {
    auto version = ccuResData_.version;
    uint32_t offset =
        static_cast<uint32_t>(msId) * HcclSim::BYTE_NUM_4K; // 一个MS容量为4KB
    if (version == RunnerCcuVersion::CCU_V1) {
...
uint64_t *CcuResourceManager::GetXnAddr(int rankId, int dieId,
                                        uint16_t xnId) const {
    auto version = ccuResData_.version;
...
const uint64_t XN_MS_SIZE_A6 =
    4096; // 每个MS占 4KB（与 GenSliceFromMs 的 msId*4096 口径一致）
...
const uint64_t GROUP_TOTAL_MS =
    MS_INTERLEAVE_NUM * CCUA_NUM_A6; // 一组总共32个MS

const uint64_t CCU_MS_START_ADDR[4] = {XN_CCUA0_MS_ADDR_A6, XN_CCUA1_MS_ADDR_A6,
                                       XN_CCUA2_MS_ADDR_A6,
                                       XN_CCUA3_MS_ADDR_A6};

bool CcuResourceManager::GetMSIdByAddr(uint32_t dieId, uint64_t addr,
                                       uint16_t &msId) {
    ...
    if (ccuIndex == UINT64_MAX) {
        HCCL_VM_ERROR("msAddr is out of range, msAddr=[{}],addr=[{}]", msAddr,
                      addr);
        return false;
    }
    ...
    msId =
        groupIndex * GROUP_TOTAL_MS + ccuIndex * MS_INTERLEAVE_NUM + posInGroup;
    return true;
}
...
uint16_t findBaseSize(CcuComponerntType type) {
    if (type == CcuComponerntType::XN_A6 || type == CcuComponerntType::CKE_A6 ||
        ...
        return 8;
               ...
               type == CcuComponerntType::JETTY_A6 ||
...
// 通过XnId所在的地址值来找到XnId
bool CcuResourceManager::GetXnAndTypeIdByAddr(uint32_t dieId, uint64_t xnAddr,
                                              CcuComponerntType &type,
                                              uint16_t &xnId) {
    if (dieId >= ccuResourceBaseAddr_.size()) {
        HCCL_VM_ERROR("dieId is out of range, dieId=[{}]", dieId);
        return false;
    ...
    if (type == CcuComponerntType::UNKNOWN) {
        HCCL_VM_ERROR("unknown type, "
                      "addr=[{}],dieId=[{}],ccuResourceBaseAddr_[dieId]=[{}]",
                      xnAddr, dieId, ccuResourceBaseAddr_[dieId]);
        return false;
    }
    uint64_t baseAddr = findBaseAddr(type) + ccuResourceBaseAddr_[dieId];
    uint16_t sizeofXn = findBaseSize(type);
    xnId = static_cast<uint16_t>((xnAddr - baseAddr) /
                                 static_cast<uint64_t>(sizeofXn));
    return true;
}

// 通过XnId所在的地址值来找到XnId
bool CcuResourceManager::GetXnIdByAddr(uint32_t dieId, CcuComponerntType type,
...
// 通过XnId所在的地址值来找到XnId
bool CcuResourceManager::GetAddrByXnId(uint32_t dieId, CcuComponerntType type,
...
std::vector<hcomm::CcuRep::CcuInstr>
CcuResourceManager::GetInstrData(int rankId, int dieId) const {
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
...
std::string CcuResourceManager::GetInstrDescribe(int rankId, int dieId,
                                                 int instrId) const {
#ifndef DEVICE_STUB
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
        // return
        // hcomm::CcuRep::ParseInstr(&(ccuResData_.v1Res[rankId]->instrSpace_[dieId].instrData[instrId]));
    } else if (ccuResData_.version == RunnerCcuVersion::CCU_V2) {
        // return
        // hcomm::CcuRep::ParseInstr(&(ccuResData_.v2Res[rankId]->instrSpace_[dieId].instrData[instrId]));
    } else {
        HCCL_VM_ERROR("ccu version {} not supported",
                      static_cast<int>(version));
        return "";
...
CcuResourceManager::InitSimulator(int rankId, int dieId, uint16_t instrStartId,
                                  uint16_t endInstrId, uint16_t instCnt) {
    if (dieId < 0 || dieId >= HcclSim::DIE_NUM || !HasResource(rankId)) {
        HCCL_VM_ERROR("invalid CCU simulator resource: rankId={}, dieId={}",
                      rankId, dieId);
        return nullptr;
    }
    auto version = ccuResData_.version;
    if (version == RunnerCcuVersion::CCU_V1) {
        if (ccuResData_.v1Res[rankId]->simulators_[dieId] == nullptr) {
            ccuResData_.v1Res[rankId]->simulators_[dieId] =
                std::make_shared<CcuSimulator>(rankId, dieId, instrStartId,
                                               endInstrId, instCnt, version);
            return ccuResData_.v1Res[rankId]->simulators_[dieId];
        } else {
            ccuResData_.v1Res[rankId]->simulators_[dieId]->Init(
                instrStartId, endInstrId, instCnt, version);
            return ccuResData_.v1Res[rankId]->simulators_[dieId];
        }
        ...
        if (ccuResData_.v2Res[rankId]->simulators_[dieId] == nullptr) {
            ...
            ccuResData_.v2Res[rankId]->simulators_[dieId]->Init(
                instrStartId, endInstrId, instCnt, version);
            return ccuResData_.v2Res[rankId]->simulators_[dieId];
        }
    } else {
...
void CcuResourceManager::DumpCcuInstructions(int rankId) const {
    ...
    for (uint32_t dieId = 0; dieId < HcclSim::DIE_NUM; dieId++) {
        HCCL_VM_DEBUG("==========================print ccu instructions start: "
                      "rank[{}], dieId[{}]==========================",
                      rankId, dieId);
        auto instrInfoCnt = GetInstrCnt(rankId, dieId);
        HCCL_VM_DEBUG(
            "*******************rankId=[{}], dieId=[{}], instrCnt=[{}]", rankId,
            dieId, instrInfoCnt);

        for (uint32_t i = 0; i < instrInfoCnt; i++) {
            HCCL_VM_TRACE("ccu instruction info: {}: {}", i,
                          GetInstrDescribe(rankId, dieId, i));
        }
        HCCL_VM_DEBUG("==========================print ccu instructions end: "
                      "rank[{}], dieId[{}]==========================",
                      rankId, dieId);
    }
#endif
...
void CcuResourceManager::DumpCcuXnResouceInfo(int rankId) const {
    for (uint32_t dieId = 0; dieId < HcclSim::DIE_NUM; dieId++) {
        HCCL_VM_DEBUG("==========================All CCU XN Resouce Info "
                      "Start: rank[{}], dieId[{}]===========================",
                      rankId, dieId);
        HCCL_VM_DEBUG("-------------------------DieId[{}] XN Resouce Info "
                      "Start-------------------------",
                      dieId);
        ...
        HCCL_VM_DEBUG("-------------------------DieId[{}] XN Resouce Info "
                      "End-------------------------",
                      dieId);
        HCCL_VM_DEBUG("==========================All CCU XN Resouce Info End: "
                      "rank[{}], dieId[{}]===========================",
                      rankId, dieId);
    }
}
...
void CcuResourceManager::DumpCcuGsaResouceInfo(int rankId) const {
    for (uint32_t dieId = 0; dieId < HcclSim::DIE_NUM; dieId++) {
        HCCL_VM_DEBUG("==========================All CCU GSA Resouce Info "
                      "Start: rank[{}], dieId[{}]===========================",
                      rankId, dieId);
        HCCL_VM_DEBUG("-------------------------DieId[{}] GSA Resouce Info "
                      "Start-------------------------",
                      dieId);
        for (uint32_t gsaId = 0; gsaId < SimCcuV1::CCU_RESOURCE_GSA_MAX;
             gsaId++) {
        ...
        HCCL_VM_DEBUG("-------------------------DieId[{}] GSA Resouce Info "
                      "End-------------------------",
                      dieId);
        HCCL_VM_DEBUG("==========================All CCU GSA Resouce Info End: "
                      "rank[{}], dieId[{}]===========================",
                      rankId, dieId);
    }
}
...
void CcuResourceManager::DumpCcuCkeResouceInfo(int rankId) const {
    for (uint32_t dieId = 0; dieId < HcclSim::DIE_NUM; dieId++) {
        HCCL_VM_DEBUG("==========================All CCU CKE Resouce Info "
                      "Start: rank[{}], dieId[{}]===========================",
                      rankId, dieId);
        HCCL_VM_DEBUG("-------------------------DieId[{}] CKE Resouce Info "
                      "Start-------------------------",
                      dieId);
        for (uint32_t ckeId = 0; ckeId < SimCcuV1::CCU_RESOURCE_MS_NUM;
             ckeId++) {
        ...
        HCCL_VM_DEBUG("-------------------------DieId[{}] CKE Resouce Info "
                      "End-------------------------",
                      dieId);
        HCCL_VM_DEBUG("==========================All CCU CKE Resouce Info End: "
                      "rank[{}], dieId[{}]===========================",
                      rankId, dieId);
    }
}
...
void CcuResourceManager::DumpCcuChannelResouceInfo(int rankId) const {
    for (int dieId = 0; dieId < HcclSim::DIE_NUM; dieId++) {
        HCCL_VM_DEBUG("==========================All CCU CHANNEL Resouce Info "
                      "Start: rank[{}], dieId[{}]===========================",
                      rankId, dieId);
        HCCL_VM_DEBUG("-------------------------DieId[{}] CHANNEL Resouce Info "
                      "Start-------------------------",
                      dieId);
        for (uint32_t chId = 0; chId < SimCcuV1::MAX_CCU_CHANNEL_NUM; chId++) {
            ...
            if (rmtCcu.first != S32_INVALID && rmtCcu.second != S32_INVALID) {
                HCCL_VM_TRACE(
                    "[CHANNEL: Id = {}, Local[{}:{}] -> Remote[{}:{}]]", chId,
                    rankId, dieId, rmtCcu.first, rmtCcu.second);
            }
        }
        HCCL_VM_DEBUG("-------------------------DieId[{}] CHANNEL Resouce Info "
                      "End-------------------------",
...
void CcuResourceManager::DumpChannelId2RmtRank(int rankId, int dieId) const {
    ...
    for (uint32_t i = 0; i < SimCcuV1::MAX_CCU_CHANNEL_NUM; i++) {
        ...
        HCCL_VM_DEBUG("channelId[{}], ccu[{}:{} --> {}:{}]", i, rankId, dieId,
                      rmtCcu.first, rmtCcu.second);
    }
}
