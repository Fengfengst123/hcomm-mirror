...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor -- load gsa to xn
 * Author: caiyifan
 */
...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::LOAD_TYPE, SimCcuV1::LOADGSAXN_CODE,
                             LoadGsaXnExecutor);
...
std::string LoadGsaXnExecutor::Describe() {
    return HcclSim::StringFormat(
        "[Simulation Execute] Load GSA[%u] + Xn[%u] to GSA[%u]\n", gsAmId_,
        xnId_, gsAdId_);
}

CcuTrace::CcuInstrTraceDetail LoadGsaXnExecutor::CollectTraceDetail() {
    CcuTrace::CcuInstrTraceDetail detail;
    detail.typeName = "LoadGsaXn";
    auto &ccuResMgr = CcuResourceManager::GetInstance();
    detail.args["gsaVal"] =
        std::to_string(ccuResMgr.GetGsaValue(rankId_, dieId_, gsAmId_));
    detail.args["xnVal"] =
