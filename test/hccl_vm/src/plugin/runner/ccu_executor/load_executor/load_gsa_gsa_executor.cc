...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor -- load gsa to
 * gsa Author: caiyifan
 */
...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::LOAD_TYPE, SimCcuV1::LOADGSAGSA_CODE,
                             LoadGsaGsaExecutor);
...
std::string LoadGsaGsaExecutor::Describe() {
    return HcclSim::StringFormat(
        "[Simulation Execute] Load GSA[%u] + GSA[%u] to GSA[%u]\n", gsAmId_,
        gsAnId_, gsAdId_);
}
...
CcuTrace::CcuInstrTraceDetail LoadGsaGsaExecutor::CollectTraceDetail() {
    ...
    detail.args["gsa2"] =
        std::to_string(ccuResMgr.GetGsaValue(rankId_, dieId_, gsAnId_));
    return detail;
}
