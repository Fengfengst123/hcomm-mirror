 ...
 * for the full text of the License. Description: ccu executor -- Or Executor
 * Author: caiyifan
 */

#include "or_executor.h"
...
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::LOAD_TYPE, SimCcuV2::OR_CODE, OrExecutor);

void OrExecutor::Parser()
{
    ValidateVersionExclusive(RunnerCcuVersion::CCU_V2, "OrExecutor");
...
void OrExecutor::Run()
{
    ...
    HCCL_VM_INFO("Or Xn{}{} | Xm{}{} to Xd{}{}", xnId, xnValue, xmId, xmValue, xdId_, xdValue);
...
std::string OrExecutor::Describe()
{
    return HcclSim::StringFormat("[OrExecutor] xdId:[%u],xnId_[%u],xmId[%u]\n", xdId_, xnId_, xmId_);
}

CcuTrace::CcuInstrTraceDetail OrExecutor::CollectTraceDetail()
{
