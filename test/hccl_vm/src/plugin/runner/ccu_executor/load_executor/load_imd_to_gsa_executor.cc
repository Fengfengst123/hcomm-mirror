...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor -- load imd to
 * gsa Author: caiyifan
 */
...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::LOAD_TYPE, SimCcuV1::LOADIMDTOGSA_CODE,
                             LoadImdToGSAExecutor);
...
std::string LoadImdToGSAExecutor::Describe() {
    return HcclSim::StringFormat(
        "[Simulation Execute] Load immediate[%lu] to GSA[%u]\n", immediate_,
        gsaId_);
}
