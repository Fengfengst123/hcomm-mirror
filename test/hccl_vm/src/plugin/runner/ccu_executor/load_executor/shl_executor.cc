...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor -- Shl Executor
 * Author: caiyifan
 */
...
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::LOAD_TYPE, SimCcuV2::SHL_CODE,
                                ShlExecutor);
...
void ShlExecutor::Run() {
    ...
    case 0:
        ...
        HCCL_VM_INFO("Shl logical Xn{}{} << Xm{}{} to Xd{}{}", xnId, xnValue,
                     xmId, xmValue, xdId_, xdValue);
        break;
    case 1:
        xdValue = static_cast<int64_t>(xnValue) << xmValue;
        HCCL_VM_INFO("Shl arithmetic Xn{}{} << Xm{}{} to Xd{}{}", xnId, xnValue,
                     xmId, xmValue, xdId_, xdValue);
        break;
    case 2:
        xdValue = (xnValue << (xmValue & 0x3F)) |
                  (xnValue >> (64 - (xmValue & 0x3F)));
        HCCL_VM_INFO("Shl rotate Xn{}{} <<< Xm{}{} to Xd{}{}", xnId, xnValue,
                     xmId, xmValue, xdId_, xdValue);
        break;
    default:
