...
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::LOAD_TYPE, SimCcuV2::LOAD_CODE,
                                LoadExecutor);
...
void LoadExecutor::Run() {
    ...
    if (!ccuResMgr.TransMemToXn(rankId_, dieId_, xdValue, xsStartAddr,
                                length)) {
        ccuSimulator_->SetExecState(CcuExecState::EXEC_FAIL);
        return;
    }
