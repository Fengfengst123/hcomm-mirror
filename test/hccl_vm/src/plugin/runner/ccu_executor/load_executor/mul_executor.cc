...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor -- Mul Executor
 * Author: caiyifan
 */
...
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::LOAD_TYPE, SimCcuV2::MUL_CODE,
                                MulExecutor);
...
void MulExecutor::Run() {
    ...
    if (parMode_ == 0) {
        ...
        HCCL_VM_INFO("Mul Xn{}{} * immed{} to Xd{}{}", xnId_,
                     xnValue & 0xFFFFFFFF, xmId_, xdId_, xdValue);
    } else {
        uint64_t xmValue = ccuResMgr.GetXnValue(rankId_, dieId_, xmId);
        xdValue = (xnValue & 0xFFFFFFFF) * (xmValue & 0xFFFFFFFF);
        HCCL_VM_INFO("Mul Xn{}{} * Xm{}{} to Xd{}{}", xnId,
                     xnValue & 0xFFFFFFFF, xmId, xmValue & 0xFFFFFFFF, xdId_,
                     xdValue);
    }
