...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor -- Add Executor
 * Author: caiyifan
 */
...
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::LOAD_TYPE, SimCcuV2::ADD_CODE,
                                AddExecutor);
...
void AddExecutor::Run() {
    ...
    if (parMode_ == 0) {
        ...
        HCCL_VM_INFO("Add Xn{}{} + immed{} to Xd{}{}", xnId_, xnValue, xmId_,
                     xdId_, xdValue);
    } else {
        uint64_t xmValue = ccuResMgr.GetXnValue(rankId_, dieId_, xmId);
        xdValue = xnValue + xmValue;
        HCCL_VM_INFO("Add Xn{} {}+ Xm{}{} to Xd{}{}", xnId, xnValue, xmId,
