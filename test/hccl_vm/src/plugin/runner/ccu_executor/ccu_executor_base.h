...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor base header file
 * Author: caiyifan
 */
...
class CcuExecutorBase {
  public:
    CcuExecutorBase(int streamId, int rankId, int dieId,
                    const hcomm::CcuRep::CcuInstr &instr,
                    CcuSimulator *ccuSimulator)
        : streamId_(streamId), rankId_(rankId), dieId_(dieId), instr_(instr),
    ...
    void SetCkeSignal(CcuResourceManager &ccuResMgr, uint16_t setCKEId,
                      uint16_t setCKEMask);
    void SetRmtCKESignal(CcuResourceManager &ccuResMgr, int rmtRank, int rmtDie,
                         uint16_t setRmtCKEId, uint16_t setRmtCKEMask);
    void ClearCkeSignal(CcuResourceManager &ccuResMgr, uint16_t clearCKEId,
                        uint16_t clearMask);
    void WaitCkeProcess(uint16_t waitCKEId, uint16_t waitCKEMask,
                        uint16_t clearType, const std::string &instrName);
  ...
  protected:
    void ValidateVersion(RunnerCcuVersion expectedVersion,
                         const std::string &instrName) {
        if (version_ != expectedVersion) {
            HCCL_VM_ERROR("[{}] Version mismatch: expected={}, actual={}",
                          instrName, static_cast<int>(expectedVersion),
                          static_cast<int>(version_));
        }
    }

    void ValidateVersionExclusive(RunnerCcuVersion allowedVersion,
                                  const std::string &instrName) {
        if (version_ != allowedVersion) {
            HCCL_VM_ERROR(
                "[{}] Instruction is exclusive to CCU_V{}, but running on V{}",
                instrName, static_cast<int>(allowedVersion),
                static_cast<int>(version_));
        }
    }
