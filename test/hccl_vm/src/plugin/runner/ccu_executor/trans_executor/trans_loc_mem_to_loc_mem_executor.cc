...
REG_CCU_EXECUTOR_CREATE_FUNC_V1(SimCcuV1::TRANS_TYPE,
                                SimCcuV1::TRANSLOCMEMTOLOCMEM_CODE,
                                TransLocMemToLocMemExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::TRANS_TYPE,
                                SimCcuV2::TRANSLOCMEMTOLOCMEM_CODE,
                                TransLocMemToLocMemExecutor);
...
void TransLocMemToLocMemExecutor::Process(CcuResourceManager &ccuResMgr) {
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        setCKEId_ += ckeOffset;
        HCCL_VM_DEBUG(
            "locCcu[{}:{}], Get gsa addr offset = [{:x}], cke id offset = [{}]",
            rankId_, dieId_, gsaOffset, ckeOffset);
    }
    HCCL_VM_DEBUG("locCcu[{}:{}] Trans data from srcLocGSAId_[{}] "
                  "srcLocAddr[{:x}] to dstLocGSAId_[{}] "
                  "dstLocAddr[{:x}], with lengthXnId[{}] transLength[{}].",
                  rankId_, dieId_, srcGSAId_, srcLocAddr, dstGSAId_, dstLocAddr,
                  lengthXnId_, transLength_);
    ccuResMgr.TransMemToMem(reinterpret_cast<void *>(srcLocAddr),
                            reinterpret_cast<void *>(dstLocAddr), transLength_,
                            false, 0, 0);
...
void TransLocMemToLocMemExecutor::RunV2() {
    ...
    HCCL_VM_DEBUG(
        "Trans data from srcMemAddr[0x{:x}] to dstMemAddr[0x{:x}], length[{}].",
        srcLocMemAddr, dstLocMemAddr, len);

    ccuResMgr.TransMemToMem(reinterpret_cast<void *>(srcLocMemAddr),
                            reinterpret_cast<void *>(dstLocMemAddr), len, false,
                            0, 0);
...
std::string TransLocMemToLocMemExecutor::Describe() {
    if (version_ == RunnerCcuVersion::CCU_V1) {
        return HcclSim::StringFormat(
            ...
            waitCKEId_, waitCKEMask_, srcGSAId_, srcXnId_, dstGSAId_, dstXnId_,
            lengthXnId_, channelId_, setCKEId_, setCKEMask_, clearType_,
            lengthEn_);
    } else {
        return HcclSim::StringFormat(
            ...
            xdId_, xdtId_, xsId_, xstId_, xlId_, usedMSId_, msNum_, setCKEId_,
            setCKEMask_);
    }
}

CcuTrace::CcuInstrTraceDetail
TransLocMemToLocMemExecutor::CollectTraceDetail() {
