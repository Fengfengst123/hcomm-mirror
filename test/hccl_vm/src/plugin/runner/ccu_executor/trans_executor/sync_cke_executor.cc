...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::TRANS_TYPE, SimCcuV1::SYNCCKE_CODE,
                             SyncCkeExecutor);
...
void SyncCkeExecutor::Process(CcuResourceManager &ccuResMgr) {
    ...
    uint16_t rmtCke =
        ccuResMgr.GetCkeValue(rmtCcu.first, rmtCcu.second, rmtCKEId_);
    ...
    HCCL_VM_DEBUG("locCcu[{}:{}], channelId=[{}], "
                  ...
                  rankId_, dieId_, channelId_, rmtCcu.first, rmtCcu.second,
                  locCKEId_, locCKEMask_, locCKE, rmtCKEId_, locCKEMask_,
                  rmtCke, newRmtCke);
...
std::string SyncCkeExecutor::Describe() {
    return HcclSim::StringFormat(
        ...
        waitCKEId_, waitCKEMask_, locCKEId_, locCKEMask_, rmtCKEId_,
        locCKEMask_, channelId_, setCKEId_, setCKEMask_, clearType_);
}
