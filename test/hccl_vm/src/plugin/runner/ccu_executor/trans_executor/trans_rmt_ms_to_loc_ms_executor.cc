...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::TRANS_TYPE,
                             SimCcuV1::TRANSRMTMSTOLOCMS_CODE,
                             TransRmtMSToLocMSExecutor);
...
void TransRmtMSToLocMSExecutor::Process(CcuResourceManager &ccuResMgr) {
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        setCKEId_ += ckeOffset;
        HCCL_VM_DEBUG(
            "ccuId=[{}:{}], Get ms offset = [{:04x}], cke offset = [{:04x}]",
    ...
    HCCL_VM_DEBUG("ccuId=[{}:{}-{}:{}] Trans data "
                  ...
                  locMSId_, lengthXnId_, transLength_);
    ccuResMgr.TransMSToMS(rmtCcu.first, rmtCcu.second, rankId_, dieId_,
                          rmtMSId_, locMSId_, transLength_);
    // 5.设置本端的cke
...
std::string TransRmtMSToLocMSExecutor::Describe() {
    return HcclSim::StringFormat(
        ...
        locMSId_ / 0x8000, locMSId_ % 0x8000, lengthXnId_, channelId_,
