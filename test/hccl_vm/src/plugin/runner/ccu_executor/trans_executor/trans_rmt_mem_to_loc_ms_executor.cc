...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::TRANS_TYPE,
                             SimCcuV1::TRANSRMTMEMTOLOCMS_CODE,
                             TransRmtMemToLocMSExecutor);
...
void TransRmtMemToLocMSExecutor::Process(CcuResourceManager &ccuResMgr) {
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        locMSId_ += msOffset;
        setCKEId_ += ckeOffset;
        HCCL_VM_DEBUG("locCcu[{}:{}], Get gsa addr offset = [{:04x}], ms "
    ...
    HCCL_VM_DEBUG("locCcu[{}:{}] Trans data "
                  ...
                  rankId_, dieId_, rmtGSAId_, rmtAddr, locMSId_, lengthXnId_,
                  transLength_);
    bool ret =
        ccuResMgr.TransMemToMS(rankId_, dieId_, locMSId_,
...
std::string TransRmtMemToLocMSExecutor::Describe() {
    return HcclSim::StringFormat("[Simulation Execute] Wait CKE[%u:%04x], "
                                 "Trans RmtMem[%u:%u] To LocMS[%u:%u] With "
                                 "LengthXn[%u] Use Channel[%u], Set "
                                 "CKE[%u:%04x], clearType[%u], lengthEn[%u]\n",
                                 waitCKEId_, waitCKEMask_, rmtGSAId_, rmtXnId_,
                                 locMSId_ / 0x8000, locMSId_ % 0x8000,
                                 lengthXnId_, channelId_, setCKEId_,
                                 setCKEMask_, clearType_, lengthEn_);
}

CcuTrace::CcuInstrTraceDetail TransRmtMemToLocMSExecutor::CollectTraceDetail() {
    CcuTrace::CcuInstrTraceDetail detail;
    detail.typeName = "TransRmtMemToLocMS";
    return detail;
}
