...
REG_CCU_EXECUTOR_CREATE_FUNC_V1(SimCcuV1::TRANS_TYPE,
                                SimCcuV1::TRANSLOCMSTOLOCMEM_CODE,
                                TransLocMSToLocMemExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::TRANS_TYPE,
                                SimCcuV2::TRANSLOCMSTOLOCMEM_CODE,
                                TransLocMSToLocMemExecutor);
...
void TransLocMSToLocMemExecutor::Process(CcuResourceManager &ccuResMgr) {
    ...
    bool ret =
        ccuResMgr.TransMSToMem(rankId_, dieId_, locMSId_,
...
void TransLocMSToLocMemExecutor::RunV2() {
    ...
    bool ret = ccuResMgr.TransMSToMem(
        rankId_, dieId_, locMsId, reinterpret_cast<void *>(dstLocMemAddr), len);
    if (!ret) {
        ccuSimulator_->SetExecState(CcuExecState::EXEC_FAIL);
        return;
    }
...
std::string TransLocMSToLocMemExecutor::Describe() {
    if (version_ == RunnerCcuVersion::CCU_V1) {
        return HcclSim::StringFormat(
            ...
            "LocMS[%u:%u] To LocMem[%u:%u] With LengthXn[%u] Use Channel[%u], "
            "Set "
            "CKE[%u:%04x], clearType[%u], lengthEn[%u]",
            waitCKEId_, waitCKEMask_, locMSId_ / 0x8000, locMSId_ % 0x8000,
            locGSAId_, locXnId_, lengthXnId_, channelId_, setCKEId_,
