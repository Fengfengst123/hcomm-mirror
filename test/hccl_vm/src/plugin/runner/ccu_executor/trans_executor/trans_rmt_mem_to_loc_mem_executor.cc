...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor -- trans rmt mem
...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::TRANS_TYPE,
                             SimCcuV1::TRANSRMTMEMTOLOCMEM_CODE,
                             TransRmtMemToLocMemExecutor);
...
void TransRmtMemToLocMemExecutor::Process(CcuResourceManager &ccuResMgr) {
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        locAddr += offset;
        setCKEId_ += ckeOffset;
        HCCL_VM_DEBUG("locCcu[{}:{}], Get gsa addr offset = [{:04x}], cke "
                      "offset = [{:04x}]",
    ...
    HCCL_VM_DEBUG(
        ...
        rankId_, dieId_, rmtGSAId_, rmtAddr, locGSAId_, locAddr, lengthXnId_,
        transLength_, reduceEn_, reduceOpCode_);
    ccuResMgr.TransMemToMem(reinterpret_cast<void *>(rmtAddr),
                            reinterpret_cast<void *>(locAddr), transLength_,
                            reduceEn_, reduceOpCode_, reduceDataType_);
    // 4.设置本端的cke
...
std::string TransRmtMemToLocMemExecutor::Describe() {
    return HcclSim::StringFormat("[Simulation Execute] Wait CKE[%u:%04x], "
                                 "Trans RmtMem[%u:%u] To LocMem[%u:%u] With "
                                 "LengthXn[%u] Use Channel[%u], Set "
                                 "CKE[%u:%04x], clearType[%u], lengthEn[%u], "
                                 "DataType[%u], ReduceType[%u] reduceEn[%u]\n",
                                 waitCKEId_, waitCKEMask_, rmtGSAId_, rmtXnId_,
                                 locGSAId_, locXnId_, lengthXnId_, channelId_,
                                 setCKEId_, setCKEMask_, clearType_, lengthEn_,
                                 reduceDataType_, reduceOpCode_, reduceEn_);
}

CcuTrace::CcuInstrTraceDetail
TransRmtMemToLocMemExecutor::CollectTraceDetail() {
