...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::TRANS_TYPE,
                             SimCcuV1::TRANSLOCMSTORMTMS_CODE,
                             TransLocMSToRmtMSExecutor);
...
void TransLocMSToRmtMSExecutor::Process(CcuResourceManager &ccuResMgr) {
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        setCKEId_ += ckeOffset;
        HCCL_VM_DEBUG(
            "curCcu[{}:{}], Get ms offset = [{:04x}], cke offset = [{:04x}]",
    ...
    HCCL_VM_DEBUG("ccuId[{}:{}-{}:{}] Trans data "
                  ...
                  rankId_, dieId_, rmtCcu.first, rmtCcu.second, locMSId_,
                  rmtMSId_, lengthXnId_, transLength_);
    ccuResMgr.TransMSToMS(rankId_, dieId_, rmtCcu.first, rmtCcu.second,
                          locMSId_, rmtMSId_, transLength_);
    // 5.设置对端的cke
...
std::string TransLocMSToRmtMSExecutor::Describe() {
    return HcclSim::StringFormat(
        ...
        waitCKEId_, waitCKEMask_, locMSId_ / 0x8000, locMSId_ % 0x8000,
        rmtMSId_ / 0x8000, rmtMSId_ % 0x8000, lengthXnId_, channelId_,
        setRmtCKEId_, setRmtCKEMask_, setCKEId_, setCKEMask_, clearType_,
        lengthEn_);
}

CcuTrace::CcuInstrTraceDetail TransLocMSToRmtMSExecutor::CollectTraceDetail() {
    CcuTrace::CcuInstrTraceDetail detail;
    detail.typeName = "TransLocMSToRmtMS";
    return detail;
}
