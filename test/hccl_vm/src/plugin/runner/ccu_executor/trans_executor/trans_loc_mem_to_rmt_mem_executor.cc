...
REG_CCU_EXECUTOR_CREATE_FUNC(SimCcuV1::TRANS_TYPE,
                             SimCcuV1::TRANSLOCMEMTORMTMEM_CODE,
                             TransLocMemToRmtMemExecutor);
...
void TransLocMemToRmtMemExecutor::Process(CcuResourceManager &ccuResMgr) {
    ...
    HCCL_VM_DEBUG(
        ...
        rankId_, dieId_, locGSAId_, locAddr, rmtGSAId_, rmtAddr, lengthXnId_,
        transLength_, reduceEn_, reduceOpCode_);
    ccuResMgr.TransMemToMem(reinterpret_cast<void *>(locAddr),
                            reinterpret_cast<void *>(rmtAddr), transLength_,
