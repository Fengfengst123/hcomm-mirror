...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: ccu executor -- Sync XnWt
 * Author: caiyifan
 */
...
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::TRANS_TYPE, SimCcuV2::SYNCXNWTX_CODE,
                                SyncXnWtExecutor);
...
void SyncXnWtExecutor::Run() {
    ...
    if (ccuType == CcuComponerntType::XN_A6) {
        ...
        HCCL_VM_ERROR("TransformSyncXnWtxInstr is not supported. xdId:[{}]",
                      xdId);
        ccuSimulator_->SetExecState(CcuExecState::EXEC_FAIL);
        return;
    }
    ...
    if (notifyValid_ == 1) {
        ...
        if (!ccuResMgr.GetXnIdByAddr(dieId_, CcuComponerntType::CKE_A6, xnAddr,
                                     cekIdRmtId)) {
            ccuSimulator_->SetExecState(CcuExecState::EXEC_FAIL);
            return;
        }
...
std::string SyncXnWtExecutor::Describe() {
    return HcclSim::StringFormat(
        ...
        xdId_, xdtId_, xsId_, xcId_, xnId_, xntId_, value_, notifyValid_,
        parMode_, setCKEId_, setCKEMask_);
}
