...
REG_CCU_EXECUTOR_CREATE_FUNC_V1(SimCcuV1::TRANS_TYPE,
                                SimCcuV1::TRANSLOCMEMTOLOCMS_CODE,
                                TransLocMemToLocMSExecutor);
REG_CCU_EXECUTOR_CREATE_FUNC_V2(SimCcuV2::TRANS_TYPE,
                                SimCcuV2::TRANSLOCMEMTOLOCMS_CODE,
                                TransLocMemToLocMSExecutor);
...
void TransLocMemToLocMSExecutor::RunV2() {
    ...
    HCCL_VM_DEBUG("Trans data from srcMemAddr[0x{:x}] to msId[{}], length[{}].",
                  srcLocMemAddr, locMsId, len);

    bool ret = ccuResMgr.TransMemToMS(
        rankId_, dieId_, locMsId, reinterpret_cast<void *>(srcLocMemAddr), len);
    if (!ret) {
        ccuSimulator_->SetExecState(CcuExecState::EXEC_FAIL);
        return;
    }
...
std::string TransLocMemToLocMSExecutor::Describe() {
    if (version_ == RunnerCcuVersion::CCU_V1) {
        return HcclSim::StringFormat(
            ...
            waitCKEId_, waitCKEMask_, locGSAId_, locXnId_, locMSId_ / 0x8000,
            locMSId_ % 0x8000, lengthXnId_, channelId_, setCKEId_, setCKEMask_,
