...
class CcuTraceCollector {
  public:
    ...
    void RecordWaitSpin(int32_t rankId, uint16_t dieId, uint16_t instrId,
                        ...
                        uint16_t ckeValue);
    // CKE 通过后调用，获取并重置自旋信息
    CcuWaitInfo FinalizeWaitInfo(int32_t rankId, uint16_t dieId,
                                 uint16_t instrId, uint16_t ckeValueOnPass);
    ...
    // ===== 设置最终快照 =====
    void SetFinalSnapshot(int32_t rankId, uint16_t dieId,
  ...
  private:
    ...
    std::string MakeWaitKey(int32_t rankId, uint16_t dieId,
