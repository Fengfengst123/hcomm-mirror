...
class CcuTraceSerializer {
  public:
    ...
    // 将 JSON 字符串写入文件
    static bool WriteToFile(const std::string &jsonStr,
                            const std::string &outputPath);

    // 一步完成：序列化并写入文件
    static bool DumpToFile(const CcuTraceRun &traceRun,
                           const std::string &outputPath);
  ...
  private:
    ...
    static std::string
    SerializeCcuRegistry(const std::vector<CcuIdentity> &registry);
    static std::string
    SerializeSqeTaskRegistry(const std::vector<CcuSqeTask> &registry);
    static std::string
    SerializeInstrSpaces(const std::vector<CcuInstrSpace> &instrSpaces);
    static std::string
    SerializeChannelSpaces(const std::vector<CcuChannelSpace> &channelSpaces);
    static std::string
    SerializeGlobalEntries(const std::vector<CcuTraceEntry> &entries);
    static std::string
    SerializeNonCcuEntries(const std::vector<CcuTraceNonCcuEntry> &entries);
    static std::string SerializeFinalSnapshots(
        const std::map<std::string, CcuResourceSnapshot> &snapshots);
    static std::string SerializeSummary(const CcuRunSummary &summary);

    static std::string
    SerializeResourceSnapshot(const CcuResourceSnapshot &snapshot);
    ...
    static std::string
    SerializeCrossCcuChanges(const CcuCrossCcuChanges &changes);
