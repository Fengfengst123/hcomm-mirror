...
/**
 * AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * for the full text of the License. Description: CCU trace JSON serializer
 * implementation
 */
...
std::string CcuTraceSerializer::SerializeToJson(const CcuTraceRun &traceRun) {
    ...
    oss << "  \"ccuRegistry\": " << SerializeCcuRegistry(traceRun.ccuRegistry)
        << ",\n";
    oss << "  \"sqeTaskRegistry\": "
        << SerializeSqeTaskRegistry(traceRun.sqeTaskRegistry) << ",\n";
    oss << "  \"instrSpaces\": " << SerializeInstrSpaces(traceRun.instrSpaces)
        << ",\n";
    oss << "  \"channelSpaces\": "
        << SerializeChannelSpaces(traceRun.channelSpaces) << ",\n";
    oss << "  \"globalEntries\": "
        << SerializeGlobalEntries(traceRun.globalEntries) << ",\n";
    oss << "  \"nonCcuEntries\": "
        << SerializeNonCcuEntries(traceRun.nonCcuEntries) << ",\n";
    oss << "  \"ccuFinalSnapshots\": "
        << SerializeFinalSnapshots(traceRun.ccuFinalSnapshots) << ",\n";
...
bool CcuTraceSerializer::WriteToFile(const std::string &jsonStr,
    ...
    ofs << jsonStr;
    ofs.close();
    HCCL_VM_INFO("Trace written to: {}, size={} bytes", outputPath,
                 jsonStr.size());
    return true;
}

bool CcuTraceSerializer::DumpToFile(const CcuTraceRun &traceRun,
                                    const std::string &outputPath) {
...
std::string
CcuTraceSerializer::SerializeResourceSnapshot(const CcuResourceSnapshot &snap) {
    std::ostringstream oss;
    oss << "{";
    ...
    for (size_t i = 0; i < snap.xnRecords.size(); i++) {
        if (i > 0)
            oss << ", ";
        oss << "{\"id\": " << snap.xnRecords[i].id << ", \"value\": \""
            << ToHexString(snap.xnRecords[i].value) << "\"}";
    }
    oss << "], ";
    ...
    for (size_t i = 0; i < snap.gsaRecords.size(); i++) {
        if (i > 0)
            oss << ", ";
        oss << "{\"id\": " << snap.gsaRecords[i].id << ", \"value\": \""
            << ToHexString(snap.gsaRecords[i].value) << "\"}";
    }
    oss << "], ";
    ...
    for (size_t i = 0; i < snap.ckeRecords.size(); i++) {
        if (i > 0)
            oss << ", ";
        oss << "{\"id\": " << snap.ckeRecords[i].id
            << ", \"value\": " << snap.ckeRecords[i].value << "}";
    }
    oss << "]";
...
std::string CcuTraceSerializer::SerializeCcuRegistry(
    const std::vector<CcuIdentity> &registry) {
    std::ostringstream oss;
    oss << "[\n";
    for (size_t i = 0; i < registry.size(); i++) {
        const auto &ccu = registry[i];
        oss << "    {\"rankId\": " << ccu.rankId << ", \"dieId\": " << ccu.dieId
            << ", \"ccuVersion\": \""
            << EscapeJsonString(RunnerCcuVersionToString(ccu.ccuVersion))
            << "\""
            << ", \"initialSnapshot\": "
            << SerializeResourceSnapshot(ccu.initialSnapshot) << "}";
...
std::string CcuTraceSerializer::SerializeSqeTaskRegistry(
    const std::vector<CcuSqeTask> &registry) {
    std::ostringstream oss;
    oss << "[\n";
    for (size_t i = 0; i < registry.size(); i++) {
        const auto &task = registry[i];
        oss << "    {\"sqeTaskId\": " << task.sqeTaskId
            << ", \"rankId\": " << task.rankId << ", \"dieId\": " << task.dieId
            << ", \"missionId\": " << static_cast<int>(task.missionId)
            << ", \"instStartId\": " << task.instStartId
            << ", \"instCnt\": " << task.instCnt << ", \"key\": " << task.key
            << ", \"args\": [";
...
std::string CcuTraceSerializer::SerializeInstrSpaces(
    const std::vector<CcuInstrSpace> &instrSpaces) {
    std::ostringstream oss;
    oss << "[\n";
    for (size_t i = 0; i < instrSpaces.size(); i++) {
        const auto &space = instrSpaces[i];
        oss << "    {\"rankId\": " << space.rankId
            << ", \"dieId\": " << space.dieId << ", \"instructions\": [\n";
...
std::string CcuTraceSerializer::SerializeChannelSpaces(
    const std::vector<CcuChannelSpace> &channelSpaces) {
    std::ostringstream oss;
    oss << "[\n";
    for (size_t i = 0; i < channelSpaces.size(); i++) {
        const auto &space = channelSpaces[i];
        oss << "    {\"rankId\": " << space.rankId
            << ", \"dieId\": " << space.dieId << ", \"channels\": [\n";
...
std::string
CcuTraceSerializer::SerializeResourceDelta(const CcuResourceDelta &delta) {
    std::ostringstream oss;
    oss << "{";
    ...
    for (size_t i = 0; i < delta.xnChanges.size(); i++) {
        ...
        oss << "{\"id\": " << delta.xnChanges[i].id << ", \"valueBefore\": \""
            ...
            << ", \"valueAfter\": \""
            << ToHexString(delta.xnChanges[i].valueAfter) << "\"}";
    }
    oss << "], ";
    ...
    for (size_t i = 0; i < delta.gsaChanges.size(); i++) {
        ...
        oss << "{\"id\": " << delta.gsaChanges[i].id << ", \"valueBefore\": \""
            ...
            << ", \"valueAfter\": \""
            << ToHexString(delta.gsaChanges[i].valueAfter) << "\"}";
    }
    oss << "], ";
...
std::string
CcuTraceSerializer::SerializeContext(const CcuExecutionContext &ctx) {
    std::ostringstream oss;
...
std::string CcuTraceSerializer::SerializeCrossCcuChanges(
    const CcuCrossCcuChanges &changes) {
    std::ostringstream oss;
    oss << "{\"hasCrossCcuChange\": "
        << (changes.hasCrossCcuChange ? "true" : "false");
...
std::string
CcuTraceSerializer::SerializeDetail(const CcuInstrTraceDetail &detail) {
    std::ostringstream oss;
...
std::string CcuTraceSerializer::SerializeGlobalEntries(
    const std::vector<CcuTraceEntry> &entries) {
    std::ostringstream oss;
    oss << "[\n";
    for (size_t i = 0; i < entries.size(); i++) {
        ...
        oss << "\"globalSeqId\": " << e.globalSeqId
            ...
            << ", \"rankId\": " << e.rankId << ", \"dieId\": " << e.dieId
            << ", \"instrId\": " << e.instrId
            << ", \"sqeTaskId\": " << e.sqeTaskId << ", \"category\": \""
            << CcuInstrCategoryToString(e.category) << "\""
            << ", \"execState\": " << static_cast<int>(e.execState)
            << ", \"context\": " << SerializeContext(e.context)
            << ", \"resourceDelta\": "
            << SerializeResourceDelta(e.resourceDelta)
            << ", \"crossCcuChanges\": "
            << SerializeCrossCcuChanges(e.crossCcuChanges)
...
std::string CcuTraceSerializer::SerializeNonCcuEntries(
    const std::vector<CcuTraceNonCcuEntry> &entries) {
    std::ostringstream oss;
    oss << "[";
    for (size_t i = 0; i < entries.size(); i++) {
        const auto &e = entries[i];
...
std::string CcuTraceSerializer::SerializeFinalSnapshots(
    const std::map<std::string, CcuResourceSnapshot> &snapshots) {
    std::ostringstream oss;
    oss << "{";
    bool first = true;
    for (const auto &kv : snapshots) {
        if (!first)
            oss << ", ";
        oss << "\"" << kv.first
            << "\": " << SerializeResourceSnapshot(kv.second);
        first = false;
    }
    oss << "}";
    return oss.str();
