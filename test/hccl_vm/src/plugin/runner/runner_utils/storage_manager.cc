...
static const std::string HCCLVM_ALL_RANK_INPUT_OUTPUT_FILE =
    "/all_rank_input_output.txt";
...
HcclVmResult StorageManager::PrintAllRankInputBuffer() {
    ...
    for (uint32_t i = 0; i < m_synData.memory_info.count; i++) {
        if (m_synData.memory_info.data[i].buffer_type == 0) {
            ...
            auto srcAddr =
                sim::AcquireDevPtrInNoHostProcess((void *)startAddr, srcPhyMem);
            if (srcAddr == nullptr) {
                HCCL_VM_ERROR("无法获取startAddr的设备地址(addr= {:x})！",
                              reinterpret_cast<uintptr_t>((void *)startAddr));
            ...
            HCCL_VM_DEBUG("rankId={}, input buffer= {:x}, size={}",
                          m_synData.memory_info.data[i].rank_id, startAddr,
                          size);
            for (uint32_t idx = 0; idx < size; idx++) {
                HCCL_VM_TRACE("idx={}, value={:x}", idx,
                              *((char *)srcAddr + idx));
            }
        }
    }
...
HcclVmResult StorageManager::Trans2CheckerParam(sim::OpDetailTab &detailTab,
                                                ::OpDetails &detail) {
...
HcclVmResult StorageManager::LoadHcclVmSynthesisData(
    sim::OpDetailTab &detailTab, std::vector<sim::CcuChannelTab> &channels) {
    ...
    for (auto &channel : channels) {
        ...
        HCCL_VM_INFO("[Channel info] channelId={}, srcRank={}, srcDie={}, "
                     "dstRank={}, dstDie={} srcEid={}, dstEid={}",
                     channel.channelId, channel.srcRankId,
                     static_cast<uint32_t>(channel.srcDieId), channel.dstRankId,
                     channel.dstDieId, EidToHexString(channel.leid),
                     EidToHexString(channel.reid));
        m_allRankChannelInfo[channel.srcRankId][channel.srcDieId]
...
HcclVmResult StorageManager::LoadHcclVmInstrData(
    std::vector<sim::CcuInstrResTab> &instrRes) {
    for (auto &ccuInstr : instrRes) {
        HCCL_VM_DEBUG("deviceId={}, dieId={}, count={}", ccuInstr.deviceId,
                      ccuInstr.dieId, ccuInstr.instrCount);
    }
...
HcclVmResult
StorageManager::LoadHcclVmTaskMetaData(std::vector<sim::OpTaskTab> &tasks) {
    ...
    for (auto &taskMeta : taskMeataData.task_meta) {
        HCCL_VM_DEBUG(
            "rankId={}, dieId={}, instrCnt={}, argSize={}, streamId={}",
            taskMeta.rankId, static_cast<uint32_t>(taskMeta.taskData.ccu.dieId),
            taskMeta.taskData.ccu.instCnt, taskMeta.taskData.ccu.argSize,
            taskMeta.streamId);
    }
...
HcclVmResult
StorageManager::GetHcclVmFlagData(HcclSim::HcclVmFlagData &waitFlag) {
    // 1. 构造路径
...
std::string StorageManager::FindRootPath() {
    ...
    for (int i = 0; i <= 3; ++i) {
        ...
        if (realpath(current_search_path.c_str(), abs_path) != nullptr) {
            ...
            HCCL_VM_WARN("Iteration {}: realpath failed for {}", i,
                         current_search_path);
        }
...
HcclVmResult
StorageManager::ConvertTaskQueue(const HcclVmTaskMetaData &taskMeataData) {
    auto rankSize = GetRankSize();
    ...
    for (uint32_t i = 0; i < size; i++) {
        ...
        if (rankId >= rankSize) {
            HCCL_VM_ERROR("rankId={} is out of range. rankSize={}", rankId,
                          rankSize);
...
HcclVmResult
StorageManager::InitCcuResource(std::vector<sim::CcuInstrResTab> &instrRes) {
    ...
    for (auto &instr : instrRes) {
        ...
        for (uint32_t i = 0; i < instr.instrCount; ++i) {
            ...
            std::memcpy(&tmp, instr.instrSpace[i],
...
void StorageManager::DumpAllRankInputOutput(
    std::vector<std::map<uint32_t, sim::CompositeOpDetail>> &compositeDataMap) {
    ...
    HCCL_VM_INFO("Dump All Rank Input Output Data. rankSize={}, fullPath={}",
                 rankSize, fullPath);
    for (size_t opIdx = 0; opIdx < compositeDataMap.size(); opIdx++) {
        ...
        for (auto &it : rankTask) {
            ...
            if (memInfo.inputAddr != 0 && memInfo.inputSize > 0) {
                sim::PhyMemBlock srcPhyMem{};
                auto startAddr = sim::AcquireDevPtrInNoHostProcess(
                    (void *)(memInfo.inputAddr), srcPhyMem);
                ...
                HCCL_VM_INFO("opIdx={}, rankId={}, inputAddr={:x}, size={}",
                             opIdx, it.first, memInfo.inputAddr,
                             memInfo.inputSize);
            }
            ...
            if (memInfo.outputAddr != 0 && memInfo.outputSize > 0) {
                sim::PhyMemBlock srcPhyMem{};
                auto startAddr = sim::AcquireDevPtrInNoHostProcess(
                    (void *)(memInfo.outputAddr), srcPhyMem);
                if (startAddr == nullptr) {
                    HCCL_VM_ERROR("fail addr(type={}, addr={:x})!",
                ...
                HCCL_VM_INFO("opIdx={}, rankId={}, outputAddr={:x}, size={}",
                             opIdx, it.first, memInfo.outputAddr,
                             memInfo.outputSize);
            }
        }
        ...
        if (firstComp.detail.opDetail.size() < sizeof(::OpDetails)) {
            HCCL_VM_ERROR("opDetail BLOB too small, opIdx={}, size={}", opIdx,
                          firstComp.detail.opDetail.size());
            continue;
        }
        ::OpDetails opDetails{};
        std::memcpy(&opDetails, firstComp.detail.opDetail.data(),
                    sizeof(::OpDetails));
...
void StorageManager::FlexiblePrintData(std::ofstream &ofs,
                                       const BufferInfo &buffer) {
    if (buffer.size <= PRINT_DATA_SIZE_THRESHOLD) {
        for (uint32_t i = 0; i < buffer.size; i++) {
            ofs << std::hex
                << static_cast<int>(
                       static_cast<unsigned char>(buffer.phyAddr[i]));
        }
        return;
    }
    ...
    for (uint32_t i = 0; i < PRINT_DATA_PREFIX; i++) {
        ofs << std::hex
            << static_cast<int>(static_cast<unsigned char>(buffer.phyAddr[i]));
    }
    ofs << "\n...[skipped " << std::dec
        << buffer.size - PRINT_DATA_SIZE_THRESHOLD << " bytes]...\n";
...
void StorageManager::PrintOpData1(std::ofstream &ofs,
                                  const std::vector<BufferInfo> &allRankInput) {
    auto rankSize = GetRankSize();
    for (uint32_t idx = 0; idx < rankSize; idx++) {
        ofs << std::dec << "Rank " << idx
            << " Input: " << (uint64_t)(void *)allRankInput[idx].virtualAddr
            << ", size=" << allRankInput[idx].size << std::endl;
        FlexiblePrintData(ofs, allRankInput[idx]);
        ofs << std::endl;
    }
}

void StorageManager::PrintOpData2(
    std::ofstream &ofs, const std::vector<BufferInfo> &allRankInput,
    ...
    for (uint32_t idx = 0; idx < rankSize; idx++) {
        ...
        ofs << std::endl;
        ofs << std::dec << "Rank " << idx
            << " Output: " << (uint64_t)(void *)allRankOutput[idx].virtualAddr
            << ", size=" << allRankOutput[idx].size << std::endl;
