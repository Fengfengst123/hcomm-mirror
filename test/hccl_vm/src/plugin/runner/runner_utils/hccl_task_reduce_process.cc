...
// 对不同类型的做Sum的Reduce操作
template <typename T>
void MemReduceSumTemplate(void *dst_addr, const void *src_addr,
                          uint64_t length) {
...
// 对不同类型的做MIN的Reduce操作
template <typename T>
void MemReduceMinTemplate(void *dst_addr, const void *src_addr,
                          uint64_t length) {
    T *dst = reinterpret_cast<T *>(dst_addr);
    const T *src = reinterpret_cast<const T *>(src_addr);
    auto count = length / sizeof(T);
...
// 对不同类型的做MAX的Reduce操作
template <typename T>
void MemReduceMaxTemplate(void *dst_addr, const void *src_addr,
                          uint64_t length) {
    T *dst = reinterpret_cast<T *>(dst_addr);
    const T *src = reinterpret_cast<const T *>(src_addr);
    auto count = length / sizeof(T);
...
void MemReduceSum(void *src, void *dst, uint32_t length,
...
void MemReduceMin(void *src, void *dst, uint32_t length,
                  HcclDataType dataType) {
    switch (dataType) {
    case HcclDataType::HCCL_DATA_TYPE_INT8:
...
void MemReduceMax(void *src, void *dst, uint32_t length,
                  HcclDataType dataType) {
    switch (dataType) {
    case HcclDataType::HCCL_DATA_TYPE_INT8:
