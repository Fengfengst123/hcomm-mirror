...
void DeviceResourceManager::NotifyRecord(uint32_t rankId, uint32_t streamId,
                                         uint32_t notifyId,
                                         uint32_t notifyCnt) {
    deviceResData_.v1Res->notifyResources_[rankId][streamId]
        .notifyCnts_[notifyId] += notifyCnt;
}

bool DeviceResourceManager::NotifyWait(uint32_t rankId, uint32_t streamId,
                                       uint32_t notifyId, uint32_t notifyCnt) {
    deviceResData_.v1Res->notifyResources_[rankId][streamId]
        .notifyCnts_[notifyId] -= notifyCnt;
    return deviceResData_.v1Res->notifyResources_[rankId][streamId]
               .notifyCnts_[notifyId] >= 0;
}

bool DeviceResourceManager::CheckNotifyWait(uint32_t rankId, uint32_t streamId,
                                            uint32_t notifyId,
                                            uint32_t notifyCnt) {
    auto notifyRemain = deviceResData_.v1Res->notifyResources_[rankId][streamId]
                            .notifyCnts_[notifyId] -
                        notifyCnt;
    return deviceResData_.v1Res->notifyResources_[rankId][streamId]
               .notifyCnts_[notifyId] >= notifyRemain >= 0;
}
