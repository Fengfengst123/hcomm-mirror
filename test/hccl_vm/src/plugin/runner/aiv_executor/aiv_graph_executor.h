...
class AivGraphExecutor {
  public:
    AivGraphExecutor(uint64_t deviceId, uint64_t launchIdx)
        : deviceId_(deviceId), launchIdx_{launchIdx} {}
  ...
  private:
    ...
    ExecuteTask(std::shared_ptr<AivSim::AivTaskMemCopy> task);
    HcclSim::HcclVmResult
    ExecuteTask(std::shared_ptr<AivSim::AivTaskReduce> task);
    HcclSim::HcclVmResult
    ExecuteTask(std::shared_ptr<AivSim::AivTaskSetFlag> task);
    HcclSim::HcclVmResult
    ExecuteTask(std::shared_ptr<AivSim::AivTaskWaitFlag> task);
    HcclSim::HcclVmResult
    ExecuteTask(std::shared_ptr<AivSim::AivTaskPipeBarrier> task);
    HcclSim::HcclVmResult
    ExecuteTask(std::shared_ptr<AivSim::AivTaskSendFlag> task);
    HcclSim::HcclVmResult
    ExecuteTask(std::shared_ptr<AivSim::AivTaskRecvFlag> task);
    HcclSim::HcclVmResult
    ExecuteTask(std::shared_ptr<AivSim::AivTaskSyncAll> task);

    template <typename T>
    HcclSim::HcclVmResult Reduce(void *src, void *dst, size_t len,
                                 uint32_t reduceOp);
