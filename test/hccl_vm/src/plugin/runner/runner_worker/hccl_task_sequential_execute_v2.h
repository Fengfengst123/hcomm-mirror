...
class SequentialExecutorV2 {
  ...
  private:
    ...
    std::shared_ptr<AivGraphExecutor> GetAivGraphExecutor(uint64_t deviceId,
                                                          uint64_t launchIdx);
    void RemoveAivGraphExecutor(uint64_t deviceId, uint64_t launchIdx);

    // CCU
    HcclVmResult
    EnsureCcuResource(); // 按DB变更签名增量刷新CCU资源(设备资源/通道映射/微码指令空间)
  ...
  private:
    ...
    std::map<std::pair<uint64_t, uint64_t>, std::shared_ptr<AivGraphExecutor>>
        aivExecutorMap_{};
