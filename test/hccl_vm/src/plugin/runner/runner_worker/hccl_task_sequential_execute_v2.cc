...
HcclVmResult SequentialExecutorV2::EnsureCcuResource() {
    ...
    if (sim::QueryCcuResourceMetaCount(instrLoadCnt, channelCnt) != 0) {
    ...
    if (ccuResourceInited_ && instrLoadCnt == lastInstrLoadCnt_ &&
        channelCnt == lastChannelCnt_) {
        return HcclVmResult::HCCL_SIM_SUCCESS;
    }
    ...
    for (const auto &channel : channels) {
        ...
        maxDeviceId = std::max(
            maxDeviceId, std::max(channel.srcDeviceId, channel.dstDeviceId));
    }
    if (deviceIds.empty()) {
    ...
    if (maxDeviceId > static_cast<uint32_t>(std::numeric_limits<int>::max())) {
        HCCL_VM_ERROR("CCU device id {} exceeds resource manager index range",
                      maxDeviceId);
        return HcclVmResult::HCCL_SIM_E_INTERNAL;
    }
    ...
    for (uint32_t deviceId : deviceIds) {
        auto device = RunnerDB::GetById<sim::Device>(deviceId);
        if (!device.has_value()) {
            HCCL_VM_ERROR("Device {:d} not found in runner db", deviceId);
            return HcclVmResult::HCCL_SIM_E_INTERNAL;
        }
        RunnerCcuVersion ver{RunnerCcuVersion::CCU_INVALID};
        if (strncmp(device->soc_version, "Ascend950", strlen("Ascend950")) ==
            0) {
            ver = RunnerCcuVersion::CCU_V1;
        } else if (strncmp(device->soc_version, "Ascend960",
                           strlen("Ascend960")) == 0) {
            ver = RunnerCcuVersion::CCU_V2;
        } else {
            HCCL_VM_ERROR("Unknown soc version: {} for device {:d}",
                          device->soc_version, deviceId);
            return HcclVmResult::HCCL_SIM_E_INTERNAL;
        }
        if (ccuVersion == RunnerCcuVersion::CCU_INVALID) {
            ccuVersion = ver;
        } else if (ccuVersion != ver) {
            HCCL_VM_ERROR("Mixed ccu version: {} and {:s}",
                          RunnerCcuVersionToString(ccuVersion),
                          RunnerCcuVersionToString(ver));
            return HcclVmResult::HCCL_SIM_E_INTERNAL;
        }
    }
    ...
    for (uint32_t deviceId : deviceIds) {
        ...
        ccuResMgr.Init(static_cast<int>(deviceId), capacity, ccuVersion,
                       CCU_RESOURCE_BASE_ADDR);
    }

    // 通道映射刷新: channelId → 远端CcuInfo(远端deviceId, dieId)，按源设备分组
    std::map<uint32_t, RankChannelInfo> channelMapByDevice{};
    for (const auto &channel : channels) {
        if (channel.srcDieId >= HcclSim::DIE_NUM ||
            channel.dstDieId >= HcclSim::DIE_NUM ||
            channel.channelId >= SimCcuV1::MAX_CCU_CHANNEL_NUM) {
            HCCL_VM_ERROR("Invalid channel info: srcDevice={:d}, srcDie={:d}, "
                          "dstDevice={:d}, dstDie={:d}, channelId={:d}",
                          channel.srcDeviceId, channel.srcDieId,
        ...
        channelMapByDevice[channel.srcDeviceId][channel.srcDieId]
                          [channel.channelId] = rmtInfo;
    }
    ...
    for (const auto &instr : instrRes) {
        if (instr.dieId >= HcclSim::DIE_NUM) {
            HCCL_VM_ERROR("Invalid instr res dieId: device={:d}, die={:d}",
                          instr.deviceId, instr.dieId);
            return HcclVmResult::HCCL_SIM_E_INTERNAL;
        }
        if (instr.instrCount > HcclSim::CCU_INSTRUCTION_NUM) {
            HCCL_VM_ERROR("Invalid instr count: device={:d}, die={:d}, "
                          "count={:d}, capacity={:d}",
                          instr.deviceId, instr.dieId, instr.instrCount,
                          HcclSim::CCU_INSTRUCTION_NUM);
            return HcclVmResult::HCCL_SIM_E_INTERNAL;
        }
        ...
        ccuResMgr.InitInstrInfo(static_cast<int>(instr.deviceId), instr.dieId,
                                ccuInstr);
    }
    ...
    HCCL_VM_INFO("CCU resources refreshed: version={}, devices={}, "
                 "channels={}, instrRes={}",
                 RunnerCcuVersionToString(ccuVersion), deviceIds.size(),
                 channels.size(), instrRes.size());
    return HcclVmResult::HCCL_SIM_SUCCESS;
}
...
HcclVmResult SequentialExecutorV2::Execute() {
    ...
    std::vector<sim::OpTaskTab> streamHeadTasks;
    ...
    HCCL_VM_DEBUG("current task num {}, head task num {}", tasks.size(),
                  streamHeadTasks.size());
    ...
    for (const auto &task : streamHeadTasks) {
        ...
        HCCL_VM_DEBUG(
            "Executing Task: id={}, pid={}, opDetailId={}, taskMeta=[{:s}]",
            task.id, task.pid, task.opDetailId, taskMeta.Describe());
        auto ret = ExecuteOneTask(taskMeta);

        if (ret == HcclVmResult::HCCL_SIM_VRT_HOLD_CMD) {
            // 无法完整执行当前Task，让出Executor，继续执行其他Task（可恢复，保留未完成态）
            HCCL_VM_DEBUG("Hold Task: id={}, pid={}", task.id, task.pid);
            continue;
        }

        // 与宿主侧的契约：任务必须到达终态（isDone），宿主侧
        // aclrtSynchronizeStream 才可能
        // 等到全部完成。硬失败时若把任务留在"未完成"态返回，则剩余任务永不变
        // isDone， 宿主会无限打印 "Waiting for stream... N/M"
        // 死等。故此处对硬失败也刷新 isDone（终态），
        // 并继续处理其它任务；失败本身由 ERROR 日志与最终数据校验体现。
        if (ret != HcclVmResult::HCCL_SIM_SUCCESS) {
            HCCL_VM_ERROR("Task Execute failed, ret={}, mark done to unblock "
                          "waiters, taskMeta=[{:s}]",
                          static_cast<int>(ret), taskMeta.Describe());
        }
...
HcclVmResult SequentialExecutorV2::TaskAivGraph(const HcclTaskMetaData &task) {
    ...
    if (!aivGraphExecutor->Init()) {
        HCCL_VM_ERROR("Failed to init AivGraphExecutor, deviceId={}, "
    ...
    if (ret != HcclVmResult::HCCL_SIM_SUCCESS &&
        ret != HcclVmResult::HCCL_SIM_VRT_HOLD_CMD) {
        HCCL_VM_ERROR("AivGraphExecutor failed, deviceId={}, streamId={}, "
                      "launchIndex={}, ret={}",
...
std::shared_ptr<AivGraphExecutor>
SequentialExecutorV2::GetAivGraphExecutor(uint64_t deviceId,
                                          uint64_t launchIdx) {
    const auto key = std::make_pair(deviceId, launchIdx);
    auto iter = aivExecutorMap_.find(key);
    if (iter == aivExecutorMap_.end()) {
        iter = aivExecutorMap_
                   .emplace(key, std::make_shared<AivGraphExecutor>(deviceId,
                                                                    launchIdx))
                   .first;
    }
    return iter->second;
...
void SequentialExecutorV2::RemoveAivGraphExecutor(uint64_t deviceId,
                                                  uint64_t launchIdx) {
    const auto key = std::make_pair(deviceId, launchIdx);
    auto iter = aivExecutorMap_.find(key);
    if (iter != aivExecutorMap_.end()) {
