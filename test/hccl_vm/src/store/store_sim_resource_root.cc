...
std::string ResolveRoot() {
    ...
    return std::string(kShmBase) + "/" + kShmPrefix +
...
SimResourceRoot::SimResourceRoot()
    : m_ownerPid(-1) // 构造时未确定 owner；由 Init() 显式设为当前进程 pid。
      ,
      m_root(ResolveRoot()), m_syncDir(m_root + "/sync") {}
...
bool SimResourceRoot::Init() {
    ...
    RemoveTree(m_root);
    if (!MkdirP(m_syncDir)) {
        HCCL_VM_ERROR("[SimResourceRoot] create {} failed: errno={}", m_syncDir,
                      errno);
        return false;
    }
    if (::setenv(kEnvKey, m_root.c_str(), 1) != 0) {
        HCCL_VM_ERROR("[SimResourceRoot] setenv {} failed, errno={}", kEnvKey,
                      errno);
        return false;
    }
...
std::string SimResourceRoot::GetRunnerLockPath() {
    return BuildName("hccl_vm_runner.lock");
}
...
// unlink 本实例在 mqueue 命名空间里创建的两条消息队列（名字由 tag 推导，
// 与 cmd_base_utils.cc 的 ToPosixMqName/MQ_REQ_NAME/MQ_RESP_NAME 保持一致）。
// 不写日志，供信号 handler 调用。正常退出时 HcclVmExit 已显式
// unlink，此处为无害 no-op 兜底。
static void RemoveOwnMessageQueues() {
    const std::string tag = SimResourceRoot::GetTag();
    (void)::mq_unlink(("/host_mq_request_" + tag).c_str());
    (void)::mq_unlink(("/host_mq_response_" + tag).c_str());
}
...
void SimResourceRoot::RegisterExitCleaner() {
    ...
    // 不同，CleanupQuiet 已兜底。
    if (GetInstance().m_ownerPid < 0 ||
        static_cast<int>(::getpid()) != GetInstance().m_ownerPid) {
        return;
    }

    // 正常 return/exit 路径。
    std::atexit([]() { sim::SimResourceRoot::GetInstance().CleanupQuiet(); });

    // 崩溃/终止信号路径（SIGKILL/SIGSTOP 不可捕获，属「外部主动
    // kill」边界，无法也不应清理）。
    const int handledSignals[] = {SIGINT,  SIGTERM, SIGHUP, SIGQUIT, SIGABRT,
...
void SimResourceRoot::CleanShmByPrefix(
    ...
    while ((entry = ::readdir(dir)) != nullptr) {
        ...
        struct stat entrySt;
        if (::lstat(filePath.c_str(), &entrySt) != 0 ||
            !S_ISREG(entrySt.st_mode)) {
            continue;
        }
        const std::string fileName = entry->d_name;
        bool matched = std::any_of(prefixes.begin(), prefixes.end(),
