...
void *DeviceMemoryManager::AllocVirMem(uint64_t devPhyId, size_t size) {
    ...
    HCCL_VM_INFO("dev:{:d} alloc vir mem:{:p}", devPhyId,
                 reinterpret_cast<void *>(virAddr));
...
void *DeviceMemoryManager::AllocPhyMem(const char *name, uint64_t deviceId,
                                       size_t size) {
    if (name == nullptr) {
        return nullptr;
    }
    ...
    if (CommPoolPolicy::ExceedsCeiling(size, checkOnlyMode)) {
        HCCL_VM_ERROR(
            "dev:{:d} alloc phy mem:{} size:{} exceeds pool ceiling:{}, reject",
            deviceId, name, size, CommPoolPolicy::kPoolSize);
        return nullptr;
    }
    if (CommPoolPolicy::ShouldRedirect(size, checkOnlyMode)) {
        return MemoryManager::GetInstance().AcquireMemByName(
            CommPoolPolicy::kPoolName);
    }
...
void *DeviceMemoryManager::AcquirePhyMem(const char *name, uint64_t deviceId,
                                         size_t size) {
    if (name == nullptr) {
        return nullptr;
    }
    ...
    // 超过复用区上界直接报错，不回退真实分配。
    if (CommPoolPolicy::ExceedsCeiling(size, checkOnlyMode)) {
        HCCL_VM_ERROR("dev:{:d} acquire phy mem:{} size:{} exceeds pool "
                      "ceiling:{}, reject",
                      deviceId, name, size, CommPoolPolicy::kPoolSize);
        return nullptr;
    }
