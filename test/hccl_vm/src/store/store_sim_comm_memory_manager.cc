...
static void SlotUnlock(volatile uint32_t *lock) {
...
uint32_t CommunicationMemoryManager::AllocSlot() {
    ...
    if (m_batchCache.current < m_batchCache.batch_size) {
        ...
        // 缓存耗尽 → __atomic_fetch_add(&next_free, 1024) 抢新批次
        uint32_t batchStart = __atomic_fetch_add(
            &h0->next_free, kAllocBatchSize, __ATOMIC_RELAXED);
        if (batchStart > UINT32_MAX - kAllocBatchSize) {
            HCCL_VM_ERROR("AllocSlot: next_free overflow, batchStart={}",
                          batchStart);
            return UINT32_MAX;
        }
        uint32_t totalSlots = kSlotsPerPage * kMaxPages;
        if (batchStart >= totalSlots) {
            HCCL_VM_ERROR("AllocSlot: slot pool exhausted, batchStart={}",
                          batchStart);
            return UINT32_MAX;
        }
        m_batchCache.start = batchStart;
        m_batchCache.current = 1;
        m_batchCache.batch_size =
            std::min((uint32_t)kAllocBatchSize, totalSlots - batchStart);
        slotIdx = batchStart;
    }
    ...
    if (!m_pages[pageId]) {
        HCCL_VM_ERROR("AllocSlot: GetOrCreatePage failed for pageId={}",
                      pageId);
        return UINT32_MAX;
    }
    ...
    // 计算 slot 在共享内存中的绝对地址
    char *base =
        (char *)m_pages[pageId] + sizeof(PageHeader) + offset * kSlotSize;
...
SlotHandle CommunicationMemoryManager::GetSlotHandle(uint32_t slotIdx,
                                                     uint32_t role) {
    SlotHandle handle{};
    // 计算 slot 所在页和页内偏移量
...
int64_t CommunicationMemoryManager::Send(const SlotHandle &handle,
...
int64_t CommunicationMemoryManager::Recv(const SlotHandle &handle, void *data,
                                         size_t size) {
...
void *CommunicationMemoryManager::GetOrCreatePage(uint32_t pageId) {
    if (pageId >= kMaxPages) {
        HCCL_VM_ERROR("GetOrCreatePage: pageId {} out of range (max {})",
                      pageId, kMaxPages);
        return nullptr;
    }
    // 先检查是否已存在该页
    ...
    if (fd != -1) {
        ...
        if (ftruncate(fd, kCommPageSize) != 0) {
            HCCL_VM_ERROR(
                "GetOrCreatePage: ftruncate failed for page {}, errno={} ({})",
        ...
        // 映射共享内存到进程地址空间
        addr = mmap(nullptr, kCommPageSize, PROT_READ | PROT_WRITE, MAP_SHARED,
                    fd, 0);
        close(fd);
        if (addr == MAP_FAILED) {
            HCCL_VM_ERROR(
                "GetOrCreatePage: mmap failed for page {}, errno={} ({})",
                pageId, errno, strerror(errno));
            unlink(name.c_str());
            return nullptr;
        }
        ...
        if (fd == -1) {
            HCCL_VM_ERROR("GetOrCreatePage: open (consumer) failed for page "
                          "{}, errno={} ({})",
                          pageId, errno, strerror(errno));
            return nullptr;
        }
        ...
        close(fd);
        if (addr == MAP_FAILED) {
            HCCL_VM_ERROR("GetOrCreatePage: mmap (consumer) failed for page "
                          "{}, errno={} ({})",
                          pageId, errno, strerror(errno));
            return nullptr;
        }
        ...
        HCCL_VM_ERROR(
            "GetOrCreatePage: open (creator) failed for page {}, errno={} ({})",
            pageId, errno, strerror(errno));
        return nullptr;
    }
