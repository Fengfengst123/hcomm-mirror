...
void *ShmCreate(const char *name, size_t size) {
    ...
    if (shmFd == -1) {
        HCCL_VM_ERROR("create: open failed, name: {} errno:{}", fullName,
                      errno);
        return nullptr;
    }
    ...
    if (ftruncate(shmFd, totalSize) == -1) {
        HCCL_VM_ERROR("create: ftruncate failed, name: {} errno:{}", fullName,
                      errno);
        close(shmFd);
    ...
    head->lock = 0;
    head->refCount = 1;
    HCCL_VM_DEBUG("create name: {}, size: {:d}, ptr: {:p} ref:{:d}", head->name,
                  size, (char *)addr + SHM_HEAD_SIZE, (int)head->refCount);
...
void *ShmOpen(const char *name, size_t *size) {
    ...
    void *addr = mmap(nullptr, statBuf.st_size, PROT_READ | PROT_WRITE,
                      MAP_SHARED, shmFd, 0);
    int err = errno;
    close(shmFd);
    ...
    *size = FromLe64(head->size);
    ...
    while (!__sync_bool_compare_and_swap(&head->refCount, ToLe32(current_count),
                                         ToLe32(new_count))) {
        current_count = FromLe32(head->refCount);
        new_count = current_count + 1;
    }
    HCCL_VM_DEBUG("open name: {}, size: {:d}, ptr: {:p}", head->name, *size,
                  (char *)addr + SHM_HEAD_SIZE);
...
void ShmClose(void *shm) {
    ...
    if (refCount == 1) {
        ...
        HCCL_VM_DEBUG("close name: {}, ptr: {:p}, refCount: {} {}", tmp,
                      (char *)head, refCount, ref);
    }
}
