...
class SubProcessManager {
  public:
    ...
    int Request(uint8_t reqCmd, const void *reqData, uint32_t reqLen,
                uint8_t &rspCmd, void *rspData, uint32_t rspMaxLen,
                uint32_t &rspLen);

    int RequestV(uint8_t reqCmd, const struct iovec *reqIov, uint32_t reqSegCnt,
                 uint8_t &rspCmd, void *rspData, uint32_t rspMaxLen,
                 uint32_t &rspLen);
  ...
  private:
    ...
    int HostRecvMsg(uint8_t &outCmd, void *outData, uint32_t maxLen,
                    uint32_t &outLen);
};

SubProcessConfig CreateAicpuDeviceConfig(uint32_t rankId,
