...
class HcclPluginManager {
  public:
    ...
    // 广播给所有运行中的插件
    HcclSim::HcclVmResult BroadcastToAllPlugin(
    ...
    StartPlugins(const std::vector<std::string> &tag);
    // 关闭并卸载插件
    std::vector<HcclSim::HcclVmResult>
    StopPlugins(const std::vector<std::string> &tag);
  ...
  private:
    ...
    bool GetPluginFolderPath(const std::string &pluginTag,
                             std::string &pluginFolderPath);
    bool IsMatchingPlugin(const std::string &manifestPath,
                          const std::string &targetTag);
    std::map<std::string, std::shared_ptr<HcclPlugin>>
        m_plugins; // 插件Tag - 插件
