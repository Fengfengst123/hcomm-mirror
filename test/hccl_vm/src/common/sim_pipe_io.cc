...
int PipeBlockRead(int fd, void *buf, size_t len) {
    ...
    while (remaining > 0) {
        ...
        if (n > 0) {
            ...
            HCCL_VM_ERROR("read failed: {} (errno={}).", strerror(errno),
                          errno);
            return -1;
        }
    }
...
int PipeBlockWrite(int fd, const void *buf, size_t len) {
    ...
    while (remaining > 0) {
        ...
        if (n > 0) {
            ...
            HCCL_VM_ERROR("write failed: {} (errno={}).", strerror(errno),
                          errno);
            return -1;
        }
    }
    ...
    while (curCnt > 0) {
        ...
        if (n > 0) {
            ...
            if (curCnt > 0 && written > 0) {
                cur[0].iov_base =
                    static_cast<uint8_t *>(cur[0].iov_base) + written;
            ...
            HCCL_VM_ERROR("writev failed: {} (errno={}).", strerror(errno),
                          errno);
            return -1;
        }
    }
...
int PipeSendMsgV(int fd, uint8_t cmd, const struct iovec *iov,
                 uint32_t iovcnt) {
    uint32_t totalLen = 0;
    ...
    if (totalLen > IPC_MSG_LEN_MAX) {
        HCCL_VM_ERROR("PipeSendMsgV payload too large: {} > {}", totalLen,
                      IPC_MSG_LEN_MAX);
        return -1;
    }
...
int PipeRecvMsg(int fd, uint8_t &outCmd, void *outData, uint32_t maxLen,
                uint32_t &outLen) {
    constexpr uint32_t hdrSize = offsetof(PipeMessage, payload);
    PipeMessage hdr{};
...
int PipeChildSetup(PipePair &h2d, PipePair &d2h, int targetReadFd,
                   int targetWriteFd) {
    // 子进程不写 h2d，不读 d2h
    close(h2d.writeFd);
    close(d2h.readFd);
