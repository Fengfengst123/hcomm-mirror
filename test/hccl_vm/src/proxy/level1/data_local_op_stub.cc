...
HcclSim::HcclVmResult InsertLevel1Task(HcclTaskMetaData *task,
                                       uint32_t *index) {
    return NormalizeLevel1Task(task) ? InsertTaskToCollection(task, index)
                                     : HcclSim::HCCL_SIM_E_INTERNAL;
}
...
int32_t HcommLocalReduceOnThread(ThreadHandle thread, void *dst,
    ...
    HCCL_VM_INFO("{}: thread={:d}, dst={:p}, src={:p}, count={:d}, "
                 ...
                 __func__, thread, dst, src, count, static_cast<int>(dataType),
                 static_cast<int>(reduceOp));
    ...
    HCCL_VM_INFO("{} success, thread={:d}, taskIndex={:d}", __func__, thread,
                 taskIndex);
    return 0;
}
...
int32_t HcommThreadNotifyRecordOnThread(ThreadHandle thread,
    ...
    auto optDstThread = RunnerDB::GetById<sim::HcclThread>(dstThread);
    if (optDstThread.has_value() && dstNotifyIdx < optDstThread->notifyNum) {
        notifyIdVal =
            static_cast<uint64_t>(optDstThread->notifyId[dstNotifyIdx]);
    } else {
        HCCL_VM_WARN("{}: dstThread {:d} not found or idx {:d} out of range",
                     __func__, dstThread, dstNotifyIdx);
    }
...
int32_t HcommThreadNotifyWaitOnThread(ThreadHandle thread, uint32_t notifyIdx,
    ...
    if (optThread.has_value()) {
        if (notifyIdx < optThread->notifyNum) {
            ...
            HCCL_VM_WARN("{}: idx {:d} out of range (notifyNum={:d})", __func__,
                         notifyIdx, optThread->notifyNum);
        }
    }
    ...
    taskMetaData.taskData.notify.srcDeviceId =
        curRank; // 期望的发送方（同节点匹配）
