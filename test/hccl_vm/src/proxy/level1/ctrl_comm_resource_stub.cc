...
static void TryLaunchAicpuDevProcForRank(int32_t rankId, uint32_t deviceKey) {
    ...
    auto config =
        sim::CreateAicpuDeviceConfig(static_cast<uint32_t>(rankId), deviceKey);
...
// 与 sim::HcclThread.notifyId[40] / sim::HcclChannel.notifyId[64] 数组容量对齐
...
static inline void ReleaseThreadNotifies(const sim::HcclThread &rec) {
    for (uint32_t n = 0; n < rec.notifyNum && n < MAX_NOTIFY_PER_THREAD; ++n) {
        if (rec.notifyId[n] != 0u) {
            RunnerDB::Delete<sim::Notify>(rec.notifyId[n]);
        }
    }
}

// 释放通道记录上挂载的 Notify 记录（按 notifyId 数组逐条删除）
static inline void ReleaseChannelNotifies(const sim::HcclChannel &rec) {
...
HcclResult HcclGetHcclBuffer(HcclComm comm, void **buffer, uint64_t *size) {
    ...
    auto buffers = RunnerDB::GetByPred<sim::HcclBuffer>(
        [commId](const sim::HcclBuffer &b) { return b.commId == commId; });
    if (buffers.empty() || buffers.size() != 1) {
        // size != 1 表示数据不一致（可能是并发问题或数据库损坏）
        HCCL_VM_ERROR("{}: no HcclBuffer found for commId={:d},size={:d}",
                      __func__, commId, buffers.size());
        return HCCL_E_INTERNAL;
    }
    ...
    *size = hcclBuf.size;
    sim::UpdateOpMemCclBuffer(
        sim::GetVirPtrByDevPtr(reinterpret_cast<uint64_t>(*buffer)), *size);

    HCCL_VM_INFO("{} success, commId={}, buffer={}, size={}", __func__, commId,
                 hcclBuf.addr, *size);
    return HCCL_SUCCESS;
}
 ...
 */
HcclResult HcclThreadAcquire(HcclComm comm, CommEngine engine,
                             uint32_t threadNum, uint32_t notifyNumPerThread,
    ...
    auto existingThreads = RunnerDB::GetByPred<sim::HcclThread>(
        [commId](const sim::HcclThread &t) { return t.commId == commId; });

    uint32_t existingThreadCount =
        static_cast<uint32_t>(existingThreads.size());
    if (existingThreadCount + threadNum > MAX_THREAD_PER_COMM) {
        HCCL_VM_ERROR(
            "{}: threadNum {:d} exceeds limit, existing {:d}, max {:d}",
            __func__, threadNum, existingThreadCount, MAX_THREAD_PER_COMM);
        return HCCL_E_INTERNAL;
    }
    ...
    // 可能很大）
    uint64_t newNotifyCount =
        static_cast<uint64_t>(threadNum) * notifyNumPerThread;
    if (existingNotifyCount + newNotifyCount > MAX_NOTIFY_PER_COMM) {
        HCCL_VM_ERROR(
            "{}: notify count exceeds limit, existing {:d}, new {:d}, max {:d}",
            __func__, existingNotifyCount, newNotifyCount, MAX_NOTIFY_PER_COMM);
        return HCCL_E_INTERNAL;
    }

    // 单线程 Notify 数量不得超过 HcclThread.notifyId 数组容量
    if (notifyNumPerThread > MAX_NOTIFY_PER_THREAD) {
        HCCL_VM_ERROR("{}: notifyNumPerThread={:d} exceeds per-thread cap {:d}",
                      __func__, notifyNumPerThread, MAX_NOTIFY_PER_THREAD);
        return HCCL_E_PARA;
    }

    // 获取当前 runner，用于为 Notify 记录填写 create_ctx_id
    sim::Runner runner;
    if (!sim::GetCurrRunnerTls(g_cur_server_key, runner)) {
    ...
    auto rollbackPrevThreads = [threads](uint32_t upTo) {
        for (uint32_t j = 0; j < upTo; ++j) {
            ...
            auto prev = RunnerDB::GetById<sim::HcclThread>(threads[j]);
            if (prev.has_value()) {
    ...
    for (uint32_t i = 0; i < threadNum; i++) {
        ...
        for (uint32_t n = 0; n < notifyNumPerThread; ++n) {
            ...
            auto nid = RunnerDB::Add<sim::Notify>(notify);
            if (nid == 0u) {
                HCCL_VM_ERROR(
                    "{}: failed to add Notify record (i={:d}, n={:d})",
                    __func__, i, n);
                for (uint32_t k = 0; k < n; ++k) {
                    RunnerDB::Delete<sim::Notify>(hcclThread.notifyId[k]);
                }
                notifyOk = false;
                break;
            }
            hcclThread.notifyId[n] = static_cast<uint32_t>(nid);
        }
        if (!notifyOk) {
            rollbackPrevThreads(i);
            return HCCL_E_INTERNAL;
        ...
        auto threadId = RunnerDB::Add<sim::HcclThread>(hcclThread);
        if (threadId == 0) {
            HCCL_VM_ERROR("{}: failed to add HcclThread index {:d}", __func__,
                          i);
            ReleaseThreadNotifies(hcclThread);
            rollbackPrevThreads(i);
            return HCCL_E_INTERNAL;
        }
        threads[i] = static_cast<ThreadHandle>(threadId);
    }

    HCCL_VM_INFO("{} success, commId={:d}, engine={:d}, threadNum={:d}, "
                 "notifyNumPerThread={:d}",
                 __func__, commId, static_cast<int>(engine), threadNum,
                 notifyNumPerThread);
    return HCCL_SUCCESS;
}
 ...
 */
HcclResult HcclThreadAcquireWithConfig(HcclComm comm, CommEngine engine,
                                       uint32_t threadNum, ThreadType type,
    ...
    if (type == THREAD_TYPE_INVALID) {
        HCCL_VM_ERROR("{}: thread type {:d} is invalid", __func__,
    ...
    for (uint32_t i = 0; i < threadNum; ++i) {
        if (config[i].header.magicWord != HCOMM_THREAD_CONFIG_MAGIC_WORD) {
            HCCL_VM_ERROR("{}: config[{:d}] magicWord=0x{:08x} mismatch, "
                          "expected=0x{:08x}, call ThreadConfigInit first",
                          __func__, i, config[i].header.magicWord,
                          HCOMM_THREAD_CONFIG_MAGIC_WORD);
            return HCCL_E_PARA;
        }
        if (config[i].notifyNumPerThread > MAX_NOTIFY_PER_THREAD) {
            HCCL_VM_ERROR("{}: config[{:d}] notifyNumPerThread={:d} "
                          "exceeds per-thread cap {:d}",
                          __func__, i, config[i].notifyNumPerThread,
    ...
    HCCL_VM_INFO("{} success, commId={:d}, engine={:d}, threadNum={:d}, "
                 ...
                 __func__, commId, static_cast<int>(engine), threadNum,
                 static_cast<int>(type), newNotifyCount);
    return HCCL_SUCCESS;
}
...
HcclResult HcclThreadAcquireWithStream(HcclComm comm, CommEngine engine,
                                       aclrtStream stream, uint32_t notifyNum,
                                       ThreadHandle *thread) {
    if (comm == nullptr) {
        HCCL_VM_ERROR("{}: comm is nullptr", __func__);
        return HCCL_E_PTR;
    ...
    if (existingThreadCount + 1 > MAX_THREAD_PER_COMM) {
        HCCL_VM_ERROR("{}: thread count exceeds limit, existing {:d}, max {:d}",
                      __func__, existingThreadCount, MAX_THREAD_PER_COMM);
        return HCCL_E_INTERNAL;
    }
    ...
    if (existingNotifyCount + notifyNum > MAX_NOTIFY_PER_COMM) {
        HCCL_VM_ERROR(
            "{}: notify count exceeds limit, existing {:d}, new {:d}, max {:d}",
            __func__, existingNotifyCount, notifyNum, MAX_NOTIFY_PER_COMM);
        return HCCL_E_INTERNAL;
    }

    // 单线程 Notify 数量不得超过 HcclThread.notifyId 数组容量
    if (notifyNum > MAX_NOTIFY_PER_THREAD) {
        HCCL_VM_ERROR("{}: notifyNum={:d} exceeds per-thread cap {:d}",
                      __func__, notifyNum, MAX_NOTIFY_PER_THREAD);
        return HCCL_E_PARA;
    }
    ...
    for (uint32_t n = 0; n < notifyNum; ++n) {
        ...
        auto nid = RunnerDB::Add<sim::Notify>(notify);
        if (nid == 0u) {
            HCCL_VM_ERROR("{}: failed to add Notify record (n={:d})", __func__,
                          n);
            // 回滚本线程已创建的 Notify
            for (uint32_t k = 0; k < n; ++k) {
                RunnerDB::Delete<sim::Notify>(hcclThread.notifyId[k]);
    ...
    *thread = static_cast<ThreadHandle>(threadId);
    HCCL_VM_INFO("{} success, commId={:d}, engine={:d}, stream={:p}, "
                 "notifyNum={:d}, threadId={:d}",
                 __func__, commId, static_cast<int>(engine), stream, notifyNum,
                 threadId);
    return HCCL_SUCCESS;
}
 ...
 */
static HcclResult SupplementDedicatedThreadNotify(ThreadHandle handle,
                                                  uint32_t target,
                                                  uint64_t createCtxId) {
    if (target > MAX_NOTIFY_PER_THREAD) {
        HCCL_VM_ERROR("{}: target notifyNum={:d} exceeds per-thread cap {:d}",
                      __func__, target, MAX_NOTIFY_PER_THREAD);
        return HCCL_E_PARA;
    }
    auto optThread =
        RunnerDB::GetById<sim::HcclThread>(static_cast<uint64_t>(handle));
    if (!optThread.has_value()) {
        HCCL_VM_ERROR("{}: dedicated thread {:d} not found in DB", __func__,
                      handle);
        return HCCL_E_INTERNAL;
    }
    ...
    for (uint32_t n = current; n < target; ++n) {
        ...
        if (nid == 0u) {
            ...
            for (uint32_t id : newNotifyIds) {
                RunnerDB::Delete<sim::Notify>(id);
            }
            return HCCL_E_INTERNAL;
        }
        newNotifyIds.push_back(static_cast<uint32_t>(nid));
    }

    // 追加到线程记录的 notifyId 数组并更新 notifyNum
    bool ok = RunnerDB::Update<sim::HcclThread>(
        static_cast<uint64_t>(handle), [&newNotifyIds](sim::HcclThread &t) {
            for (uint32_t id : newNotifyIds) {
                if (t.notifyNum >= MAX_NOTIFY_PER_THREAD) {
                    break;
                }
                t.notifyId[t.notifyNum] = id;
                t.notifyNum++;
            }
        });
    if (!ok) {
 ...
 * @retval HCCL_E_INTERNAL 通信域不存在、DB写入失败或超出资源上限
 */
HcclResult HcclDedicatedThreadAcquire(HcclComm comm,
                                      HcclDedicatedThreadType useType,
    ...
    if (useType == HCCL_DED_THREAD_TYPE_INVALID) {
        HCCL_VM_ERROR("{}: useType {:d} is invalid", __func__,
                      static_cast<int>(useType));
        return HCCL_E_PARA;
    }
    if (notifyNumPerThread > MAX_NOTIFY_PER_THREAD) {
        HCCL_VM_ERROR("{}: notifyNumPerThread={:d} exceeds per-thread cap {:d}",
    ...
    *thread = 0;
    ...
    if (it != g_dedicatedThreadMap.end()) {
        ...
        auto optThread = RunnerDB::GetById<sim::HcclThread>(
            static_cast<uint64_t>(it->second));
        if (optThread.has_value()) {
            *thread = it->second;
            HcclResult ret = SupplementDedicatedThreadNotify(
                *thread, notifyNumPerThread, createCtxId);
            if (ret != HCCL_SUCCESS) {
                HCCL_VM_ERROR(
                    "{}: supplement notify failed, thread={:d}, target={:d}",
                    __func__, it->second, notifyNumPerThread);
                return ret;
            }
            HCCL_VM_INFO(
                "{} reuse dedicated thread, commId={:d}, useType={:d}, "
                "thread={:d}, notifyNumPerThread={:d}",
                __func__, commId, static_cast<int>(useType), *thread,
                notifyNumPerThread);
            return HCCL_SUCCESS;
        }
        // 缓存失效，清除并走创建路径
    ...
    if (useType == HCCL_DED_THREAD_TYPE_AICPU_LAUNCH_GE) {
        HCCL_VM_INFO("{} dedicated thread not found for AICPU_LAUNCH_GE, "
    ...
    auto existingThreads = RunnerDB::GetByPred<sim::HcclThread>(
        [commId](const sim::HcclThread &t) { return t.commId == commId; });
    if (static_cast<uint32_t>(existingThreads.size()) + 1 >
        MAX_THREAD_PER_COMM) {
        HCCL_VM_ERROR("{}: thread count exceeds limit, existing {:d}, max {:d}",
                      __func__, existingThreads.size(), MAX_THREAD_PER_COMM);
        return HCCL_E_INTERNAL;
    }
    uint64_t existingNotifyCount = 0;
    for (const auto &t : existingThreads) {
    ...
    if (existingNotifyCount + notifyNumPerThread > MAX_NOTIFY_PER_COMM) {
        HCCL_VM_ERROR(
            "{}: notify count exceeds limit, existing {:d}, new {:d}, max {:d}",
            __func__, existingNotifyCount, notifyNumPerThread,
            MAX_NOTIFY_PER_COMM);
        return HCCL_E_INTERNAL;
    }
    ...
    g_dedicatedThreadMap[key] = static_cast<ThreadHandle>(threadId);
    *thread = static_cast<ThreadHandle>(threadId);

    HCCL_VM_INFO("{} success, commId={:d}, useType={:d}, thread={:d}, "
                 "notifyNumPerThread={:d}",
                 __func__, commId, static_cast<int>(useType), *thread,
                 notifyNumPerThread);
    return HCCL_SUCCESS;
 ...
 */
HcclResult HcclThreadResGetInfo(HcclComm comm, ThreadHandle thread,
                                ThreadResType resType, uint32_t infoLen,
                                void **info) {
    if (comm == nullptr) {
        HCCL_VM_ERROR("{}: comm is nullptr", __func__);
        return HCCL_E_PTR;
    ...
    if (resType != THREAD_RES_TYPE_STREAM) {
        HCCL_VM_ERROR("{}: unsupported resType {:d}", __func__,
                      static_cast<int>(resType));
        return HCCL_E_PARA;
    }
    if (infoLen != sizeof(ThreadResTypeStream)) {
        HCCL_VM_ERROR(
            "{}: infoLen {:d} does not match ThreadResTypeStream size {:d}",
            __func__, infoLen,
    ...
    ThreadResTypeStream stream =
        reinterpret_cast<aclrtStream>(optThread->streamId);
    *reinterpret_cast<ThreadResTypeStream *>(info) = stream;

    HCCL_VM_INFO("{} success, thread={:d}, stream={:p}", __func__, thread,
                 stream);
    return HCCL_SUCCESS;
}
...
HcclResult HcclChannelAcquire(HcclComm comm, CommEngine engine,
    ...
    for (uint32_t i = 0; i < channelNum; i++) {
        ...
        uint64_t notifyNum = (engine == COMM_ENGINE_CCU)
                                 ? CCU_DEFAULT_NOTIFY_NUM
                                 : desc.notifyNum;

        // CCU 引擎不支持外部 memHandle 交换（仅使用通信域内置缓冲区）
        bool isCcuWithExtraMem =
            (engine == COMM_ENGINE_CCU) && (desc.memHandleNum > 0);
        if (isCcuWithExtraMem) {
            HCCL_VM_ERROR(
                "{}: CCU engine does not support external memHandle exchange",
                __func__);
            return HCCL_E_PARA;
        }
        ...
        if (notifyNum > MAX_NOTIFY_PER_CHANNEL) {
            HCCL_VM_ERROR(
                "{}: notifyNum={:d} exceeds per-channel cap {:d} (i={:d})",
                __func__, notifyNum, MAX_NOTIFY_PER_CHANNEL, i);
            return HCCL_E_PARA;
        }

        // 通道复用逻辑：检查是否已存在相同 (commId, engine, remoteRankId)
        // 的通道 如果存在则直接返回已有通道的句柄，避免重复创建
        auto existingChannel = RunnerDB::GetByPred<sim::HcclChannel>(
            [commId, engine, remoteRankId](const sim::HcclChannel &ch) {
                return ch.commId == commId &&
                       ch.engine == static_cast<uint8_t>(engine) &&
                       ch.remoteRankId == remoteRankId;
            });
        ...
        for (uint32_t n = 0; n < notifyNum; ++n) {
            ...
            if (nid == 0u) {
                HCCL_VM_ERROR(
                    ...
                    __func__, i, n);
                for (uint32_t k = 0; k < n; ++k) {
                    RunnerDB::Delete<sim::Notify>(hcclChannel.notifyId[k]);
                }
                notifyOk = false;
                break;
            }
            hcclChannel.notifyId[n] = nid;
        }
        if (!notifyOk) {
            return HCCL_E_INTERNAL;
        }

        auto channelId = RunnerDB::Add<sim::HcclChannel>(hcclChannel);
        if (channelId == 0) {
            HCCL_VM_ERROR("{}: failed to add HcclChannel index {:d}", __func__,
                          i);
            ReleaseChannelNotifies(hcclChannel);
            return HCCL_E_INTERNAL;
        }
        channels[i] = static_cast<ChannelHandle>(channelId);
    }

    HCCL_VM_INFO("{} success, commId={:d}, engine={:d}, channelNum={:d}",
                 __func__, commId, static_cast<int>(engine), channelNum);
    return HCCL_SUCCESS;
}
 ...
 *
 * 根据传入的HcclChannelDesc数组，检查通信域中是否已存在匹配的通道
 * （commId + engine + remoteRankId）。已存在的通道在channels中返回其句柄，
 * 不存在的返回0（空句柄），由调用方决定是否新建（通常随后调用HcclChannelAcquire）。
 *
 * 该接口为CANN 9.3.0新激活的北向接口：真实HCCL在CcuAcquireKernelChannels中会先
 * 调用本接口查询可复用通道。若不在此处打桩，调用会落到真实libhcomm实现，而真实
 * 实现会按真实hcclComm解引用北向的通信域句柄（北向为DB行号假句柄），导致段错误。
 *
 * @param comm 输入：通信域句柄。
 * @param engine 输入：通信引擎类型。
 * @param channelDescs 输入：通道描述数组，由HcclChannelDescInit初始化。
 * @param channelNum 输入：通道数量，取值范围(0, 1024 * 1024]。
 * @param channels
 * 输出：查询结果句柄数组，需传入channelNum大小；不存在的通道置0。
 *
 * @return HcclResult 接口成功返回HCCL_SUCCESS，其他失败。
 * @retval HCCL_SUCCESS 查询成功
 * @retval HCCL_E_PTR comm、channelDescs或channels指针为空
 * @retval HCCL_E_PARA channelNum为0或超过上限
 * @retval HCCL_E_INTERNAL 通信域不存在
 */
HcclResult HcclChannelQuery(HcclComm comm, CommEngine engine,
                            const HcclChannelDesc *channelDescs,
                            uint32_t channelNum, ChannelHandle *channels) {
    if (comm == nullptr || channelDescs == nullptr || channels == nullptr) {
        HCCL_VM_ERROR("{}: comm, channelDescs or channels is nullptr",
                      __func__);
        return HCCL_E_PTR;
    }
 ...
 * 周期性查询 RunnerDB 中属于指定 remoteCommId 的 sim::HcclBuffer 记录，
 * 直到查询结果非空或总等待时长超过 10s 为止。
 *
 * @param[in]  remoteCommId 远端通信域的 id（sim::Communicator::id）。
 ...
 */
static bool
WaitForRemoteHcclBufferRecords(uint64_t remoteCommId,
                               std::vector<sim::HcclBuffer> &outRecords) {
    auto deadline = std::chrono::steady_clock::now() +
                    std::chrono::milliseconds(REMOTE_MEM_WAIT_MAX_MS);
    while (true) {
        outRecords = RunnerDB::GetByPred<sim::HcclBuffer>(
            [remoteCommId](const sim::HcclBuffer &b) {
                return b.commId == remoteCommId;
            });
        if (!outRecords.empty()) {
            return true;
        }
        if (std::chrono::steady_clock::now() >= deadline) {
            HCCL_VM_ERROR(
                "{}: no HcclBuffer records for remoteCommId={:d} after {:d}ms",
                __func__, remoteCommId, REMOTE_MEM_WAIT_MAX_MS);
            outRecords.clear();
            return false;
        }
        std::this_thread::sleep_for(
            std::chrono::milliseconds(REMOTE_MEM_WAIT_POLL_INTERVAL_MS));
    }
}

static bool
WaitForRemoteCommunicator(uint64_t remoteRankId,
                          const sim::Communicator &localComm,
                          std::vector<sim::Communicator> &outComms) {
    auto deadline = std::chrono::steady_clock::now() +
                    std::chrono::milliseconds(REMOTE_MEM_WAIT_MAX_MS);
    while (true) {
        outComms = RunnerDB::GetByPred<sim::Communicator>(
            [remoteRankId, &localComm](const sim::Communicator &c) {
                return c.rank_id == remoteRankId &&
                       c.rank_size == localComm.rank_size &&
                       c.comm_hash == localComm.comm_hash &&
                       std::strncmp(c.comm_id, localComm.comm_id,
                                    sizeof(c.comm_id)) == 0;
            });
        if (!outComms.empty()) {
            return true;
        ...
        if (std::chrono::steady_clock::now() >= deadline) {
            HCCL_VM_ERROR("{}: no remote communicator found after {:d}ms, "
                          "remoteRankId={:d}, rankSize={:d}",
                          __func__, REMOTE_MEM_WAIT_MAX_MS, remoteRankId,
                          localComm.rank_size);
            outComms.clear();
            return false;
        }
 ...
 */
HcclResult HcclChannelGetHcclBuffer(HcclComm comm, ChannelHandle channel,
    ...
    if (optChannel->commId != localCommId) {
        HCCL_VM_ERROR("{}: channel {:d} does not belong to comm {:d}, "
                      "channel.commId={:d}",
    ...
    auto optLocalComm = RunnerDB::GetById<sim::Communicator>(localCommId);
    if (!optLocalComm.has_value()) {
        HCCL_VM_ERROR("{}: local communicator {:d} not found", __func__,
                      localCommId);
        return HCCL_E_INTERNAL;
    }
    const uint32_t localRankSize = optLocalComm->rank_size;

    std::vector<sim::Communicator> remoteComms;
    ...
    if (remoteComms.size() != 1) {
        HCCL_VM_ERROR("{}: ambiguous remote communicator, matched={:d}, "
                      "remoteRankId={:d}, rankSize={:d}",
                      __func__, remoteComms.size(), remoteRankId,
                      localRankSize);
        return HCCL_E_INTERNAL;
    }

    const uint64_t remoteCommId = remoteComms[0].id;
    std::vector<sim::HcclBuffer> buffers;
    if (!WaitForRemoteHcclBufferRecords(remoteCommId, buffers)) {
        HCCL_VM_ERROR("{}: timed out waiting for HcclBuffer, "
                      "channel={:d}, localCommId={:d}, remoteRankId={:d}, "
                      "remoteCommId={:d}",
                      __func__, channel, localCommId, remoteRankId,
                      remoteCommId);
        return HCCL_E_INTERNAL;
    }
    ...
    *size = hcclBuf.size;
    ...
    HCCL_VM_INFO("{} success, channel={:d}, localCommId={:d}, "
 ...
 * 以 10ms 为间隔周期性查询 RunnerDB 中属于指定 remoteCommId 的
 * sim::HcclMem 记录，直到查询结果非空或总等待时长超过 10s 为止。
 *
 * @param[in]  remoteCommId 远端通信域的 id（sim::Communicator::id）。
 ...
 * @return false 等待超时仍未查到，已在内部输出 HCCL_VM_ERROR。
 */
static bool WaitForRemoteMemRecords(uint64_t remoteCommId,
                                    std::vector<sim::HcclMem> &outRecords) {
    auto deadline = std::chrono::steady_clock::now() +
                    std::chrono::milliseconds(REMOTE_MEM_WAIT_MAX_MS);
    while (true) {
        outRecords = RunnerDB::GetByPred<sim::HcclMem>(
            [remoteCommId](const sim::HcclMem &m) {
                return m.commId == remoteCommId;
            });
        if (!outRecords.empty()) {
            return true;
        }
        if (std::chrono::steady_clock::now() >= deadline) {
            HCCL_VM_ERROR(
                "{}: no HcclMem records for remoteCommId={:d} after {:d}ms",
                __func__, remoteCommId, REMOTE_MEM_WAIT_MAX_MS);
            outRecords.clear();
            return false;
        }
 ...
 * 本桩通过 sim::HcclMem 表（按远端 commId 查询）返回远端 rank 注册的内存。
 * 返回的指针在下次调用本接口前有效（与真实实现的缓存语义一致）。
 *
 * @param comm 输入：通信域句柄。
...
HcclResult HcclChannelGetRemoteMems(HcclComm comm, ChannelHandle channel,
                                    uint32_t *memNum, CommMem **remoteMems,
                                    char ***memTags) {
    if (comm == nullptr || memNum == nullptr || remoteMems == nullptr ||
        memTags == nullptr) {
        HCCL_VM_ERROR("{}: comm, memNum, remoteMems or memTags is nullptr",
                      __func__);
        return HCCL_E_PTR;
    }
    *memNum = 0;
    ...
    *memTags = nullptr;
    ...
    const uint64_t remoteCommId = remoteComms[0].id;
    std::vector<sim::HcclMem> memRecords;
    if (!WaitForRemoteMemRecords(remoteCommId, memRecords)) {
        HCCL_VM_ERROR("{}: timed out waiting for remoteCommId={:d}, "
                      "channel={:d}, localCommId={:d}, remoteRankId={:d}",
                      __func__, remoteCommId, channel, localCommId,
                      remoteRankId);
        return HCCL_E_INTERNAL;
    }
    ...
    *memTags = tagPtrBuf.data();
    ...
    HCCL_VM_INFO("{} success, channel={:d}, localCommId={:d}, "
                 ...
                 __func__, channel, localCommId, remoteRankId, remoteCommId,
                 *memNum,
                 static_cast<const void *>((*remoteMems)[*memNum - 1].addr),
                 (*remoteMems)[*memNum - 1].size, (*memTags)[*memNum - 1]);
    return HCCL_SUCCESS;
}
 ...
 */
HcclResult HcclEngineCtxCreate(HcclComm comm, const char *ctxTag,
                               CommEngine engine, uint64_t size, void **ctx) {
    if (comm == nullptr) {
        HCCL_VM_ERROR("{}: comm is nullptr", __func__);
        return HCCL_E_PTR;
    }
    if (ctxTag != nullptr && std::strlen(ctxTag) > HCCL_RES_TAG_MAX_LEN) {
        HCCL_VM_ERROR("{}: ctxTag length {:d} exceeds max {:d}", __func__,
                      std::strlen(ctxTag), HCCL_RES_TAG_MAX_LEN);
        return HCCL_E_PARA;
    }
    if (size == 0) {
    ...
    if (engine == COMM_ENGINE_AICPU || engine == COMM_ENGINE_AICPU_TS) {
        ...
        uint32_t devKey = static_cast<uint32_t>(sim::GetCurrDeviceKey());
        TryLaunchAicpuDevProcForRank(rankId, devKey);
    }

    HcclResult ret =
        EngineCtxRegistry::Instance().Create(commId, ctxTag, engine, size, ctx);
    if (ret != HCCL_SUCCESS) {
        HCCL_VM_ERROR(
            "{}: EngineCtxRegistry::Create failed, ret={:d}, size={:d}",
            __func__, static_cast<int>(ret), size);
        return ret;
    }

    HCCL_VM_INFO("{} success, commId={:d}, ctxTag={}, engine={:d}, "
                 "size={:d}, ctx={:p}",
                 __func__, commId, ctxTag ? ctxTag : "",
                 static_cast<int>(engine), size, *ctx);
    return HCCL_SUCCESS;
}
 ...
 */
HcclResult HcclEngineCtxGet(HcclComm comm, const char *ctxTag,
                            CommEngine engine, void **ctx, uint64_t *size) {
    ...
    if (!EngineCtxRegistry::Instance().Get(commId, tagHash, engineVal, ctx,
                                           size)) {
    ...
    HCCL_VM_INFO("{} success, commId={:d}, ctxTag={}, engine={:d}, "
                 "ctx={:p}, size={:d}",
                 __func__, commId, ctxTag ? ctxTag : "",
                 static_cast<int>(engine), *ctx, *size);
    return HCCL_SUCCESS;
}
 ...
 */
HcclResult HcclEngineCtxCopy(HcclComm comm, CommEngine engine,
                             const char *ctxTag, const void *srcCtx,
    ...
    HcclResult ret = EngineCtxRegistry::Instance().Copy(
        commId, tagHash, engineVal, srcCtx, size, dstCtxOffset);
    if (ret == HCCL_E_NOT_FOUND) {
        HCCL_VM_ERROR("{}: dst ctx not found, commId={:d}, ctxTag={}", __func__,
                      commId, ctxTag ? ctxTag : "");
        return HCCL_E_NOT_FOUND;
    }
    if (ret == HCCL_E_PARA) {
        HCCL_VM_ERROR("{}: dstCtx overflow, offset={:d} + size={:d}", __func__,
                      dstCtxOffset, size);
        return HCCL_E_PARA;
    }
    if (ret != HCCL_SUCCESS) {
        HCCL_VM_ERROR("{}: aclrtMemcpy failed, ret={:d}", __func__,
                      static_cast<int>(ret));
        return ret;
    }

    HCCL_VM_INFO("{} success, commId={:d}, engine={:d}, ctxTag={}, "
                 "srcCtx={:p}, size={:d}, dstCtxOffset={:d}",
                 __func__, commId, static_cast<int>(engine),
                 ctxTag ? ctxTag : "", srcCtx, size, dstCtxOffset);
    return HCCL_SUCCESS;
}
 ...
 * @retval HCCL_E_PARA ctxTag超长
 */
HcclResult HcclEngineCtxDestroy(HcclComm comm, const char *ctxTag,
                                CommEngine engine) {
    if (comm == nullptr) {
        HCCL_VM_ERROR("{}: comm is nullptr", __func__);
        return HCCL_E_PTR;
    ...
    uint32_t removed =
        EngineCtxRegistry::Instance().Destroy(commId, tagHash, engineVal);

    HCCL_VM_INFO("{} done, commId={:d}, ctxTag={}, engine={:d}, removed={:d}",
                 __func__, commId, ctxTag ? ctxTag : "",
...
HcclResult HcclCommMemReg(HcclComm comm, const char *memTag, const CommMem *mem,
    ...
    if (std::strlen(memTag) > HCCL_RES_TAG_MAX_LEN) {
        HCCL_VM_ERROR("{}: memTag length {:d} exceeds max {:d}", __func__,
                      std::strlen(memTag), HCCL_RES_TAG_MAX_LEN);
        return HCCL_E_PARA;
    }
    ...
    auto existing = RunnerDB::GetByPred<sim::HcclMem>(
        [commId, tagHash](const sim::HcclMem &m) {
            return m.commId == commId && m.memTag == tagHash;
        });
    if (!existing.empty()) {
        HCCL_VM_ERROR("{}: memTag '{}' already registered in comm {:d}",
                      __func__, memTag, commId);
        return HCCL_E_INTERNAL;
    }
    ...
    auto memId = RunnerDB::Add<sim::HcclMem>(hcclMem);
    if (memId == 0) {
    ...
    // 后续可通过此句柄查询或注销内存注册
    *memHandle = reinterpret_cast<HcclMemHandle>(memId);
    HCCL_VM_INFO("{} success, commId={:d}, memTag={}, memType={:d}, "
                 "memAddr={:p}, memSize={:d}, handleId={:d}",
                 __func__, commId, memTag, static_cast<int>(mem->type),
                 mem->addr, mem->size, memId);
    return HCCL_SUCCESS;
}
