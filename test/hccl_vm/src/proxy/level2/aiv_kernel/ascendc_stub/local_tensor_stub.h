...
template <typename T> class LocalTensor {
  public:
    ...
    __aicore__ __inout_pipe__(S) void SetValue(const uint32_t index,
                                               const T value) const {
    ...
    __aicore__ __inout_pipe__(S) T GetValue(const uint32_t offset) const {
        if (offset >= size_) {
            HCCL_VM_ERROR("LocalTensor GetValue out-of-bounds, offset={:d}",
                          offset);
        }
        return *(ptr_ + offset);
    }
    ...
    __aicore__ uint64_t GetPhyAddr() const {
    ...
    __aicore__ uint32_t GetLength() const {
