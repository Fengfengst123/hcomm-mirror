...
extern __aicore__ inline void PRINTF(const char* __restrict format, ...)
{
    ...
    const int messageSize = std::vsnprintf(nullptr, 0, format, argsCopy);
    va_end(argsCopy);
    if (messageSize < 0) {
        va_end(args);
        HCCL_VM_INFO("AIV PRINTF format failed: {}", format);
        return;
    }

    std::string message(static_cast<size_t>(messageSize) + 1, '\0');
    std::vsnprintf(message.data(), message.size(), format, args);
    va_end(args);
    message.resize(static_cast<size_t>(messageSize));
    HCCL_VM_INFO("[ASCENDC::PRINTF] {}", message);
}
