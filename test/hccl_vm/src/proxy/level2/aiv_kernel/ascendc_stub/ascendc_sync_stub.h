...
inline void send_flag(uint32_t targetRank, uint64_t commInfoOffset, int32_t curTag)
{
    ...
    const auto flagBuffer = executor.ResolveGlobalDataSlice(
        executor.GetAivCommInfoBuffer(targetRank).addr + commInfoOffset, sizeof(AivSim::flag_t));
    aiv->AppendScalar(std::make_shared<AivSim::AivTaskSendFlag>(targetRank, flagBuffer, curTag));
}
...
inline void recv_flag(uint32_t targetRank, uint64_t commInfoOffset, int32_t curTag)
{
    ...
    const auto flagBuffer = executor.ResolveGlobalDataSlice(
        executor.GetAivCommInfoBuffer(targetRank).addr + commInfoOffset, sizeof(AivSim::flag_t));
    aiv->AppendScalar(std::make_shared<AivSim::AivTaskRecvFlag>(targetRank, flagBuffer, curTag));
}
