...
inline void pipe_barrier(pipe_t pipe) {
    ...
    default:
        HCCL_VM_ERROR("PipeBarrier type[{:d}] not support",
                      static_cast<uint32_t>(pipe));
    }
}

inline void send_flag(uint32_t targetRank, uint64_t commInfoOffset,
                      int32_t curTag) {
    auto curBlockIdx = GetBlockIdx();
    auto aiv = AivSim::AivKernelExecutor::GetInstance().GetAivCore(curBlockIdx);
    if (aiv == nullptr) {
    ...
    const auto flagBuffer = executor.ResolveGlobalDataSlice(
        executor.GetAivCommInfoBuffer(targetRank).addr + commInfoOffset,
        sizeof(AivSim::flag_t));
    aiv->AppendScalar(std::make_shared<AivSim::AivTaskSendFlag>(
        targetRank, flagBuffer, curTag));
}

inline void recv_flag(uint32_t targetRank, uint64_t commInfoOffset,
                      int32_t curTag) {
    auto curBlockIdx = GetBlockIdx();
    auto aiv = AivSim::AivKernelExecutor::GetInstance().GetAivCore(curBlockIdx);
    if (aiv == nullptr) {
    ...
    const auto flagBuffer = executor.ResolveGlobalDataSlice(
        executor.GetAivCommInfoBuffer(targetRank).addr + commInfoOffset,
        sizeof(AivSim::flag_t));
    aiv->AppendScalar(std::make_shared<AivSim::AivTaskRecvFlag>(
        targetRank, flagBuffer, curTag));
}

template <pipe_t pipe> __aicore__ void PipeBarrier() { pipe_barrier(pipe); }
...
template <HardEvent event> __aicore__ void SetFlag(TEventID eventID) {
    auto curBlockIdx = GetBlockIdx();
    auto aiv = AivSim::AivKernelExecutor::GetInstance().GetAivCore(curBlockIdx);
    if (aiv == nullptr) {
        HCCL_VM_ERROR("Get AIV core failed, curBlockIdx={:d}", curBlockIdx);
    ...
    auto task =
        std::make_shared<AivSim::AivTaskSetFlag>(srcPipe, dstPipe, eventID);
    ...
    case pipe_t::PIPE_MTE3:
        ...
        break;
    default:
        HCCL_VM_ERROR("SetFlag pipe type[{:d}] not support",
                      static_cast<uint32_t>(srcPipe));
    }
}

template <HardEvent event> __aicore__ void WaitFlag(TEventID eventID) {
    auto curBlockIdx = GetBlockIdx();
    auto aiv = AivSim::AivKernelExecutor::GetInstance().GetAivCore(curBlockIdx);
    if (aiv == nullptr) {
        HCCL_VM_ERROR("Get AIV core failed, curBlockIdx={:d}", curBlockIdx);
    ...
    pipe_t dstPipe = GetEventDstPipe(event);
    auto task =
        std::make_shared<AivSim::AivTaskWaitFlag>(srcPipe, dstPipe, eventID);
    ...
    case pipe_t::PIPE_MTE3:
        ...
        break;
    default:
        HCCL_VM_ERROR("WaitFlag pipe type[{:d}] not support",
                      static_cast<uint32_t>(dstPipe));
    }
}

template <bool isAIVOnly = true> __aicore__ void SyncAll() {
    auto curBlockIdx = GetBlockIdx();
    auto aiv = AivSim::AivKernelExecutor::GetInstance().GetAivCore(curBlockIdx);
    if (aiv == nullptr) {
        HCCL_VM_ERROR("Get AIV core failed, curBlockIdx={:d}", curBlockIdx);
