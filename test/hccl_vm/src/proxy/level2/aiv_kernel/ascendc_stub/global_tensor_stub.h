...
template <typename T> class GlobalTensor {
  public:
    ...
    __aicore__ __inout_pipe__(S) T GetValue(const uint64_t offset) const {
        if (offset >= size_) {
            HCCL_VM_ERROR("GlobalTensor GetValue out-of-bounds, offset={:d}",
                          offset);
        }
        return *(ptr_ + offset);
    }
    ...
    __aicore__ GlobalTensor operator[](const uint64_t offset) const {
        if (offset >= size_) {
            HCCL_VM_ERROR("LocalTensor operator[] out-of-bounds, offset={:d}",
                          offset);
        }
    ...
    __aicore__ uint64_t GetSize() const {
        return size_ == UINT64_MAX ? 0 : size_;
    }
    __aicore__ uint64_t GetLength() const {
        return size_ == UINT64_MAX ? 0 : size_ * sizeof(T);
    }
