...
AivKernelExecutor::AivKernelExecutor() {
    std::fill(std::begin(rankDeviceIds_), std::end(rankDeviceIds_), UINT32_MAX);
}
...
void AivKernelExecutor::Init(RankId rankId, size_t blockNum,
                             uint32_t rankSize) {
    Reset();
    rankId_ = rankId;
    rankSize_ = std::min(rankSize, MAX_RANK_NUM);
    if (rankSize > MAX_RANK_NUM) {
        HCCL_VM_WARN("Init rankSize={} out of range, clamp to MAX_RANK_NUM={}",
                     rankSize, MAX_RANK_NUM);
    }
...
void AivKernelExecutor::Reset() {
    ...
    std::fill(std::begin(aivCommInfoBuffer_), std::end(aivCommInfoBuffer_),
              Mem{});
    // UB is persistent hardware state. Reset only clears per-launch state and
    // keeps the same address and communication-domain state for every launch.
}

bool AivKernelExecutor::SetCommIdentity(uint64_t commId,
                                        const std::string &commName) {
    if (commId == 0 || commName.empty()) {
        HCCL_VM_ERROR(
            "invalid AIV communicator identity, commId={}, commName={}", commId,
            commName);
        return false;
    }
    if (commId_ == commId) {
...
bool AivKernelExecutor::SetDeviceId(RankId rankId, DeviceId deviceId) {
    if (rankId >= rankSize_ || deviceId == UINT32_MAX) {
        HCCL_VM_ERROR(
            "SetDeviceId invalid mapping, rankId={}, deviceId={}, rankSize={}",
            rankId, deviceId, rankSize_);
        return false;
    }
...
uint64_t AivKernelExecutor::ResolveDataSliceAddress(const AivDataSlice &slice,
                                                    RankId rankId,
    ...
    if (buffer.addr == 0 || slice.GetOffset() > buffer.size ||
        slice.GetSize() > buffer.size - slice.GetOffset()) {
        return 0;
    }
...
std::shared_ptr<AivCore> AivKernelExecutor::GetAivCore(int64_t blockIdx) {
    return (blockIdx >= 0 && blockIdx < aivCores_.size()) ? aivCores_[blockIdx]
                                                          : nullptr;
}

void AivKernelExecutor::SetIoBuffer(uint64_t inBuffer, uint64_t inBufferSize,
...
void AivKernelExecutor::SetCommBuffer(RankId rankId, uint64_t cclBuffer,
...
bool AivBufferMatch(uint64_t addr, uint64_t size, const Mem &buffer,
...
AivDataSlice AivKernelExecutor::ResolveGlobalDataSlice(
    uint64_t addr, uint64_t size, RankId *rankId, BlockId ubBlockId) const {
    if (rankId != nullptr) {
        *rankId = UINT32_MAX;
    }
    ...
    if (ubBlockId != UINT32_MAX &&
        AivBufferMatch(addr, size, GetUbBuffer(ubBlockId), AivBufferType::UB,
                       rankId_, matchedSlice, rankId)) {
        matchedSlice.SetDeviceId(GetDeviceId(rankId_));
        matchedSlice.SetVirtualAddr(addr);
        return matchedSlice;
    }
    if (AivBufferMatch(addr, size, inBuffer_, AivBufferType::INPUT, rankId_,
                       matchedSlice, rankId)) {
        matchedSlice.SetDeviceId(GetDeviceId(rankId_));
        matchedSlice.SetVirtualAddr(addr);
        return matchedSlice;
    }
    if (AivBufferMatch(addr, size, outBuffer_, AivBufferType::OUTPUT, rankId_,
                       matchedSlice, rankId)) {
        matchedSlice.SetDeviceId(GetDeviceId(rankId_));
        matchedSlice.SetVirtualAddr(addr);
        return matchedSlice;
    ...
    for (RankId i = 0; i < rankSize_; ++i) {
        if (AivBufferMatch(addr, size, cclBuffer_[i], AivBufferType::CCL, i,
                           matchedSlice, rankId)) {
            matchedSlice.SetDeviceId(GetDeviceId(i));
            matchedSlice.SetVirtualAddr(addr);
            return matchedSlice;
        }
        if (AivBufferMatch(addr, size, aivCommInfoBuffer_[i],
                           AivBufferType::AIV_COMM, i, matchedSlice, rankId)) {
            matchedSlice.SetDeviceId(GetDeviceId(i));
            matchedSlice.SetVirtualAddr(addr);
            return matchedSlice;
        }
