...
public:
    ...
    __aicore__ inline GM_ADDR GetGmIn(uint32_t rank)
    {
        ...
        const auto buffer = (gmInOffset_ == 0) ? executor.GetCclBuffer(rank) : executor.GetAivCommInfoBuffer(rank);
    ...
    {
        const auto& executor = AivSim::AivKernelExecutor::GetInstance();
        return reinterpret_cast<GM_ADDR>(executor.GetAivCommInfoBuffer(rank).addr + gmOutOffset_);
    }
...
{
    const uint64_t commInfoOffset = gmOutOffset_ + flag_offset * AivCommInfoLayout::SYNC_CELL_BYTES;
