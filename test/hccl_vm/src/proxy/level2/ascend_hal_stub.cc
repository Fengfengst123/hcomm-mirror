...
extern bool GetPhyMemBlockByVirPtr(void *virPtr, uint32_t &offset,
...
drvError_t halGetChipCapability(uint32_t devId,
                                struct halCapabilityInfo *info) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halGetCapabilityGroupInfo(int device_id, int ts_id, int group_id,
                                     struct capability_group_info *group_info,
                                     int group_count) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t drvGetDevNum(uint32_t *num_dev) {
    ...
    auto devs = RunnerDB::GetByPred<sim::Device>(
        [](const sim::Device &d) { return d.status == 0; });
...
drvError_t drvGetLocalDevIDByHostDevID(uint32_t host_dev_id,
                                       uint32_t *local_dev_id) {
    local_dev_id = &host_dev_id;
    HCCL_VM_INFO("host_dev_id:{} local_dev_id:{}", host_dev_id, *local_dev_id);
    return DRV_ERROR_NONE;
}

drvError_t halSensorNodeRegister(uint32_t devId, struct halSensorNodeCfg *cfg,
                                 uint64_t *handle) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halGetDeviceInfo(uint32_t devId, int32_t moduleType,
                            int32_t infoType, int64_t *value) {
    if (moduleType == (int32_t)MODULE_TYPE_SYSTEM &&
        infoType == (int32_t)INFO_TYPE_VERSION) {
        int64_t hardwareVersion = 0xf00;
        // torch_npu 需要赋值
        // *value = hardwareVersion;
    } else if ((moduleType == (int32_t)MODULE_TYPE_SYSTEM) &&
               (infoType == (int32_t)INFO_TYPE_CORE_NUM)) {
        *value = 1;
    } else if ((moduleType == (int32_t)MODULE_TYPE_AICORE) &&
               (infoType == (int32_t)INFO_TYPE_CORE_NUM)) {
        // 模拟昇腾 AI CORE 数量
        *value = 16;
    } else if ((moduleType == (int32_t)MODULE_TYPE_VECTOR_CORE) &&
               (infoType == (int32_t)INFO_TYPE_CORE_NUM)) {
        // 模拟昇腾 VECTOR CORE
        *value = 16;
    } else {
...
drvError_t halGetDeviceInfoByBuff(uint32_t devId, int32_t moduleType,
...
drvError_t halGetPhyDeviceInfo(uint32_t phyId, int32_t moduleType,
                               int32_t infoType, int64_t *value) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halGetPairDevicesInfo(uint32_t devId, uint32_t otherDevId,
                                 int32_t infoType, int64_t *value) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halGetPairPhyDevicesInfo(uint32_t devId, uint32_t otherDevId,
                                    int32_t infoType, int64_t *value) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t drvDeviceExceptionHookRegister(
    drvDeviceExceptionReporFunc exception_callback_func) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t drvDeviceGetPhyIdByIndex(uint32_t devIndex, uint32_t *phyId) {
    auto ret = RunnerDB::GetOneByPred<sim::Device>(
        [devIndex](const sim::Device &d) { return d.logic_id == devIndex; });
    if (!ret.second) {
...
drvError_t drvDeviceGetIndexByPhyId(uint32_t phyId, uint32_t *devIndex) {
    ...
    // 从进程通信域配置缓存推导phyId→device序号(与Device.logic_id一致)
    uint32_t logicDevId = 0;
    if (sim::GetDevIndexByPhyId(g_cur_server_key, phyId, logicDevId) !=
        ACL_SUCCESS) {
        HCCL_VM_ERROR("cannot find devIndex by phyId:{}, serverKey:{}", phyId,
                      g_cur_server_key);
        return DRV_ERROR_NO_DEVICE;
    }
    *devIndex = logicDevId;
...
drvError_t drvQueryProcessHostPid(int pid, unsigned int *chip_id,
                                  unsigned int *vfid, unsigned int *host_pid,
...
drvError_t drvMemRead(uint32_t devId, MEM_CTRL_TYPE memType, uint32_t offset,
                      uint8_t *value, uint32_t len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t drvMemWrite(uint32_t devId, MEM_CTRL_TYPE memType, uint32_t offset,
                       uint8_t *value, uint32_t len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halDeviceCanAccessPeer(int *can_access_peer, uint32_t dev,
                                  uint32_t peer_dev) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t drvCreateIpcNotify(struct drvIpcNotifyInfo *info, char *name,
                              unsigned int len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t drvDestroyIpcNotify(const char *name,
                               struct drvIpcNotifyInfo *info) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halShrIdCreate(struct drvShrIdInfo *info, char *name,
                          uint32_t name_len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halShrIdSetAttribute(const char *name, enum shrIdAttrType type,
                                struct shrIdAttr attr) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halShrIdGetAttribute(const char *name, enum shrIdAttrType type,
                                struct shrIdAttr *attr) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halSafeIslandTimeSyncMsgSend(uint32_t devId, void *msg,
                                        size_t msgSize) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

DVresult drvMbindHbm(DVdeviceptr devPtr, size_t len, uint32_t type,
                     uint32_t dev_id) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

DVresult drvLoadProgram(DVdevice device_id, void *program, unsigned int offset,
...
DVresult drvMemcpy(DVdeviceptr dst, size_t dest_max, DVdeviceptr src,
                   size_t byte_count) {
...
drvError_t halMemcpy(void *dst, size_t dst_size, void *src, size_t count,
                     struct memcpy_info *info) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

DVresult halMemCpyAsync(DVdeviceptr dst, size_t dest_max, DVdeviceptr src,
...
DVresult halMemcpyBatch(uint64_t dst[], uint64_t src[], size_t size[],
                        size_t count) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halSdmaCopy(DVdeviceptr dst, size_t dst_size, DVdeviceptr src,
                       size_t len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halSdmaBatchCopy(void *dst[], void *src[], size_t size[],
                            int count) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

DVresult drvMemConvertAddr(DVdeviceptr p_src, DVdeviceptr p_dst, UINT32 len,
...
DVresult drvMemPrefetchToDevice(DVdeviceptr dev_ptr, size_t len,
                                DVdevice device) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

DVresult halShmemCreateHandle(DVdeviceptr vptr, size_t byte_count, char *name,
                              uint32_t name_len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
DVresult halShmemSetPodPid(const char *name, uint32_t sdid, int pid[],
                           int num) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
DVresult halShmemOpenHandleByDevId(DVdevice dev_id, const char *name,
                                   DVdeviceptr *vptr) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
DVresult halShmemGetAttribute(const char *name, enum ShmemAttrType type,
                              uint64_t *attr) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halHostRegister(void *src_ptr, UINT64 size, UINT32 flag,
                           UINT32 devid, void **dst_ptr) {
    if (src_ptr == nullptr || dst_ptr == nullptr) {
        HCCL_VM_ERROR(
            "halHostRegister: invalid params, src_ptr={:p}, dst_ptr={:p}",
    ...
    uint64_t devPtr =
        static_cast<uint64_t>(reinterpret_cast<uintptr_t>(src_ptr));
    ...
    if (!virMemRes.second) {
        HCCL_VM_ERROR("can not find VirtualMemBlock by virPtr:{:p}", src_ptr);
        return DRV_ERROR_INVALID_VALUE;
    }
    ...
    uint64_t offset = devPtr - virMemRes.first.dev_mapped_ptr;
    sim::PhyMemBlock phyMem = *phyMemRes;
    ...
    if (hostPtr == nullptr) {
        HCCL_VM_ERROR(
            "halHostRegister: AcquirePhyMem failed for name={}, size={:d}",
            phyMem.name, size);
        return DRV_ERROR_INNER_ERR;
    }

    *dst_ptr = hostPtr + offset;
    HCCL_VM_INFO("halHostRegister stub: src_ptr={:p} -> hostPtr={:p}, name={}, "
                 "size={:d}",
                 (void *)src_ptr, *dst_ptr, phyMem.name, size);

    // 记录host侧映射过的hostPtr到VirtualMemBlock表
    void *finalHostAddr = *dst_ptr;
    RunnerDB::Update<sim::VirtualMemBlock>(
        virMemRes.first.id,
        [finalHostAddr](sim::VirtualMemBlock &virtMemBlock) {
            virtMemBlock.host_ptr = static_cast<uint64_t>(
                reinterpret_cast<uintptr_t>(finalHostAddr));
        });
...
drvError_t halHostUnregister(void *src_ptr, UINT32 devid) {
    if (src_ptr == nullptr) {
        HCCL_VM_WARN("halHostUnregister: src_ptr is null");
        return DRV_ERROR_INVALID_VALUE;
    }

    sim::PhyMemBlock phyMem{};
    uint32_t offset = 0;
    if (!GetPhyMemBlockByVirPtr(src_ptr, offset, phyMem)) {
        HCCL_VM_INFO("halHostUnregister: src_ptr={:p} not a dev mem, skip",
                     (void *)src_ptr);
        return DRV_ERROR_NONE;
    }
    ...
    // 清空 VirtualMemBlock.host_ptr，避免 unregister 后
    // GetDevMapperAddrByHostPtr 命中已失效映射
    uint64_t devPtr =
        static_cast<uint64_t>(reinterpret_cast<uintptr_t>(src_ptr));
    uint64_t deviceKey = sim::GetCurrDeviceKey();
    auto virMemRes = RunnerDB::GetOneByPred<sim::VirtualMemBlock>(
    ...
    if (virMemRes.second) {
        RunnerDB::Update<sim::VirtualMemBlock>(
            virMemRes.first.id, [](sim::VirtualMemBlock &virtMemBlock) {
                virtMemBlock.host_ptr = 0;
            });
    }

    HCCL_VM_INFO("halHostUnregister: src_ptr={:p}, name={}", (void *)src_ptr,
                 phyMem.name);
    return DRV_ERROR_NONE;
}

drvError_t halHostUnregisterEx(void *src_ptr, UINT32 devid, UINT32 flag) {
    return halHostUnregister(src_ptr, devid);
}
...
drvError_t halMemAdvise(DVdeviceptr ptr, size_t count, unsigned int type,
                        DVdevice device) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halCheckProcessStatus(DVdevice device, processType_t process_type,
                                 processStatus_t status, bool *is_matched) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halCheckProcessStatusEx(DVdevice device, processType_t process_type,
...
drvError_t halMemGetAddressReserveRange(void **ptr, size_t *size,
                                        drv_mem_addr_reserve_type type,
                                        uint64_t flag) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halMemAddressReserve(void **ptr, size_t size, size_t alignment,
...
drvError_t halMemCreate(drv_mem_handle_t **handle, size_t size,
...
drvError_t halMemMap(void *ptr, size_t size, size_t offset,
...
drvError_t halMemSetAccess(void *ptr, size_t size,
                           struct drv_mem_access_desc *desc, size_t count) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halMemGetAccess(void *ptr, struct drv_mem_location *location,
                           uint64_t *flags) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halMemExportToShareableHandle(drv_mem_handle_t *handle,
...
drvError_t
halMemExportToShareableHandleV2(drv_mem_handle_t *handle,
                                drv_mem_handle_type handle_type, uint64_t flags,
...
drvError_t
halMemImportFromShareableHandleV2(drv_mem_handle_type handle_type,
                                  struct MemShareHandle *share_handle,
...
drvError_t halMemTransShareableHandle(drv_mem_handle_type handle_type,
                                      struct MemShareHandle *share_handle,
                                      uint32_t *server_id,
                                      uint64_t *shareable_handle) {
    HCCL_VM_WARN("is empty.");
...
drvError_t halMemSetPidToShareableHandle(uint64_t shareable_handle, int pid[],
                                         uint32_t pid_num) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halMemShareHandleSetAttribute(uint64_t shareable_handle,
                                         enum ShareHandleAttrType type,
                                         struct ShareHandleAttr attr) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halMemShareHandleGetAttribute(uint64_t shareable_handle,
                                         enum ShareHandleAttrType type,
                                         struct ShareHandleAttr *attr) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
...
drvError_t halMemShareHandleInfoGet(uint64_t shareable_handle,
...
DVresult halMemGetInfoEx(DVdevice device, unsigned int type,
                         struct MemInfo *info) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

DVresult halMemGetInfo(DVdevice device, unsigned int type,
                       struct MemInfo *info) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t drvHdcClientCreate(HDC_CLIENT *client, int maxSessionNum,
                              int serviceType, int flag) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t drvHdcSessionConnect(int peer_node, int peer_devid,
                                HDC_CLIENT client, HDC_SESSION *session) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

hdcError_t halHdcSessionConnectEx(int peer_node, int peer_devid, int peer_pid,
...
drvError_t drvHdcAllocMsg(HDC_SESSION session, struct drvHdcMsg **ppMsg,
                          int count) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t drvHdcGetMsgBuffer(struct drvHdcMsg *msg, int index, char **pBuf,
                              int *pLen) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t drvHdcRecvBuf(HDC_SESSION session, char *pBuf, int bufLen,
                         int *msgLen) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t drvHdcGetTrustedBasePath(int peer_node, int peer_devid,
                                    char *base_path, unsigned int path_len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t drvHdcSendFile(int peer_node, int peer_devid, const char *file,
                          const char *dst_path,
...
void *drvHdcMallocEx(enum drvHdcMemType mem_type, void *addr,
                     unsigned int align, unsigned int len, int devid,
                     unsigned int flag) {
    return nullptr;
}
...
drvError_t drvHdcEpollCtl(HDC_EPOLL epoll, int op, void *target,
                          struct drvHdcEvent *event) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t drvHdcEpollWait(HDC_EPOLL epoll, struct drvHdcEvent *events,
                           int maxevents, int timeout, int *eventnum) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halHdcGetSessionInfo(HDC_SESSION session,
                                struct drvHdcSessionInfo *info) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

hdcError_t halHdcSend(HDC_SESSION session, struct drvHdcMsg *pMsg, UINT64 flag,
                      UINT32 timeout) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
hdcError_t halHdcRecvEx(HDC_SESSION session, struct drvHdcMsg *pMsg, int bufLen,
                        int *recvBufCount,
...
hdcError_t halHdcNotifyRegister(int service_type,
...
int log_read_by_type(int device_id, char *buf, unsigned int *size, int timeout,
                     enum log_channel_type channel_type) {
    HCCL_VM_WARN("is empty.");
    return 0;
}
...
int prof_drv_start(unsigned int device_id, unsigned int channel_id,
...
int prof_channel_read(unsigned int device_id, unsigned int channel_id,
...
int halProfDataFlush(unsigned int device_id, unsigned int channel_id,
...
int halMbufAllocEx(uint64_t size, unsigned int align, unsigned long flag,
                   int grp_id, Mbuf **mbuf) {
    HCCL_VM_WARN("is empty.");
    return 0;
}
...
drvError_t halGrpCacheAlloc(const char *name, unsigned int devId,
                            GrpCacheAllocPara *para) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
int halGrpQuery(GroupQueryCmdType cmd, void *inBuff, unsigned int inLen,
...
drvError_t halBuffGetDQSPooInfoById(unsigned int poolId,
                                    DqsPoolInfo *poolInfo) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halBuffCreateInterGrp(unsigned int grpId, const char *name,
                                 unsigned int len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halResourceIdAlloc(uint32_t devId, struct halResourceIdInputInfo *in,
                              struct halResourceIdOutputInfo *out) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halResourceIdFree(uint32_t devId,
                             struct halResourceIdInputInfo *in) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halResourceEnable(uint32_t devId,
                             struct halResourceIdInputInfo *in) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halResourceDisable(uint32_t devId,
                              struct halResourceIdInputInfo *in) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halResourceDetailQuery(uint32_t devId,
                                  struct halResourceIdInputInfo *in,
...
drvError_t halResourceIdInfoGet(struct drvResIdKey *key, drvResIdProcType type,
                                uint64_t *value) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halTsdrvCtl(uint32_t devId, int cmd, void *param, size_t paramSize,
                       void *out, size_t *outSize) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halSqCqAllocate(uint32_t devId, struct halSqCqInputInfo *in,
...
drvError_t halSqMemGet(uint32_t devId, struct halSqMemGetInput *in,
...
drvError_t halCqReportIrqWait(uint32_t devId, struct halReportInfoInput *in,
                              struct halReportInfoOutput *out) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halCqReportGet(uint32_t devId, struct halReportGetInput *in,
...
drvError_t halSqTaskArgsAsyncCopy(uint32_t devId,
                                  struct halSqTaskArgsInfo *info) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halAsyncDmaWqeCreate(uint32_t devId, struct halAsyncDmaInputPara *in,
                                struct halAsyncDmaOutputPara *out) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halAsyncDmaWqeDestory(uint32_t devId,
                                 struct halAsyncDmaDestoryPara *para) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halAsyncDmaCreate(uint32_t devId, struct halAsyncDmaInputPara *in,
                             struct halAsyncDmaOutputPara *out) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halAsyncDmaDestory(uint32_t devId,
                              struct halAsyncDmaDestoryPara *para) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halCtl(int cmd, void *param_value, size_t param_value_size,
                  void *out_value, size_t *out_size_ret) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halCdqCreate(unsigned int devId, unsigned int tsId,
                        struct halCdqPara *cdqPara, unsigned int *queId) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halCdqDestroy(unsigned int devId, unsigned int tsId,
                         unsigned int queId) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halCdqAllocBatch(unsigned int devId, unsigned int tsId,
                            unsigned int queId, unsigned int timeout,
                            unsigned int *batchId) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halCdqGetReadyBatch(const char *queName, DVdeviceptr *batchAddr,
...
drvError_t halEschedCreateGrpEx(uint32_t devId, struct esched_grp_para *grpPara,
                                unsigned int *grpId) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halEschedSetEventPriority(unsigned int devId, EVENT_ID eventId,
                                     SCHEDULE_PRIORITY priority) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halEschedRegisterFinishFunc(
...
drvError_t halEschedDumpEventTrace(unsigned int devId, char *buff,
...
drvError_t halEschedTraceRecord(unsigned int devId, const char *recordReason,
                                const char *key) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halEschedSubmitEvent(unsigned int devId,
                                struct event_summary *event) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halEschedSubmitEventToThread(uint32_t devId,
                                        struct event_summary *event) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halEschedSubmitEventSync(unsigned int devId,
                                    struct event_summary *event, int timeout,
                                    struct event_reply *reply) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halEschedThreadSwapout(unsigned int devId, unsigned int grpId,
                                  unsigned int threadId) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halEschedThreadGiveup(unsigned int devId, unsigned int grpId,
                                 unsigned int threadId) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halEschedGetEvent(unsigned int devId, unsigned int grpId,
                             unsigned int threadId, EVENT_ID eventId,
                             struct event_info *event) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halEschedAckEvent(unsigned int devId, EVENT_ID eventId,
                             unsigned int subeventId, char *msg,
                             unsigned int msgLen) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halEschedRegisterAckFunc(unsigned int grpId, EVENT_ID eventId,
                                    void (*ackFunc)(unsigned int devId,
...
drvError_t halEschedTableAddEntry(unsigned int devId, unsigned int tableId,
...
drvError_t halEschedTableDelEntry(unsigned int devId, unsigned int tableId,
                                  struct esched_table_key *key) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t
halEschedTableQueryEntryStat(unsigned int devId, unsigned int tableId,
                             struct esched_table_key *key,
...
drvError_t halQueueCreate(unsigned int devId, const QueueAttr *queAttr,
                          unsigned int *qid) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueueGrant(unsigned int devId, int qid, int pid,
                         QueueShareAttr attr) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halQueueSubscribe(unsigned int devId, unsigned int qid,
                             unsigned int groupId, int type) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halQueueSubF2NFEvent(unsigned int devId, unsigned int qid,
...
drvError_t halQueueGetQidbyName(unsigned int devId, const char *name,
                                unsigned int *qid) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueueCtrlEvent(struct QueueSubscriber *subscriber,
                             QUE_EVENT_CMD cmdType) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueuePeek(unsigned int devId, unsigned int qid, uint64_t *buf_len,
                        int timeout) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueuePeekData(unsigned int devId, unsigned int qid,
                            unsigned int flag, QueuePeekDataType type,
                            void **mbuf) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueueEnQueueBuff(unsigned int devId, unsigned int qid,
                               struct buff_iovec *vector, int timeout) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueueDeQueueBuff(unsigned int devId, unsigned int qid,
                               struct buff_iovec *vector, int timeout) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halQueueQuery(unsigned int devId, QueueQueryCmdType cmd,
                         QueueQueryInputPara *inPut,
                         QueueQueryOutputPara *outPut) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueueSet(unsigned int devId, QueueSetCmdType cmd,
...
drvError_t halQueueExport(unsigned int devId, unsigned int qid,
                          struct shareQueInfo *queInfo) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueueUnexport(unsigned int devId, unsigned int qid,
                            struct shareQueInfo *queInfo) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueueImport(unsigned int devId, struct shareQueInfo *queInfo,
                          unsigned int *qid) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueueUnimport(unsigned int devId, unsigned int qid,
                            struct shareQueInfo *queInfo) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halQueueGetDqsQueInfo(unsigned int devId, unsigned int qid,
                                 DqsQueueInfo *info) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halNotifyGetInfo(uint32_t devId, uint32_t tsId, uint32_t type,
                            uint32_t *val) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halGetChipInfo(unsigned int devId, halChipInfo *chipInfo) {
    if (chipInfo == nullptr) {
        HCCL_VM_ERROR("invalid param, chipInfo is nullptr, devId={}", devId);
        return DRV_ERROR_INVALID_VALUE;
    }
    ...
    strncpy(reinterpret_cast<char *>(chipInfo->type), locDevice.soc_version,
            MAX_CHIP_NAME - 1);
    chipInfo->type[MAX_CHIP_NAME - 1] = '\0';

    HCCL_VM_INFO("get chipInfo success, devId={}, chipInfo->type={}", devId,
                 reinterpret_cast<char *>(chipInfo->type));
...
drvError_t halGetFaultEvent(uint32_t devId, struct halEventFilter *filter,
                            struct halFaultEventInfo *eventInfo, uint32_t len,
                            uint32_t *eventCount) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
int halGetUserConfig(unsigned int devId, const char *name, unsigned char *buf,
                     unsigned int *bufSize) {
    HCCL_VM_WARN("is empty.");
    return 0;
}

int halSetUserConfig(unsigned int devId, const char *name, unsigned char *buf,
                     unsigned int bufSize) {
    HCCL_VM_WARN("is empty.");
    return 0;
}
...
int halGetDeviceVfList(unsigned int devId, unsigned int *vf_list,
                       unsigned int list_len, unsigned int *vf_num) {
    HCCL_VM_WARN("is empty.");
    return 0;
}

int halTsDevRecord(unsigned int devId, unsigned int tsId,
                   unsigned int record_type, unsigned int record_Id) {
    HCCL_VM_WARN("is empty.");
    return 0;
}

DVresult halMemInitSvmDevice(int hostpid, unsigned int vfid,
                             unsigned int dev_id) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

DVresult halMemBindSibling(int hostPid, int aicpuPid, unsigned int vfid,
                           unsigned int dev_id, unsigned int flag) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halResMap(unsigned int devId, struct res_map_info *res_info,
                     unsigned long *va, unsigned int *len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halResRead(unsigned int dev_id, struct res_map_info *res_info,
                      void *data, unsigned int len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}

drvError_t halResWrite(unsigned int dev_id, struct res_map_info *res_info,
                       void *data, unsigned int len) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
int halBuffAllocAlignEx(uint64_t size, unsigned int align, unsigned long flag,
                        int grp_id, void **buff) {
    HCCL_VM_WARN("is empty.");
    return 0;
}
...
drvError_t halEschedCreateGrp(unsigned int devId, unsigned int grpId,
                              GROUP_TYPE type) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halEschedWaitEvent(unsigned int devId, unsigned int grpId,
                              unsigned int threadId, int timeout,
                              struct event_info *event) {
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
