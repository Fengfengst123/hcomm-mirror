...
extern bool GetPhyMemBlockByVirPtr(const void* virPtr, uint64_t& offset, sim::runtime::PhyMemBlock& phyMem);
...
drvError_t drvGetDevNum(uint32_t* num_dev)
{
    ...
    // 等价替换：旧实现 GetByPred<Device>(Eq(status,0)) 物化全部行后只做
    // empty()+size()，不使用任何行字段；数据库端 COUNT 聚合返回相同计数，
    // 消除 Device 行物化。查询条件（status=0、无 server 限定）与错误语义不变：
    // 查询失败 → DRV_ERROR_NO_DEVICE（原路径无日志，保持一致）；
    // 空集 → 原日志 + DRV_ERROR_NO_DEVICE。
    auto result = sim::runtime::Db::Count<sim::runtime::Device>(HcclSim::Storage::Eq(&sim::runtime::Device::status, 0));
    if (!result.ok() || !result.value.has_value()) {
        return DRV_ERROR_NO_DEVICE;
    }
    const uint64_t devCount = *result.value;

    if (devCount == 0) {
        HCCL_VM_ERROR("no available device found");
        return DRV_ERROR_NO_DEVICE;
    }
    *num_dev = static_cast<uint32_t>(devCount);
...
{
    auto locDevice = sim::runtime::Db::GetOneByPred<sim::runtime::Device>(HcclSim::Storage::And(
        HcclSim::Storage::Eq(&sim::runtime::Device::server_id, g_cur_server_key),
        HcclSim::Storage::Eq(&sim::runtime::Device::physical_id, devId)));
    if (!locDevice.ok()) {
...
{
    auto ret = sim::runtime::Db::GetOneByPred<sim::runtime::Device>(
        HcclSim::Storage::Eq(&sim::runtime::Device::logic_id, devIndex));
    if (!ret.ok()) {
        HCCL_VM_ERROR("cannot find device by devIndex id {}.", devIndex);
        return DRV_ERROR_NO_DEVICE;
    }
    *phyId = (uint32_t)ret->physical_id;
...
drvError_t drvDeviceGetIndexByPhyId(uint32_t phyId, uint32_t* devIndex)
{
    if (g_cur_server_key == 0) {
        uint64_t serverKey = sim::runtime::GetCurServerId();
        if (serverKey == 0) {
    ...
    if (sim::runtime::GetDevIndexByPhyId(g_cur_server_key, phyId, logicDevId) != ACL_SUCCESS) {
        HCCL_VM_ERROR("cannot find devIndex by phyId:{}, serverKey:{}", phyId, g_cur_server_key);
        return DRV_ERROR_NO_DEVICE;
    }
    *devIndex = logicDevId;
...
DVresult drvMemcpy(DVdeviceptr dst, size_t dest_max, DVdeviceptr src, size_t byte_count)
{
    ...
    uint64_t offset = 0;
    // 地址分类：src/dst 各做一次数据库包含查询。分类失败（CONFLICT/
    // RESOURCE_EXHAUSTED/断连等）不得冒充 host 分类——HOST_TO_HOST 裸 memcpy
    // 设备虚拟地址会 SIGSEGV（128p 拆卸期实际崩溃链），先失败即返回驱动错误，
    // 不进入底层拷贝。
    const auto srcClass = sim::IsDeviceAddress((void*)src);
    if (!srcClass.ok()) {
        HCCL_VM_ERROR(
            "drvMemcpy src address classification failed: code={} "
            "diagnostic={} src={:p}",
            static_cast<int>(srcClass.code), srcClass.diagnostic, (void*)src);
        return DRV_ERROR_INNER_ERR;
    }
    if (srcClass.value.value()) {
        isSrcHost = false;
    }

    const auto dstClass = sim::IsDeviceAddress((void*)dst);
    if (!dstClass.ok()) {
        HCCL_VM_ERROR(
            "drvMemcpy dst address classification failed: code={} "
            "diagnostic={} dst={:p}",
            static_cast<int>(dstClass.code), dstClass.diagnostic, (void*)dst);
        return DRV_ERROR_INNER_ERR;
    }
    if (dstClass.value.value()) {
        isDstHost = false;
    }
    ...
    const aclError copied = aclrtMemcpy((void*)dst, dest_max, (void*)src, byte_count, kind);
    if (copied != ACL_SUCCESS) {
        HCCL_VM_ERROR(
            "drvMemcpy aclrtMemcpy failed: aclError={} kind={} "
            "count={} destMax={} dst={:p} src={:p}",
            static_cast<int>(copied), static_cast<int>(kind), byte_count, dest_max, (void*)dst, (void*)src);
        return DRV_ERROR_INNER_ERR;
    }
    return DRV_ERROR_NONE;
}
...
drvError_t halHostRegister(void* src_ptr, UINT64 size, UINT32 flag, UINT32 devid, void** dst_ptr)
{
    if (src_ptr == nullptr || dst_ptr == nullptr) {
        HCCL_VM_ERROR("halHostRegister: invalid params, src_ptr={:p}, dst_ptr={:p}", (void*)src_ptr, (void*)dst_ptr);
        return DRV_ERROR_INVALID_VALUE;
    }

    // host侧根据虚拟地址查询时需要根据device_id匹配；地址包含查询经派生端点列下推。
    uint64_t devPtr = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(src_ptr));
    ...
    uint64_t offset = devPtr - virMemRes->dev_mapped_ptr;
    sim::runtime::PhyMemBlock phyMem = *phyMemRes.value;

    char* hostPtr
        = (char*)sim::DeviceMemoryManager::GetInstance().AcquirePhyMem(phyMem.name, phyMem.device_id, phyMem.size);
    if (hostPtr == nullptr) {
        HCCL_VM_ERROR("halHostRegister: AcquirePhyMem failed for name={}, size={:d}", phyMem.name, size);
        return DRV_ERROR_INNER_ERR;
    }

    *dst_ptr = hostPtr + offset;
    HCCL_VM_INFO(
        "halHostRegister stub: src_ptr={:p} -> hostPtr={:p}, name={}, "
        "size={:d}",
        (void*)src_ptr, *dst_ptr, phyMem.name, size);

    // 记录host侧映射过的hostPtr到VirtualMemBlock表
    void* finalHostAddr = *dst_ptr;
    (void)sim::runtime::Db::Update<sim::runtime::VirtualMemBlock>(
        HcclSim::Storage::Eq(&sim::runtime::VirtualMemBlock::id, virMemRes->id),
        HcclSim::Storage::Set(
            &sim::runtime::VirtualMemBlock::host_ptr,
            static_cast<uint64_t>(reinterpret_cast<uintptr_t>(finalHostAddr))));
...
drvError_t halHostUnregister(void* src_ptr, UINT32 devid)
{
    ...
    if (!GetPhyMemBlockByVirPtr(src_ptr, offset, phyMem)) {
        HCCL_VM_INFO("halHostUnregister: src_ptr={:p} not a dev mem, skip", (void*)src_ptr);
        return DRV_ERROR_NONE;
    }
    ...
    if (virMemRes.ok()) {
        (void)sim::runtime::Db::Update<sim::runtime::VirtualMemBlock>(
            HcclSim::Storage::Eq(&sim::runtime::VirtualMemBlock::id, virMemRes->id),
            HcclSim::Storage::Set(&sim::runtime::VirtualMemBlock::host_ptr, uint64_t{0}));
    }

    HCCL_VM_INFO("halHostUnregister: src_ptr={:p}, name={}", (void*)src_ptr, phyMem.name);
    return DRV_ERROR_NONE;
}

drvError_t halHostUnregisterEx(void* src_ptr, UINT32 devid, UINT32 flag) { return halHostUnregister(src_ptr, devid); }
...
drvError_t
halMemGetAllocationGranularity(const struct drv_mem_prop* prop, drv_mem_granularity_options option, size_t* granularity)
{
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
int dsmi_cmd_get_network_device_info(
...
void* log_type_alloc_mem(uint32_t device_id, uint32_t type, uint32_t* size) { return nullptr; }
...
drvError_t
halBufEventSubscribe(const char* grpName, unsigned int threadGrpId, unsigned int event_id, unsigned int devid)
{
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t
halCdqAllocBatch(unsigned int devId, unsigned int tsId, unsigned int queId, unsigned int timeout, unsigned int* batchId)
{
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t
halEschedSubmitEventSync(unsigned int devId, struct event_summary* event, int timeout, struct event_reply* reply)
{
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t
halEschedAckEvent(unsigned int devId, EVENT_ID eventId, unsigned int subeventId, char* msg, unsigned int msgLen)
{
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halEschedTableAddEntry(
...
drvError_t halEschedTableQueryEntryStat(
...
drvError_t
halQueueQuery(unsigned int devId, QueueQueryCmdType cmd, QueueQueryInputPara* inPut, QueueQueryOutputPara* outPut)
{
    HCCL_VM_WARN("is empty.");
    return DRV_ERROR_NONE;
}
...
drvError_t halGetChipInfo(unsigned int devId, halChipInfo* chipInfo)
{
    ...
    sim::runtime::Device locDevice{};
    if (sim::runtime::GetDeviceByPhysicalId(devId, locDevice) != ACL_SUCCESS) {
        HCCL_VM_ERROR("get device by physic id {} failed.", devId);
        return DRV_ERROR_NO_DEVICE;
    }

    strncpy(reinterpret_cast<char*>(chipInfo->type), locDevice.soc_version, MAX_CHIP_NAME - 1);
    chipInfo->type[MAX_CHIP_NAME - 1] = '\0';

    HCCL_VM_INFO("get chipInfo success, devId={}, chipInfo->type={}", devId, reinterpret_cast<char*>(chipInfo->type));
