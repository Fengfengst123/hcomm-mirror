...
void MockCommCommand::Setup(CLI::App &app) {
    auto sub_run =
        app.add_subcommand("mock-comm", "mock-comm: 启动一个算子的通信域");

    sub_run
        ->add_option("configFile", configFileName, "加载昇腾集群组网配置目录")
        ->required()
        ->check(FileInModelDir);
    sub_run->add_option("--level", g_hcclVmLevel,
...
void MockCommCommand::Execute() {
    HCCL_VM_INFO("Initializing mock communication domain: Config={}, Level={}",
                 configFileName, g_hcclVmLevel);
    ...
    if (configFileName != "ranktable") {
        ...
        if (!sim::RefreshTopoMetaConfig(configFileName, &topoMeta)) {
