...
void TableCommand::Setup(CLI::App &app) {
    ...
    updateCmd
        ->add_option("table", tableName,
                     "Table name (e.g., Device, Server, Host)")
        ->required();
    updateCmd->add_option("id", rowId, "Row ID to update")->required();
    updateCmd
        ->add_option("column", columnName,
                     "Column to update (e.g., soc_version)")
        ->required();
    updateCmd->add_option("value", newValue, "New value (as string)")
        ->required();
