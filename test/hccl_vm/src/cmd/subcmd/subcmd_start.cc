...
void StartCommand::Setup(CLI::App &app) {
    auto sub_start = app.add_subcommand(
        "start", "start: 启动仿真环境,请勿在子bash中重复启用");

    sub_start
        ->add_option("configCluster", configClusterName,
                     "加载昇腾集群组网配置目录")
        ->required()
        ->check(CheckClusterConfigFile);
    sub_start->add_option(
        "--level", g_hcclVmLevel,
...
void StartCommand::Execute() {
    ...
    HCCL_VM_INFO("Initializing: Model={}, Level={}", configClusterName,
                 g_hcclVmLevel);
    g_configClusterDir = InstallPath::ResolveToInstallRoot(
