...
void LogCommand::Setup(CLI::App &app) {
    ...
    auto *levelOpt =
        subCmdLog->add_option("-l,--level", level_, "设置日志等级")
            ->transform(CLI::CheckedTransformer(levelMap_, CLI::ignore_case));
    auto *consoleLevelOpt =
        subCmdLog
            ->add_option("--console-level", consoleLevel_,
                         "设置console日志等级")
            ->transform(CLI::CheckedTransformer(levelMap_, CLI::ignore_case));
    auto *fileLevelOpt =
        subCmdLog->add_option("--file-level", fileLevel_, "设置file日志等级")
            ->transform(CLI::CheckedTransformer(levelMap_, CLI::ignore_case));
