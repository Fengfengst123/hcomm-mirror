...
namespace {
    ...
    bool IsValidCommunicatorName(const char* commName)
    {
        ...
        return commName != nullptr
               && ::strnlen(commName, sizeof(sim::Communicator::comm_id)) < sizeof(sim::Communicator::comm_id);
    }
    ...
    bool IsSameCommunicator(const sim::Communicator& record, const char* commName, uint64_t commHash)
    {
        return std::strncmp(record.comm_id, commName, sizeof(record.comm_id)) == 0 && record.comm_hash == commHash;
    }
} // namespace
...
{
    if (!IsValidCommunicatorName(commName) || commName[0] == '\0' || rankSize == 0 || rankId >= rankSize
        || deviceId == 0) {
        HCCL_VM_ERROR(
            "invalid communicator parameters: name={}, rankSize={}, "
            "rankId={}, deviceId={}",
            SafeCommunicatorName(commName), rankSize, rankId, deviceId);
        return false;
    }

    const auto members = RunnerDB::GetByPred<sim::Communicator>([commName, commHash](const sim::Communicator& record) {
        return IsSameCommunicator(record, commName, commHash);
    });
    if (!members.empty()) {
        for (const auto& member : members) {
            if (member.rank_size != rankSize) {
                HCCL_VM_ERROR("communicator domain conflict: name={}, rankSize={}", commName, rankSize);
                return false;
            }
            if (member.rank_id == rankId) {
                if (member.device_id != deviceId) {
                    HCCL_VM_ERROR(
                        "communicator rank mapping conflict: "
                        "name={}, rankId={}, "
                ...
                (void)RunnerDB::Update<sim::Communicator>(commId, [](sim::Communicator& record) {
            ...
            if (member.device_id == deviceId) {
                HCCL_VM_ERROR(
                    "communicator device mapping conflict: name={}, "
                    "deviceId={}, "
                    "oldRankId={}, newRankId={}",
    ...
    if (memberId == 0) {
        HCCL_VM_ERROR(
            "insert communicator member failed: name={}, rankId={}, "
            "deviceId={}",
    ...
    commId = memberId;
    HCCL_VM_DEBUG(
        "Insert Communicator member, id={}, commName={}, "
        "commHash={}, rankSize={}, rankId={}, deviceId={}",
        commId, std::string(commName), record.comm_hash, record.rank_size, record.rank_id, record.device_id);
    return true;
}

bool WaitCommunicatorReady(const char* commName, uint64_t commHash, uint32_t rankSize)
{
    if (!IsValidCommunicatorName(commName) || commName[0] == '\0' || rankSize == 0) {
        HCCL_VM_ERROR(
            "invalid communicator readiness parameters: name={}, rankSize={}", SafeCommunicatorName(commName),
            rankSize);
        return false;
    }

    const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(kCommunicatorWaitTimeoutMs);
    while (std::chrono::steady_clock::now() < deadline) {
        const auto records
            = RunnerDB::GetByPred<sim::Communicator>([commName, commHash](const sim::Communicator& record) {
                  return IsSameCommunicator(record, commName, commHash);
              });
        ...
        if (std::all_of(rankSeen.begin(), rankSeen.end(), [](bool seen) {
                return seen;
            })) {
            return true;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(kCommunicatorWaitIntervalMs));
    }

    HCCL_VM_ERROR("wait communicator ready timeout: name={}, commHash={}, rankSize={}", commName, commHash, rankSize);
    return false;
}
...
bool WaitCommunicatorDestroyReady(uint64_t commId)
{
    ...
    if (!self.has_value() || !IsValidCommunicatorName(self->comm_id) || self->comm_id[0] == '\0' || self->rank_size == 0
        || self->rank_id >= self->rank_size) {
        HCCL_VM_ERROR("invalid communicator destroy barrier member id={}", commId);
        return false;
    }
    ...
    if (RunnerDB::Add<sim::CommunicatorDestroySync>(record) == 0) {
        HCCL_VM_ERROR("failed to mark communicator destroy ready: id={}", commId);
        return false;
    }
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        const auto records
            = RunnerDB::GetByPred<sim::CommunicatorDestroySync>([&self](const sim::CommunicatorDestroySync& record) {
                  return std::strncmp(record.comm_id, self->comm_id, sizeof(record.comm_id)) == 0
                         && record.comm_hash == self->comm_hash;
              });
    ...
    HCCL_VM_ERROR(
        "wait communicator destroy barrier timeout: name={}, "
        "commHash={}, rankSize={}",
...
bool MarkAndSyncCommunicatorDestroy(uint64_t commId)
{
    ...
    if (!RunnerDB::Update<sim::Communicator>(commId, [](sim::Communicator& record) {
            record.status = COMM_STATUS_DESTROYED;
        })) {
        HCCL_VM_ERROR("failed to mark communicator destroy status: id={}", commId);
        return false;
    }
    ...
    const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(kCommunicatorWaitTimeoutMs);
    while (std::chrono::steady_clock::now() < deadline) {
        const auto records = RunnerDB::GetByPred<sim::Communicator>([&self](const sim::Communicator& record) {
            return IsSameCommunicator(record, self->comm_id, self->comm_hash);
        });
    ...
    HCCL_VM_ERROR(
        "wait communicator destroy sync timeout: name={}, "
        "commHash={}, rankSize={}",
        self->comm_id, self->comm_hash, self->rank_size);
    return false;
}
...
bool GetCommunicatorMembers(uint64_t commId, std::vector<CommunicatorMemberInfo>& members)
{
    ...
    const auto communicatorMembers
        = RunnerDB::GetByPred<sim::Communicator>([&selfMember](const sim::Communicator& member) {
              return IsSameCommunicator(member, selfMember->comm_id, selfMember->comm_hash);
          });
    if (communicatorMembers.empty()) {
        HCCL_VM_ERROR("communicator has no members: commName={}", selfMember->comm_id);
        return false;
    }
...
bool GetCommunicatorName(uint64_t commId, std::string& commName)
{
    ...
    if (!IsValidCommunicatorName(selfMember->comm_id) || selfMember->comm_id[0] == '\0') {
...
bool GetCommunicatorIdentity(uint64_t commId, std::string& commName, uint64_t& commHash)
{
    ...
    const auto selfMember = RunnerDB::GetById<sim::Communicator>(commId);
    if (!selfMember.has_value() || !IsValidCommunicatorName(selfMember->comm_id) || selfMember->comm_id[0] == '\0') {
...
aclError GetDeviceByCommRank(uint64_t commId, uint32_t rankId, sim::Device& device)
{
    ...
    auto rankRet = RunnerDB::GetOneByPred<sim::Communicator>([&selfMember, rankId](const sim::Communicator& member) {
        return IsSameCommunicator(member, selfMember->comm_id, selfMember->comm_hash) && member.rank_id == rankId;
    });
    if (!rankRet.second) {
        HCCL_VM_ERROR("cannot find communicator member: commName={}, rankId={}", selfMember->comm_id, rankId);
        return ACL_ERROR_INVALID_PARAM;
    }
    ...
    if (!deviceRet.has_value()) {
        // not find
        HCCL_VM_ERROR(
            "cannot find device for communicator member: "
            "commName={}, rankId={}",
...
aclError GetDeviceByPhysicalId(uint32_t deviceId, sim::Device& device)
{
    ...
    HCCL_VM_INFO(
        "GetDeviceByPhysicalId: device.physical_id = {:d}, serverkey = {:d}", device.physical_id, g_cur_server_key);
...
{
    auto ret = RunnerDB::GetOneByPred<sim::EndPoint>([addr](const sim::EndPoint& ep) {
        return memcmp(ep.eid, addr.GetEid().raw, URMA_EID_LEN) == 0;
    });
    if (!ret.second) {
        HCCL_VM_ERROR("cannot find EndPoint by eid:{}", addr.EidToHexString().c_str());
        return ACL_ERROR_INVALID_PARAM;
    }
    endPoint = ret.first;
    return ACL_SUCCESS;
...
bool GetCommRankByDeviceId(uint64_t commId, uint32_t deviceId, uint32_t& rankId)
{
    ...
    auto member = RunnerDB::GetOneByPred<sim::Communicator>([&selfMember, deviceId](const sim::Communicator& record) {
        return IsSameCommunicator(record, selfMember->comm_id, selfMember->comm_hash) && record.device_id == deviceId;
    });
    if (!member.second) {
        HCCL_VM_ERROR("cannot find communicator rank: commName={}, deviceId={}", selfMember->comm_id, deviceId);
        return false;
    }
...
std::string GetHardwareTypeByDevice(uint64_t deviceId)
{
    ...
    auto device = RunnerDB::GetOneByPred<sim::Device>([deviceId](const sim::Device& d) {
        return d.logic_id == deviceId;
    });
    if (!device.second) {
        HCCL_VM_ERROR("cannot find device by physical id {:d}", deviceId);
...
namespace {
    ...
    {
        std::string ranktablePath = InstallPath::ResolveToInstallRoot("data/ranktable.json");
        ...
        for (const auto& rank : rankTable["rank_list"]) {
            ...
            for (const auto& level : levelList) {
                ...
                if (sscanf(netInstanceId.c_str(), "sp_%u_srv_%u", &spIdx, &srvIdx) == 2
                    || sscanf(netInstanceId.c_str(), "superPod%u_rack%u", &spIdx, &srvIdx) == 2) {
                    found = true;
                    break;
                }
            }
    ...
    bool BuildCommConfigFromTopoMeta(const TopoMeta& topoMeta, CommConfigData& data)
    {
        auto allServers = RunnerDB::GetByPred<sim::Server>([](const sim::Server& r) {
            return true;
        });
        ...
        for (const auto& [podId, superPod] : topoMeta) {
            for (const auto& [serverId, serverMeta] : superPod) {
                ...
                if (it == serverKeyIndex.end()) {
                    HCCL_VM_ERROR(
                        "server not found in static topo: podId={:d}, "
                        "serverId={:d}",
                        podId, serverId);
                    return false;
                }
    ...
    bool LoadCommConfigData()
    {
        ...
        auto ret = RunnerDB::GetOneByPred<sim::TopoMetaConfig>([](const sim::TopoMetaConfig&) {
            return true;
        });
        if (!ret.second) {
        ...
        std::string fileName(ret.first.file_name, strnlen(ret.first.file_name, sizeof(ret.first.file_name)));
        CommConfigData data;
...
bool RefreshTopoMetaConfig(const std::string& fileName, const TopoMeta* topoMeta)
{
    if (fileName.empty() || fileName.size() >= sizeof(sim::TopoMetaConfig::file_name)) {
    ...
    g_comm_config_data = std::move(data);
    g_comm_config_loaded = true;
    HCCL_VM_INFO("refresh comm config: file={}, server num={}", fileName, g_comm_config_data.servers.size());
    return true;
}
...
aclError GetDevIndexByPhyId(uint64_t serverKey, uint32_t phyId, uint32_t& devIndex)
{
    ...
    HCCL_VM_ERROR("dev index not found by phyId:{:d}, serverKey:{:d}", phyId, serverKey);
    return ACL_ERROR_INVALID_PARAM;
}
