...
std::atomic<uint32_t> g_curOpIter{
    0}; // 当前进程正在处理的算子迭代（供 CCU 交换表隔离 op）

/** @brief 读取当前进程正在处理的算子迭代（见头文件声明）。 */
uint32_t GetCurOpIter() { return g_curOpIter.load(std::memory_order_relaxed); }
...
int InsertOpDetail(OpDetailTab &rec) {
    ...
    if (!GetCommunicatorIdentity(rec.commId, commName, commHash)) {
        HCCL_VM_ERROR("Failed to resolve communicator identity before "
    ...
    }
    {
        ...
        auto iter =
            g_opIterCounter.try_emplace(std::make_pair(commName, commHash), 0u)
                .first;
        rec.opIter = iter->second.fetch_add(1, std::memory_order_relaxed);
        // 记录"当前 op"：本 op 的 CCU launch 随后会在同进程发生，交换表据此打
        // op 身份。
        g_curOpIter.store(rec.opIter, std::memory_order_relaxed);
    }
    rec.syncIter = g_syncIterCounter;

    const std::string sql =
        "INSERT INTO opDetails (pid, deviceId, rankId, commId, opIter, "
...
int InsertOpMem(OpMemInfoTab &rec) {
    ...
    const std::string sql =
        "INSERT INTO opMemInfo (opDetailId, inputAddr, inputSize, outputAddr, "
        "outputSize, cclAddr, cclSize) VALUES (?, ?, ?, ?, ?, ?, ?)";
    std::vector<Value> params = {
        (int64_t)rec.opDetailId, (int64_t)rec.inputAddr,
        (int64_t)rec.inputSize,  (int64_t)rec.outputAddr,
        (int64_t)rec.outputSize, (int64_t)rec.cclAddr,
        (int64_t)rec.cclSize};
...
int InsertCcuChannel(CcuChannelTab &rec) {
    const std::string sql =
        "INSERT INTO ccuChannels (channelId, srcDieId, dstDieId, srcDeviceId, "
        "dstDeviceId, srcRankId, dstRankId, leid, reid, protocol, jettyNum, "
        "jettyId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    std::vector<uint8_t> jettyId((uint8_t *)rec.jettyId,
                                 (uint8_t *)rec.jettyId + rec.jettyNum * 4);
    std::vector<Value> params = {
        (int64_t)rec.channelId,   (int64_t)rec.srcDieId,
        (int64_t)rec.dstDieId,    (int64_t)rec.srcDeviceId,
        (int64_t)rec.dstDeviceId, (int64_t)rec.srcRankId,
...
int InsertJettyMap(JettyMapTab &rec) {
    rec.opDetailId = g_currOpDetailId;
    const std::string sql =
        "INSERT INTO JettyMaps (opDetailId, srcDieId, dstDieId, srcRankId, "
        "dstRankId, leid, reid, protocol) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    std::vector<Value> params = {
        (int64_t)rec.opDetailId, (int64_t)rec.srcDieId,  (int64_t)rec.dstDieId,
        (int64_t)rec.srcRankId,  (int64_t)rec.dstRankId, BlobData{rec.leid, 16},
        BlobData{rec.reid, 16},  (int64_t)rec.protocol};
    uint32_t dummy;
    return OpDbOps::Instance().ExecInsert(sql, params, dummy);
}
...
int InsertSyncRecord(SyncRecordTab &rec) {
    rec.syncIter = g_syncIterCounter++;
    const std::string sql =
        "INSERT INTO syncRecords (pid, rankId, rankSize, syncIter, streamId, "
        "status) VALUES (?, ?, ?, ?, ?, ?)";
    std::vector<Value> params = {(int64_t)rec.pid,      (int64_t)rec.rankId,
...
int InsertCcuInstr(CcuInstrTab &rec) {
    ...
    std::vector<Value> params = {(int64_t)rec.ccuInstrResId,
                                 (int64_t)rec.startId,
                                 (int64_t)rec.instrInfoSize};
    return OpDbOps::Instance().ExecInsert(sql, params, rec.id);
}
...
int UpdateAndInsertByCcuId(uint64_t &ccuId, uint32_t deviceId, uint32_t dieId,
                           ...
                           uint32_t *totalInstrCount) {
    constexpr size_t kInstrSpaceSize =
        HcclSim::CCU_INSTRUCTION_NUM * HcclSim::CCU_INSTRUCTION_SIZE;

    // 使用减法形式校验写入范围，避免 offset + size 的整数溢出。
    if (instrInfo == nullptr || instrOffset > kInstrSpaceSize ||
        instrInfoSize > kInstrSpaceSize - instrOffset) {
        HCCL_VM_ERROR("invalid instruction range: offset={}, size={}, "
                      "space={}, instrInfo={}",
                      instrOffset, instrInfoSize, kInstrSpaceSize,
                      instrInfo == nullptr ? "null" : "valid");
        return -1;
    }
    ...
    if (ret != 0) {
        HCCL_VM_ERROR("query ccuInstrRes failed for deviceId: {} dieId: {}",
                      deviceId, dieId);
        return -1;
    }
    ...
    if (!rows.empty() && rows[0].size() >= 3) {
        ...
        if (std::holds_alternative<std::vector<uint8_t>>(rows[0][1])) {
            ...
            std::memcpy(space.data(), existBlob.data(),
                        std::min(existBlob.size(), kInstrSpaceSize));
        }
        ...
        const std::string updateSql = "UPDATE ccuInstrRes SET instrSpace = ?, "
                                      "instrCount = ? WHERE id = ?";
        std::vector<Value> updateParams = {BlobData{space.data(), space.size()},
                                           (int64_t)newCount, (int64_t)existId};
        ...
        const std::string insertSql =
            "INSERT INTO ccuInstrRes (deviceId, dieId, instrCount, instrSpace) "
            "VALUES (?, ?, ?, ?)";
        std::vector<Value> insertParams = {
            (int64_t)deviceId, (int64_t)dieId, (int64_t)newCount,
...
int InsertHalfRTT(HalfRTTTab &rec) {
    const std::string sql =
        "INSERT INTO halfRTT (deviceId, dieId, wishCntXnIdBegin, "
        "wishCntXnIdEnd, totalCntId) VALUES (?, ?, ?, ?, ?)";
    std::vector<Value> params = {(int64_t)rec.deviceId, (int64_t)rec.dieId,
                                 (int64_t)rec.wishCntXnIdBegin,
                                 (int64_t)rec.wishCntXnIdEnd,
...
int QuerySyncRecordByStatus(uint8_t status, std::vector<SyncRecordTab> &out) {
    ...
    const std::string sql =
        "SELECT id, pid, rankId, rankSize, syncIter, streamId, status FROM "
...
int UpdateSyncRecordStatus(std::vector<SyncRecordTab> &syncRecord) {
    return OpDbOps::Instance().RunInTransaction([&]() -> int {
        const std::string sql =
            "UPDATE syncRecords SET status = ? WHERE id = ?";
...
int UpdateOpMemCclBuffer(uint64_t cclAddr, uint64_t cclSize) {
    const std::string sql =
        "UPDATE opMemInfo SET cclAddr = ?, cclSize = ? WHERE id = ?";
    std::vector<Value> params = {(int64_t)cclAddr, (int64_t)cclSize,
                                 (int64_t)g_currOpMemId};
    int ret = OpDbOps::Instance().ExecUpdate(sql, params);
    if (ret != 0) {
        return ret;
    }
...
int QueryCurrentOpMemInfo(uint64_t commId, uint32_t deviceId,
    ...
    if (commId == 0 || deviceId == 0) {
        HCCL_VM_ERROR("invalid current opMemInfo query, commId={}, deviceId={}",
                      commId, deviceId);
        return -1;
    }
    ...
    if (OpDbOps::Instance().ExecQuery(sql, params, rows) != 0 || rows.empty()) {
        HCCL_VM_ERROR("query current opMemInfo failed, commId={}, deviceId={}",
                      commId, deviceId);
        return -1;
    }
    ...
    if (row.size() < 8) {
        HCCL_VM_ERROR(
            "invalid current opMemInfo row size={}, commId={}, deviceId={}",
            row.size(), commId, deviceId);
        return -1;
    }
...
int QueryOpMemInfoByOpDetailId(uint32_t opDetailId, OpMemInfoTab &out) {
    const std::string sql = "SELECT id, opDetailId, inputAddr, inputSize, "
    ...
    if (OpDbOps::Instance().ExecQuery(sql, params, rows) != 0 || rows.empty() ||
        rows[0].size() < 8) {
        HCCL_VM_ERROR("query opMemInfo by opDetailId failed, opDetailId={}",
                      opDetailId);
        return -1;
    }
...
int UpdateOpExpansionMode(uint8_t mode) {
    const std::string sql =
        "UPDATE opDetails SET opExpansionMode = ? WHERE id = ?";
    std::vector<Value> params = {(int64_t)mode, (int64_t)g_currOpDetailId};
    int ret = OpDbOps::Instance().ExecUpdate(sql, params);
    if (ret != 0) {
        HCCL_VM_ERROR("Failed to update mode={}, id={}", mode,
                      g_currOpDetailId);
    }
    return ret;
}
...
int UpdateOpDetailMainStream(uint32_t opDetailId, uint64_t streamId) {
    const std::string sql = "UPDATE opDetails SET streamId = ? WHERE id = ?";
    std::vector<Value> params = {(int64_t)streamId, (int64_t)opDetailId};
    int ret = OpDbOps::Instance().ExecUpdate(sql, params);
    if (ret != 0) {
        HCCL_VM_ERROR(
            "Failed to update opDetail main stream, id={}, streamId={}",
            opDetailId, streamId);
    }
    return ret;
}
...
int QueryLatestOpExpansionMode() {
    const std::string sql =
        "SELECT opExpansionMode FROM opDetails WHERE id = ?";
    ...
    if (OpDbOps::Instance().ExecQuery(sql, params, rows) != 0 || rows.empty() ||
        ...
        return static_cast<int>(
            SimOpExpansionMode::SIM_OP_EXPANSION_MODE_RESERVED);
    }
...
int QueryOpExecutionIndexEntries(std::vector<OpExecutionIndexEntry> &out) {
    ...
    const std::string sql =
        "SELECT id, deviceId, rankId, commId, opIter, rankSize "
    ...
    for (const auto &row : rows) {
        if (row.size() < COLUMN_COUNT) {
            HCCL_VM_ERROR(
                "Invalid operator execution index row, columnCount={}",
                row.size());
            return -1;
        }
...
int QueryCompositeOpDetailBySyncIter(
    ...
    const std::string opSql =
        "SELECT id, pid, deviceId, rankId, commId, opIter, syncIter, streamId, "
    ...
    for (const auto &r : opRows) {
        ...
        comp.deviceId =
            static_cast<uint32_t>(std::stoul(toStr(r[OD_DEVICE_ID])));
        ...
        if (std::holds_alternative<std::vector<uint8_t>>(r[OD_OP_DETAIL])) {
            comp.detail.opDetail =
                std::get<std::vector<uint8_t>>(r[OD_OP_DETAIL]);
        }
        if (std::holds_alternative<std::vector<uint8_t>>(r[OD_OP_EXT_INFO])) {
            comp.detail.opExtInfo =
                std::get<std::vector<uint8_t>>(r[OD_OP_EXT_INFO]);
        }
        ...
        if (OpDbOps::Instance().ExecQuery(memSql, memParams, memRows) == 0 &&
            !memRows.empty()) {
        ...
        const std::string taskSql =
            "SELECT id, pid, opDetailId, deviceId, streamId, taskType, isDone, "
            "opTaskMeta FROM " +
            taskTabName + " WHERE opDetailId = ?";
        std::vector<Value> taskParams = {(int64_t)opDetailId};
        HcclSim::DB::ExRows taskRows;
        if (OpDbOps::Instance().ExecQueryEx(taskSql, taskParams, taskRows) ==
            0) {
            for (const auto &tr : taskRows) {
                ...
                if (std::holds_alternative<std::vector<uint8_t>>(
                        tr[TASK_META])) {
                    taskRec.optaskMeta =
                        std::get<std::vector<uint8_t>>(tr[TASK_META]);
                }
...
int QueryOpDetailIdentity(uint32_t opDetailId, uint64_t &commId,
    ...
    const std::string sql =
        "SELECT commId, rankId, deviceId FROM opDetails WHERE id = ?";
    std::vector<Value> params = {(int64_t)opDetailId};
    std::vector<std::vector<std::string>> rows;
    if (OpDbOps::Instance().ExecQuery(sql, params, rows) != 0 || rows.empty() ||
        rows[0].size() < 3) {
...
// 从 opDetails 一行（16 列）解析 OpDetailTab，QueryOpDetailById /
// QueryLatestOpDetailByDeviceId 共用。
static int ParseOpDetailRow(const ExRows::value_type &r, OpDetailTab &out) {
    enum : int {
        OD_ID,
        OD_PID,
        OD_DEVICE_ID,
    ...
    out.id = static_cast<uint32_t>(std::stoul(toStr(r[OD_ID])));
    out.pid = static_cast<uint32_t>(std::stoul(toStr(r[OD_PID])));
    out.deviceId = static_cast<uint32_t>(std::stoul(toStr(r[OD_DEVICE_ID])));
    out.rankId = static_cast<uint32_t>(std::stoul(toStr(r[OD_RANK_ID])));
    out.commId = std::stoull(toStr(r[OD_COMM_ID]));
    out.opIter = static_cast<uint32_t>(std::stoul(toStr(r[OD_OP_ITER])));
    out.syncIter = static_cast<uint32_t>(std::stoul(toStr(r[OD_SYNC_ITER])));
    out.streamId = std::stoull(toStr(r[OD_STREAM_ID]));
    out.root = static_cast<uint32_t>(std::stoul(toStr(r[OD_ROOT])));
    out.opExpansionMode =
        static_cast<uint32_t>(std::stoul(toStr(r[OD_OP_EXPANSION])));
    out.devType = static_cast<uint32_t>(std::stoul(toStr(r[OD_DEV_TYPE])));
    out.rankSize = static_cast<uint32_t>(std::stoul(toStr(r[OD_RANK_SIZE])));
    out.srcRank = static_cast<uint32_t>(std::stoul(toStr(r[OD_SRC_RANK])));
    out.dstRank = static_cast<uint32_t>(std::stoul(toStr(r[OD_DST_RANK])));
    if (std::holds_alternative<std::vector<uint8_t>>(r[OD_OP_DETAIL])) {
        out.opDetail = std::get<std::vector<uint8_t>>(r[OD_OP_DETAIL]);
    }
    if (std::holds_alternative<std::vector<uint8_t>>(r[OD_OP_EXT_INFO])) {
        out.opExtInfo = std::get<std::vector<uint8_t>>(r[OD_OP_EXT_INFO]);
    }
    return 0;
}
...
int QueryOpDetailById(uint32_t opDetailId, OpDetailTab &out) {
    const std::string sql =
        "SELECT id, pid, deviceId, rankId, commId, opIter, syncIter, streamId, "
        "root, opExpansionMode, devType, rankSize, srcRank, dstRank, opDetail, "
        "opExtInfo "
        "FROM opDetails WHERE id = ?";
    std::vector<Value> params = {(int64_t)opDetailId};
    ExRows rows;
    if (OpDbOps::Instance().ExecQueryEx(sql, params, rows) != 0 ||
        rows.empty()) {
        HCCL_VM_ERROR("Query opDetails by id failed, opDetailId={}",
                      opDetailId);
        return -1;
    }
    return ParseOpDetailRow(rows[0], out);
}
...
int QueryLatestOpDetailByDeviceId(uint32_t deviceId, OpDetailTab &out) {
    const std::string sql =
        ...
        "root, opExpansionMode, devType, rankSize, srcRank, dstRank, opDetail, "
        "opExtInfo "
        "FROM opDetails WHERE deviceId = ? ORDER BY id DESC LIMIT 1";
    std::vector<Value> params = {(int64_t)deviceId};
    ExRows rows;
    if (OpDbOps::Instance().ExecQueryEx(sql, params, rows) != 0 ||
        rows.empty()) {
        HCCL_VM_WARN(
            "Failed to query latest opDetail by deviceId={}, OpDetail is empty",
            deviceId);
        return -1;
    }
    return ParseOpDetailRow(rows[0], out);
...
int QueryCcuChannelAll(std::vector<CcuChannelTab> &out) {
    ...
    const std::string sql =
        "SELECT id, channelId, srcDieId, dstDieId, srcDeviceId, dstDeviceId, "
...
int QueryJettyMapAll(std::vector<JettyMapTab> &out) {
    ...
    const std::string sql =
        "SELECT id, opDetailId, srcDieId, dstDieId, srcRankId, dstRankId, "
...
int QueryCcuInstrResAll(std::vector<CcuInstrResTab> &out) {
    ...
    const std::string sql =
        "SELECT id, deviceId, dieId, instrCount, instrSpace FROM ccuInstrRes";
    ExRows rows;
    if (OpDbOps::Instance().ExecQueryEx(sql, {}, rows) != 0) {
        return -1;
    }
    ...
    for (const auto &r : rows) {
        ...
        std::memcpy(rec.instrSpace, blob.data(),
                    std::min(blob.size(), sizeof(rec.instrSpace)));
        out.push_back(std::move(rec));
    }
    return 0;
}
...
int QueryCcuResourceMetaCount(uint32_t &instrLoadCnt, uint32_t &channelCnt) {
    ...
    std::vector<std::vector<std::string>> rows;
    if (OpDbOps::Instance().ExecQuery(sql, {}, rows) != 0 || rows.empty() ||
        rows[0].size() < 2) {
...
void AddSyncStreamIter(uint64_t streamId) {
    g_syncStreamIterMap.try_emplace(streamId, 0u);
}

void RemoveSyncStreamIter(uint64_t streamId) {
    g_syncStreamIterMap.erase(streamId);
}
...
int QueryCompositeOpDetailByOpIter(const std::string &commName,
    ...
    auto commMembers = RunnerDB::GetByPred<sim::Communicator>(
        [&commName, commHash](const sim::Communicator &record) -> bool {
            return std::string(record.comm_id) == commName &&
                   record.comm_hash == commHash;
        });

    for (const auto &comm : commMembers) {
        const std::string opSql =
            "SELECT id, pid, deviceId, rankId, commId, opIter, syncIter, "
            "streamId, "
            "root, opExpansionMode, devType, rankSize, srcRank, dstRank, "
        ...
        HcclSim::DB::ExRows opRows;
        if (OpDbOps::Instance().ExecQueryEx(opSql, opParams, opRows) != 0 ||
            opRows.empty() || opRows[0].size() < OD_COL_COUNT) {
            HCCL_VM_ERROR("Query opDetails failed, commId={}, opIter={}",
                          comm.id, opIter);
            return -1;
        }
        ...
        CompositeOpDetail comp = {};
        comp.deviceId =
            static_cast<uint32_t>(std::stoul(toStr(opRow[OD_DEVICE_ID])));
        comp.rankId =
            static_cast<uint32_t>(std::stoul(toStr(opRow[OD_RANK_ID])));
        ...
        if (std::holds_alternative<std::vector<uint8_t>>(opRow[OD_OP_DETAIL])) {
            comp.detail.opDetail =
                std::get<std::vector<uint8_t>>(opRow[OD_OP_DETAIL]);
        }
        if (std::holds_alternative<std::vector<uint8_t>>(
                opRow[OD_OP_EXT_INFO])) {
            comp.detail.opExtInfo =
                std::get<std::vector<uint8_t>>(opRow[OD_OP_EXT_INFO]);
        }

        // 查询 MemInfo
        const std::string memSql =
            "SELECT id, opDetailId, inputAddr, inputSize, "
            "outputAddr, outputSize, cclAddr, cclSize "
            "FROM opMemInfo WHERE opDetailId = ?";
        const uint32_t opDetailId = comp.detail.id;
        std::vector<Value> memParams = {(int64_t)opDetailId};
        std::vector<std::vector<std::string>> memRows;
        if (OpDbOps::Instance().ExecQuery(memSql, memParams, memRows) != 0 ||
            memRows.empty() || memRows[0].size() < MEM_COL_COUNT) {
            HCCL_VM_ERROR("Query opDetails mem failed, opDetailId={}",
                          opDetailId);
            return -1;
        }
        ...
        if (OpDbOps::Instance().ExecQueryEx(taskSql, taskParams, taskRows) !=
                0 ||
            taskRows.empty()) {
            HCCL_VM_ERROR("Query opDetails tasks failed, pid={}, opDetailId={}",
                          pid, opDetailId);
            return -1;
        }
        for (const auto &taskRow : taskRows) {
            if (taskRow.size() < TASK_COL_COUNT) {
                HCCL_VM_ERROR(
                    "Query opDetails tasks failed, pid={}, opDetailId={}", pid,
                    opDetailId);
                return -1;
            }
            OpTaskTab taskRec;
            ...
            if (std::holds_alternative<std::vector<uint8_t>>(
                    taskRow[TASK_META])) {
                taskRec.optaskMeta =
                    std::get<std::vector<uint8_t>>(taskRow[TASK_META]);
            }
            comp.tasks.push_back(std::move(taskRec));
        }
...
int QueryAllOpTasks(std::vector<OpTaskTab> &tasks, bool filterDone) {
    ...
    for (const std::string &taskTabName : taskTabNames) {
        ...
        for (const auto &tr : taskRows) {
            // 校验列数
            if (tr.size() < TASK_COL_COUNT) {
                HCCL_VM_ERROR("Query OpTasks failed, tableName={}",
                              taskTabName);
                continue;
            }
            OpTaskTab taskRec;
            ...
            taskRec.opDetailId =
                std::stoul(std::get<std::string>(tr[TASK_OP_DETAIL_ID]));
            taskRec.deviceId =
                std::stoull(std::get<std::string>(tr[TASK_DEVICE_ID]));
            taskRec.streamId =
                std::stoull(std::get<std::string>(tr[TASK_STREAM_ID]));
            taskRec.taskType = std::stoul(std::get<std::string>(tr[TASK_TYPE]));
            taskRec.isDone =
                std::stoul(std::get<std::string>(tr[TASK_IS_DONE]));
            // 安全访问 Meta 数据
...
int QueryOpTasksByStreamId(uint64_t streamId, std::vector<OpTaskTab> &tasks) {
    ...
    if (ret != 0) {
        HCCL_VM_ERROR(
            "Query OpTasks by streamId failed, tableName={:s}, streamId={}",
            tabName, streamId);
        return ret;
    }
...
int FinishOpTask(const OpTaskTab &task) {
    ...
    int ret = OpDbOps::Instance().ExecUpdate(sql, params);
    if (ret != 0) {
        HCCL_VM_ERROR("Update OpTask isDone failed, taskTable:{:s}, id:{}",
                      taskTabName, task.id);
    }
    return ret;
}
...
int QueryHalfRTTAll(std::vector<HalfRTTTab> &out) {
    ...
    for (const auto &r : rows) {
        ...
        rec.deviceId = std::stoul(std::get<std::string>(r[DEVICE_ID]));
        rec.dieId = std::stoul(std::get<std::string>(r[DIE_ID]));
        rec.wishCntXnIdBegin =
            std::stoul(std::get<std::string>(r[WISH_CNT_XN_BEGIN]));
        rec.wishCntXnIdEnd =
            std::stoul(std::get<std::string>(r[WISH_CNT_XN_END]));
