...
HcclDBSqlite::~HcclDBSqlite() { Close(); }
...
HcclVmResult HcclDBSqlite::Connect(const sim::DBConfig &config) {
    ...
    sqlite3_exec(m_db, "PRAGMA cache_size = -10000;", nullptr, nullptr,
                 nullptr);
    sqlite3_exec(m_db, "PRAGMA temp_store = MEMORY;", nullptr, nullptr,
                 nullptr);
    sqlite3_exec(m_db, "PRAGMA journal_size_limit = 67108864;", nullptr,
                 nullptr, nullptr);
    sqlite3_exec(m_db, "PRAGMA wal_autocheckpoint = 200;", nullptr, nullptr,
                 nullptr);
    sqlite3_exec(m_db, "PRAGMA mmap_size = 268435456;", nullptr, nullptr,
                 nullptr);
...
bool HcclDBSqlite::IsConnected() const { return m_db != nullptr; }
...
HcclVmResult HcclDBSqlite::Execute(const std::string &sql,
    ...
    for (size_t i = 0; i < params.size(); ++i) {
        ...
        if (std::holds_alternative<int64_t>(val)) {
            ...
            sqlite3_bind_text(stmt, i + 1, str.c_str(), str.size(),
                              SQLITE_STATIC);
...
HcclVmResult HcclDBSqlite::Query(const std::string &sql,
                                 const std::vector<Value> &params,
...
HcclVmResult HcclDBSqlite::QueryBlob(const std::string &sql,
                                     const std::vector<Value> &params,
    ...
    if (ret == SQLITE_ROW) {
        ...
        if (blob && blobSize > 0) {
            outBlob.assign(static_cast<const uint8_t *>(blob),
...
HcclVmResult
HcclDBSqlite::QueryBlobColumns(const std::string &sql,
                               const std::vector<Value> &params,
    ...
    if (ret == SQLITE_ROW) {
        ...
        for (int i = 0; i < colCount; ++i) {
            ...
            if (blob && blobSize > 0) {
                outBlobs[i].assign(static_cast<const uint8_t *>(blob),
...
HcclVmResult HcclDBSqlite::QueryEx(const std::string &sql,
                                   const std::vector<Value> &params,
                                   ExRows &rows) {
    std::lock_guard lock(m_mutex);
...
std::string HcclDBSqlite::GetLastError() const { return m_lastError; }

sim::DbType HcclDBSqlite::GetDbType() const { return sim::DbType::SQLITE3; }

void *HcclDBSqlite::GetNativeHandle() { return m_db; }
