...
class OpDbOps {
  public:
    ...
    int ExecInsert(const std::string &sql, const std::vector<Value> &params,
                   uint32_t &outId);

    int ExecInsertNoId(const std::string &sql,
                       const std::vector<Value> &params);

    int ExecQuery(const std::string &sql, const std::vector<Value> &params,
                  std::vector<std::vector<std::string>> &rows);

    int ExecUpdate(const std::string &sql, const std::vector<Value> &params);

    int QuerySingleBlob(const std::string &sql,
                        const std::vector<Value> &params,
                        std::vector<uint8_t> &outBlob);

    int QueryBlobColumns(const std::string &sql,
                         const std::vector<Value> &params,
                         std::vector<std::vector<uint8_t>> &outBlobs);

    int ExecQueryEx(const std::string &sql, const std::vector<Value> &params,
                    ExRows &rows);
