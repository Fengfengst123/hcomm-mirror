...
class HcclDB {
  public:
    ...
    virtual HcclVmResult Execute(const std::string &sql,
                                 const std::vector<Value> &params) = 0;

    virtual HcclVmResult Query(const std::string &sql,
    ...
    virtual HcclVmResult QueryBlob(const std::string &sql,
                                   const std::vector<Value> &params,
                                   std::vector<uint8_t> &outBlob) = 0;

    virtual HcclVmResult
    QueryBlobColumns(const std::string &sql, const std::vector<Value> &params,
                     std::vector<std::vector<uint8_t>> &outBlobs) = 0;

    virtual HcclVmResult QueryEx(const std::string &sql,
                                 const std::vector<Value> &params,
                                 ExRows &rows) = 0;
