...
class HcclDBSqlite : public HcclDB {
  public:
    ...
    HcclVmResult Execute(const std::string &sql,
                         const std::vector<Value> &params) override;

    HcclVmResult Query(const std::string &sql,
                       std::vector<std::vector<std::string>> &rows) override;

    HcclVmResult Query(const std::string &sql, const std::vector<Value> &params,
                       std::vector<std::vector<std::string>> &rows) override;

    HcclVmResult QueryBlob(const std::string &sql,
                           const std::vector<Value> &params,
                           std::vector<uint8_t> &outBlob) override;

    HcclVmResult
    QueryBlobColumns(const std::string &sql, const std::vector<Value> &params,
                     std::vector<std::vector<uint8_t>> &outBlobs) override;

    HcclVmResult QueryEx(const std::string &sql,
                         const std::vector<Value> &params,
                         ExRows &rows) override;
