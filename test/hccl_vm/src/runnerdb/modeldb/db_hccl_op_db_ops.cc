...
void OpDbOps::SetDB(std::unique_ptr<HcclDB> db) { m_db = std::move(db); }

HcclDB *OpDbOps::GetDB() const { return m_db.get(); }
...
HcclVmResult OpDbOps::InitSchema() {
    ...
    auto ret = m_db->RunInTransaction([this, &opTaskTable]() -> HcclVmResult {
        ...
        HCCLVM_CHK_RET(m_db->Execute(
            "CREATE INDEX IF NOT EXISTS idx_opdetail_pid_synciter ON "
            "opDetails(pid, syncIter);"));
        HCCLVM_CHK_RET(m_db->Execute(
            "CREATE INDEX IF NOT EXISTS idx_opdetail_comm_device_id ON "
        ...
        HCCLVM_CHK_RET(m_db->Execute(
            "CREATE INDEX IF NOT EXISTS idx_opmeminfo_op_detail_id ON "
        ...
        HCCLVM_CHK_RET(
            m_db->Execute("CREATE INDEX IF NOT EXISTS idx_sync_status ON "
                          "syncRecords(status);"));
        HCCLVM_CHK_RET(
            m_db->Execute("CREATE INDEX IF NOT EXISTS idx_sync_pid_status ON "
        ...
        HCCLVM_CHK_RET(
            m_db->Execute("CREATE INDEX IF NOT EXISTS idx_ccuchannel_channelid "
        ...
        HCCLVM_CHK_RET(m_db->Execute(
            "CREATE INDEX IF NOT EXISTS idx_jettymap_op_detail_id ON "
            "JettyMaps(opDetailId);"));
        HCCLVM_CHK_RET(m_db->Execute(
            "CREATE INDEX IF NOT EXISTS idx_jettymap_srcdie_dstdie ON "
        ...
        HCCLVM_CHK_RET(m_db->Execute("CREATE TABLE IF NOT EXISTS " +
                                     opTaskTable +
                                     " ("
                                     "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                                     "pid INTEGER, "
        ...
        HCCLVM_CHK_RET(
            m_db->Execute("CREATE TABLE IF NOT EXISTS halfRTT ("
                          "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                          "deviceId INTEGER, dieId INTEGER, "
        ...
        HCCLVM_CHK_RET(
            m_db->Execute("CREATE INDEX IF NOT EXISTS idx_halfrtt_rank_dieid "
...
int OpDbOps::ExecInsert(const std::string &sql,
                        const std::vector<Value> &params, uint32_t &outId) {
    HCCLVM_CHK_PTR(m_db);
...
int OpDbOps::ExecInsertNoId(const std::string &sql,
                            const std::vector<Value> &params) {
    HCCLVM_CHK_PTR(m_db);
    return m_db->Execute(sql, params) == HCCL_SIM_SUCCESS ? 0 : -1;
}

int OpDbOps::ExecQuery(const std::string &sql, const std::vector<Value> &params,
                       std::vector<std::vector<std::string>> &rows) {
    HCCLVM_CHK_PTR(m_db);
    return m_db->Query(sql, params, rows) == HCCL_SIM_SUCCESS ? 0 : -1;
}

int OpDbOps::ExecUpdate(const std::string &sql,
                        const std::vector<Value> &params) {
    HCCLVM_CHK_PTR(m_db);
    return m_db->Execute(sql, params) == HCCL_SIM_SUCCESS ? 0 : -1;
}

int OpDbOps::QuerySingleBlob(const std::string &sql,
                             const std::vector<Value> &params,
...
int OpDbOps::QueryBlobColumns(const std::string &sql,
                              const std::vector<Value> &params,
                              std::vector<std::vector<uint8_t>> &outBlobs) {
    HCCLVM_CHK_PTR(m_db);
    return m_db->QueryBlobColumns(sql, params, outBlobs) == HCCL_SIM_SUCCESS
               ? 0
               : -1;
}

int OpDbOps::ExecQueryEx(const std::string &sql,
                         const std::vector<Value> &params, ExRows &rows) {
    HCCLVM_CHK_PTR(m_db);
