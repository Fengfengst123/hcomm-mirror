...
namespace DB {
    ...
    int
    OpDbOps::QuerySingleBlob(const std::string& sql, const std::vector<Value>& params, std::vector<uint8_t>& outBlob)
    {
        HCCLVM_CHK_PTR(m_db);
        return m_db->QueryBlob(sql, params, outBlob) == HCCL_SIM_SUCCESS ? 0 : -1;
    }

    int OpDbOps::QueryBlobColumns(
