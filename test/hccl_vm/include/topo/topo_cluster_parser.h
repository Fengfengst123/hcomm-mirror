...
class ClusterTopoParser {
  ...
  private:
    ...
    ParseStatus ScanServerDirs(const std::string &superpodDir,
                               SuperPod &superpod);
    ...
    ParseStatus ParseRootinfoJson(const std::string &rootinfoPath,
                                  Server &server);
    ParseStatus ParseServersInfoJson(const std::string &serversInfoPath);

    void MergeTopoAndRootinfo(Server &server);

    std::string FindFileBySuffix(const std::string &dir,
