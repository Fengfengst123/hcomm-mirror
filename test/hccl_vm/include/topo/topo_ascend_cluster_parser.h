...
class AscendClusterTopoParser {
  public:
    ...
    AscendClusterTopoParser &
    operator=(const AscendClusterTopoParser &) = delete;
    ...
    HcclVmResult
    ParseRanktableAndInitCommDomain(const std::string &ranktablePath);
    // 初始化通信域表项
    HcclVmResult InitCommunicationDomain(const TopoMeta &topoMeta,
                                         bool withRanktable = false);
    // 获取集群状态
  ...
  private:
    ...
    json BuildRankEntry(const Server &server, int srcDevPhyId,
                        const std::set<int> &commDomainLocalIds, uint32_t spIdx,
                        uint32_t srvIdx, uint32_t rankId, bool skipLayerAbove2,
                        const std::set<int> &allowedNetLayers);
    ...
    HcclVmResult AddLinkInfo(const LinkPortRef *srcPort,
                             const LinkPortRef *dstPort, uint32_t netLayer,
                             const std::vector<Protocol> &protocols);
    HcclVmResult CreateRankTableFile(const TopoMeta &topoMeta);
    HcclVmResult ParseRanktable(const std::string &ranktablePath,
                                TopoMeta &topoMeta);
