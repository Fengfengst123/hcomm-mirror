...
template <typename T> uint64_t Add(T &rec) {
    return SimRunnerSqliteDB::Instance().Add<T>(rec);
}

template <typename T> std::vector<uint64_t> AddBatch(std::vector<T> &recs) {
    return SimRunnerSqliteDB::Instance().AddBatch<T>(recs);
}

template <typename T> std::optional<T> GetById(uint64_t id) {
...
template <typename T> bool Delete(uint64_t id) {
