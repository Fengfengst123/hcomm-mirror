...
struct CommConfigData {
    std::string
        fileName; // topo_meta yaml文件名; "ranktable"表示ranktable.json模式
    std::vector<CommConfigServer>
        servers; // 按(podId, serverId)升序, 与rankId分配顺序一致
};
} // namespace sim
...
bool RefreshTopoMetaConfig(const std::string &fileName,
                           const TopoMeta *topoMeta = nullptr);
