...
template <typename T> class SqliteTable : public TableBase {
  public:
    ...
    KeyType Add(T &rec) {
        ...
        for (int attempt = 0; attempt <= kMaxRetries; ++attempt) {
            ...
            std::string sql =
                "INSERT INTO " + m_tableName + " (data) VALUES (?)";

            sqlite3_stmt *rawStmt;
            int rc =
                sqlite3_prepare_v2(m_db, sql.c_str(), -1, &rawStmt, nullptr);
            if (rc != SQLITE_OK) {
                HCCL_VM_ERROR(
                    "[SqliteTable::Add] SQL prepare failed [{}]: {} (rc={})",
                    m_tableName.c_str(), sqlite3_errmsg(m_db), rc);
                return 0;
            }

            std::unique_ptr<sqlite3_stmt, decltype(&sqlite3_finalize)> stmt(
                rawStmt, sqlite3_finalize);
            ...
            if (!isRetryable || attempt == kMaxRetries) {
                HCCL_VM_ERROR("[SqliteTable::Add] SQL insert failed [{}]: "
                              "rc={}, errc={}, ext={}, msg={}",
                              m_tableName.c_str(), stepRc, errc, extErrc,
                              sqlite3_errmsg(m_db));
                return 0;
            }
    ...
    std::vector<KeyType> AddBatch(std::vector<T> &recs) {
        ...
        if (sqlite3_prepare_v2(m_db, sql.c_str(), -1, &rawStmt, nullptr) !=
            SQLITE_OK) {
            HCCL_VM_ERROR("[SqliteTable::AddBatch] prepare failed [{}]: {}",
        ...
        for (auto &rec : recs) {
            ...
            if (stepRc != SQLITE_DONE) {
                int errc = sqlite3_errcode(m_db);
                int extErrc = sqlite3_extended_errcode(m_db);
                HCCL_VM_ERROR("[SqliteTable::AddBatch] step failed [{}]: "
                              "rc={}, errc={}, ext={}, msg={}",
                              m_tableName.c_str(), stepRc, errc, extErrc,
                              sqlite3_errmsg(m_db));
                if (ownTx) {
    ...
    bool Update(KeyType id, std::function<void(T &)> updater) {
        ...
        for (int attempt = 0; attempt <= kMaxRetries; ++attempt) {
            ...
            bool ownTx = (sqlite3_get_autocommit(m_db) != 0);
            if (ownTx) {
                sqlite3_exec(m_db, "BEGIN IMMEDIATE", nullptr, nullptr,
                             nullptr);
            }
            ...
            std::string sql =
                "UPDATE " + m_tableName + " SET data = ? WHERE id = ?";
            ...
            if (sqlite3_prepare_v2(m_db, sql.c_str(), -1, &rawStmt, nullptr) !=
                ...
                HCCL_VM_ERROR(
                    "[SqliteTable::Update] SQL prepare failed [{}]: {}",
                    m_tableName.c_str(), sqlite3_errmsg(m_db));
                return false;
            }
            ...
            bool changed =
                (result == SQLITE_DONE) && (sqlite3_changes(m_db) > 0);
            ...
            if (!changed && result != SQLITE_DONE) {
                ...
                bool isRetryable =
                    (errc == SQLITE_BUSY || errc == SQLITE_LOCKED);

                connLock.unlock();

                if (!isRetryable || attempt == kMaxRetries) {
                    HCCL_VM_ERROR("[SqliteTable::Update] SQL step failed [{}]: "
                                  "rc={}, errc={}, msg={}",
            ...
            if (ownTx) {
                sqlite3_exec(m_db, changed ? "COMMIT" : "ROLLBACK", nullptr,
                             nullptr, nullptr);
            }
            return changed;
        }
    ...
    bool Delete(KeyType id) {
        ...
        for (int attempt = 0; attempt < 5; ++attempt) {
            ...
            if (sqlite3_prepare_v2(m_db, sql.c_str(), -1, &rawStmt, nullptr) !=
                SQLITE_OK) {
                HCCL_VM_ERROR(
                    "[SqliteTable::Delete] SQL prepare failed [{}]: {}",
                    m_tableName.c_str(), sqlite3_errmsg(m_db));
                return false;
            }
            ...
            if (errc != SQLITE_BUSY && errc != SQLITE_LOCKED) {
                HCCL_VM_ERROR("[SqliteTable::Delete] SQL step failed [{}]: "
                              "rc={}, errc={}",
                              m_tableName.c_str(), result, errc);
                return false;
            }
    ...
    bool DeleteAll() {
        ...
        if (sqlite3_exec(m_db, sql.c_str(), nullptr, nullptr, nullptr) !=
            SQLITE_OK) {
            HCCL_VM_ERROR(
                "[SqliteTable::DeleteAll] SQL delete all failed [{}]: {}",
                m_tableName.c_str(), sqlite3_errmsg(m_db));
            return false;
        }
        ...
        sqlite3_stmt *rawStmt;
        if (sqlite3_prepare_v2(m_db, seqSql.c_str(), -1, &rawStmt, nullptr) ==
            SQLITE_OK) {
            std::unique_ptr<sqlite3_stmt, decltype(&sqlite3_finalize)> stmt(
                rawStmt, sqlite3_finalize);
            sqlite3_bind_text(stmt.get(), 1, m_tableName.c_str(), -1,
                              SQLITE_TRANSIENT);
    ...
    QueryList(std::function<bool(const T &)> pred) const {
        std::shared_lock<std::shared_mutex> connLock(GetConnectionMutex());
        ...
        for (int attempt = 0; attempt <= kMaxRetries; ++attempt) {
            ...
            if (sqlite3_prepare_v2(m_db, sql.c_str(), -1, &rawStmt, nullptr) !=
                SQLITE_OK) {
                HCCL_VM_ERROR(
                    "[SqliteTable::QueryList] SQL prepare failed [{}]: {}",
                    m_tableName.c_str(), sqlite3_errmsg(m_db));
                return result;
            }
        ...
        HCCL_VM_ERROR(
            "[SqliteTable::QueryList] SQLITE_BUSY after {} retries [{}]",
            kMaxRetries, m_tableName.c_str());
        return result;
    }

    std::pair<ValueType, bool>
    Query(std::function<bool(const T &)> pred) const {
        std::shared_lock<std::shared_mutex> connLock(GetConnectionMutex());
        std::string sql = "SELECT id, data FROM " + m_tableName;
        static constexpr int kMaxRetries = 30;
        for (int attempt = 0; attempt <= kMaxRetries; ++attempt) {
            ...
            if (sqlite3_prepare_v2(m_db, sql.c_str(), -1, &rawStmt, nullptr) !=
                SQLITE_OK) {
                HCCL_VM_ERROR(
                    "[SqliteTable::Query] SQL prepare failed [{}]: {}",
                    m_tableName.c_str(), sqlite3_errmsg(m_db));
  ...
  private:
    void CreateTableIfNotExists() {
        ...
        if (sqlite3_exec(m_db, sql.c_str(), nullptr, nullptr, nullptr) !=
            SQLITE_OK) {
            HCCL_VM_ERROR("[SqliteTable::CreateTableIfNotExists] SQL create "
                          "table failed [{}]: {}",
                          m_tableName.c_str(), sqlite3_errmsg(m_db));
        }
    }
    ...
    bool FindInternal(KeyType id, T &rec) const {
        std::string sql =
            "SELECT id, data FROM " + m_tableName + " WHERE id = ?";

        sqlite3_stmt *rawStmt;
        if (sqlite3_prepare_v2(m_db, sql.c_str(), -1, &rawStmt, nullptr) !=
            SQLITE_OK) {
            HCCL_VM_ERROR(
                "[SqliteTable::FindInternal] SQL prepare failed [{}]: {}",
                m_tableName.c_str(), sqlite3_errmsg(m_db));
            return false;
        }
        ...
        if (sqlite3_step(stmt.get()) == SQLITE_ROW) {
            ...
            if (blobSize == static_cast<int>(sizeof(T))) {
                std::memcpy(&rec, blob, sizeof(T));
                rec.id =
                    static_cast<KeyType>(sqlite3_column_int64(stmt.get(), 0));
                return true;
            }
        }
...
class SqliteDatabase {
  public:
    ...
    SqliteTable<T> &GetTable(const std::string &tableName) {
        {
            ...
            if (it != m_tables.end()) {
                return *static_cast<SqliteTable<T> *>(
                    m_tables[tableName].get());
            }
        }
    ...
    bool BeginTransaction() {
        ...
        return sqlite3_exec(m_db, "BEGIN IMMEDIATE", nullptr, nullptr,
                            nullptr) == SQLITE_OK;
    }
    ...
    bool CommitTransaction() {
        ...
        return sqlite3_exec(m_db, "COMMIT", nullptr, nullptr, nullptr) ==
               SQLITE_OK;
    }
  ...
  public:
    ...
    SqliteDatabase() : m_db(nullptr) {
        std::string actualPath =
            s_dbPath.empty()
                ? InstallPath::ResolveToInstallRoot("data/hccl_sim.db")
                : s_dbPath;
        if (sqlite3_open_v2(actualPath.c_str(), &m_db,
                            ...
                            nullptr) != SQLITE_OK) {
            HCCL_VM_ERROR(
                "[SqliteDatabase] CRITICAL: Failed to open SQLite database: {}",
                sqlite3_errmsg(m_db));
        } else {
            sqlite3_exec(m_db, "PRAGMA journal_mode     = WAL", nullptr,
                         nullptr, nullptr);
            sqlite3_exec(m_db, "PRAGMA synchronous      = NORMAL", nullptr,
                         nullptr, nullptr);
            sqlite3_exec(m_db, "PRAGMA cache_size       = -10000", nullptr,
                         nullptr, nullptr);
            sqlite3_exec(m_db, "PRAGMA temp_store       = MEMORY", nullptr,
                         nullptr, nullptr);
            sqlite3_exec(m_db, "PRAGMA locking_mode     = NORMAL", nullptr,
                         nullptr, nullptr);
            sqlite3_exec(m_db, "PRAGMA wal_autocheckpoint = 1000", nullptr,
                         nullptr, nullptr);
            sqlite3_exec(m_db, "PRAGMA busy_timeout     = 60000", nullptr,
                         nullptr, nullptr);
