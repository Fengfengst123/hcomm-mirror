...
class Loader {
  public:
    ...
    HcclResult
    GetSyncRecordsByStatus(uint8_t status,
                           std::vector<sim::SyncRecordTab> &syncRecords);

    HcclResult LoadOpExecutionKeys(std::vector<sim::OpExecutionKey> &keys);
    HcclResult LoadOpExecutionByKey(const sim::OpExecutionKey &key,
                                    sim::OpExecution &opExecution);
    HcclResult LoadAllOpExecutions(std::vector<sim::OpExecution> &opExecutions);

    // Runner: 每 sync 一次调用一次，outSyncIter: 输出本次加载的 syncIter
    HcclResult
    LoadRunnerSingleSync(const uint32_t &outSyncIter,
                         std::map<uint32_t, std::vector<sim::CompositeOpDetail>>
                             &compositeDataMap);
    ...
    HcclResult GetHalfRTTInfo(std::vector<sim::HalfRTTTab> &halfRTT);
    ...
    HcclResult LoadAllOpTasks(std::vector<sim::OpTaskTab> &tasks,
                              bool filterDone = false);
    HcclResult
    LoadCompositeOpDetailByOpIter(const std::string &commName,
                                  uint64_t commHash, uint32_t opIter,
                                  std::vector<sim::CompositeOpDetail> &details);
    HcclResult FinishOpTask(const sim::OpTaskTab &task);

  private:
    // 成员变量
    OpPipeline opTaskCache_;
    using OpExecutionIdentity = std::tuple<std::string, uint64_t, uint32_t>;
    std::map<OpExecutionIdentity, std::vector<sim::OpExecutionIndexEntry>>
        opExecutionEntriesByKey_;
};
} // namespace loader
