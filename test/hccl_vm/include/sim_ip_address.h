...
class IpAddress {
  public:
    ...
    explicit IpAddress(const union BinaryAddr &ip, s32 family, s32 scopeID = 0)
        : family_(family), scopeID_(scopeID) {
        binaryAddr_ = ip;
        // 区分ipv4和ipv6转eid
        if (family_ == AF_INET6) {
            (void)memcpy(eid_.raw, binaryAddr_.addr6.s6_addr,
                         sizeof(binaryAddr_.addr6.s6_addr));
        } else {
    ...
    explicit IpAddress(const std::string &ip, s32 family = AF_INET)
        : family_(family) {
        void *dst;
        if (family != AF_INET && family != AF_INET6) {
            throw std::invalid_argument(
                "[IpAddress] Unsupported Address Family");
        }
        if (family == AF_INET6) {
    ...
    std::string GetIpStr() const {
        ...
        if (res == nullptr) {
            throw std::invalid_argument(
                "[IpAddress] Invalid Binary Network Address");
        }
        return dst;
    }
